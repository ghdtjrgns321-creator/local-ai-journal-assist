...
- 발화 후 순서 부여 : 판정은 이진이고, 발화한 건에 대해서만 **즉시 / 검토 / 후순위** 검토 순서를 매김. 순서가 낮아져도 발화 기록은 지우지 않음
...
```
 ━━━ 데이터 정합성 — 데이터 결함 ━━━
    ...
    L1-01  차대변 균형     차변 합 ≠ 대변 합 (허용 오차 0.01)
 ...
 ━━━ 승인·권한 ━━━
    ...
    L1-04  승인한도 초과   승인자의 전결 한도보다 전표 금액이 큼
           순서 : 한도 경계 근처 · 시스템 계정 승인은 후순위
    L1-05  자기 승인       작성자와 승인자가 같은 사람
           면제 : 자동·반복 전기는 발화 자체에서 빠진다 (사람이 아닌 처리)
                  단 배치번호 없이 자동으로 위장한 행은 면제를 취소한다
    L1-06  직무분리 위반   한 사람이 상충하는 업무를 함께 수행
    L1-07  승인 생략       승인자 칸이 공란이면 예외 없이 전부 발화
           순서 : 수기 입력에 증거 2개 이상이면 즉시 · 시스템 전기는 후순위
