...
- **3레이어 22개 룰 기반 탐지**: Layer A(데이터 무결성 3개) + Layer B(부정 탐지 10개) + Layer C(이상 징후 9개)
...
| 데이터 생성   | EY-ASU DataSynth (Rust, 1,068K건 합성 전표) |

## 아키텍처

```
Excel/CSV -> ingest (자동 매핑) -> feature (18개 파생변수) -> validation (L1~L3)
  -> detection (22개 룰 + XGBoost + VAE+IF) -> score_aggregator -> DuckDB
...
| 1b    | detection (22개 룰 3레이어) + DuckDB + pipeline           | 미착수 |
...
메인 데이터는 EY-ASU DataSynth(Rust)로 생성한 합성 전표 1,068K건.
PCAOB/ISA/COSO/SOX 감사기준 코드 레벨 구현, 132종 anomaly 유형, seed 고정 재현 가능.

```
data/journal/
  primary/datasynth/    # 메인: 합성 전표 (fraud 2%, 8가지 유형)
...
| docs/AUDIT_DOMAIN_FINAL.md   | 22개 룰, Benford 판정, 도메인 용어      |
