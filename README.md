...
1. 전표 생성    : 회계적으로 성립하는 합성 원장 생성 (EY 스위스 Assurance R&D 가 공개한 오픈소스 DataSynth 기반 → K-IFRS 중견 제조업으로 개조. 원 저장소는 이후 비공개 전환)
2. 룰 탐지      : 결정론 룰을 설정하여 단위 규칙 위반을 걸러냄
...
**읽기·파생** — 필수 컬럼이 표준 이름과 정확히 맞아 매핑 없이 통과한다. 이 경로는 표준 밖 컬럼을 전부 미매핑으로 두므로, 스키마 51컬럼 대비 원장 73컬럼 중 배치 ID·변형 이력 같은 생성기 메타 26개가 남는다.
...
실제 기업 원장은 구할 수 없으므로 회계적으로 성립하는 원장을 생성한다. 생성기는 EY 스위스 Assurance R&D 가 Apache 2.0 으로 공개한 오픈소스 DataSynth 를 K-IFRS 제조업 기준으로 개편한 Rust 워크스페이스 15 크레이트다. 원 저장소는 이후 비공개로 전환됐다.
