...
```
 ...
 [생성]  시나리오 할당 ───▶ 계정 결정 ──────▶ 금액·일시 ──────────▶ 균형 검증
         고객 입금 반제       1000 현금      2,176,500            차변 = 대변
 ...
 [탐지]  정상 원장임에도 불구하고 3개의 탐지 룰 위반 발생
         ┣ L1-05 자기 승인   작성자 = 승인자     직무분리 위반 — 전결 규정이면 정당
         ┣ L3-05 주말 전기   일요일 전기         맥락 꼬리표 — 결산 전 주말 처리
         ┗ L3-04 기말 집중   월말 ±5일 창        맥락 꼬리표 — 기간 경계

 [목록]  대상(기말결산) × 수법(자기승인·수기입력) 충족 ─▶ 검토 목록에 오름
```

침묵한 룰에도 근거가 있다. 전기 시각이 오전 09:54라 심야 룰이 조용하고, 증빙 괴리가 0일이라 소급기표 룰이, 금액이 217만원이라 절대 고액 룰이 각각 조용하다.

**L1-05의 발화가 이 예시의 핵심이다.** 작성자와 승인자가 같은 사람이면 직무분리가 깨진다. 다만 이 원장에는 전결로 자기승인이 허용되는 비율이 의도적으로 들어 있어, 이 전표도 규정상 정당할 수 있다. 도구는 그 판단을 하지 않고 근거만 붙여 목록에 올린다.
