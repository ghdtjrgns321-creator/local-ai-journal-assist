...
- **31개 룰 기반 감사 검토 후보 선별**: L1(확정 오류/명시 위반 9개) + L2(강한 통제우회·자금유출 검토 신호 5개) + L3(검토 필요 이상징후 12개, L3-12 업무범위 집중 검토 포함) + L4(통계적 이상치 5개; L4-02 Benford macro는 PHASE1-2 family)
...
| LLM (Phase 3) | OpenAI (gpt-5.4 / gpt-5.4-mini 2티어) — Review Queue Narrator |
| NLP (Phase 3) | kiwipiepy (적요 형태소 분석, Narrator 보조 입력) |
...
Excel/CSV -> ingest (자동 매핑) -> feature (18개 파생변수) -> validation (L1~L3)
  -> detection (31개 룰 + XGBoost + VAE+IF) -> score_aggregator -> DuckDB
    -> Streamlit 대시보드 (Summary / Benford / Explorer)
      -> Phase 3 Review Queue Narrator (룰 히트+ML 스코어+전표 메타 → LLM 서술/우선순위)
```
...
> 상세: [docs/guide/ux-flow.md](docs/guide/ux-flow.md)

## Phase 로드맵

탐지는 C안 3-surface 구조다(근거 SoT: [docs/spec/PHASE1_TIER_EVIDENCE_BASIS.md](docs/spec/PHASE1_TIER_EVIDENCE_BASIS.md)): **PHASE1-1 룰**(전표 단위 결정론 룰) · **PHASE1-2 family**(graph/relational/시계열 구조 단위, 순환거래의 정식 집) · **PHASE2 VAE**(비지도 companion surface, 부정 확정 아님). 세 surface는 비병합(독립 탭/큐).

| Phase | 범위                                                     | 상태   |
|:------|:---------------------------------------------------------|:-------|
| 1a    | ingest + feature + validation + EDA 프로파일링            | 완료   |
| 1b (PHASE1-1) | detection 룰 (31개) + 순서형 tier + DuckDB + pipeline | 미착수 |
| PHASE1-2 | family 탐지기 (graph/relational/시계열 구조 단위)       | 미착수 |
| 1c    | Streamlit 대시보드 3탭                                    | 미착수 |
| 2a (PHASE2 VAE) | ML 전처리 파이프라인 (pipeline_builder, cv_selector, VAE) | 미착수 |
| 2b    | ML 탐지기 (SupervisedDetector, VAEDetector, SHAP)         | 미착수 |
| 2c    | 추가 탐지기 (Duplicate, Timeseries, Intercompany)         | 미착수 |
| 3     | Review Queue Narrator (LLM이 룰 히트+ML 스코어+전표 메타 해석, 의심 후보 + 근거 + 다음 행동 제안). Text-to-SQL/Export는 비범위 — 상세 [docs/archive/abandoned/PHASE3_REVIEW_NARRATOR_SPEC.md](docs/archive/abandoned/PHASE3_REVIEW_NARRATOR_SPEC.md), 완료 리포트 [docs/archive/abandoned/phase3_review_narrator_completion.md](docs/archive/abandoned/phase3_review_narrator_completion.md) | 완료 후 폐기 |
...
| 문서                         | 내용                                    |
|:-----------------------------|:----------------------------------------|
| dev/active/                  | 진행 중 작업별 plan/context/tasks       |
| docs/spec/DECISION.md        | 아키텍처/기술 선택 결정 로그            |
| docs/archive/completed/raw-plan/ | 기능 영역별 과거 구현 가이드         |
| docs/spec/DETECTION_RULES.md | 전체 탐지 룰 목록, 점수 체계, 컬럼 스키마 |
