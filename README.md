...
```
 검증 통과 원장
        ...
        ├─▶ 부정 계층      ── 승인·분할·자본화 등 14개 룰 ───────┤ 스레드풀 병렬
        ├─▶ 이상 계층      ── 시점·고액·역분개 등 11개 룰 ───────┤ 결과는 입력 순서로
        └─▶ 증빙 검사기    ── 수익 컷오프 ───────────────────────┘
                        │
                        ▼
 ...
 ※ 벤포드·계정 단위 신호는 같은 파이프라인에서 함께 돌지만
    행 점수 기여가 0이고 소속 표면이 다르다 → 분석적 검토(3-4)
```

**룰 설계 원칙**

- 판정은 위반 / 비위반 **둘 중 하나** — 룰이 위험 점수나 확률을 만들지 않는다
- 같은 데이터를 넣으면 언제나 같은 결과가 나온다 (학습·난수 없음)
- 임계는 전부 설정 파일에 둔다 — 전결 한도·결산 일정·근무시간은 회사마다 다르기 때문
- "컬럼이 없어 못 돈 룰"과 "돌았는데 0건인 룰"을 구분해 기록한다

**룰을 어떻게 도출했나**

1. **감리 지적 사례 원문 정독** — 금융감독원이 2009년부터 공개한 지적 사례 문서 231개
2. **전표 흔적으로 번역** — 사례마다 "이 조작이 GL 전표에 무엇을 남기는가"를 룰과 1:1 대조해 **731행 지도**로 정리
3. **조작 대상은 실증 빈도로 선별** — 추정계정 147 · 관계사 80 · 컷오프 23 · 비용자산화 22 · 기말결산 20 · 매출과대 20건
4. **조작 방법은 기준서에서 가져옴** — 감리 원문에서 확정된 것이 8건뿐이었다(직무분리 3 · 수기 3 · 승인생략 2). 원문이 "수기로 · 승인 없이 · 심야에" 같은 전표 통제를 거의 쓰지 않기 때문에, 이 축은 감사기준서 240의 부정 분개 특징으로 채웠다
5. **감리 사례 전부를 룰로 옮길 수는 없다** — 고위험 36건 중 **25건(69%)**은 실물 재고·부외자금·증빙 위조·타사 원장이라 GL 원장만 봐서는 신호가 존재하지 않는다
