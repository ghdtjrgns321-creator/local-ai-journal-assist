...
### 주제별 룰 — 무엇을 잡고 무엇을 예외로 두나

```
 ━━━ 데이터 정합성 — 부정 신호가 아니라 데이터 결함 ━━━

    L1-01  차대변 균형     전표 단위 차변 합 ≠ 대변 합 (허용 오차 0.01)
    L1-02  필수필드 누락   전표번호·일자·계정·금액 등 필수 칸이 공란
    L1-03  무효 계정       계정과목표에 없는 계정이 원장에 등장
    L1-08  기간 불일치     전기일의 월과 회계기간이 어긋남
           예외 : 회사가 특수기간을 쓰면 감사인이 엄격 모드를 끌 수 있다

 ━━━ 승인·권한 ━━━

    L1-04  승인한도 초과   승인자의 전결 한도보다 전표 금액이 큼
           예외 : 한도 경계 근처 · 시스템 계정 승인은 검토 대상으로만
    L1-05  자기 승인       작성자와 승인자가 같은 사람
           예외 : 자동·반복 전기는 면제 · 결산/자산 프로세스는 검토 대상
                  단 중요성 금액을 넘으면 예외를 취소하고 즉시 위반
    L1-06  직무분리 위반   한 사람이 상충하는 업무를 함께 수행
    L1-07  승인 생략       승인자 칸이 공란
           예외 : 자동·배치·인터페이스 전기는 제외 · 반복 전기는 검토 대상
                  (상시 승인이나 마스터 승인으로 대체됐을 수 있음)
 ...
 ━━━ 금액·분할 ━━━
    ...
    L2-03  중복 전표       90일 안에 같은 지급이 반복 (적요 유사도 80 · 금액 오차 2%)
    L4-01  매출 이상치     매출 계정(4xxx)에서 금액이 크게 튐
    L4-03  절대 고액       수행중요성 초과 (세전이익의 5%를 중요성으로, 그중 75%)

 ━━━ 시점 ━━━

    L3-04  기말 집중       월말 앞뒤 5일 창에 전기
    L3-05  주말 전기       토·일 전기
    L3-06  심야 전기       22시 ~ 06시 전기
    L3-07  전기-증빙 괴리  증빙일보다 30일 넘게 지나 전기
    L4-05  심야 배치       배치 단위로 볼 때 심야에 쏠림
    L4-06  배치 이상       배치 단위로 볼 때 기말·금액 분포가 치우침

 ━━━ 계정·거래 성격 ━━━

    L2-04  비용 자본화     비용 성격 계정(5~8xxx)이 자산 계정(12·15xxx)으로
    L2-05  역분개 거울쌍   금액이 같고 방향이 반대인 전표 쌍
           예외 : 입고미착 청산(2900) · 관계사 정산(1150·2050)은 정상 프로세스
    L3-02  수기 전환       입력 방식이 수기 또는 결산조정
    L3-03  관계사 꼬리표   관계사 계정쌍(1150↔2050 · 4500↔2700) 사용
    L3-09  가계정 미정리   가지급금·가수금·미결산 계정이 장기 잔존
                           계정코드 1190·2190·2900·9990 또는 적요 키워드로 판별
    L3-10  추정계정        경영진 추정이 들어가는 계정 — 대손충당금·손상차손·퇴직급여 등
    L3-11  수익 컷오프     기간 경계에서 수익 인식 시점이 어긋남
    L3-12  업무범위 집중   한 사람의 당기 활동 범위가 과도하게 넓음
    L4-04  희귀 차대쌍     그 원장에서 거의 나오지 않는 계정 조합
```
