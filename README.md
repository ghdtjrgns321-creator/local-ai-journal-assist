...
```
 ...
 ━━━ 계정 성격 ━━━
    ...
    L2-04  비용 자본화     비용 성격 계정(5~8xxx)이 자산 계정(12xxx, 15xxx)으로
...
- 분해값의 정확성 : 점수가 항목별 복구 오차의 합이므로 분해값이 정확값으로 존재, SHAP 등 근사 기법 미사용
- 근거의 타당성 : 가공 매출은 인도 부재 또는 증빙일 불일치를 수반, 기여도 1위 항목이 해당 간격
- 생성 흔적 학습 배제 : 전기시각, 요일, 월중일자 기여도 평탄, 이상 전표가 정상 전표 복제본이라 학습 대상 흔적 부재
...
<sub>룰 29종 중 L1-07-02(유령 승인자)는 L1-07(승인 생략)에 합산 계측되어 판정 분모는 28</sub>
...
| L1-04 승인한도 초과 · L2-01 한도 직하 분할 · L2-02 분할 거래<br>L3-07 전기-증빙 괴리 · L4-01 매출 이상치 · L4-03 절대 고액 |          0% |   0.01~0.6% |    **0** |
