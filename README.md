...
| 측정 항목                  | 결과                                   | 해석                                                                                                                                                   |
| -------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 룰 기반 이상 데이터 회수율 | 99.4\~100% (328\~330 / 330)            | 전체 이상 데이터 330건 중 328~330건이 검토 대상 등재, 정상을 더 많이 지목하는 무정보 룰(lift ≤ 1.0) 제외 시 89.7\~90.9% → [룰별 lift 표](#룰-기반-검증) |
...
- 룰별 판별력 지표 **lift** : 이상 데이터 발현율 ÷ 정상 데이터 발현율
  - **1.0 = 이상과 정상에 같은 비율로 발현**
  - lift가 높은 룰 : 정상 전표를 거의 지목하지 않아 검증률 높음
  - lift가 1.0 이하인 룰 : 쓰레기
...
- 편중 한계 : 상위 구간에 오르는 이상 데이터가 가공매출 계열에 몰림, 유령급여 등 다른 계열은 거의 오르지 않음
