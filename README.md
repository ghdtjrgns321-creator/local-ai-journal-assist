...
```
 ◆ 자기 큐 (별도 목록 5종)                  ◆ 꼬리표 (전표 줄에 표시만)
   벤포드 · 라운드넘버 밀집                   심야집중 · 배치입력 · 업무범위
   첫등장/희소 거래처                         라운드넘버 단건 · 첫등장 전표
   계정 활동 변동 · 결산월 집중 변동
        │                                          │
   계정·거래처 단위로 봐야 보이는 신호        정상 업무에도 흔해 단독 목록은 노이즈
   전표 목록과 분리된 자기 화면을 가짐        전표 옆 맥락으로만 붙음
        └────────────────┬─────────────────────────┘
                         ▼
   룰 표면과 합치지 않는다 — 이 신호들은 전표 목록의 순서에 관여하지 않음
   모집단 통계는 "이 전표가 문제"가 아니라 "이 계정·거래처를 보라"는 뜻이라서
```
