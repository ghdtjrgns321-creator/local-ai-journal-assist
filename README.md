# 감사 전표 테스트 자동화 — Local AI Audit Assistant

> ⚠️ **개발 중** — 분석 엔진과 검증 체계는 완주, Streamlit 화면은 다듬는 중이다.

![Python](https://img.shields.io/badge/Python-3.11+-3776AB?logo=python&logoColor=white)
![Rust](https://img.shields.io/badge/DataSynth-Rust%2015crates-000000?logo=rust&logoColor=white)
![DuckDB](https://img.shields.io/badge/DuckDB-회사·연도%20격리-FFF000?logo=duckdb&logoColor=black)
![Streamlit](https://img.shields.io/badge/Streamlit-6화면-FF4B4B?logo=streamlit&logoColor=white)
...
![VAE](https://img.shields.io/badge/행단위%20VAE-검증%20불성립-C62828)

---

## 개요

전표 수십만 건에서 **감사인이 먼저 볼 항목**을 고르는 1차 스크리닝 도구. 외부 전송 없이 노트북에서 돈다.

1. 생성 : 회계적으로 성립하는 합성 원장 확보 (Rust 생성기 · K-IFRS 중견 제조업)
2. 수집 : ERP마다 다른 컬럼명을 표준 형태로 → 파생변수 → 3계층 검증
3. 탐지 : 결정론 룰 · 모집단 신호 · 비지도 모델을 **각각 따로** 실행
4. 표면 : 감사인이 조합을 골라 검토 목록을 만듦 — 시스템은 등급을 선언하지 않음

```
   원장 CSV
       │
       ▼
 ━━━ 수집 · 검증 : ERP마다 다른 원장을 표준 형태로 ━━━

    헤더탐지 ─▶ 컬럼매핑 ─▶ 타입정제 ─▶ 파생변수 ─▶ 구조검증 ─▶ 회계검증
                                                      │          │
                                            둘 중 하나라도 치명이면 탐지 진입 차단

 ━━━ 탐지 : 세 표면을 합치지 않는다 ━━━

    ◆ 룰 29종           전표·행 단위 규칙 위반을 명명
      ┗ 조합 빌더       대상 10 × 수법 10 — 감사인이 조합을 고른다
    ◆ 모집단 신호 5종   계정·거래처를 모아야 보이는 쏠림
      ┗ 자기 큐 + 꼬리표                        (점수 미합산)
    ◆ 비지도 VAE        학습된 정상 분포 밖 비정형 후보
      ┗ 별도 실행 · 별도 표시                   (점수 미합산)

 ━━━ 적재 · 화면 ━━━

    DuckDB (회사·감사연도별 파일 분리) ─▶ 화면 6개
                                            │
                                            ▼
                               감사인이 조합을 고르고 최종 판단
```

세 표면의 점수를 하나로 합치지 않는 것이 이 설계의 핵심이다. 척도가 다른 신호를 더하면 큰 쪽이 작은 쪽을 가리고, 실패한 표면이 성공한 표면을 오염시킨다.

**실제 화면**

> **입력 원장 —** `journal_entries_2022.csv` · 전표 37,895건 · 총 거래금액 1.36조 · 활성 계정 39개 · 기표자 149명 · 거래처 1,270곳

<p align="center">
  <img src="images/screenshot-overview.png" width="860" alt="개요 화면 — 데이터 개요 KPI, 주요 거래 유형 도넛, 월별 전표 추이, 품질 진단 8항목, 세 표면 안내 카드">
</p>
<p align="center"><sub>분석 실행 전 품질을 먼저 진단한다. 수기 라인 18.3%는 <b>검토 권장</b>, 자기승인 363건(1.1%)은 <b>통제 취약</b>으로 표시하고, 차대변 대사·벤포드 적합은 통과로 둔다.</sub></p>

---

## 1. 문제 인식

외부감사에서 전표 테스트는 선택이 아니다. 감사기준서는 전 기간 분개에서 부정 위험이 높은 항목을 골라 검사하도록 요구한다.

문제는 규모다 — 중견 제조업 3개년 원장이 35만 행을 넘고, 실제로 열어볼 수 있는 것은 수십 건이다.
...
실제 부정은 금액이 아니라 **처리 방식**에 흔적을 남긴다. 수기 입력·승인 생략·심야 전기·기말 집중이 그 축인데 금액 정렬에는 들어오지 않는다.

### 통상적 접근과의 차이

| 구분      | 통상적 접근                          | 본 시스템                                           |
| --------- | ------------------------------------ | --------------------------------------------------- |
| 위험 판정 | HIGH/MEDIUM 등급 자동 부여           | **등급 비선언** — 근거 표기 가능한 어휘만 제공      |
| 신호 통합 | 룰·통계·ML 점수를 가중합해 단일 순위 | **세 표면 비병합** — 독립 실행 · 독립 표시          |
| 판정 기준 | 측정 후 결과를 보고 기준 설정        | **측정 전 대역 선언** — 대역 밖은 3분류 후 조사     |
| 완료 선언 | "전 룰 검증 완료" 같은 서술          | **스크립트 종료 코드** — 분모 고정 후 차집합 0      |
| 실패 처리 | 잘 나온 지표만 보고                  | 실패를 같은 비중으로 기록 · 과거 좋은 수치도 무효화 |

빅4 도구를 조사한 결과도 같은 방향이었다. EY·KPMG·PwC·Deloitte 네 도구 모두 룰·기준이 주력이고 ML은 보조이며, 비지도 딥러닝 단독 탐지는 실무에 없다.

---

## 2. 실증 예시 — 전표 한 장이 전 구간을 지나는 과정

정상 원장의 첫 전표를 끝까지 따라간다. 값은 전부 실제 레코드다.

**생성** — 시나리오 추첨에서 매입송장이 뽑힌다. 20종 중 가중치가 가장 크고, 실제 기업 원장에서 매입송장이 가장 흔하다는 사실의 반영이다.

  - 계정결정표가 차변 사무용품비를 계정 `6500`으로, 세액 라인을 매입부가세로 해소
  - 공급가액 **45,987,521원** · 세율 10% → 세액 4,598,752.1원 · 총액 50,586,273원
  - 2023-10-04 11:22:56(평일 오전) · 작성자 `CWHITE048`(중간 경력 회계담당 페르소나)
  - 전기 방식이 **반복 처리**라 개별 승인 기록이 없음 → 승인자 칸 공란
  - 차대 균형 검증 통과 → 문서 ID `ef8bb035-…` 확정 · 부정 라벨 **false**

**수집·파생** — 이 데이터는 이미 표준 컬럼명을 쓰므로 정확 일치 단계에서 해소되고 유사도 매칭은 개입하지 않는다. 스키마 51컬럼 대비 원장 73컬럼이라 생성기 메타 26개가 미매핑으로 남는다.

  - 금액축 : 계정 `6500` 그룹 안에서 이상 정도 계산 (그룹 표본 충분)
  - 시간축 : 11시 22분 → 정상 근무시간 · 주말·공휴일·심야 해당 없음 · 증빙 괴리 0일
  - 패턴축 : 둥근 금액 아님 (45,987,521은 유효숫자 8자리)

**검증** — 필수 10컬럼이 모두 있고 차·대변이 음수가 아니라 구조 검증을 통과한다. 전표 단위 차대 차액이 허용오차 0.01 이내라 회계 검증도 통과한다.

**탐지** — 부정이 한 건도 없는 정상 원장의 전표인데도 **세 룰이 발화한다.**

| 룰              | 왜 발화하나                   | 성격                                            |
| --------------- | ----------------------------- | ----------------------------------------------- |
| L1-07 승인 생략 | 승인자 칸이 공란              | 정당 — 반복 처리 전표는 개별 승인을 남기지 않음 |
| L3-04 기말 집중 | 전기일이 월초 5일 창에 들어감 | 정당 — 기간 경계 맥락 꼬리표                    |
| L4-03 절대 고액 | 4,598만원이 임계를 넘음       | **결함의 실물** — 아래                          |

침묵한 룰에도 근거가 있다. 시간이 정상 근무시간이라 주말·심야 룰이 조용하고, 증빙 괴리가 0일이라 소급기표 룰이, 가계정이 아니라 미정리 룰이 각각 조용하다.

**L4-03의 발화가 이 예시의 핵심이다.** 4,598만원은 중견 제조업 매입송장으로 이상한 금액이 아니다. 이 룰의 임계 산출이 깨져 있었다 — 마감분개 식별 키가 현행 데이터와 맞지 않아 순이익이 잘못 계산되고 임계가 연 2,018만~3,007만원까지 내려갔다. 전표 금액 90분위가 1,944만원이므로 사실상 상위 10%가 전부 걸린다.

**표면화** — 이 전표는 대상(이상고액·기말결산)과 수법(승인생략)을 모두 갖고 있어 조합 조건을 만족한다. 정상 전표가 검토 목록에 오르는 것이고, 이것은 1차 스크리닝의 정상 동작이다. 목록에 오른 것 중 무엇이 실제 문제인지는 감사인이 증빙으로 판단한다.

---

## 3. 합성데이터 — 검증할 원장을 직접 만든다

실제 기업 원장은 구할 수 없으므로 회계적으로 성립하는 원장을 생성한다. 생성기는 EY-ASU 오픈소스(Apache 2.0)를 K-IFRS 제조업 기준으로 개편한 Rust 워크스페이스 15 크레이트다.

```
 ━━━ [1] 설정 : YAML 17섹션 → 검증 함수 28개(비율∈[0,1] · 분포합 1.0±0.01 · 임계 오름차순) ━━━

    ◆ 함정 : 필드 전부 기본값 허용 → 섹션 누락이 에러가 아니라 조용한 달러 기본값 (§10)

 ━━━ [2] 생성 : 오케스트레이터가 8단계를 순서대로 ━━━

    계정과목표 ─▶ 마스터데이터 ─▶ 문서흐름 ─▶ 전표 생성 ─▶ 잔액 추적 ─▶ 출력
    감사룰이 이름으로  거래처·직원   3-way match   아래 [3]    자산=부채+자본   73컬럼 CSV
    참조하는 계정 먼저 ·자재·자산    가격 5%·수량 2%           +(수익−비용)     + 사이드카
                                    ·과납품 10%               허용오차 0.01

 ━━━ [3] 전표 한 건이 만들어지는 경로 ━━━

    ◆ 시나리오 추첨   20종 가중치 룰렛 (매입송장이 최대 가중치)
    ◆ 계정 결정       결정표 조회 ┳ 시나리오·방향·성격 3중키 예외 → 2건
                                  ┗ 없으면 기본 매핑 → 40여 건
    ◆ 금액            로그정규분포 · 하한 100원 · 상한 1,000억
    ◆ 일시            월말 2.5배 · 분기말 4.0배 · 연말 6.0배 · 오전 피크 1.8배
    ◆ 승인자          한도 이상 승인권자 중 선택 ┳ 생성자 본인 제외
                                                 ┗ 단 2%는 전결로 자기승인 허용
    ◆ 의미 게이트     시나리오별 허용 계정 화이트리스트 위반 → 생성 차단
    ◆ 균형            총차변 = 총대변
```
...
| 실무 예외는 남긴다   | 승인 생략 · 자기승인 2% · 결산기 야근 — 0으로 소독 금지 (룰 검증 불가) |
| 노이즈는 라벨과 무관 | 결측 2%(완전 무작위) · 오타 0.15% — 정상·부정에 **같은 비율**          |
| 계정은 무작위 아님   | 성격에서 계정으로 가는 결정표를 생성기에 내장                          |


### 부정 시나리오 — 금감원 감리 사례에서 따옴

정상 원장은 그대로 두고 별도 파일로 부정을 얹는다. 14개 수법이 각각 3~4단계 구성요소를 갖는다.
...
부정이 전표 한 장이 아니라 **여러 장에 걸친 이야기**로 심기는 것이 핵심이다. 가공매출은 매출 계상 → 가짜 회수 → 매출채권 할인 차입 → 차기 역분개 4단계로 들어간다.

그중 일부는 그 자체로 아무 규칙도 위반하지 않는 "다리" 전표이고, 이것이 표면 커버리지가 60% 안팎에 머무는 이유다.

지름길도 막았다. 부정 전용 계정을 새로 만들면 "본 적 없는 계정 = 부정"이 되므로 정상이 많이 쓰는 22개 계정으로 전면 교체했고, 시나리오 이름이 적요에 노출되지 않도록 고빈도 정상 헤더로 치환했다.

---

## 4. 룰 엔진 — 전표 단위 규칙 위반

탐지기 5종을 병렬로 돌리고 결과는 완료 순서가 아니라 **입력 순서로 재정렬**한다. 실행할 때마다 룰 순서가 바뀌면 같은 데이터에서 같은 화면이 나오지 않는다.

```
 검증 통과 원장
        │
        ├─▶ 정합성 검사기  ── 차대균형 · 필수필드 · 무효계정 ────┐
        ├─▶ 부정 계층      ── 승인·분할·자본화 등 14개 룰 ───────┤
        ├─▶ 이상 계층      ── 시점·고액·역분개 등 11개 룰 ───────┤ 스레드풀 병렬
        ├─▶ 벤포드 트랙    ── 첫자리 분포 (행 점수 0) ───────────┤ 결과는 입력 순서로
        └─▶ 증빙 검사기    ── 수익 컷오프 ───────────────────────┘
                        │
                        ▼
        각 탐지기는 실패해도 결과 객체를 반환한다
        ◆ 실행됨 / 건너뜀 / 오류  +  건너뛴 사유를 계약 필드로 보유
          ┗ "컬럼이 없어 못 돈 룰"과 "돌았는데 0건"은 전혀 다른 의미
                        │
                        ▼
        점수 정규화 = 신호강도 × 심각도 × 증거강도 × 역할 × 기여가중치
                        │
                        ├─▶ 행 종합 점수 : 계층별 최댓값의 가중합
                        │                  (L1 0.40 · L2 0.25 · L3 0.20 · L4 0.15)
                        └─▶ 표면 구성 : 흐름 단위 → 전표 단위
```
...
| 주제           | 룰                                                                    | 잡는 것                              |
| -------------- | --------------------------------------------------------------------- | ------------------------------------ |
| 데이터 정합성  | L1-01 · L1-02 · L1-03 · L1-08                                         | 원천 추출·매핑 단계의 구조 결함      |
| 승인·권한      | L1-04 · L1-05 · L1-06 · L1-07 · L1-07-02                              | 전결 한도 초과 · 자기승인 · 직무분리 |
| 금액·분할      | L2-01 · L2-02 · L2-03 · L4-01 · L4-03                                 | 한도 직하 분할 · 중복 · 이상 고액    |
| 시점           | L3-04 · L3-05 · L3-06 · L3-07 · L4-05 · L4-06                         | 기말 집중 · 휴일·심야 · 소급기표     |
| 계정·거래 성격 | L2-04 · L2-05 · L3-02 · L3-03 · L3-09 · L3-10 · L3-11 · L3-12 · L4-04 | 비용 자본화 · 역분개 · 수기 · 관계사 |
| 분포 통계      | L4-02                                                                 | 첫자리 분포 이탈 (계정·월 단위)      |
...
| 10  | **이 합성데이터 전용**  | 다른 ERP 원장은 사용자가 컬럼 매핑을 손봐야 함 — 무설정 투입 단계는 아님                           |
...
| 레이어          | 기술                                            | 비고                                              |
| --------------- | ----------------------------------------------- | ------------------------------------------------- |
| 합성데이터 생성 | Rust (워크스페이스 15 크레이트)                 | EY-ASU 오픈소스를 K-IFRS 제조업 기준으로 개편     |
| 언어·패키지     | Python 3.11+ / uv + pyproject dependency-groups | core·ml·dashboard·dev 그룹 분리                   |
| 수집·전처리     | pandas 2.x · openpyxl · rapidfuzz               | 컬럼 헤더 유사도 매핑                             |
