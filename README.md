...
| VAE 추가 회수              | 룰 기반 탐지가 놓친 124건 중 27건 회수, AUROC 0.8845 | 추가 회수는 가공매출로 한정                          |
...
1. [분석 범위와 다루는 자료](#1-분석-범위와-다루는-자료)
2. [실증 예시](#2-실증-예시---전표-1건-처리-과정)
3. 기술 설명
...
- 분석 대상 : 단일 법인 총계정원장 1개, 전표에 남는 신호만 사용
- 대조 제외 : 거래처 마스터, 은행 거래내역, 타사 장부 등 원장 외부 자료 미사용
- 범위 밖 수법 : 원장 외부 자료가 있어야 확인 가능한 수법은 분석 대상에서 제외
- 분석 단위 : 룰 기반 검증과 비지도 학습은 전표 단위, 분석적 검토는 계정·거래처·작성자 집계 단위
- 산출물 : 감사인이 우선 검토할 전표 목록, 부정 여부는 미확정
...
| 근거      | 사례 태깅 결과       | 적발 계정·거래 유형 전수 태깅        |      731행 | [3-2](#3-2-기술-설명---룰-기반-검증)     |
| 근거      | 감사기준서           | 240 부정 분개 특징·520 분석적 검토   |        2종 | [3-3](#3-3-기술-설명---검토-조합-구성)   |
| 원장      | 정상 데이터          | K-IFRS 제조업 3개년·73컬럼           |  355,786행 | [3-1](#3-1-기술-설명---합성-데이터-생성) |
| 원장      | 전표                 | 행을 전표 단위로 묶은 수             |  113,135건 | [3-1](#3-1-기술-설명---합성-데이터-생성) |
| 원장      | 거래 시나리오        | 구매·판매·급여·결산·자산·자금·관계사 |       20종 | [3-1](#3-1-기술-설명---합성-데이터-생성) |
| 원장      | 이상 데이터          | FS01~FS14·전표 330건                 |       14종 | [3-1](#3-1-기술-설명---합성-데이터-생성) |
| 분석 모델 | 룰                   | 데이터 정합성 4 + 이상 징후 25       |       29종 | [3-2](#3-2-기술-설명---룰-기반-검증)     |
| 분석 모델 | 검토 조합 어휘       | 조작 대상 9 × 조작 수법 10           | 프리셋 5종 | [3-3](#3-3-기술-설명---검토-조합-구성)   |
| 분석 모델 | 분석적 검토 지표     | 독립 목록 5 + 전표 꼬리표 5          |       10종 | [3-4](#3-4-기술-설명---분석적-검토)      |
| 검증 자산 | 정상 데이터 검증지표 | 일반 57 + 계정 9                     |       66개 | [4-1](#4-1-정상-데이터-베이스라인-검증)  |

---

## 2. 실증 예시 - 전표 1건 처리 과정

> 정상 데이터 첫 전표의 생성부터 검토 목록 등재까지 전 과정, 표기된 값은 모두 실제 생성 레코드
...
```
 ...
 [목록]  조작 대상(기말결산) × 조작 수법(자기승인·수기입력) 충족
         ─▶ 검토 대상 전표 목록에 등재
```

- 정상 동작 : 정상 전표의 검토 목록 등재는 1차 스크리닝의 정상 결과
- 최종 판단 : 목록 내 실제 문제 여부는 감사인이 증빙으로 확인
...
> - 생성기 본체 : Rust 벤더 코드로 저장소 외부에 위치, 빌드 바이너리와 위 설정만 사용
...
- 설정 관리 : 기업 규모, 거래 비중, 직원 수를 YAML 17개 파일로 관리, 로드 시 위 3축 우선 점검
...
| L4-04 희귀 차대쌍    | 미사용 계정 조합    | 차변·대변 계정 짝의 원장 내 출현 빈도        | 분기당 1회 미만     |

- 임계 외부화 근거 : 전결 한도 6단계, 결산 일정, 근무 시간이 회사마다 상이 → 동일 원장도 회사 설정에 따라 다른 결과 산출
...
| 라운드 넘버 밀집도 | 계정·전기월·작성자 | 그룹의 라운드 넘버 비율이 원장 전체 비율보다 높은지 이항검정                   |
...
- 폐기 근거 실측 : 100만원 단위 기준 적용 시 거래의 64.9%가 판정 대상에서 제외, 원장의 0.04%만 적발
...
| 단계         | 방식                                                         |
| ------------ | ------------------------------------------------------------ |
| 기준선       | 해당 원장 전체의 라운드 넘버 비율, 상수 미사용               |
...
- **기준선의 데이터 산출**
  - 라운드 넘버 기준선 : 정상 비율이 산업·회사마다 상이해 상수 대신 해당 원장 전체 비율 사용
  - 결산월 기준 : 12월 고정 없이 해당 원장에서 관측된 최대 회계기간으로 결정, 3월·6월 결산 법인 대응
...
- **명칭 표기 원칙**
  - 신규 거래처 대신 원장 첫 등장 : 거래처 마스터 등록일이 아닌 해당 원장 최초 등장일 기준
...
<p align="center"><sub>벤포드 첫째 자릿수, 원장 전체 적합 판정 후 계정별 이탈도 상위 별도 제시</sub></p>

<p align="center">
  <img src="images/screenshot-round-density.png" width="860" alt="라운드 넘버 밀집 차트 — 축별 수평 막대와 원장 기준선 4.1% 수직 점선, 계정·전기월·작성자가 색으로 구분">
</p>
<p align="center"><sub>라운드 넘버 밀집도, 점선은 해당 원장에서 산출한 기준선</sub></p>

<p align="center">
  <img src="images/screenshot-vendor-signal.png" width="860" alt="거래처 단위 신호 화면 — 첫등장·희소·휴면 후 재활성 정의와 신호 필터, 거래처별 신호·거래건수·총금액 목록">
</p>
<p align="center"><sub>거래처 단위 신호 3종, 가공거래처 확정이 아닌 원장 범위 내 근사 신호로 표기</sub></p>
...
- **허용 대역의 출처와 한계**
  - 설정 방식 : 실제 기업 원장 통계 확보 불가로 자체 실측과 LLM 휴리스틱 기반 설정, 외부 채점 아님
...
| 항목      | 내용                                                    |
| --------- | ------------------------------------------------------- |
| 배경 원장 | 정상 데이터의 2024회계연도 구간 119,205행               |
...
| ---- | ---------- | ------------------------------- | --------------------------------------------- |
| 1    | 2026-07-18 | 모델의 탐지 실패                | 데이터에 지름길 존재, 측정 자체가 무효        |
...
- 반대 방향 오류 : 원장 핵심인 차대변 금액이 2개월 전 근거로 차단 상태 유지
...
| 2   | 정답 자체 주입 구조   | 이상 데이터를 직접 주입 후 탐지하는 구조, 유리하게 심을 경우 자문자답 성립    |
...
- 2번 잔존 한계 : 주입한 대상을 탐지하는 구조 자체는 해소 불가
...
| #   | 한계                       | 내용                                                                             |
| --- | -------------------------- | -------------------------------------------------------------------------------- |
| 4   | 단일 법인 원장 단일 소스   | 은행 계좌망·타사 장부·거래처 마스터 부재, 순환거래·가공거래처·유령급여 확인 불가 |
...
- 4번 규모 실측 : 금감원 고위험 사례 36건 중 **25건(69%)** 이 실물 재고·부외 자금·증빙 위조·타사 원장 대사에 기인해 원장 내 신호 부재
...
| 10  | 현 합성 데이터 전용 배선 | 읽기 단계가 해당 원장 컬럼에 고정, 타사 원장은 코드 수정 필요   |
| 11  | 이상 데이터 주입 반작용  | 자기승인·수기 조건 충족 과정에서 승인생략 룰 판별력 0.64로 역전 |

- 8번 영향 범위 : 룰이 손익 총액을 미참조해 탐지 결과와 무관, 재무제표 현실성만 미달
