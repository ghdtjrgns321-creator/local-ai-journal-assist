...
```
 ━━━ [1] 설정 : 회사 규모·거래 비중·직원 수를 YAML 17개 묶음으로 ━━━

    ◆ 읽을 때 28가지를 먼저 확인 (비율이 0~1 안인가 · 분포 합이 1인가 · 임계가 오름차순인가)
    ◆ 함정 : 빠뜨린 항목이 에러가 아니라 조용히 기본값으로 채워짐 → 통화가 달러로 바뀐 사고 (§10)

 ━━━ [2] 생성 : 순서대로 8단계 ━━━

    계정과목표 ─▶ 마스터데이터 ─▶ 문서흐름 ─▶ 전표 생성 ─▶ 잔액 추적 ─▶ 출력
    룰이 이름으로     거래처·직원   주문·입고·송장   아래 [3]   자산 = 부채+자본   73칸 CSV
    찾는 계정 먼저   ·자재·자산     셋을 맞춰본다              + (수익−비용)     + 부속 파일
                                   가격 5%·수량 2%            오차 0.01 이내
                                   ·과납품 10% 어긋남

 ━━━ [3] 전표 한 건이 만들어지는 순서 ━━━

    ◆ 거래 종류    20종을 실제 빈도대로 뽑음 (매입송장이 가장 흔함)
    ◆ 계정         거래 종류·차대 방향·성격 3개로 계정을 찾음 (표에 없으면 기본 매핑)
    ◆ 금액         소액이 많고 고액이 드문 분포 · 100원 ~ 1,000억
    ◆ 일시         월말 2.5배 · 분기말 4배 · 연말 6배 · 오전에 몰림
    ◆ 승인자       한도 넘는 금액은 권한 있는 사람이 · 본인 승인 금지 (단 2%는 전결 허용)
    ◆ 금지 조합    이 거래에 쓸 수 없는 계정이 나오면 만들다 말고 버림
    ◆ 균형         차변 합 = 대변 합

 ━━━ [4] 현실성 검사 : 만든 원장이 진짜 회사 장부처럼 보이는가 ━━━

    검사 57개 + 계정 검사 9개 · 판정 통과 / 실패 / 보류 / 관찰 / 참고
    ┣ 복식부기   전표마다 차변 합 = 대변 합 · 자산 = 부채 + 자본 + 당기순이익
    ┣ 금액       원가율 0.55~0.92 · 마이너스 잔액 비율 2% 이하
    ┣ 세무       세액이 공급가액의 9~11%
    ┣ 승인       자동 처리 행은 배치번호가 있고 사람이 만든 행은 비어 있음
    ┣ 관계사     단일 법인 범위를 부속 파일까지 지켰는가
    ┗ 생성기 티  같은 값이 한 거래 종류에 100건 넘게 몰리거나 같은 시각이 50건 넘으면 적발
                 ┗ 실제로 두 번 걸렸다 (계정 162건 · 은행 거래처 3건)
```
