...
| 측정 항목                  | 결과                                                                                    | 해석                                                                                   |
| -------------------------- | --------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| 룰 기반 이상 데이터 회수율 | 최대 탐지율 : 99.4\~100% (328\~330 / 330)<br>실질 탐지율 : 89.7\~90.9% (296\~300 / 330) | 실질 탐지율 : 변별력 상실 지표(lift ≤ 1.0)를 전면 배제 → [룰별 lift 표](#룰-기반-검증) |
...
- 주입 규모 : 14개 시나리오, 전표 330건, 전체 정상 전표 113,135건의 0.29%
...
| 지표                  | 분석 단위              | 판정 대상                                                 |
| --------------------- | ---------------------- | --------------------------------------------------------- |
| 벤포드 법칙 이탈      | 계정, 발생월별         | 첫 자릿수 분포와 이론값의 평균절대편차                    |
| 라운드 넘버 집중도    | 계정, 발생월, 작성자별 | 전체 데이터 비율 대비 특정 그룹의 라운드 넘버 비율 분석   |
| 신규/희소 거래처 발생 | 거래처별               | 전기 미등장, 거래 건수 하위 10%, 직전 연도 휴면 후 재등장 |
| 계정별 활동성 변동    | 계정별(전기대비)       | 금액, 건수, 건당 금액의 변동 절대값 순                    |
| 월 집중도 변화        | 계정별(전기대비)       | 전기 월 비중 대비 당기 월 비중 분석                       |
...
```
 ━━━ 비지도 학습(VAE) 파이프라인 ━━━
  ...
  전표 데이터
    │
    ▼
  [1] 입력 변수 선정     데이터 컬럼 71칸 중 원본 속성 39개 채택 + 생성 파생 변수 11개 = 50개
    │                  
    ▼
...
| 사유            |  칸 | 해당 칸                                                                                       |
| --------------- | --: | --------------------------------------------------------------------------------------------- |
| 정답, 판정 결과 |   4 | 직무분리 위반 여부와 유형, 적요 품질, 한도 초과 여부                                          |
| 합성 생성 흔적  |   7 | 합성 여부, 변형 여부, 시나리오 ID 2종, 배치 ID와 유형, 잡 ID                                  |
| 전표 고유 값    |   7 | 전표 ID와 번호, 참조번호, IP, 행번호, 원전표 ID, 역분개 전표 ID                               |
| 결측치 과다     |  10 | 소인 2종, 미결제 잔액, 결제 상태와 일자, 역분개 사유 3종, 승인 한도 2종 (결측 98.2% ~ 100.0%) |
...
- 기여도 분해 신뢰성 검증 근거
  - 
- 정확 분해 : 점수 = 항목별 복구 오차 합, 별도 설명 도구 불필요
- 근거 일치 : 가공 매출 = 인도 부재 또는 증빙일 불일치, 기여 1위가 해당 간격
- 생성 흔적 배제 : 전기시각, 요일, 월중일자 기여 평탄, 이상 전표는 정상 전표 복제본
- 배정 편향 없음 : 사람, 승인 축 기여 0.1%
...
- 검증 결과 : **통과 28 / 실패 0**<br>
(총 355,786건의 정상 전표 투입 - 실측된 상위 항목 발췌)<br>
...
- 전체 룰 기준
  - 최대 발현율 : **328\~330건(99.4\~100%)** 
  - 실질 발현율 : **296\~300건(89.7\~90.9%)** (변별력 상실 지표(lift ≤ 1.0) 제외) 
- 룰별 판별력 지표 **lift** : 이상 데이터 내 발현율 ÷ 정상 데이터 내 발현율
...
- 제외 사유 : 초기에는 시스템이 위험도(High/Medium/Low)를 자동 부여하도록 설계 → 각 탐지 결과에 부여되는 가중치와 점수가 임의적임, 객관적인 통계적 근거가 부족

| 구분      | 변경 전                             | 변경 후                                              |
| --------- | ----------------------------------- | ---------------------------------------------------- |
| 산출 방식 | HIGH/MEDIUM/LOW/맥락 등급 자동 부여 | 시스템 등급화 폐지                                   |
