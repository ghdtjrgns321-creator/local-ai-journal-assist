...
총계정원장의 **전표를 표본 없이 전수 조사**하는 도구 — 감사 실증절차의 전표 테스트(Journal Entry Testing)를 로컬에서 자동화 · 외부 API 호출 없음

- 검사 대상 (경계와 한계 : [1. 문제 인식과 스코프](#1-문제-인식과-스코프))
  - 원장 : K-IFRS 제조업 3개년 합성 원장 355,786행 · 113,135전표 · 73컬럼
  - 부정 : 금융감독원 감리 지적사례에서 딴 14개 수법(FS01~FS14)을 정상 원장에 주입
- 세 검토 표면 — 보는 대상이 달라 셋을 따로 세움 (상술 : [4. 핵심 설계](#4-핵심-설계))
  - 룰 29종 — **이름 붙은 위반을 집는 역할** : 전표 한 장이 규칙을 어겼는지 이진 판정, 어긴 룰의 이름이 그대로 검토 사유
  - 분석적 검토 5종 — **모집단을 훑는 역할** : 전표 한 장으론 안 보이고 계정·거래처를 모아야 드러나는 쏠림을 별도 목록으로
  - 비지도 VAE — **룰로 미리 적어둘 수 없는 것을 올리는 역할** : 정상 분포를 학습해 그 밖을 후보로, 회수는 가공매출 계열에 한정
- 산출물의 성격
  - 검토 우선순위 : "먼저 볼 목록"이며 목록에 올랐다는 것이 부정이라는 뜻은 아님
  - 등급 비선언 : 위험 등급을 만들지 않음 — 근거 표기 가능한 어휘 제공과 정확한 집계까지가 책임
  - 표면 비병합 : 판정 단위·척도가 달라 한 점수로 합치지 않음 → **실패한 표면이 성공한 표면을 오염시키지 않음**
...
- **의무 절차와 규모의 불일치**
  - PCAOB AS 2401 · ISA 240 : 경영진의 통제 무력화 위험 때문에 **전 기간 분개**를 대상으로 선별 검사를 요구
  - 규모의 벽 : 중견 제조업 3개년 원장 35만 행 대비 감사인이 실제로 열어보는 것은 수십 건
- **금액 상위 정렬의 편향**
  - 고액 정상 거래가 목록을 채우고 소액으로 쪼갠 은폐는 통째로 빠짐
  - 부정의 흔적은 금액이 아니라 **처리 방식**(수기 입력 · 승인 생략 · 심야 전기 · 기말 집중)에 남는데 그 축이 정렬 기준에 없음
- **도구가 등급을 선언하면 감사인이 그 등급을 방어해야 함**
  - "이 전표는 HIGH 위험"의 근거가 "합성데이터에서 드물었다"이면 감사 증거가 아님
  - 이 프로젝트도 등급 체계를 구현했다가 근거가 무너지는 것을 확인하고 폐기 → [5-2](#2-위험-등급-자동-산출-폐지--사용자-주도-필터링)

#### 표본 검사에서 전수 검사로

| 구분             | 표본 검사             | 전수 스크리닝                       |
| ---------------- | --------------------- | ----------------------------------- |
| 검사 범위        | 금액 상위 N건 등 표본 | 원장 전 건                          |
| 선별 주체        | 정렬 기준 하나        | 룰 판정 · 모집단 통계 · 비지도 학습 |
| 놓치는 것        | 소액 분할 · 처리 방식 | 원장 밖 증거가 필요한 수법          |
| 대가로 생기는 것 | —                     | 전 건에 걸면 대량 발화              |

- 기계가 전 건을 판정하면서 표본 추출이 더는 필수 조건이 아님 — 이 도구가 서 있는 자리
- 전 건 적용의 대가 : 기말 집중 룰 하나가 정상 원장의 **44.5%** 에 발화
- 이 프로젝트의 답 : 등급으로 자르지 않고 **조합으로 좁힘** — 조작 대상 9 × 조작 수법 10 ([3-3](#3-3-기술-설명--룰-위반-조합-검토))

### 3. 이 프로젝트를 통해 얻을 수 있는 것

| 하려는 일      | 통상적 방법               | 이 도구                          |
| -------------- | ------------------------- | -------------------------------- |
| 볼 항목 고르기 | 금액 상위 N건 정렬        | 위반 룰의 조합으로 좁힘          |
| 처리 방식 축   | 정렬 기준에 없음          | 수기·승인생략·심야·기말이 1급 축 |
| 위험 판정      | 도구가 등급을 선언        | 등급 없음 · 감사인이 조합 선택   |
| 판정 근거      | 점수 산식(설명·방어 불가) | 감리 확정 건수 + 기준서 문언     |
| 실행 위치      | 원장을 외부로 업로드      | 로컬 · 회사·연도별 DB 격리       |

- **무엇이 달라지는가**
  - 금액으로는 안 보이는 것이 올라옴 : 희귀 차대 계정쌍은 부정에서 정상의 **810배** 발화(17.0% 대 0.02%)인데 금액이 작아 정렬에서 목록 밖
  - 좁힐수록 밀도가 오르는 것을 건수로 봄 : 비용자산화 조합 234건 중 25건(**10.7%**) 대 결산 조합 5만여 건 중 230여 건(**0.5%**)
  - 조서에 "왜 이 등급인가"를 쓸 필요가 없음 : 등급이 없어 방어할 휴리스틱 0 — 남는 것은 "추정계정(감리 확정 147건) + 수기 입력(기준서 240 명문)"이라는 사실 진술

### 4. 핵심 설계

```mermaid
graph LR
    L["원장 전 건<br/>355,786행 · 113,135전표"]
    L --> R["① 룰 29종<br/>전표 한 장의 규칙 위반"]
    L --> A["② 분석적 검토 5종<br/>계정·거래처 단위 쏠림"]
    L --> V["③ 비지도 VAE<br/>학습한 정상 분포의 밖"]
    R --> J["감사인이 조합을 고르고 판단"]
    A --> J
    V --> J
```

| 표면              | 판정 단위             | 근거                           | 무엇을 올리나    | 상세                                                                           |
| ----------------- | --------------------- | ------------------------------ | ---------------- | ------------------------------------------------------------------------------ |
| ① 룰 29종         | 전표·행               | 금감원 감리 731행 + 기준서 240 | 명명된 규칙 위반 | [3-2](#3-2-기술-설명--룰-위반-검증) · [3-3](#3-3-기술-설명--룰-위반-조합-검토) |
| ② 분석적 검토 5종 | 계정·거래처·작성자·월 | 감사기준서 520                 | 모집단 단위 쏠림 | [3-4](#3-4-기술-설명--분석적-검토)                                             |
| ③ 비지도 VAE      | 행                    | 정상 분포 학습                 | 학습한 정상의 밖 | [3-5](#3-5-기술-설명--비지도-vae)                                              |

#### ① 룰 — 감리 사례에서 뽑았다

```
 금융감독원 공개 감리 지적사례 원문 230건 (2009~2025)
        │  전수 태깅
        ▼
 사례 731행 ──▶ 적발된 대상 계정·거래 유형 ──▶ 몸통 9   조작의 대상
        │                                       추정계정 147 · 관계사 80 · 컷오프 23 …
        │
 감사기준서 240 부정 분개의 특징 ────────────▶ 특징 10   전표의 모양
        │                                       수기 · 자기승인 · 심야 · 희소계정쌍 …
        ▼
 검토 조합 = (몸통 중 1개 이상) AND (특징 중 1개 이상)
```
...
- 몸통 자격 : 감리 확정 1건 이상 **그리고** 감사기준서 240 조작대상 조항 매핑
- 특징 자격 : 기준서 240 특징 목록 매핑, 화면에 문언과 출처 병기
- 어휘 밖 활성 룰 : 조합이 아니라 커버리지 큐에서 룰별 전수로 소비
- 데이터 정합성 4종은 조합에서 제외 : 부정 징후가 아니라 데이터가 깨진 것이므로 별도 패널

#### ② 분석적 검토 — 전표 한 장으론 안 보이는 것

| 신호               | 단위               | 무엇을 보나                                          |
| ------------------ | ------------------ | ---------------------------------------------------- |
| 벤포드 이탈        | 계정·월            | 금액 첫 자릿수 분포가 자연 발생 분포에서 벗어난 계정 |
| 라운드넘버 밀집    | 계정·전기월·작성자 | 끝자리가 떨어지는 금액이 유독 몰린 그룹              |
| 첫등장/희소 거래처 | 거래처             | 당기 첫 등장 · 거래 극소 · 휴면 후 재활성            |
| 계정 활동 변동     | 계정·전기대비      | 전기 대비 금액·건수·건당 금액이 크게 달라진 계정     |
| 결산월 집중 변동   | 계정·전기대비      | 연간 금액 중 결산월 비중이 전기보다 늘어난 계정      |

- 검토 목록을 소유하지 않음 : 생성 경로에서 절단하지 않고 전량 전달, 절단 지점은 화면과 감사인이 결정
- 전표 점수 기여 0 : 3중으로 차단
...
| 항목                  | 값                                                           |
| --------------------- | ------------------------------------------------------------ |
| 문서 AUROC (4벌 평균) | **0.8845** · 폭 0.0154 · 시드 고정으로 재현                  |
| 룰이 놓친 부정 회수   | **27 / 124 (21.8%)**                                         |
| 회수되는 수법         | **가공매출 계열만** — 가공매출 42.9% · 순환거래 31.6%        |
| 회수되지 않는 수법    | 유령급여 · 부적절 자본화 · 충당금 누락 · 손상 회피 = **0건** |
| 비용                  | 9건을 얻기 위해 **1,134건 검토**                             |

- 주장은 수법 단위로만 : "룰이 놓친 부정의 21.8% 회수"가 아니라 **"룰이 놓친 가공매출의 34% 회수, 다른 계열은 회수 못 함"**
- 판정 이력 4회 : 이 값에 도달하기까지 세 라운드가 성립하지 않았음 → [3-5](#3-5-기술-설명--비지도-vae) · [5-3](#3-vae-판정--네-차례-정정-끝에-표면-유지)

### 5. 주요 성능 평가

정상 원장 · 표적 위반 주입 · 부정 시나리오 주입 3단계로 나눠 측정 (전 과정 : [4. 검증](#4-검증))
