...
```
 ━━━ 전표 ef8bb035 · 매입송장 · 45,987,521원 · 2023-10-04 11:22 ━━━

 [생성]  시나리오 추첨 ─▶ 계정 결정 ─▶ 금액·일시 ─▶ 균형 검증
         매입송장         6500          공급가액 45,987,521   차변 = 대변
         20종 중 최대     사무용품비    + 세액 4,598,752      부정 라벨 없음
         가중치
         ◆ 전기 방식이 반복 처리 → 승인자 칸 공란

 [읽기]  컬럼명이 표준이라 매핑 없이 통과
         ◆ 표준 51칸 밖의 생성기 메타 26칸은 미매핑으로 남음

 [파생]  금액 ─ 계정 6500 안에서 얼마나 튀는지 계산
         시간 ─ 11:22 정상 근무시간 · 주말·공휴일·심야 아님 · 증빙 괴리 0일
         패턴 ─ 둥근 금액 아님

 [검증]  필수 10칸 있음 · 차·대변 음수 아님 · 차대 차액 0.01 이내
         └─▶ 탐지 진입

 [탐지]  부정이 한 건도 없는 원장인데 룰 3개가 발화
         ┣ L1-07 승인 생략   승인자 공란         정당 — 반복 처리는 승인 기록을 안 남김
         ┣ L3-04 기말 집중   전기일이 월초 5일   정당 — 기간 경계 꼬리표
         ┗ L4-03 절대 고액   4,598만원 > 임계    결함 — 임계 산출이 깨져 있었다

 [목록]  대상(이상고액·기말결산) × 수법(승인생략) 충족 ─▶ 검토 목록에 오름
```
...
정상 전표가 검토 목록에 오르는 것 자체는 1차 스크리닝의 정상 동작이다. 목록에 오른 것 중 무엇이 실제 문제인지는 감사인이 증빙으로 판단한다.
