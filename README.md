...
```
 ━━━ [1] 환경 설정 : 기업 규모·거래 비중·직원 수 등을 YAML 17개 파일로 관리 ━━━
      ...
      ◆ 유효성 검증 : 설정을 읽을 때 28개 항목을 먼저 점검
                      ┣ 비율이 0~1 사이?    (100% 초과 방지)
                      ┣ 비중 합이 1?        (거래 20종 비중을 더해 1이 안 되면 나머지가 실종)
                      ┗ 임계가 오름차순?    (소액 < 중액 < 고액이 뒤집히면 구간이 무너짐)

 ━━━ [2] 생성 로직 : 8단계 순차 파이프라인 ━━━

    1  계정과목표    회사가 쓸 계정 목록을 먼저 만든다
                     ┗ 탐지 룰이 "가지급금 1190" 같은 실명으로 찾으므로 자동생성 계정보다 먼저 심는다
    2  마스터데이터  전표에 등장할 대상 — 거래처·고객·자재·자산·직원(승인 한도 포함)
    3  문서흐름      실제 구매·판매 과정을 흉내낸다 : 발주 ─▶ 입고 ─▶ 송장
                     ┗ 셋을 대사하고 현실처럼 어긋남을 섞는다 (가격 5% · 수량 2% · 과납품 10%)
    4  업무 이벤트   정상 진행 75% · 예외 20% · 오류 5%
    5  전표 생성     아래 [3]의 순서로 한 건씩
    6  이상치 주입   부정·오류를 심는 단계 — 정상 원장에서는 전부 0으로 꺼 둔다
    7  잔액 추적     만들면서 자산 = 부채 + 자본 + (수익−비용) 을 실시간 확인 (오차 0.01 이내)
    8  출력          73개 컬럼 CSV + 부속 명세서(거래처·문서흐름·세금·기간마감 등)

 ━━━ [3] 단위 전표 생성 메커니즘 ━━━

    ◆ 거래 유형   실제 발생 빈도에 비례하여 20종 중 추출 (매입 송장 최다 빈도)
    ◆ 계정 매핑   거래 유형·차대 방향·거래 성격 3요소 기반 할당 (미지정 시 기본 계정 처리)
    ◆ 금액 산출   Long-tail 분포(소액 다수, 고액 희소) 적용 (100원 ~ 1,000억 원)
    ◆ 일시 지정   기말 집중도 반영 (월말 2.5배 · 분기말 4배 · 연말 6배 가중치) 및 오전 시간대 편향
    ◆ 권한 승인   한도 초과 시 권한자 승인 배정 및 자기 승인(Self-approval) 배제 (단, 2% 예외 전결 허용)
    ◆ 제약 조건   해당 거래에 유효하지 않은 계정 조합 발생 시 생성 중단 및 해당 건 폐기
