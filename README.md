...
```
 ...
 ━━━ [3] 단위 전표 생성 메커니즘 ━━━
    ...
    ◆ 의미 검증   만든 전표를 11개 규칙으로 검사해 거래 성격과 어긋나면 버리고 다시 만든다
                  ┣ 적요는 인건비인데 구매 거래 · 감가상각인데 상대가 매입 거래처
                  ┣ 매출인데 상대가 고객이 아님 · 계정 성격과 적요 계열 불일치
                  ┗ 최대 25회 재생성, 그래도 못 만들면 생성 자체를 중단 (조용히 통과시키지 않음)
