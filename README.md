...
- 전표 수만 건에서 감사인이 먼저 볼 검토 큐를 만든다
- 표본 검사에서 전수 검사로 넘어간다 — 이 도구에서 AI가 맡는 역할이 이것이다
- 금융감독원 감리 지적사례로 룰을 만들고, 이진 분류와 비지도 모델로 이상치까지 잡는다

**막힌 지점**

- 실제 기업 원장을 구할 수 없어 합성데이터로 검증할 수밖에 없었다
- 그 합성데이터에서 VAE 성능 검증이 성립하지 않았다
- 비지도라 정답을 주지 않으니 무엇을 못 맞혔는지도 드러나지 않는다
- 실제 위반 데이터도 한 줄씩 보면 전부 정상이라 행 단위 모델로는 잡을 자리가 없다
- 부정은 전표 여러 장의 관계에 있는데, 그 관계를 학습시키기에 합성데이터가 부족하다
...
| 항목      | 방식                                                |
| --------- | --------------------------------------------------- |
| 위험 판정 | **등급 비선언** — 근거 표기 가능한 어휘만 제공      |
| 신호 통합 | **세 표면 비병합** — 독립 실행 · 독립 표시          |
| 판정 기준 | **측정 전 대역 선언** — 대역 밖은 3분류 후 조사     |
| 완료 선언 | **스크립트 종료 코드** — 분모 고정 후 차집합 0      |
| 실패 처리 | 실패를 같은 비중으로 기록 · 과거 좋은 수치도 무효화 |
