...
총계정원장 전 건에서 **감사인이 먼저 볼 전표**를 골라내는 1차 스크리닝 도구 · 외부 API 호출 없이 로컬 실행
...
- 세 검토 표면 (상술 : [4. 핵심 설계](#4-핵심-설계))
  - 룰 29종 : 전표 한 장의 규칙 위반 — 근거는 금감원 감리 731행 + 감사기준서 240
  - 분석적 검토 5종 : 계정·거래처 단위 쏠림 — 근거는 감사기준서 520
  - 비지도 VAE : 학습한 정상 분포의 밖 — 룰을 대체하지 않는 보조 표면
- 산출물의 성격
