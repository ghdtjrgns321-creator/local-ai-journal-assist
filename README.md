...
> 전수 전표 테스트로 **감사인이 먼저 볼 항목**을 고르는 1차 스크리닝 도구 · 외부 API 호출 없이 로컬 실행
...
- 전수 : 표본을 뽑지 않고 원장 전 건에 룰을 걸어, 걸린 것을 감사인이 조합으로 좁힘
- 검토 우선순위 : 산출물은 "먼저 볼 목록"이며 목록에 올랐다는 것이 부정이라는 뜻은 아님
- 부정 확정 아님 : 위험 등급을 만들지 않음 — 근거 표기 가능한 어휘 제공과 정확한 집계까지가 책임
- 세 표면 비병합 : 판정 단위·척도가 달라 한 점수로 합치지 않음 → **실패한 표면이 성공한 표면을 오염시키지 않음**
...
- **왜 필요한가**
  - 의무 절차 : PCAOB AS 2401 · ISA 240 이 경영진의 통제 무력화 위험 때문에 전 기간 분개 대상 선별 검사를 요구
