...
- **31개 룰 기반 탐지**: L1(확정 오류/명시 위반 9개) + L2(강한 부정 정황 5개) + L3(검토 필요 이상징후 11개) + L4(통계적 이상치 6개)
...
- **Layer C** (징후): 기말/기초 대규모, 주말/심야 전기, Benford 위반, 이상 고액 등 9개
...
| 1b    | detection (31개 룰) + DuckDB + pipeline                   | 미착수 |
...
```
data/journal/
  primary/datasynth/    # 메인: 합성 전표 (fraud 15종 + anomaly 46종 + labels.csv)
