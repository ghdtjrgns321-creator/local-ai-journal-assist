...
| 관문      | 이 전표에 실제로 일어난 일                                                                                                          |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| 생성      | 매입송장 추첨(20종 중 최대 가중치) · 계정 `6500` 사무용품비 · 공급가액 45,987,521원 + 세액 4,598,752원 · 2023-10-04 11:22 평일 오전 |
| 〃        | 전기 방식이 반복 처리라 승인자 칸 공란 · 차대 균형 통과 · **부정 라벨 없음**                                                        |
| 읽기      | 컬럼명이 표준이라 매핑 없이 통과 · 표준 밖 51→73칸 차이인 생성기 메타 26칸은 미매핑                                                 |
| 파생      | 계정 `6500` 그룹 안에서 금액 이상도 계산 · 정상 근무시간 · 주말·공휴일·심야 아님 · 증빙 괴리 0일 · 둥근 금액 아님                   |
| 검증      | 필수 10칸 모두 있음 · 차·대변 음수 아님 · 전표 차대 차액 0.01 이내 → 탐지 진입                                                      |
| 탐지      | 부정이 한 건도 없는 원장인데 **룰 3개가 발화** (아래)                                                                               |
| 검토 목록 | 대상(이상고액·기말결산)과 수법(승인생략)을 모두 갖춰 조합 조건 충족 → 목록에 오름                                                   |
