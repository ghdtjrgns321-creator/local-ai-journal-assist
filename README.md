...
```
 ...
 ━━━ [3] 단위 전표 생성 메커니즘 ━━━
    ...
    ◆ 거래 유형   업무 흐름 7종에 걸친 20개 시나리오를 발생 빈도에 비례하여 추출
                  ┗ 구매 2 · 판매 2 · 급여 2 · 결산 6 · 자산 3 · 자금 3 · 관계사 2
    ◆ 라인 수     2줄 66.7% · 4줄 16.6% · 3줄 5.8% — 전표당 평균 3.5줄
                  ┗ EY 원본 분포에서 10줄 이상 구간을 축소하고 2줄로 재배분 (원본은 평균 10.2줄)
    ◆ 계정 매핑   거래 유형·차대 방향·계정 성격 3개를 키로 계정결정표에서 계정을 찾는다
                  ┗ 후보가 여럿이면 고르지 않고 미해소로 남긴다
                    (무작위로 고르던 시절 임차료 계정에 소모품비가 기표된 사고를 막기 위함)
    ◆ 금액 산출   로그정규 분포 · 중앙값 약 120만원 · 하한 100원 · 상한 1,000억
                  ┗ EY 원본은 달러 전제(중앙값 1,097달러)라 원화 규모로 다시 잡은 값
    ◆ 일시 지정   월말 2.5배 · 분기말 4배 · 연말 1.5배 · 오전(08:30~11:30) 1.8배
                  ┗ 연말은 3배에서 내렸다 — 12월에 전표가 과도하게 몰려서
    ◆ 적요 생성   업무 프로세스·계정 성격별 한국어 문구 템플릿 2,117줄에서 조합
                  ┗ 약어·대소문자·오타 변형을 덧입혀 같은 문구가 그대로 반복되지 않게 한다
    ◆ 권한 승인   한도 초과 시 권한자 승인 배정 및 생성자 본인 배제 (단, 2% 예외 전결 허용)
    ◆ 의미 검증   거래 성격과 맞지 않는 조합 11가지를 생성 시점에 차단
                  ┗ 인건비 적요가 구매 거래에, 감가상각이 외상매입 거래처에 붙는 식 (위반 시 폐기)
    ◆ 대차 균형   차변 합계 = 대변 합계 완전 일치

 ━━━ [4] 정합성 검증 : 생성된 합성 원장의 실무 현실성 평가 ━━━

    총 66개 지표 (일반 57종 + 계정 9종) 검사 · 5단계 판정 (통과 / 실패 / 보류 / 관찰 / 참고)
    ┣ 복식부기    개별 전표 대차 일치 여부 및 재무상태표 등식(자산 = 부채 + 자본 + 당기순이익) 검증
    ┣ 금액 분포   매출원가율(55~92%) 정상 범위 및 부(-)의 잔액 비율 2% 이하 확인
...
숫자는 추첨 가중치로, 클수록 자주 뽑힌다.

| 흐름          | 시나리오                                                                   |
| ------------- | -------------------------------------------------------------------------- |
| 구매 P2P (2)  | 매입송장 300 · 대금지급 200                                                |
| 판매 O2C (2)  | 매출송장 250 · 고객입금 180                                                |
| 급여 H2R (2)  | 급여 발생 120 · 급여 지급 80                                               |
| 결산 R2R (6)  | 발생분개 120 · 마감분개 20 · 대손상각 2 · 퇴직급여 2 · 잡이익 2 · 역분개 0 |
| 자산 A2R (3)  | 취득 80 · 감가상각 80 · 개발비 자본화 5                                    |
| 자금 TRE (3)  | 차입 50 · 이자지급 50 · 임원 가지급 10                                     |
| 관계사 IC (2) | 관계사 매출 40 · 관계사 정산 40                                            |

역분개는 가중치가 0이라 추첨으로는 생성되지 않고 별도 경로로만 만들어진다.
