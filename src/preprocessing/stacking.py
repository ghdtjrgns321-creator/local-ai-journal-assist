...
Why: 8개 base model 점수를 Ridge(positive=True)로 결합하여 최종 anomaly_score 산출.
...
class StackingEnsemble(BaseEstimator):
    ...
    입력: (N, 8) base model 점수 행렬 (STACKING_BASE_MODELS 열 순서)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> StackingEnsemble:
        ...
        Args:
            X: (N, 8) base model 점수 행렬. 각 열은 STACKING_BASE_MODELS 순서.
            y: (N,) 이진 라벨 (0=정상, 1=이상).
        """
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        """(N, 8) → (N,) 이진 예측 (threshold=0.5)."""
        proba = self.predict_proba(X)[:, 1]
        return (proba >= 0.5).astype(int)

    def predict_proba(self, X: np.ndarray) -> np.ndarray:
        """(N, 8) → (N, 2) 확률 배열 [P(정상), P(이상)].
