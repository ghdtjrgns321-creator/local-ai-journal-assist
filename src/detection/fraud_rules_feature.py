...
def b08_manual_override(
    ...
    if "is_manual_je" in df.columns:
        result = df["is_manual_je"].fillna(False).astype(bool)
    elif "source" in df.columns:
        if not manual_sources:
            result = pd.Series(False, index=df.index)
        else:
            result = source_norm.isin(manual_sources)
    else:
        return pd.Series(False, index=df.index)
    ...
    control_bypass = result & (self_approval | skipped_approval | missing_approval_date)
    priority = result & (
        exceeds_threshold
        | abnormal_time
        | period_end
    ...
    bucket.loc[result & source_norm.eq("adjustment")] = "adjustment_population"
    bucket.loc[result & bucket.eq("none")] = "manual_population"
    bucket.loc[priority] = "manual_priority"
    bucket.loc[control_bypass] = "manual_control_bypass"

    score_series = pd.Series(0.0, index=df.index, dtype="float64")
    score_series.loc[result] = 0.35
    ...
    flagged_index = result[result].index
    row_annotations: dict[object, dict[str, object]] = {}
    optional_columns = [
        column
        for column in (
    ...
    bucket_values = bucket.loc[flagged_index].astype(str).to_numpy()
    score_values = score_series.loc[flagged_index].round(4).to_numpy()
    source_bucket_values = np.where(
        source_norm.loc[flagged_index].eq("adjustment").to_numpy(),
        "adjustment",
        "manual",
    )
    ...
    result.attrs["breakdown"] = {
        "flagged_rows": int(result.sum()),
        "manual_rows": int((result & ~source_norm.eq("adjustment")).sum()),
        "adjustment_rows": int((result & source_norm.eq("adjustment")).sum()),
        "priority_rows": int(priority.sum()),
        "control_bypass_rows": int(control_bypass.sum()),
        "source_counts": source_norm.loc[result].value_counts().to_dict(),
        "bucket_counts": bucket.loc[result].value_counts().to_dict(),
