...
def _estimate_account_name_text(df: pd.DataFrame) -> pd.Series:
    """Return lowercased account-name text from account_name/gl_account_name."""

    parts: list[pd.Series] = []
    for column in ("account_name", "gl_account_name"):
    ...
    return combined.str.strip().str.lower()


def _estimate_account_mask(
    df: pd.DataFrame,
    *,
    account_name_keywords: tuple[str, ...],
    exact_accounts: tuple[str, ...],
    account_prefixes: tuple[str, ...],
    name_text: pd.Series | None = None,
    gl: pd.Series | None = None,
) -> tuple[pd.Series, pd.Series, pd.Series, pd.Series]:
    """Return (combined, keyword, exact, prefix) masks for estimate-account match.

    Why: 추정계정 코드는 회사 CoA마다 달라(§3) account_name 키워드(1차)로 잡고,
    클라이언트 확정 코드/prefix(2차)를 OR로 보강한다. 둘 다 비면 전부 False.
    """

    if name_text is None:
        name_text = _estimate_account_name_text(df)
    if gl is None:
        gl = (
    ...
    for keyword in account_name_keywords:
        lowered = keyword.lower()
        if lowered:
            keyword_mask = keyword_mask | name_text.str.contains(lowered, regex=False)
    ...
    return keyword_mask | exact_mask | prefix_mask, keyword_mask, exact_mask, prefix_mask


def _estimate_account_match_annotations(
    df: pd.DataFrame,
    *,
    account_name_keywords: tuple[str, ...],
    exact_accounts: tuple[str, ...],
    account_prefixes: tuple[str, ...],
    estimate_account_groups: dict[str, tuple[str, ...]],
) -> tuple[pd.Series, dict[int, dict[str, str]], dict[str, int]]:
    """Return L3-10 binary mask plus per-row factual annotations (no 정황 분류)."""
    ...
    mask, keyword_mask, exact_mask, prefix_mask = _estimate_account_mask(
        df,
    ...
    for row_index in df.index[mask]:
        name_value = name_text.loc[row_index]
        gl_value = gl.loc[row_index]
        if keyword_mask.loc[row_index]:
            match_type = "keyword"
            matched_value = next(
                (kw for kw in account_name_keywords if kw.lower() in name_value), ""
            )
        elif exact_mask.loc[row_index]:
            match_type = "exact"
        ...
        annotations[int(row_index)] = {
            ...
            "matched_group": _matched_estimate_group(name_value, estimate_account_groups),
        }

    breakdown = {
        "keyword": int(keyword_mask.sum()),
        "exact": int((exact_mask & ~keyword_mask).sum()),
        "prefix": int((prefix_mask & ~keyword_mask & ~exact_mask).sum()),
    }
    return mask, annotations, breakdown


def _matched_estimate_group(
    name_text: str,
    estimate_account_groups: dict[str, tuple[str, ...]],
) -> str:
    """Return the configured estimate-account group whose keyword matches the name."""

    for group_name, keywords in estimate_account_groups.items():
        for keyword in keywords:
            if keyword and keyword.lower() in name_text:
                return group_name
    return ""


def _get_estimate_account_config(
    audit_rules: dict | None = None,
) -> dict[str, object]:
    """Load L3-10 estimate-account policy (account-name keyword first, client codes optional)."""

    rules = audit_rules or get_audit_rules()
    patterns = rules.get("patterns", {})
    cfg = patterns.get("estimate_account_use", {})
    groups_raw = cfg.get("estimate_account_groups", {})
    ...
    for group_name, group_cfg in groups_raw.items():
        if isinstance(group_cfg, dict):
            ...
            raw_keywords = group_cfg
        else:
            raw_keywords = []
        groups[str(group_name).strip().lower()] = tuple(
            str(v).strip() for v in raw_keywords if str(v).strip()
        )
    return {
        "account_name_keywords": tuple(
            str(v).strip() for v in cfg.get("account_name_keywords", []) if str(v).strip()
        ),
        "accounts": tuple(str(v).strip().lower() for v in cfg.get("accounts", [])),
        "account_prefixes": tuple(str(v).strip().lower() for v in cfg.get("account_prefixes", [])),
        "estimate_account_groups": groups,
    }
...
def _get_work_scope_excess_config(audit_rules: dict | None = None) -> dict[str, object]:
    ...
    estimate = _get_estimate_account_config(audit_rules)
    ...
    return {
        ...
        "sensitive_accounts": estimate["accounts"],
        "sensitive_account_prefixes": estimate["account_prefixes"],
        "sensitive_account_keywords": estimate["account_name_keywords"],
    }
...
def b14_work_scope_excess_review(
    ...
    # Why: L3-12 sensitive_account reason은 L3-10과 같은 추정계정(키워드+코드) 정의를 공유한다.
    sensitive_account, _, _, _ = _estimate_account_mask(
        df,
        account_name_keywords=tuple(cfg["sensitive_account_keywords"]),
        exact_accounts=tuple(cfg["sensitive_accounts"]),
        account_prefixes=tuple(cfg["sensitive_account_prefixes"]),
        gl=gl,
    )
...
def b13_estimate_account_use(
    df: pd.DataFrame,
    audit_rules: dict | None = None,
) -> pd.Series:
    """L3-10 estimate-account use (binary flag).

    추정계정(회계추정치 ISA 540) 접촉 시 1.0, 아니면 0.0. 정황 차등은 통합점수 소관.
    """

    cfg = _get_estimate_account_config(audit_rules)
    result, annotations, reason_counts = _estimate_account_match_annotations(
        df,
        account_name_keywords=cfg["account_name_keywords"],
        exact_accounts=cfg["accounts"],
        account_prefixes=cfg["account_prefixes"],
        estimate_account_groups=cfg["estimate_account_groups"],
    )
...
def b06_self_approval(
    ...
    if "user_persona" in df.columns and allow["user_personas"]:
        allowed = allowed | (
            _cached_text(df, "user_persona", cache).isin(allow["user_personas"]) & ~lone_automated
        )
    if "source" in df.columns and allow["sources"]:
...
def b07_segregation_of_duties(
    ...
    person_processes = (
        process[valid]
        .groupby(person[valid])
        .agg(lambda values: frozenset(value for value in values.unique() if value))
    )
