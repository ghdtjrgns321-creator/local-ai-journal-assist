...
> **단위 정책 (2026-05-18 확정)**: 본 문서의 recall / precision은 모두 **document(전표) 단위**다. PCAOB AS 2401·MindBridge 등 외부 감사 표준 단위와 일치시킨다. PHASE1 case 묶음은 감사인의 검토 효율을 위한 UI 단위일 뿐 외부 보고 KPI 단위가 아니므로 본 문서에서는 사용하지 않는다. 결정 근거: `docs/spec/TROUBLESHOOT.md` TS-12.
...
새 UI 등급 기준 상세: `docs/guide/users/12_RANK_PERCENTILE_REVIEW_BANDS.md`
...
| 큐 진입 truth 전표 | 540 (80건 PHASE1 case 미진입 → `docs/spec/TROUBLESHOOT.md` TS-13) |
...
- recall 90% 이상은 **PHASE1 case 미진입 truth 80건 때문에 현재 case queue 자체에서 달성 불가**다. 정렬 개선만으로 회수 불가. → `docs/spec/TROUBLESHOOT.md` TS-13 (PHASE1 룰 커버리지 갭).
- recall ceiling = (큐 진입 truth 540) / (원본 620) = **87.10%**. 출처: `docs/spec/TROUBLESHOOT.md` TS-13, `artifacts/ts13_uncovered_truth_80_analysis.json`.
...
본 §8 의 family complementarity 표는 PHASE2 5 family 결합 ranking 설계의 정성 근거가 되었다. 결합 설계는 다음과 같이 lock 됐다 — 자세한 거버넌스는 `docs/spec/PHASE2_GOVERNANCE_DESIGN.md` 결정 8 참조.
...
| 거버넌스 출처 | `docs/spec/PHASE2_GOVERNANCE_DESIGN.md` 결정 8, `docs/archive/completed/phase2-family-ranking/` plan/context/tasks |
...
- V4 PHASE1 측정 비교 원본: `docs/archive/completed/DETECTION_RESULTS_MANIPULATION_V4.md`
