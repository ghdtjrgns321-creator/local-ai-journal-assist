...
- PHASE1/PHASE2/PHASE3는 각각 룰, 비지도 이상 점수, 설명/절차 제안 계층이며 감사인 판단을 대체하지 않는다.
...
- 8B 양자화 로컬 LLM은 한국어 회계 도메인 이해력이 부족하여, Text-to-SQL 운영 보조 품질이 제한적한다.
...
PHASE1 로컬 실행의 의미는 원본 ERP 데이터를 외부로 내보내지 않고, 전체 모집단에서 규칙 위반 후보를 먼저 넓게 확보한다는 것이다. 이 단계의 결과는 최종 감사 판단이 아니며, PHASE2/PHASE3 또는 감사인 검토에서 중요성·예외 정책·업무 맥락으로 재분류한다.
...
- LLM 룰 피드백 루프: [08-llm.md](completed/raw-plan/08-llm.md) §Audit Rules 피드백 루프 (historical v1; Phase 3 v2에서는 비범위로 결정, [PHASE3_REVIEW_NARRATOR_SPEC.md](PHASE3_REVIEW_NARRATOR_SPEC.md))
