...
| 전표 규모      | 319,204건 / 1,107,720 라인아이템 (전표당 3.47행)      |
| 금액 분포      | LogNormal(14.0, 2.5) — 라인 중앙값 ~33.6만원, 평균 ~1,706만원 |
| 승인 한도      | 6단계 전결규정 (자동→담당자→팀장→본부장→CFO→이사회)    |
| 사용자 풀      | 1,365명 사용 / 1,422명 마스터 (5개 페르소나, 한국식 로마자 ID), SoD 위반 3.32% |
| 시간 패턴      | 한국 근무 문화 반영 (심야 1.5%, 오전 피크 29.7%, 야근 13.1%) |
| 이상 주입      | fraud 1.96% (6,262건, 15종) + anomaly 2.60% (8,294건, 46종) + anomaly_labels.csv 8,337건 (5개 카테고리) |
