...
**구현 의존 그래프:**
```
01-project-setup → 10-sample-data → 02-ingest → 03-feature → 04-validation
                                                                    ↓
                                                              05-detection → 06-db
                                                                               ↓
                                                                         07-dashboard
...
Excel/CSV → file_validator → excel_reader → header_detector → column_mapper → type_caster
  → 표준 DataFrame → feature/engine (11개 파생변수) → validation (3-Level)
