...
감사기준서 330호           실증절차, 재무제표-장부 대사              → 22개 룰 전체 (실증절차)
...
22개 룰 전체가 330호가 요구하는 전표 실증절차를 구현한 것이다.
통제테스트(TOE) vs 실증절차: 내부통제 양호 → TOE 표본 검사, 미흡 → 실증절차 확대.

```
→ 22개 룰 = 330호 실증절차의 자동화
...
현재 22개 룰은 전표 데이터 기반 패턴 탐지에 집중하며, 증빙 대조는 미포함.
...
| 컬럼명       | 타입 | 설명          | 분포                    |
|-------------|------|---------------|-------------------------|
| `is_fraud`  | bool | fraud 여부     | True 2.1%, False 97.9%  |
| `is_anomaly`| bool | anomaly 여부   | True 7.7%, False 92.3%  |
...
현재 29개 컬럼에는 없지만, DataSynth에 컬럼을 추가 생성하면 구현 가능한 탐지 항목:
- 증빙 관련: `has_attachment`, `supporting_doc_type`, `invoice_amount`, `delivery_date`
- 승인 관련: `approved_by`, `approval_timestamp`, `approval_level` (approval.rs 활성화)
