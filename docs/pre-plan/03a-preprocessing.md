...
### ② sklearn Pipeline 전처리 — ⬜ 구현 예정 (Phase 2)
...
- **profiler.py:** 수치/범주/시간형 컬럼별 프로파일링 + 대용량 샘플링 동작 검증 ⬜
- **report.py:** EDAProfile → 대시보드 요약 변환 + JSON 직렬화 ⬜
- **sklearn Pipeline:** Pipeline별 fit/predict + cross_val_score 동작 검증 ⬜
