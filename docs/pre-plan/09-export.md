...
원본 데이터 기준: DataSynth v1.2.0 (106,489건 전표, 1,106,356 라인아이템, 39컬럼).
...
| 프로세스별 분포     | `general_ledger` GROUP BY | P2P(23.3%), O2C(25.9%), R2R(26.5%), H2R(8.9%), TRE(8.5%), A2R(6.9%) |
| 법인별 분포         | `general_ledger` GROUP BY | C001(본사), C002(울산공장), C003(천안공장)  |
| 이상징후 요약       | `anomaly_flag_summary`   | 이상 전표 7.5%, 부정 의심 1.9%, SoD 위반 1.0% |
...
- KPI 카드: 전표 건수, 이상 비율(7.5%), 부정 의심(1.9%), SoD 위반(1.0%)
...
- 시간대별 분포 히트맵 (심야 0.8%, 오전피크 36.8%, 마감러시 22.1%)
...
DataSynth 기준 1,106,356 라인아이템은 Excel 행 제한(1,048,576)을 초과할 수 있다.

| 전략           | 적용 시트       | 방법                                                  |
|----------------|----------------|-------------------------------------------------------|
| 전표 단위 집계 | Raw Data       | document_id 기준 1행으로 집계 (106,489행, 제한 이내)  |
