...
`column_mapper.py` — rapidfuzz로 원본 컬럼명을 표준 스키마에 매핑한다.

**구현할 것:**
- `keywords.yaml`에서 각 표준 컬럼의 알려진 이름(별칭) 로드
- `rapidfuzz.process.extractOne`으로 최적 매칭
- threshold(80%) 미만이면 `unmapped`에 추가, `needs_review=True`
- `MappingResult` dataclass: mapping, confidence, unmapped, needs_review

**매핑 확인 UI (Phase 1c):**
```
confidence ≥ 80%  → 자동 확정 (초록)
40% ≤ conf < 80%  → 추천 + 사용자 확인 (노랑)
conf < 40%        → 수동 선택 (빨강)
```
- 필수 9컬럼 미매핑 시 진행 차단
- DataSynth CSV는 헤더 일치 → UI 스킵
