...
또한 NLP & Transformer를 통해 **텍스트 데이터(적요, 계약서 등)**에서
...
```
사례: 횡령범이 1,000만원을 빼돌리면서 금액은 정상 입력
  - 숫자 분석: 금액 정상, 계정 정상, 날짜 정상 → 룰/ML 미탐지
  - 텍스트 단서: 적요란에 "접대비(A유흥주점)" 기재
  - NLP 분석: "유흥주점" 문맥 파악 → 부정 징후 탐지
```
...
| Phase  | 방식                     | 기술              | 한계                          |
|:-------|:-------------------------|:------------------|:------------------------------|
| 1b     | 키워드 정확 매칭          | if "유흥" in text | 은어·동의어·우회 표현 못 잡음  |
| 3      | 형태소 분석 + 임베딩      | kiwipiepy + LLM   | 문맥 이해, 의미 유사도 탐지    |

### Transformer의 역할

Transformer는 NLP의 핵심 엔진이다. ChatGPT, Qwen3 등 현대 LLM의 기반 아키텍처.

- **키워드 매칭**: "유흥" 단어가 있으면 잡음. "룸살롱"은 못 잡음.
- **Transformer 기반 NLP**: "룸살롱"과 "유흥"이 의미적으로 유사함을 이해. 은어·동의어·우회 표현 탐지 가능.
...
| 유형           | 키워드 매칭 결과 | NLP/Transformer 결과 | 설명                              |
|:---------------|:----------------|:---------------------|:----------------------------------|
| 동의어         | ❌ 미탐지        | ✅ 탐지               | "기프트카드" = 상품권              |
| 은어           | ❌ 미탐지        | ✅ 탐지               | "일시 보관금" = 가수금(가계정)     |
| 우회 표현      | ❌ 미탐지        | ✅ 탐지               | "임직원 선물비", "경조사 지원"     |
| 영문 혼재      | ❌ 미탐지        | ✅ 탐지               | "gift card", "suspense clearing"  |
| 정보 부재      | △ 길이만 판정   | ✅ 문맥 판정          | 적요 "잡손실" — 짧지만 의미 부족   |
...
- **근거**: 감사기준서 315호, 240호
- **입력**: `line_text`, `header_text`, `gl_account`, `auxiliary_account_number`
- **로직**: 적요 텍스트에서 거래 성격을 NLP로 추론하고, 계정 분류와 불일치 시 플래그
  - 예: 적요에 "대여금" 키워드 + 계정이 매출(4xx) → 실질 불일치 의심
- **한국 실무**: kiwipiepy 형태소 분석으로 한국어 적요 처리
...
- **로직**: LLM이 탐지된 이상 전표를 분석하여 "사업상 합리성" 보조 의견 제공
  - 프롬프트: "이 전표(계정: {gl_account}, 금액: {amount}, 적요: {text})의 사업상 합리성을 평가하세요"
