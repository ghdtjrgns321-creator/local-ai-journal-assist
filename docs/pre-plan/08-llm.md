...
# 부정 유형 (fraud_type, 16종)
vn.train(documentation="""
fraud_type 값 (총 2,046건, 1.9%):
  DuplicatePayment(397), FictitiousTransaction(351), RevenueManipulation(299),
  SplitTransaction(294), TimingAnomaly(196), UnauthorizedAccess(178),
  ExpenseCapitalization(105), SuspenseAccountAbuse(100),
  JustBelowThreshold(29), RoundDollarManipulation(28),
  ExceededApprovalLimit(20), SelfApproval(19), ImproperCapitalization(11),
  SegregationOfDutiesViolation(10), FictitiousEntry(7), FictitiousVendor(2)
""")

# 이상징후 유형 (anomaly_type, 상위 10종)
vn.train(documentation="""
anomaly_type 값 (총 8,001건, 7.5%):
  NewCounterparty(1,312), UnusualAccountPair(1,086), MissingRelationship(896),
  DormantAccountActivity(826), UnmatchedIntercompany(704),
  TransferPricingAnomaly(472), CentralityAnomaly(444),
  CircularTransaction(416), CircularIntercompany(233), UnusualTiming(183)
""")

# SoD 충돌 유형 (sod_conflict_type, 6종)
vn.train(documentation="""
sod_conflict_type 값 (총 12,419건, 11.7%):
  preparer_approver(10,842), system_access_conflict(1,040), requester_approver(175),
  payment_releaser(140), journal_entry_poster(74), master_data_maintainer(72),
  reconciler_poster(71)
""")

# GL 계정 체계
vn.train(documentation="""
gl_account 체계 (402개 GL 계정, K-IFRS):
...
전표 규모: 106,489건(document_id), 1,104,914건(라인아이템)
""")
```
...
class InsightGenerator:
    ...
    def generate_summary_insight(self, pipeline_result) -> str:
        ...
        예: '106,489건 전표 중 2,046건(1.9%)이 부정으로, 8,001건(7.5%)이
            이상징후로 탐지되었습니다. 부정 유형별: DuplicatePayment 397건(19.4%),
            FictitiousTransaction 351건(17.2%), RevenueManipulation 299건(14.6%)이
            상위 3종입니다. SoD 위반 12,419건 중 preparer_approver 충돌이
            10,842건(87.3%)으로 가장 빈번합니다.'
        """
    ...
    def generate_benford_insight(self, benford_result: dict) -> str:
        ...
        예: '1,104,914 라인아이템의 first_digit 분포는 MAD 0.008로
...
호출 조건:
  ...
  - 예상 대상: 부정 2,046건 + 이상징후 8,001건 중 HIGH 이상 = ~3,000~5,000건
