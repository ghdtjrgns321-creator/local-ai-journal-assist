...
│     └ 매핑 엄격도 슬라이더 (확정≥80 / 추천≥40 조정) + 중복 금액 퀵픽스
│  ③ 타입 캐스팅 (자동 완료)
...
| 라인아이템         | 1,106,356건 (전표당 평균 10.4라인)            |
...
| 지표              | 건수    | 비율   | UX 영향                              |
|:------------------|:--------|:-------|:-------------------------------------|
| 이상징후 전표     | 7,959   | 7.5%   | 탐지 결과 목록의 기본 볼륨           |
| 부정 전표         | 2,008   | 1.9%   | 고위험 플래그 대상                   |
| SoD 위반          | 1,080   | 1.0%   | 통제 위반 탭 대상                    |
| 차대변 불일치     | 44      | 0.04%  | A01 무결성 검증 대상                 |
...
**DB 스키마 (Phase 1c 구현 시 `src/db/schema.py`에 추가):**
...
| UX 단계 | 적용 항목                                                    |
|:--------|:------------------------------------------------------------|
| 1단계   | 인코딩=Auto, 시트=최고점수 자동선택, fuzzy_threshold=80(확정)/40(추천 경계) |
...
| 4단계   | 민감도 조절 + HITL + 프리셋     | 1c ⬜               | AuditSettings 오버라이드 + whitelist 테이블 + preset YAML | [07-dashboard.md](07-dashboard.md)           |
