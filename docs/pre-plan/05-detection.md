...
검증 완료된 DataFrame에 **3레이어 24개 룰(if/threshold 기반)**을 적용하여 이상을 탐지하고,
...
② integrity_layer.detect(df)                 → A01~A03 무결성 검사
       ↓ (A 위반 시 경고 플래그, 계속 진행)
③ fraud_layer.detect(df)                     → B01~B11 부정 탐지
       ↓
④ anomaly_layer.detect(df)                   → C01~C10 이상 징후 (C07=Benford)
       ↓
...
├── fraud_layer.py           # Layer B: 오케스트레이터 (B01~B11) — MVP
├── fraud_rules_feature.py   # Layer B: 피처 기반 룰 (B01, B02, B03, B08)
├── fraud_rules_groupby.py   # Layer B: groupby 기반 룰 (B04, B05, B11)
├── fraud_rules_access.py    # Layer B: 접근통제 룰 (B06, B07, B09, B10)
├── anomaly_layer.py              # Layer C: 오케스트레이터 (C01~C06, C08~C10)
├── anomaly_rules_simple.py       # Layer C: 피처 기반 룰 (C01~C06, C08, C10)
...
11개 룰(B01~B11)이 각각 독립적으로 부정 패턴 하나를 탐지하며,
...
├── fraud_rules_groupby.py     # groupby 기반 룰: B04, B05, B11 — 원본 컬럼 집계/중복 판정
...
| `fraud_rules_groupby.py`  | 원본 컬럼 groupby + window 비교. 연산 비용 높음          | B04, B05, B11      |
...
- `sod_process_threshold` → B07 (fallback, user_persona 미존재 시)
- `audit_rules.yaml sod_toxic_pairs` → B07 Toxic Pair 5쌍
- `audit_rules.yaml sod_role_thresholds` → B07 직급별 임계값
...
##### fraud_rules_groupby.py — groupby 기반 룰 (B04, B05, B11)
...
b07_segregation_of_duties  (하이브리드 3단계)
  ...
  ④ Role-based: junior >1, senior >3 → Medium (controller/manager 제한 없음)
...
Toxic Pair(위험 쌍) + In-Process Conflict(프로세스 내 상충) + Role-based(직급별 임계값).
설정은 `audit_rules.yaml`의 `sod_toxic_pairs`와 `sod_role_thresholds`에서 관리.
...
# 4. Role-based: persona별 허용 프로세스 수 초과 여부
```
...
| B07 | —                                      | `created_by`, `business_process`, `user_persona`, `sod_conflict_type` | 하이브리드 3단계 SoD |
| B08 | `is_manual_je`, `exceeds_threshold`    | —                                      | 피처 조합                    |
| B09 | `exceeds_threshold`                    | `source`, `created_by`                 | 피처 + 원본 혼합             |
| B10 | `is_intercompany`                      | `company_code`, `trading_partner`      | MVP: 관계사 존재 감지 + IC 상대 식별 |
| B11 | —                                      | `gl_account`, `document_id`            | merge 기반 계정 조합 탐지    |
...
| B07 하이브리드 SoD              | Toxic Pair + In-Process + Role-based 3단계   | 단순 nunique는 99.96% 과탐. automated 제외 + 위험 쌍 + 직급별 임계값 |
...
| B11 비용→자산 계정 기준        | 차변 `15xx`(자산) + 대변 `6xxx`(비용)          | K-IFRS 계정과목 체계. audit_rules.yaml로 확장 가능           |
| B11 탐지 방식                  | C09와 동일한 merge 기반 Cartesian Product      | document_id 기준 N:M 복합 분개 대응. 벡터화 연산             |

#### settings.py 추가 설정

| 설정                            | 타입  | 기본값 | 환경변수                              | 사용 룰 |
|:--------------------------------|:------|:-------|:--------------------------------------|:--------|
| `approval_thresholds`           | `list[int]` | `[10M, 100M, 1B, 5B, 10B, 50B]` | `AUDIT_APPROVAL_THRESHOLDS` | B02, B03 |
| `duplicate_payment_window_days` | `int` | `30`   | `AUDIT_DUPLICATE_PAYMENT_WINDOW_DAYS` | B04     |
| `sod_process_threshold`         | `int` | `3`    | `AUDIT_SOD_PROCESS_THRESHOLD`         | B07 (fallback) |

> `sod_process_threshold`: user_persona 미존재 시 fallback용. 하이브리드 로직에서는
> `audit_rules.yaml`의 `sod_toxic_pairs`(5쌍) + `sod_role_thresholds`(직급별) 우선 적용.
...
tests/test_detection/test_fraud_rules_access.py — 12개 통과
  ...
  B07: toxic pair flagged, in-process conflict flagged, junior 초과 flagged,
...
```
src/detection/
├── anomaly_layer.py              # AnomalyDetector 오케스트레이터 — C01~C06, C08~C10 (9개)
├── anomaly_rules_simple.py       # 피처 기반 룰: C01~C06, C08, C10 — bool 컬럼 마스크 연산
...
10개 룰(C01~C10)이 시점·금액·적요·분포·가계정 관점에서 이상 패턴을 감지하며,
...
##### anomaly_rules_simple.py — 피처 기반 룰 (C01~C06, C08, C10)
...
c10_suspense_account    is_suspense_account == True
```
...
| C10 | 외감법 §8①2호, FSS 횡령 은폐 사례            | 가수금·가지급 등 가계정 장기 체류 전표       |
...
per-rule: flagged ? (severity / 5) : 0.0
  ...
  C10 (severity 3) → 위반 시 0.6
row_score = max(C01_score, ..., C10_score)
```
...
| 이슈                            | 결정                                                     | 사유                                                           |
|:--------------------------------|:---------------------------------------------------------|:---------------------------------------------------------------|
| Benford 별도 가중치 vs C07 포함 | Benford를 C07과 별도 가중치(0.15)로 분리                 | DETECTION_RULES.md §7 설계 — 통계적 배경 점수로 독립 취급      |
...
| Time     | `is_holiday`              | C02                    | is_weekend와 OR 조합 |
...
| Pattern  | `is_suspense_account`     | C10                    | 직접 사용            |
...
| B02 | 승인한도 직하   | JustBelowThreshold              | 3   | 240-A45(e), SOX        | 6단계 한도 중 가장 가까운 한도×0.9 이상      | is_near_threshold                    |
| B03 | 승인한도 초과   | ExceededApprovalLimit           | 3   | SOX, 240§32            | 6단계 한도 중 가장 가까운 한도 초과           | exceeds_threshold                    |
...
| B06 | 자기 승인       | SelfApproval                    | 3   | SOX직무분리, FSS오스템  | created_by == approved_by (직접 비교)        | created_by, approved_by, source      |
| B07 | 직무분리 위반   | SegregationOfDutiesViolation    | 4   | SOX직무분리, FSS오스템  | 하이브리드 3단계 SoD (Toxic Pair+In-Process+Role) | created_by, business_process, user_persona, sod_conflict_type |
...
| B10 | 관계사 순환거래  | CircularIntercompany            | 4   | 550호, FSS순환거래      | company_code+trading_partner 순환 패턴       | is_intercompany, company_code, trading_partner |
| B11 | 비용 자산화     | ExpenseCapitalization           | 4   | 240§32, FSS분식회계     | 동일 doc 내 차변=자산(15xx)+대변=비용(6xxx)  | gl_account, document_id              |
...
| C04 | 소급 전기      | TimingAnomaly          | 3   | 240-A45(c)            | posting_date < document_date - N일  | days_backdated                       |
...
| C10 | 가수금 장기체류 | SuspenseAccountAbuse  | 3   | 외감법§8①2호, FSS횡령  | is_suspense_account == True         | is_suspense_account                  |
...
5. - [ ] `fraud_layer.py` — B01~B11 (부정 탐지)
6. - [ ] `anomaly_layer.py` — C01~C10 (이상 징후, C07=Benford)
...
| FraudLayer (B01~B11)                        | MVP (Phase 1b) |
| AnomalyLayer (C01~C10, C07=Benford)        | MVP (Phase 1b) |
...
- C09 recall=10% (1,039건 라벨 중 105건 탐지). 통계적 하위 1% 기준의 C09 룰은 정상 작동하나,
...
**현재 한계**: B04/B05는 정확 매칭(exact match) 기반. 유사 중복/분할 거래 미탐.

**실증 근거 (2026-03-28 E2E 전수조사, v7)**:
- B05 recall=9% (134건 라벨 중 12건 탐지). 샘플 20건 중 18건(90%)이 동일 날짜+GL+금액의
...
**실증 근거 (2026-03-28 E2E 전수조사, v7)**:
- C09 recall=10% (1,039건 라벨 중 105건 탐지): 통계적 하위 1% 기준과 DataSynth 도메인 라벨 기준이 불일치.
  전수조사 결과 라벨 중 ~56%가 실제로는 흔한 GL 조합. 통계 룰로는 구조적으로 포착 불가.
  C09 룰 자체는 정상 작동(하위 1% 빈도 쌍 탐지). 도메인 기반 "비정상"은 Phase 2 ML(GNN/VAE)로 이관.
- B10 recall=7% (643건 중 48건 탐지): 643건 중 640건이 trading_partner 자체가 NULL.
  2-hop cycle 형성 0건. 3자 이상 cycle은 Phase 3 GraphDetector로 이관.
- B05 recall=9% (134건 중 12건 탐지): 샘플 20건 중 18건이 실제 중복 쌍 부재.
  exact match 한계 → Phase 2 DuplicateDetector(fuzzy + split + embedding)로 이관.

| 유형                   | 라벨 | Phase 1 탐지     | 룰 한계                                    | 보완 전략                               | Phase |
|:-----------------------|-----:|:-----------------|:-------------------------------------------|:----------------------------------------|:------|
| UnusualAccountPair     | 1039 | C09(105건/10%)   | 빈도 기반만 → 흔하지만 도메인상 이상한 조합 미탐 | VAE/GNN 잠재공간 학습                   | 2c    |
...
| CircularTransaction    |  643 | B10(48건/7%)     | 2-hop만, 640건 trading_partner NULL         | 그래프 DFS/BFS N-hop 순환 경로          | 3     |
...
**실증 근거 (2026-03-28 E2E 전수조사, v7)**:
B10 recall=7% (643건 라벨 중 48건만 탐지). 전수조사 결과 643건 중 640건이
...
| `test_fraud_layer.py`          | fraud_layer.py         | ~28건          | B01~B11 각 위반/정상, 권장 컬럼 미존재 skip        |
| `test_anomaly_layer.py`        | anomaly_layer.py       | ~24건          | C01~C10 각 위반/정상, C07 최소 샘플 미달           |
...
- **Layer B:** B01~B11 각각에 대해 DataSynth anomaly 레이블 데이터로 적발률 검증
  - 권장 컬럼(created_by 등) 미존재 시 skip + warning 반환 확인
  - B11: ExpenseCapitalization(90건) fraud_type 레이블 대비 탐지율 측정
- **Layer C:** C01~C10 각각 위반/미위반 데이터 검증
  - C10: SuspenseAccountAbuse(102건) fraud_type 레이블 대비 탐지율 측정
...
- **SoD Ground Truth:** `sod_violation`/`sod_conflict_type` 레이블 대비 B07 성능 측정
...
- 상세 사양: [DETECTION_RULES.md §2.3 C11](../DETECTION_RULES.md) 참조

#### Top-side JE (경영진 조정 전표) — B19 ✅
...
- 로직 (게이트키퍼 + 가점 방식):
  ...
  - **가점** (각 1점, 최대 5점):
    1. 기말 시점 (C01 > 0)
    2. 자기승인/승인 생략 (B06 > 0 OR B09 > 0)
    3. 비정상 계정 (A03 > 0 OR C09 > 0)
    4. 이상 고액 (C08 > 0)
    5. 위험 적요 (C06 > 0)
  - 판정: 수기 AND 가점 ≥ `topside_threshold`(기본 2) → B19 플래그, risk_level=High 승격
