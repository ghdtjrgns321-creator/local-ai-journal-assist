...
`docs/DETECTION_REFERENCE.md` 기반 PCAOB AS 2401 / ISA 240 / K-SOX 매핑:
...
`docs/CONSTRAINTS.md:253-292`에 명시:
> "모든 개발은 DataSynth로만 수행. 실제 회사 데이터는 검증 단계에서 투입 예정"

현황:
- DataSynth는 **SAP ACDOCA 네이티브** → 더존 iCUBE, 한앤철, Oracle EBS 등 로컬 ERP 특성 미반영
- 5종 공개 데이터셋(BPI 2019 등) E2E 통과는 `docs/DECISION.md:107`에 기록되어 있으나, 모두 프로세스 마이닝용 영문 데이터셋 — **한국 회계 ERP 아님**
...
> #### 문서 주장의 범위
>
> - "5종 공개 데이터셋 E2E 통과" (`docs/DECISION.md:107`) 기록은 **ingest 레벨 통과**일 뿐, **탐지 임계값 타당성 검증이 아님**. 문서가 이 구분을 명확히 한 건 정확.
...
- `docs/CONSTRAINTS.md:37-52`에 인지됨 — 단기는 방화벽, 중장기는 `streamlit-authenticator`
...
> | 1-3 | "max() 집계로 중복 가중 없음" | 레이어별 가중합이라 실제로는 가산 | 🟡 근거 오류 |
