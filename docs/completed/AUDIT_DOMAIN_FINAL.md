...
1호 "식별·측정·분류·기록 및 보고 방법"
  → L3-08 위험 적요 — 적요가 공백이거나 무의미하면 "기록 방법" 위반
  → L3-07 소급 전기 — 거래일 대비 전기일이 과도하게 늦으면 "적시 기록" 위반
...
```
→ L3-03 관계사 순환거래 — 합리적 사업 근거 없는 특수관계자 거래 탐지
```
...
| L3-03 | 관계사 순환거래 | `CircularIntercompany`            | 4   | **550호**, **FSS순환거래**         | company_code 간 순환 패턴                             | `company_code`, `reference`             |
...
| L3-07 | 소급 전기     | `BackdatedEntry`              | 3   | 240-A49(c), FSS횡령은폐          | `posting_date < document_date - N일`    | `posting_date`, `document_date`      |
| L1-08 | 기간 불일치   | `WrongPeriod`                 | 4   | 240§32(b)                        | `fiscal_period ≠ month(posting_date)`   | `fiscal_period`, `posting_date`      |
| L3-08 | 위험 적요     | `VagueDescription`            | 1   | 240-A49(c), SOX전표기록           | `line_text` 공백·위험키워드              | `line_text`, `header_text`           |
...
| L3-09 | 가수금 장기체류 | `SuspenseAccountAbuse`       | 3   | 외감법§8①2호, FSS횡령은폐         | `is_suspense_account == True`           | `is_suspense_account`                |
...
Phase 1의 24개 룰 결과를 pseudo-label로, DataSynth `is_fraud`/`is_anomaly`를 ground truth로.
