...
| 비교     | B10 (fraud_layer)                | GR01 (graph_rules)                |
|----------|----------------------------------|-----------------------------------|
| 방법     | `is_intercompany` 플래그 단순 패턴 | N-hop Johnson 순환               |
| 범위     | 1:1 직접 IC 거래                 | A→B→C→A N-hop 간접 순환          |
| recall   | ~7% (DataSynth)                  | 20%+ 개선 목표                    |

GR03 vs R03: R03은 `(partner, account)` 그룹 통계 편차(방향성 없음), GR03은 A→B 평균 vs B→A 평균 **방향성** price asymmetry — 차별화 명확. B10은 1차 필터, GR01은 2차 심층 분석으로 계층화되어 있다.
...
- `_query_significant_tx()`: `C08 AND B01` 동시 플래그 전표 Top N — `list_contains`로 정확 매칭 (`insight_generator.py:107-113`).
...
- `_SYSTEM_PROMPT` (`narrative_report.py:50-56`): "Cite triggered rule IDs (e.g., B02, C03)" 지시.
- `_build_prompt`에서 `flagged_rules`를 LLM에 전달하므로 참조 가능하나 **보장이 아님**.
- LLM 응답의 `cited_rules`가 실제 `flagged_rules`에 있는지 **교차 검증 로직 없음**.

**구체적 위험 시나리오**:
1. LLM이 "B02 (기말대량)" 인용 → 실제 해당 전표에는 B02 미플래그 → 감사조서에 허위 근거 기재.
