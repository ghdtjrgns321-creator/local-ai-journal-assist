...
| HIGH-6/8/10 복권 | 반영 대상. 가공거래처, 재고 과대평가 조작형, topside/연결조정은 HIGH 자격으로 복권됐지만 현재 PHASE1-1 코드가 직접 잡지 못한다. |
| KPI baseline | 고액 제거 후 정상 데이터 기준 HIGH 1.905%로 HARD ≤2%를 통과했지만, period_end가 대부분이라 L3-08/L3-10 임계 조정 후 재측정 대상이다. |
...
|-----|---------------|-----------|-----------|
| 가공거래처 | HIGH | PHASE1-1 거래처/vendor master 전용 룰 없음 | 신규 룰 또는 PHASE1-2 family 연계 필요 |
| 재고 과대평가 조작형 | HIGH | 재고 수량, 단가, NRV, 감모를 보는 룰 없음 | 신규 룰 필요 |
| topside/연결조정 | HIGH 자격 | 식별 룰 없음. 연결/통합조정 전표가 현재 입력에 들어오는지 불명확 | 데이터 단위 확인 선행 |
...
- 고액 제거 후 period_end HIGH가 정상 데이터에서 과도하게 남는지, L3-08/L3-10 임계 조정 후 재측정했는가.
