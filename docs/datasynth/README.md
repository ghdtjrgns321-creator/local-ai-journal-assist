...
오래된 v20~v134 실험 문서는 원인 추적용으로만 참조하고, 현행 설명은 2026-06-22 기준 accepted lineage를 기준으로 한다.

## 현재 기준

| 영역 | 현재 기준 | 역할 |
| --- | --- | --- |
| NORMAL base | `datasynth_semantic_v1_normal_20260621_v46b` | 단일법인 C001 정상 원장 기준본. 관계사 거래 흔적은 C002/C003 trading partner와 IC GL row로만 존재 |
| PHASE1-1 recall overlay | `datasynth_semantic_v1_recall_20260622_v46b_phase1_1_r11` | 최신 `DETECTION_RULES.md` 기준 26개 개별 룰 detector-only recall 검증용 |
| PHASE1 combo/tier overlay | `datasynth_semantic_v1_combo_tier_20260622_v46b_r1z` | HIGH/MEDIUM/LOW/CONTEXT case assembly 검증용 |
...
주의: PHASE2 r4m_h는 2026-06-14 accepted fraud overlay lineage다. 2026-06-21 v46b NORMAL에서 단일법인+관계사 흔적 기준이 갱신되었으므로, PHASE2 overlay를 새 NORMAL 위에서 다시 만들 때는 r4m_h의 gate를 그대로 재사용하되 base 경로와 normal-twin 분포를 다시 검증해야 한다.
...
7. [반복 결함과 Gate 사전](./failure-patterns.md)
8. [현재 기준과 남은 Gap](./current-lineage-and-gaps.md)
9. [의사결정 이력](./decisions-and-history.md)
...
- v43d 이후 추가된 단일법인 전환, 관계사 IC 흔적 복구, PHASE1-1 r11, combo/tier r1z의 수락 기준.
