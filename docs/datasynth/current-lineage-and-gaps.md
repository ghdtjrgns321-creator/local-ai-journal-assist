...
| 영역 | 현재 기준 | 판정 |
| --- | --- | --- |
| NORMAL | `datasynth_semantic_v1_normal_20260702_v50_approval_noise_r2` | Accepted for batch/job identity + RBAC/SoD persona-process + approver master-authority + bounded L1-04 natural exception realism successor |
...
### 5. PHASE2 r4m_h는 accepted지만 최신 NORMAL v50과 아직 동기화되지 않았다

r4m_h는 PHASE2 fraud overlay로 accepted다. 다만 NORMAL이 이후 단일법인+관계사 trace와 v47 batch/job identity 기준으로 갱신되었으므로, 다음 PHASE2 재생성에서는 base를 v47로 바꿔 같은 gate를 다시 통과해야 한다.
...
- `verification-and-tests.md`: v50 NORMAL snapshot과 r1j acceptance 기준을 반영했다.
...
| Gap | 영향 | 다음 조치 |
| --- | --- | --- |
| PHASE2 overlay가 v50 NORMAL 위에서 재생성되지 않음 | PHASE2 accepted lineage와 최신 NORMAL base가 다름 | r4m_h gate 세트를 유지하고 v50 base로 PHASE2 재생성 |
