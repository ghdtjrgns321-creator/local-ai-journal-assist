...
현재 accepted PHASE1-1 recall lineage는 `datasynth_semantic_v1_recall_20260630_v47_batchid_phase1_1_r1`이다.
v47 recall은 최신 firing matrix 기준으로 detector catch script exit 0, shortcut findings 0을 달성했다.
...
현재 accepted lineage는 `datasynth_semantic_v1_combo_tier_20260630_v47_batchid_r1j`이다.
v47 combo/tier는 `phase1-combo-tier-overlay`에서 automated/recurring 계열의 `batch_id`와 `job_id`를
둘 다 채우고, manual/adjustment는 비워 둔다. 또한 PHASE1 측정 harness는 L2-05 structural-reference
컬럼을 읽어 ERP-link reversal combo를 actual case-builder gate에서 볼 수 있어야 한다.
