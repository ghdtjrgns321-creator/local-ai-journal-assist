...
전표 부정 탐지 룰의 법적 근거, 실증 데이터, 도메인 지식을 정리한 참고 문서.
...
→ 이 프로젝트 자체가 A46의 구현체. 24개 룰 = CAATs 쿼리를 Python으로 자동화한 것.
...
```
→ B06(자기승인), B07(직무분리), B08(수기전표), B09(승인생략) = 내회관 통제 결과 추론
...
비정상 시간대          심야/주말 입력자 집중              C02, C03 + 확장       KLCA
계정·금액 무결성       Benford, 비정상 계정 조합          C07, C09             KICPA JET 3.2
승인 흐름 추적         생성→승인→포스팅 로그              B06~B09 + 확장        315호/330호 ITGC
...
감사기준서 1100호          내회관 감사 (통제 운영평가)               → B06~B09 (통제 결과 추론)
KLCA IT 체크리스트         전표 로그 무결성, 변경관리                → C02, C03, B06~B09 (부분 커버)
KICPA Issue Paper 46       Benford·계정 패턴, 완전성                → C07, C09
```
