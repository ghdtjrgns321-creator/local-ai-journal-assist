...
감사기준서 550호 §23  "특수관계자 거래의 사업상 합리성"
  → L3-03 관계사 순환거래 — 합리적 사업 근거 없는 순환 자금 이동
```
...
- `sod_process_threshold` → L1-06 (fallback, user_persona 미존재 시)
- `audit_rules.yaml sod_toxic_pairs` → L1-06 Toxic Pair 5쌍
- `audit_rules.yaml sod_role_thresholds` → L1-06 직급별 임계값
...
| L1-06 | —                                      | `created_by`, `business_process`, `user_persona`, `sod_conflict_type` | 하이브리드 3단계 SoD |
...
| L1-06 하이브리드 SoD              | Toxic Pair + In-Process + Role-based 3단계   | 단순 nunique는 99.96% 과탐. automated 제외 + 위험 쌍 + 직급별 임계값 |
...
| `sod_process_threshold`         | `int` | `3`    | `AUDIT_SOD_PROCESS_THRESHOLD`         | L1-06 (fallback) |
...
감사기준서 240호 A49(c)  "비정상적 시기에 이루어진 거래"
  ...
  → L3-07 소급 전기 — 과도한 소급은 기록 조작 은폐
  → L3-08 위험 적요 — 적요 미비/위험 키워드는 전표 추적 방해
...
| L3-08 | 감사기준서 240호 A49(c), K-SOX §8①1호        | 적요 품질 불량 또는 위험 키워드              |
...
| L3-08 | `description_quality`, `has_risk_keyword`  | —                                | 피처 2개 OR 조합                  |
...
| L3-08 OR 조건                       | quality in (missing,poor) OR risk_kw in (high,medium)        | 적요 미비 또는 위험 키워드 — 어느 하나만으로도 플래그            |
| scores 산출 방식                  | `max(severity / 5 × flagged)` per row                        | severity 5단계를 0~1 범위로 정규화. 합산 시 1.0 초과 위험       |
| 룰별 독립 실행                    | 한 룰 실패(exception)해도 나머지 계속 실행                   | try/except per rule + warning 수집                              |
...
tests/test_detection/test_anomaly_rules_simple.py — 22개 통과
  ...
  L3-08: missing/poor flagged, high/medium risk flagged, normal+low not
...
| Text     | `description_quality`     | L3-08                    | + has_risk_keyword   |
| Text     | `has_risk_keyword`        | L3-08                    | OR 조합              |
...
| L1-06 | 직무분리 위반   | SegregationOfDutiesViolation    | 4   | SOX직무분리, FSS오스템  | 하이브리드 3단계 SoD (Toxic Pair+In-Process+Role) | created_by, business_process, user_persona, sod_conflict_type |
...
| L3-03 | 관계사 순환거래  | CircularIntercompany            | 4   | 550호, FSS순환거래      | company_code+trading_partner 순환 패턴       | is_intercompany, company_code, trading_partner |
...
| L3-07 | 소급 전기      | TimingAnomaly          | 3   | 240-A45(c)            | posting_date < document_date - N일  | days_backdated                       |
| L1-08 | 기간 불일치    | WrongPeriod            | 4   | 240§32                | fiscal_period ≠ month(posting_date) | fiscal_period_mismatch               |
| L3-08 | 위험 적요      | VagueDescription       | 1   | 240-A45(c), SOX       | line_text 공백·위험키워드           | description_quality, has_risk_keyword |
...
| L3-09 | 가수금 장기체류 | SuspenseAccountAbuse  | 3   | 외감법§8①2호, FSS횡령  | is_suspense_account == True         | is_suspense_account                  |
...
**현재 한계**: L3-08은 키워드 정확 매칭 + 길이 기반. 은어/동의어/맥락 미탐.
