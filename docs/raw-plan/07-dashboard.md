...
| `user_persona`     | automated_system(76.6%), junior(9.2%), senior(7.3%), manager(3.6%), controller(3.3%) | 필터, 페르소나×위험 교차표 |
| `source`           | Automated(69.0%), Manual(20.5%), Recurring(7.6%), Adjustment(2.9%)| 필터, 수기 전표 하이라이트 |
...
| 지표              | 건수    | 비율   | 대시보드 활용                   |
|:------------------|:--------|:-------|:--------------------------------|
| 이상징후 전표     | 8,001   | 7.5%   | Summary KPI, Explorer 필터      |
| 부정 전표         | 2,046   | 1.9%   | Summary KPI, 부정유형 Treemap   |
| SoD 위반          | 12,419  | 11.7%  | SoD 분석 뷰                     |
| 차대변 균형 불일치 | 45     | 0.04%  | A01 룰 위반 카운트              |

### 부정 유형 Top 5 (16종 중)

| 유형                   | 건수 | 대응 탐지 룰         |
|:-----------------------|:-----|:---------------------|
| DuplicatePayment       | 397  | B04 중복 지급        |
| FictitiousTransaction  | 351  | B03 가공 거래        |
...
def render(result: PipelineResult):
    ...
    page_size=100 (1,104,914 라인 직접 로드 방지)
...
| 요소               | 내용                                                              |
|:-------------------|:-----------------------------------------------------------------|
| KPI 카드           | SoD 위반 건수(12,419), 위반 비율(11.7%), 위반 사용자 수           |
...
- **대용량 대응:** 1,104,914 라인아이템을 AgGrid에 직접 로드하지 않음. batch_ledger 쿼리가 anomaly_score DESC 정렬이므로 상위 N건만 초기 표시. AgGrid pagination(page_size=100) 적용.
