...
1. **Row-level anomaly score**
   ...
   - row `risk_level` threshold는 `High >= 0.7`, `Medium >= 0.4`, `Low >= 0.2`다.
...
2. **Case priority score**
   ...
   - L1-01 단독 case에서 logic 기여도는 `0.20 * L1-01 normalized_score`다. `rounding_scale`은 priority에 0.03만 더하고, `severe`는 0.18까지 더한다.
   - `priority_floors`는 심각한 통제 위반(`L1-05`, `L1-04`, `L1-06`, `L1-07`)이 단일 룰이라는 이유로 queue에서 밀리지 않도록 최소 priority를 보장한다.
...
| 버킷 | L1-01 row score | row `anomaly_score` 기여 (`* 0.40`) | case priority logic 기여 (`* 0.20`) |
| --- | ---: | ---: | ---: |
| `rounding_scale` | 0.15 | 0.06 | 0.03 |
| `minor` | 0.30 | 0.12 | 0.06 |
| `material` | 0.65 | 0.26 | 0.13 |
| `severe` | 0.90 | 0.36 | 0.18 |
...
- **탐지 로직**: `schema.yaml`에서 `required: true`로 정의된 컬럼의 행 단위 NULL 검사
...
- **필요 피처**: `gl_account`
- **DataSynth 상태**: `v59` 기준 CoA 밖 GL은 `InvalidAccount`가 소유한다. `MisclassifiedAccount`가 CoA 밖 GL을 사용해 L1-03을 오염시키는 케이스는 `0`건이며, `check_datasynth_required_truth.py`에서 승격 전 검증한다.
...
| 버킷 | 기준 | row score |
|---|---|---:|
| `boundary` | 한도 초과율 `0% < ratio <= 10%`, 기본 REVIEW | 0.00 (`review_score` 0.40) |
| `moderate` | `10% < ratio <= 50%` | 0.60 |
| `severe` | `50% < ratio <= 100%` | 0.75 |
| `critical` | `ratio > 100%` | 0.90 |
| `non_approver` | `can_approve_je = false` 또는 승인권한 없음 | 0.90 |
| `none` | L1-04 미해당 | 0.00 |
...
- **결과 표현**
  ...
  - `score_series`는 즉시 위반만 반영한다. `allowed_system`은 후보에는 포함되지만 확정 위험 점수는 0점이다.
  - `review` 자기승인은 `review_score_series`, `breakdown`, `row_annotations`에 남기되 확정 탐지 precision/recall 분모에는 넣지 않는다.
  - 화면과 점수에서는 아래 bucket을 함께 남긴다.

| 버킷 | 기준 | row score |
|---|---|---:|
| `allowed_system` | 자기승인이지만 자동처리 allowlist에 해당. 후보/evidence에는 남기고 위험 점수는 0점 | 0.00 |
| `review` | R2R/A2R 등 검토 필요 bucket이고 보강 신호 없음 | 0.40 |
| `immediate` | 일반 사람 자기승인 또는 명시 통제 위반 성격 | 0.80 |
| `escalated_materiality` | review 후보이나 수행중요성 이상 수기 전표라 즉시 위반으로 승격 | 0.80 |
| `escalated_abnormal_time` | review 후보이나 주말·공휴일·심야 등 비정상 시점이라 즉시 위반으로 승격 | 0.80 |
| `escalated_high_risk_account` | review 후보이나 민감 계정을 건드려 즉시 위반으로 승격 | 0.80 |
...
- **구현**: `fraud_rules_access.py` → `b06_self_approval()`
  ...
  - `breakdown`: candidate/actionable/immediate/review/allowed_system 행 수, bucket count, 승격 사유 수
...
- **결과 표현**
  ...
  - 행별 확정 점수는 `immediate = 0.80`, `review = 0.00`, `low_priority = 0.00`이다. 검토 우선순위 참고용으로 `review_score = 0.40`, 낮은 우선순위 후보는 `review_score = 0.10`을 저장한다.
  - `row_annotations`에는 `queue_label`, `score`, `review_score`, `evidence_count`, `evidence_reasons`, `source_category`, `has_approval_date`, `min_evidence_count`를 저장한다.
  - `breakdown`에는 `immediate_rows`, `review_rows`, `low_priority_rows`, `confirmed_rows`, `candidate_rows`, `actionable_missing_approver_rows`, `no_approval_required_rows`, `approval_level_review_rows`, `allowed_system_rows`, `missing_approver_rows`, `no_approval_trace_rows`, `evidence_count_bands`, `evidence_reason_counts`를 저장한다.
- **평가/리포트 표시 방식**
...
- **탐지 로직**: `approval_date`가 비어 있으면 우선 L1-09 후보로 전부 잡고, 승인자 존재 여부와 실무 맥락에 따라 점수를 분리한다.
  - **즉시위반**: 수기/조정 source, 고위험 업무 프로세스(`TRE`, `P2P`, `O2C`, `H2R`), 또는 한도초과 금액 신호가 있어 승인일 누락을 문서 추적성 결함으로 바로 볼 수 있는 경우
...
- **필요 피처**: `approval_date`. 점수 분리를 위해 `approved_by`, `source`, `business_process`, `exceeds_threshold`를 사용한다.
...
- **출력 방식**
  ...
  - `breakdown`에는 `reference_match_docs`, `mixed_reference_fallback_docs`, `blank_reference_fallback_docs`, `recurring_suppressed_docs`, `partner_key_coverage_ratio`를 기록한다.
...
- **DataSynth 상태**: `v58` 기준 `P2P + KZ` 지급쌍 라벨과 `labels/duplicate_payment_pairs*`, `labels/duplicate_payment_negative_controls*`를 관리한다.
- **평가 계약**
  ...
  - fallback 후보는 confirmed duplicate payment가 아니라 review candidate로 본다.
...
- **출력 방식**
  ...
  - `score_series`는 행 단위 confidence를 담고, `row_annotations`에는 `reason_code`, `matched_reason_codes`, `confidence`, `confidence_band`, `queue_label`을 기록한다.
  - `breakdown`에는 `immediate_rows`, `review_rows`, `queue_counts`, `confidence_band_counts`, `immediate_docs`, `review_docs`, `reason_counts`, `reason_doc_counts`, `modifier_row_counts`, `normal_context_suppressed_docs`를 기록한다.
...
- **합성데이터 평가**: `v59` 기준 `MisclassifiedAccount`는 CoA에 존재하는 유효 계정만 사용한다. 따라서 L3-01은 "계정 자체가 틀림"이 아니라 "업무 프로세스와 계정 성격이 어색함"을 평가하며, `labels/misclassified_account_coa_fix_cases*`에는 v59에서 CoA 밖 GL을 유효한 process-mismatched 계정으로 교체한 19건이 기록된다. 과거 `v29_candidate`의 보수적 category/denylist 평가 기록은 `tests/phase1_rulebase/test-results/l3-01-synth-2022-2024.md`를 참조한다.
- **Phase 1 이후 사용 원칙**
...
- **결과 표현**
  - 룰 hit 자체는 기존처럼 `is_manual_je` 또는 `source in manual_source_codes` Boolean으로 유지한다.
...
- **출력 메타데이터**
  - `score_series`: population 0.35, priority 0.60, control-bypass 0.75
  ...
  - `breakdown`: `manual_rows`, `adjustment_rows`, `priority_rows`, `control_bypass_rows`, `source_counts`, `bucket_counts`, `priority_reason_counts`
- **평가/리포트 표시 방식**
...
- **결과 표현**
  - 룰 hit 자체는 기존처럼 `is_period_end & (high_amount | is_manual_je)` Boolean으로 유지한다.
  - 다만 L3-04 raw hit 전체를 동일 위험도로 보지 않고 아래 bucket과 row score를 함께 남긴다.

| 버킷 | 기준 | 해석 | row score |
|---|---|---|---:|
| `closing_manual` | 기말/기초 + 수기 전표, 고액 아님 | 결산 수기 모집단 | 0.45 |
| `closing_high_amount` | 기말/기초 + Q3 초과 고액, 수기 아님 | 결산 고액 모집단 | 0.55 |
| `closing_manual_high_amount` | 기말/기초 + 수기 + Q3 초과 고액 | 우선 검토 결산 조정 | 0.65 |
| `closing_priority` | L3-04 hit + 보강 위험 신호 | 우선 검토 결산 후보 | 0.70 |
...
- **평가/리포트 표시 방식**
  ...
  - `review_queue_docs`는 L3-04에 한해 `closing_priority_docs + closing_high_docs`를 우선 사용한다.
  - `RushedPeriodEnd` 확정 라벨 기준 precision/recall은 보조 참고값이며, primary 해석은 결산 검토 모집단 coverage와 우선순위 band 분포다.
- **운영 전제**
...
- **성격**: Phase 1 review-needed 룰. 과거 이력 또는 동료 baseline을 학습하지 않고, 현재 감사기간 데이터 안에서 사용자별 업무영역 폭만 집계한다.
- **L1-06과의 경계**
  - L1-06은 금지된 업무분장 조합, 명시적 SoD conflict, 승인/작성 역할 충돌처럼 **확정 가능한 통제 위반**을 잡는다.
  - L3-12는 금지 여부를 판단하지 않는다. 한 사용자가 여러 프로세스, 회사, 전표유형, 계정군, 입력방식에 과도하게 관여하고, 해당 전표에 수기·민감계정·고액·결산 같은 문맥이 있을 때 review population으로 올린다.
  - L3-12 hit는 L1-06의 FP/FN/precision/recall에 포함하지 않는다.
- **탐지 로직**
  - `created_by`별로 현재 데이터의 `business_process`, `company_code`, `document_type`, `gl_account` 계정군, `source` distinct count를 집계한다.
  - `user_persona`가 있으면 사용자 유형별 기준을 적용한다. 없으면 default 기준을 쓴다.
  - 자동/배치 계정은 기본 제외한다. 다만 수기/조정 source와 민감계정·고액·결산 맥락이 함께 있으면 검토 후보로 올릴 수 있다.
  - L3-12는 사용자 단위 review signal이다. 한 사용자가 업무범위 집중 기준을 충족하면 해당 사용자 활동 행을 같은 review population으로 올리되, 자동/system-only와 admin/superuser 단순 breadth는 제외한다. 단독 L3-12는 High가 아니며 다른 룰과의 결합 여부로 우선순위를 조정한다.
...
| `admin`, `superuser` | 단순 다중업무 제외, 수기/민감/고액/결산 중 2개 이상 결합 시 |
| `automated_system`, `batch_user` | 기본 제외, 수기/조정 source와 보강 신호가 함께 있을 때만 |

- **결과 표현**
  - `score_series`: `0.20~0.65` review priority score를 제공한다. 이는 확정 위반 점수가 아니라 정렬/큐 노출 점수다.
  - `row_annotations`: `user`, `persona`, `bucket`, `process_count`, `company_count`, `document_type_count`, `account_group_count`, `source_count`, `reasons`, `rule_boundary`를 저장한다.
  - `breakdown`: `candidate_rows`, `candidate_users`, `bucket_counts`, `user_summaries`, `excluded_system_rows`, `excluded_admin_rows`를 기록한다.
- **평가/리포트 표시 방식**
  - L3-12는 라벨-only precision/recall로 해석하지 않는다. `work_scope_excess_review_population`은 review population truth다.
  - v80 DataSynth 기준 L3-12 truth는 `labels/rule_truth_L3_12.csv`와 `labels/work_scope_excess_review_population.csv`에 분리 저장한다.
...
| 버킷 | 기준 | row score |
|---|---|---:|
| `review_zscore` | `zscore_threshold < amount_zscore < 4.0` + 금액 가드 통과 | 0.45 |
| `strong_zscore` | `4.0 <= amount_zscore < 6.0` + 금액 가드 통과 | 0.60 |
| `extreme_zscore` | `amount_zscore >= 6.0` + 금액 가드 통과 | 0.75 |
...
- **DataSynth 평가 계약**:
  - `UnusuallyHighAmount`와 `StatisticalOutlier`는 confirmed anomaly subset이다.
  - `v48_candidate`부터 `labels/high_amount_review_population*`을 L4-03 review coverage로 사용한다.
  - `labels/high_amount_confirmed_anomalies*`는 주입 고액 이상치 recall 확인용이다.
  - `labels/high_amount_normal_controls*`는 정상 대형거래 control이며, raw L4-03 hit가 될 수 있어도 confirmed FP로 단정하지 않는다.
  - `labels/high_amount_boundary_controls*`는 z-score 임계값 근처 control이며, hard-threshold fitting 방지용이다.
  - 모든 고액 거래를 `UnusuallyHighAmount`로 라벨링하지 않는다.
- **평가 계약**:
  - L4-03은 strict pass/fail 룰이 아니라 `coverage_anchor`다. confirmed `UnusuallyHighAmount`/`StatisticalOutlier` 라벨은 direct subset이고, raw hit는 고액 검토 모집단이다.
  - `score_bands`는 `high_amount_review_docs`, `review_zscore_docs`, `strong_zscore_docs`, `extreme_zscore_docs`로 나눈다.
...
- **결과 표현**
  ...
  - row score는 0.40으로 두고, annotation에 `reason_codes`, `rare_pair_count`, `sample_pairs`, `threshold_count`를 남긴다.
...
- **DataSynth 평가 계약**
  ...
  - `labels/rare_account_pair_normal_controls*`는 정상 희소 계정쌍 control이며, raw L4-04 hit가 될 수 있어도 confirmed FP로 단정하지 않는다.
...
- **DataSynth 상태**: DataSynth 자동 전표는 `source='automated'`를 사용한다. L4-06은 이를 배치성 source로 포함하되, multi-line 전표 과탐을 막기 위해 대량 동시 생성 조건은 row count가 아니라 `document_id` distinct count 기준으로 평가한다.
- **평가 계약**:
  - L4-06은 strict pass/fail 룰이 아니라 `coverage_anchor` 보조 증거다. 현재 DataSynth에는 confirmed `BatchAnomaly` 라벨 계약이 없으므로 `no_label` 실패로 해석하지 않는다.
...
- **DataSynth 평가 계약**:
  ...
  - 운영 DataSynth `v58` 기준 D01 sidecar에 `business_event_type`, `evaluation_bucket`, `precision_policy`를 포함한다.
