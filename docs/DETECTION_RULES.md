...
- **PHASE3 LLM Narrator**는 active detector layer가 아니다. Detection rules are PHASE1/PHASE2 only. Any future semantic description analysis must be local-first and must not transmit ledger evidence to external APIs. 단일 출처: [LOCAL_FIRST_EVIDENCE_POLICY.md](LOCAL_FIRST_EVIDENCE_POLICY.md), deprecated spec [PHASE3_REVIEW_NARRATOR_SPEC.md](PHASE3_REVIEW_NARRATOR_SPEC.md).
...
이 절은 총 32개 카드 = **31개 transaction/review row 카드 + 1개 macro 카드 (L4-02 Benford)** 로 구성된다. row 카드는 `L1~L4` 전표 행 단위 구현 룰이고, macro 카드인 L4-02 Benford는 계정·월 단위 분포 검정으로 별도 가중치 트랙이다. L3-12는 L1-06의 명시적 SoD 위반과 분리된 사용자 업무범위 집중 review 룰이며, 사용자 큐와 우선순위 해석은
...
그래프 신호는 위 32개 카드 수에는 넣지 않지만, Phase 1 결과 화면에서는 Account / Process Queue 또는
관계사·연결 구조 drill-down의 보조 finding으로 결합한다. 카운트 단일 출처는 [RULE_DETAIL_METADATA_V1_LOCK.md §Rule Count Policy](RULE_DETAIL_METADATA_V1_LOCK.md#rule-count-policy)이며, 레거시 문서의 "33 rules" 표현은 `32 canonical + Benford display alias` 로 normalize한다.
...
`IC01~IC03`은 32개 L1~L4 룰 수에 포함하지 않는 관계사 보조 finding이다. `GR01/GR03`은 local graph 보조 신호이며 `L3-03` 케이스와 결합될 때 관계사·연결 구조 이상 우선순위를 높이는 보조 증거로 사용한다.
