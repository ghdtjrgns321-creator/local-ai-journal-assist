...
이 문서는 `docs/completed/DETECTION_RESULTS_CONTRACT_V2.md`의 KPI 단위 혼재를 2026-05-18 TS-12 정책에 맞춰 정리한 별도 보고서다. V2 원문은 보존하고, V3는 `datasynth_contract_v2` 결과를 전표 단위 외부 KPI로 재집계한다.
...
본 문서의 모든 외부 KPI는 전표(document) 단위로 재집계되었다. case 단위 V2 표는 `docs/completed/DETECTION_RESULTS_CONTRACT_V2.md`에 보존한다.
