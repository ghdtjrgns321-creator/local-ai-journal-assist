...
| TS-6  | 아키텍처           | Historical: 로컬 LLM 한계와 외부 API 검토 → D068에서 제거                                                                                                   | VRAM 8GB, 한국어 회계, local-first                                                                                                                    | historical                       |
...
| TS-11 | 탐지 설계/거버넌스 | PHASE1 거버넌스 V1 — 검토 후보 선별 원칙과 정답 외우기 방지                                                                                                 | 31개 canonical 룰 고정(L4-02/D01/D02 macro→PHASE1-2 family), 6개 주제 점수(7→6, intercompany_cycle→PHASE1-2 family), 정답 외우기 방지 감시, 별도 검증 | 진행 중                          |
| TS-12 | 탐지 정렬/평가     | PHASE2 점수 통합 정렬 미반영 → RRF 5-way·doc-level 시도 후 폐기 → case 단위 통일                                                                            | composite_sort, V1 lock, case-truth 1:1 매핑, 분모 case 620                                                                                           | 정정 (2026-05-19)                |
| TS-13 | 탐지 룰 커버리지   | PHASE1 룰 커버리지 갭 — 큐 미진입 truth 80건 (PHASE2 doc 큐 회수 0/80 확정)                                                                                 | V7 fixed3, ceiling 확정, Phase A.5 진단                                                                                                               | 정정 (2026-05-19)                |
| TS-14 | 제품 포지셔닝      | 부정탐지 모델에서 설명 가능한 감사 리뷰 큐 도구로 범위 고정                                                                                                 | 실데이터 부재, golden set 부재, 포트폴리오, review queue, local evidence                                                                              | 해결                             |
| TS-15 | 탐지 설계/거버넌스 | PHASE1 priority_score 분포 불균형 → 가중합·floor·band컷 폐기, 순서형 tier(HIGH/MEDIUM/LOW/CONTEXT) 전환 + HIGH 조합 근거화 + 17건 재감사 A안 셋째 다리 확장 | 0.75 클러스터링, priority_floors dead code, count fitting, tier 전환, HIGH_COMBO_GROUNDING, A안, 과탐 가드 0.334%, D060/D061                          | 해결 (2026-06-17)                |
| TS-16 | 표시/거버넌스      | PHASE2 / PHASE1+2 통합 3등급 분류 도입 — D062 정책 + wiring 검증 통과, 운영 분포 검증 별도                                                                  | phase2_review_band, phase12_review_band, Stage7 cache, sub-detector 부재, D062                                                                        | 진행 중 (2026-05-21)             |
| TS-17 | PHASE2 평가 기준   | family responsibility-map 도입 — 전체 620 recall과 owner-set target recall 분리                                                                             | owner set, no_clear_owner, detector-blind denominator, target recall, PHASE2 family                                                                   | diagnostic baseline (2026-05-30) |
| TS-18 | PHASE2 ML 설계     | VAE target 미정의 + PHASE1·family 7/7 중첩 → 다변량 조합 비정형성으로 재정의, semantic-clean 데이터 대기                                                    | anti-leakage 잔여 feature, joint coherence, PHASE1 L4 충돌, Class B, multivariate                                                                     | 확정·대기 (2026-06-02)           |

---

## TS-18: VAE target 미정의 + PHASE1·family 중첩 → 다변량 조합 비정형성으로 재정의

**상태**: 확정·대기 (2026-06-02). 상세 근거·구현방향 단일 출처는
`docs/spec/PHASE2_UNSUPERVISED_ROLE_REDESIGN_DECISION.md` §6.

### 문제

VAE/unsupervised를 document-level review surface로 재설계(P1~P5)하던 중, P5 측정에서
document-case default가 TOP500 repeated-normal pressure 1.0 / PHASE1 밖 추가 ~0(primary 0,
companion 3)으로 native row queue 실패를 재현했다. 원인 추적 결과 ordering이 아니라 더
근본적인 설계 결함이 드러났다.

1. **target 미정의**: VAE 입력은 설계된 탐지 대상이 아니라 anti-leakage 잔여 컬럼이다
   (`phase2_plan.py` include-by-default). "VAE가 무엇을 탐지한다"가 정의된 적이 없다.
2. **leakage deny가 고신호 축 제거**: 금액·승인·라운드·임계·IC·suspense를 학습에서 제외
   (`constants.py::LEAKAGE_DENY_COLUMNS`).
3. **잔여 해석 feature 7/7이 PHASE1·family와 중첩**: amount_z→L4, round→L2-01, weekend→L3-05,
   after_hours→L3-06, lag→L3-07, manual→L3-02, partner_frequency→L3-03/relational.
4. **"broad statistical review" 프레이밍이 PHASE1 L4 statistical_outlier와 충돌**.

→ PHASE1·family와 차별점 0이 PHASE1 밖 ~0의 직접 원인.

### 결정

VAE target = **다변량 조합 비정형성(multivariate joint-combination atypicality)**.
account_subtype × business_process × counterparty_type × document_type × line_text_family
조합이 jointly 희소·부정합한 전표를 감사인 주의로 올린다. 단일 축(금액·라운드·duplicate·
reciprocal·timing burst·entity novelty)은 PHASE1/family 소유라 입력·score에서 배제한다.
autoencoder의 유일한 비중첩 niche는 joint coherence이며, 단일 축이면 룰이 더 정밀해 VAE
존재 이유가 없다.

### 상태 / 대기

확정 target은 testability matrix Class B라 정상 모집단이 semantic-clean이어야 유효하다
(account×process×counterparty×text 정합). 현재 합성 정상 데이터는 이 정합이 깨져 있어 VAE
joint 신호가 노이즈로 보였다. semantic-clean 데이터는 별도 트랙(datasynth journal realism)
에서 생성 중이며, **그 데이터 도착 전까지 입력 feature 재설계·평가를 보류**하고 VAE는
diagnostic/companion으로 유지한다. 합성 평가는 mutation-type 분리 mechanism 증거 한정,
production recall 주장 금지(Class C/D는 실데이터 영역으로 제외).

### 가드

truth/owner/scenario/PHASE1 rank를 model·selector·ranking 입력 금지. q95/score/threshold/
weight를 recall에 맞춰 조정 금지. DataSynth를 VAE score에 맞춰 조작 금지. "이상치=부정" 금지.

---

## TS-14: 부정탐지 모델에서 설명 가능한 감사 리뷰 큐 도구로 범위 고정

### 문제

포트폴리오 프로젝트 조건상 실제 회사 ERP 원장과 회계사가 확정한 fraud label을 확보할 수 없다. 공개 실제 데이터는 대체로 fraud label이 없고, label이 있는 Tritscher ERP-Fraud는 외부 synthetic ERP simulation이다. DataSynth는 프로젝트 기능 시연에는 가장 적합하지만, 합성 truth만으로 supervised fraud detector 성능을 주장하면 fitting 위험을 피할 수 없다.

또한 PHASE1 + VAE 결과는 "실제 부정을 잘 잡는 모델"이라는 제품 문구를 뒷받침하기에는 부족하다. 이 상태에서 supervised/transformer/stacking을 억지로 깨우면 합성 데이터의 생성 흔적을 학습하는 모델이 되어 포트폴리오 신뢰도를 오히려 낮춘다.

### 결정

제품 포지셔닝을 다음으로 고정한다.

> 이 프로젝트는 부정을 확정하는 모델이 아니라, 전수 원장/지급 데이터에서 감사인이 검토해야 할 후보를 설명 가능한 review queue로 정렬하고, 근거와 다음 감사 절차를 제공하는 로컬 감사 분석 보조 도구다.

### 데이터셋 역할 재정의

| 데이터셋            | 역할                                                                                                                                           |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| DataSynth V7 fixed3 | 메인 포트폴리오 데모. 복식부기 전표 구조, 계정, 승인, 적요, 관계사, 의도 주입 truth가 있어 PHASE1/PHASE2 로컬 분석 기능을 보여주기에 적합      |
| OpenDataPhilly      | 실제 공개 지급 데이터 ingest와 reviewer workflow smoke test. fraud label이 없으므로 운영 성능 검증이나 supervised fraud training에는 쓰지 않음 |
| Tritscher ERP-Fraud | 외부 synthetic ERP shadow benchmark. VAE/document ranking 보조 검증에만 사용                                                                   |

### 운영 언어 가드

- 쓰지 않는 표현: "부정 적발", "fraud 확정", "실제 fraud recall", "자동 적발", "실무 운영 성능 검증 완료".
- 쓰는 표현: "감사 리뷰 후보", "예외 후보", "고위험 검토 항목", "설명 가능한 우선순위", "감사인 다음 절차".
- DataSynth truth recall/precision은 개발 검증 보조값이며 사용자-facing 성능 보장으로 쓰지 않는다.
- Local Evidence Brief는 판단자가 아니라 이미 계산된 로컬 근거 요약과 절차 제안 표시다. 외부 LLM/API 기능은 active product path가 아니다.

### 결과

이 결정으로 프로젝트는 중단되는 것이 아니라 범위가 명확해졌다. 메인 데모는 DataSynth에서 PHASE1 rule queue와 PHASE2 unsupervised/rule-style family를 end-to-end로 보여준다. 설명은 Local Evidence Brief로 로컬 근거만 요약한다. 외부 데이터는 보조 검증으로 두고, supervised 계열은 trusted golden set이 생길 때까지 dormant로 둔다.

### 관련 문서

- `docs/spec/CONSTRAINTS.md` §프로젝트 포지셔닝
- `docs/guide/users/10_PORTFOLIO_POSITIONING.md`
- `docs/guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §외부 shadow 검증
- `artifacts/external_validation/tritscher_erp_fraud_20260519/phase2_external_shadow_summary.md`

---

## TS-15: PHASE1 priority_score 분포 불균형 — Stage 1 봉합 + Stage 2-A fitting 위험으로 보류

### 문제

`config/phase1_case.yaml` 의 `priority_band.high` 임계를 0.75 → 0.90 으로 상향한 직후 fy2022 / fy2023 artifact 측정 결과 다음 분포 확인.

| Bin    | fy2022 (12,698) | fy2023 (14,926) |
| ------ | --------------- | --------------- |
| >=0.95 | 0               | 0               |
| >=0.90 | **0**           | **0**           |
| >=0.85 | 0               | 0               |
...
Top 50 ~ Top 1000 cutoff 모두 0.7500. 즉시검토 (`>=0.90`) 가 0건이고 4,000건이 0.75 단일 점수에 클러스터링. 감사인이 우선순위로 봐야 할 case 가 비어 있어 review queue 의 실무 의미가 약해진 상태.

### 원인 진단 (1차 단절)

`src/detection/phase1_case_builder.py:1602-1604` 에서 `use_topic_scoring=True` 경로가 `priority_score = max(topic_scores.values(), default=0.0)` 로 덮어쓰기.

`priority_floors` 의 `min_priority_score: 0.90` 엔트리 4종 (`multiple_core_required_fields_missing`, `sod_direct_critical`, `skipped_approval_critical`, `escalated_self_approval_material_or_sensitive`) 이 평가는 되었으나 최종 `priority_score` 에 반영되지 않는 **dead code** 상태. 두 채점 계층 (priority_floors ↔ topic_scoring) 의 단절이 1차 원인.

`topic_scoring` 의 weighted base_score 는 `max_primary_rule_score: 0.62` 가중치로 ~0.62 부근에서 형성되고, `topic_floor_policies` 와 `combo_floor_policies` 의 `*_high` 가 모두 0.75 로 끌어올려 0.75 클러스터 형성.

### Stage 1 — 봉합 (해결)

`phase1_case_builder.py:1556-1620` 의 priority_score 머지 로직 수정.
...
# ... topic_scoring (compute_topic_scores) ...
...
if use_topic_scoring:
    ...
    # macro_reasons 는 audit 사유에 추가하지 않음 (점수에 미반영되는 경로)
else:
    adjustment_reasons.extend(macro_reasons)
```

두 핵심 변경:
1. `use_topic_scoring=True` 경로의 머지 후보를 `priority_score_pre_macro` 로 사용 → priority_floors 의 0.90 entry 가 topic_scoring 에 덮이지 않음.
2. `macro_reasons` append 를 분기 후로 이연 → macro 보너스가 점수에 미반영되는 경로에서는 audit 사유에서도 제거.

#### Stage 1 측정 결과 (시뮬레이션, scripts/simulate_stage1_priority_score.py)

|                         | fy2022       | fy2023       |
| ----------------------- | ------------ | ------------ |
| 0.90+                   | **21**       | **23**       |
| 0.75+                   | 4,316 (+307) | 4,706 (+174) |
| dropped_below_threshold | 0            | 0            |

0.90+ 진입 케이스는 모두 `escalated_self_approval_material_or_sensitive` (L1-05 escalated label) 단독 floor. priority_floors 4종 중 1종만 발현 — 나머지 3종은 데이터 특성상 매우 드문 시나리오.

Stage 1 자체는 코드 구조 결함 수정이 동기, 결과는 yaml 사전 잠금 정책의 자연 발현. **fitting 위험 매우 낮음**.

### Stage 2-A — 도메인 정합 critical entry 시도 후 보류 (정정)

사용자 목표 (0.90+ 100~200건) 를 위해 priority_floors 에 `approval_bypass_critical` + `period_end_adjustment_critical` 0.90 entry 7종 추가 시도. D060 5조 원칙 (강한 seed + 중요성 + 독립 보강 + 단독 금지 + count 재조정 금지) 정합.

#### 시뮬레이션 결과 (scripts/simulate_stage2_critical.py)
...
목표 100~200 의 5~10배. period_end_adjustment_critical 폭증 (~93%) — `L3-04/L3-11/L1-08 raw>=0.60 + L4-03 + (L3-02 또는 approval_bypass rule)` 결합이 fy2022/fy2023 데이터에서 매우 흔함.

#### Fitting 진단 (scripts/fitting_audit.py)

| 위험                          | 강도     | 근거                                                            |
| ----------------------------- | -------- | --------------------------------------------------------------- |
| 머지 자체의 fitting           | 낮음     | stage_0/stage_1 의 critical 매칭 case 동일 (분포 이동만)        |
| 0.80~0.89 band 의 인공성      | **높음** | 단일 floor `escalated_self_approval_abnormal_time` 가 100% 형성 |
| critical 의 자기 floor 의존성 | **높음** | 도메인 매칭 514/493건 중 16~20% 만 진입                         |
| count 보고 조건 재설계        | **높음** | high_amount 필수 추가 동기가 분포 보정                          |
...
조건을 좁혀 count 를 맞추면 D060 5조 5번 ("건수 목표로 조건 재조정 금지") 위반. 본질 문제는 critical 정의가 아니라 priority_score 계층이 0.75/0.90 2단계만 도메인 의미를 가지고 0.80/0.85 가 단일 floor 인공물이라는 점.

#### Roll back 처리

- yaml priority_floors 신규 7 entry 제거
- `_priority_floor_corroboration_match` 의 `required_rules_any` 지원 제거
- `tests/modules/test_detection/test_phase1_case_builder_stage2.py` 삭제
- Stage 1 종료 상태 (0.90+ = 21/23) 로 복귀
- approval_bypass / period_end 결합 신호는 **검토대상 상단의 정렬/필터 축**으로 이관

### 다음 단계 — priority_score 계층 재설계 (Stage 5 별도)

| 점수  | 의미                                           | 목표 (관찰) |
| ----- | ---------------------------------------------- | ----------- |
| 0.90+ | 즉시검토 — 강한 단일 결함 또는 명시적 critical | 20~50건     |
| 0.85+ | 상위 검토대상 / supervisor review              | 100~300건   |
| 0.80+ | 검토대상 상단                                  | 500~1,000건 |
| 0.75+ | 일반 검토대상                                  | 수천 건     |

4단 계층을 priority_floors 다단계 entry 로 구현. D060 5조 (특히 5번 count 재조정 금지) 정신 유지.

### 보존 산출물 (Stage 5 에서 재사용)

| 파일                                         | 용도                                                                     |
| -------------------------------------------- | ------------------------------------------------------------------------ |
| `scripts/audit_phase1_score_distribution.py` | Stage 별 분포 측정 (`>=0.95/0.90/0.85/0.80/0.75`, Top N cutoffs, 이동표) |
| `scripts/simulate_stage1_priority_score.py`  | Stage 1 머지 효과 시뮬레이션 (YAML 기반 floor reason 추출)               |
| `scripts/simulate_stage2_critical.py`        | priority_floors 0.90 entry 신설 효과 1회 측정 도구                       |
| `scripts/analyze_stage2_candidates.py`       | 0.80~0.89 band critical 조건 분석                                        |
| `scripts/fitting_audit.py`                   | 분포 이동 vs fitting 진단                                                |
| `artifacts/phase1_cases_baseline/`           | Stage 별 baseline artifact 보관 (gitignored)                             |

### 회귀 잠금

- `tests/modules/test_detection/test_phase1_case_builder_stage1.py` — anti-disappear 4 + anti-noise 4 + composite_sort + macro 실주입 = 10 테스트 통과 (78/78 회귀 OK)

### 학습 — fitting 식별 패턴

본 케이스에서 fitting 패턴이 어떻게 전개됐는지 정리:

1. **목표 설정**: "0.90+ 100~200건" 같은 수량 목표를 코드/조건 결정의 기준으로 삼음.
2. **상위 band 분석**: 0.80~0.89 구간만 보고 그 안에서 조건 매칭률 계산.
3. **조건 재설계**: 매칭률이 목표 범위 안으로 들어오도록 조건의 폭을 조정.
4. **도메인 정당화**: 조정된 조건을 "도메인 원칙" 으로 설명.

문제: (1) 의 수량 목표가 (3) 의 조건 결정에 직접 영향을 줬다는 점. (4) 의 도메인 정당화는 사후 합리화. truth label 을 직접 학습하지 않아도 분포 보정만으로 fitting.

**회피 원칙 (D060 5조)**:
- 조건은 도메인 진술 단위로 정의 (강한 seed + 중요성 + 독립 보강 1개).
- 결과 카운트는 관찰 지표. count 보고 조건 재조정 금지.
- 다른 데이터셋 (e.g., fy2023) 은 holdout. 조건 정의는 한 데이터셋만 사용.
- 매 stage 1회 측정만 (반복 미세조정 금지).

### 관련 문서

- `docs/archive/completed/PHASE1_SCORE_DISTRIBUTION_LOG.md` — Stage 별 누적 측정 로그 (구 가중합 체계, 2026-06-16 archive 이관)
- `docs/spec/DECISION.md` D060 (0.90 critical 승격 원칙 5조), D061 (Stage 2-A 보류 + 계층 재설계)
- `docs/spec/CONSTRAINTS.md` §PHASE1 priority_score 의 명시적 제약
- `docs/guide/users/07_PHASE1_PRIORITY_RANKING.md`

### 해결 — priority_score 계층 재설계를 tier 체계 전환으로 대체 (2026-06-14 ~ 06-16)

Stage 5(priority_score 0.75/0.80/0.85/0.90 다단계 계층)는 결국 **숫자 정밀도를 더 쌓는 방향**이라 같은 fitting 함정을 반복할 위험이 있었다. 근본 진단은 "0.80/0.85가 단일 floor 인공물"이라는 것이었는데, 이는 **가중합 점수 자체가 근거 없는 임의 계수**라는 더 깊은 문제의 증상이었다. 따라서 계층을 세분화하는 대신 **연속 점수 체계를 폐기하고 명명된 순서형 tier로 전환**했다.

**전환 핵심 (SoT `docs/spec/PHASE1_TIER_EVIDENCE_BASIS.md`·`PHASE1_TIER_SCORING_SPEC.md`, 2026-06-14)**:

1. **가중합·floor 숫자·band컷 폐기**: `0.62·max_primary + 0.08·…` 가중합, `0.75/0.90` floor, `topic_score >= 0.90/0.75` band컷을 전부 제거. 근거 없는 숫자 정밀도(false precision)였다.
2. **순서형 tier 직접 매핑**: band를 HIGH/MEDIUM/LOW/CONTEXT 명명 tier로 직접 결정. tier는 "0.9만큼 위험"이 아니라 "먼저 본다"는 순서다. tier 내부 정렬만 단순 tiebreak(독립 primary 수·rule 수·materiality, 가중합 아님).
3. **HIGH 조합 근거화**: HIGH는 임의 floor가 아니라 금감원(FSS) 실증 + 감사기준서 교차검증으로 확정한 5개 명명 조합(가공전표·횡령은폐·가수금·충당금조작·비용자산화)이 발화할 때만 부여(`HIGH_COMBO_GROUNDING.md`).
4. **17건 재감사 → A안 셋째 다리 확장 (2026-06-16)**: FSS HIGH 158건 전수 재집계에서 기존 5조합 밖 17건을 발견·원문 분석한 결과, 11건이 같은 스토리(셋째 정황만 다름)였다. 신규 조합을 만들지 않고 조합1(가공전표)·조합2(횡령은폐)의 인정 2차정황 풀을 실증에 맞게 확장(A안). B안(메타규칙 일반화)·C안(신규 조합)은 과탐·관리부담으로 기각. 상세: `HIGH_COMBO_GROUNDING.md` §5b.

구 가중합 점수체계 문서(`PHASE1_TOPIC_SCORING_V1_LOCK.md`·`PHASE1_SCORE_DISTRIBUTION_LOG.md`)는 `docs/archive/completed/`로 이관(2026-06-16). 본 TS-15의 0.75 클러스터링·dead code 문제는 이 전환으로 해소됐다.

**A안 코드 반영·측정 완료(2026-06-17)**: `topic_scoring.py::_FICTITIOUS_SECONDARY_RULES` 신설로 조합1 셋째 다리 확장, 조합2 `(역분개 L2-05 + 수기 L3-02 + 고액 L4-03)` 분기 추가. 과탐 HARD 가드 측정(정상 v42j 2022, 330,914행 → 14,070 case): wide(6 신규 다리) **5.245% FAIL**(기말 L3-04 734/738·승인일공백 L1-09 334 과발화) → 기말(closing_timing 영역)·L1-09(근거 약함) 제외 → narrowed **0.334% PASS**(≤2%). 부수: `phase1_case_builder._precomputed_string_column` nullable-boolean fillna 버그 수정(전체연도 case 빌드 차단 선행버그). 측정 도구 `tools/scripts/measure_a_an_high_ratio.py`, 결과 `artifacts/a_an_{wide_diag,narrowed}_2022.txt`.

### 상태

해결 (2026-06-17). 가중합 폐기 → 순서형 tier 전환으로 priority_score 계층 재설계 대체 완료. A안 셋째 다리 확장 코드·측정·문서 반영 완료(narrowed HIGH 0.334% ≤ 2%).

---

## TS-16: PHASE2 / PHASE1+2 통합 3등급 분류 도입 — wiring 검증 통과, 운영 분포 검증 별도

### 문제

PHASE1 은 `priority_band` (high/medium/low) 로 3등급 표시가 이미 있으나 PHASE2 단독 큐와 PHASE1+2 통합 큐에는 동일 등급 표시가 없다. 사용자가 감사인 입장에서 모든 큐를 같은 언어 (즉시검토 / 검토대상 / 후보) 로 보고 싶다는 요청.

PHASE2 의 점수 (`phase2_internal_noisy_or_score`) 는 ECDF 기반 상대값이라 절대 임계의 도메인 의미가 약하고, 통합 큐의 RRF score 도 데이터 크기에 따라 분포가 달라진다. PHASE1 의 priority_floors 같은 yaml 도메인 잠금이 직접 적용되지 않는 영역.

### 결정 (D062, 2026-05-21)

#### 분류 정책 — yaml 도메인 잠금 + 정렬 키 보존

PHASE2 단독 등급 (v2 채택, strong + coverage 보수적 정의):

```
즉시검토:  max_evidence_tier == "strong" AND coverage_breadth_q95 >= 2
검토대상:  max_evidence_tier == "strong" AND coverage_breadth_q95 < 2
...
후보:     phase2_noisy_or > 0 AND 위 조건 미달
신호없음:  phase2_noisy_or == 0
```

통합 등급 — "max band 취하되 즉시검토만 교집합":

```
통합 즉시검토:  PHASE1 즉시검토 AND PHASE2 즉시검토   (양측 모두 강한 경우만)
통합 검토대상:  PHASE1 즉시검토 OR PHASE2 즉시검토
              OR PHASE1 검토대상 OR PHASE2 검토대상  (한쪽이라도 강하면)
통합 후보:    PHASE1 후보 OR PHASE2 후보
통합 신호없음:  양측 모두 신호없음
```

신규 필드 (가상 점수 없이 band 만):
- `phase1_review_band` (priority_band alias)
- `phase2_review_band` (신규)
- `phase12_review_band` (신규)

정렬 키 변경 없음 — PHASE1 composite_sort, PHASE2 noisy_or, 통합 RRF 모두 보존. band 는 분류/표시/필터/KPI/badge 용도.

#### v3 안 (strong 단독 즉시검토 + max(P1, P2) 통합) 거부 사유

PHASE1 Stage 2-A 가 "도메인상 맞아 보여도 모집단에서 흔하면 즉시검토 폭증" 패턴으로 fitting 위험 보였다 (TS-15). v3 의 strong 단독 즉시검토 + max(P1, P2) 합집합 통합 은 같은 폭증 위험. v2 의 strong+coverage>=2 + 통합 교집합 이 보수적이고 TS-15 학습 정합.

### 구현 단계

| 단계      | 작업                                                                                                                                                         | 결과 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---- |
| P2-prep   | D062 결정 잠금 + `config/phase2_review_band.yaml` 신규 (정책 잠금 문서, 코드는 상수 lock)                                                                    | 완료 |
| P2-A      | `Phase2CaseOverlay.phase2_review_band` 필드 추가 + `classify_phase2_review_band` helper                                                                      | 완료 |
| P2-B      | `phase12_review_band` 통합 build 위치 추가 + `classify_phase12_review_band` helper                                                                           | 완료 |
| P2-B 보강 | Stage7 의 `build_phase2_case_overlays` 호출에 family_ecdf_by_case + family_q95_thresholds + family_roles 인자 보강. unsupervised/ml_unsupervised alias 통일. | 완료 |
| P2-D      | Stage7 cache 옵션 B 측정 (wiring 검증)                                                                                                                       | 완료 |
| P2-C      | dashboard UI 라벨 통일 — phase2 탭 + 통합 review queue 브라우저만 변경 (phase1 결과 탭 + 전체개요 탭 제약)                                                   | 완료 |
| P2-E      | code-reviewer + 문서 갱신                                                                                                                                    | 완료 |

### P2-D 측정 결과 (2026-05-21, 옵션 B)

#### 측정 방식

기존 Stage7 cache/parquet 위에서 새 review_band 컬럼 분포 산출. 전체 재추론 X, wiring 검증 목적.

- 입력: `artifacts/stage7_phase1_case_result.pkl` (May 17, PHASE1 case cache)
- 입력: `artifacts/stage7_phase2_family_by_doc.parquet` (PHASE2 5-family by-doc cache)
- 실행: `uv run python tools/scripts/phase1_phase2_integration_stage7.py` (44.3초)
- 산출: `data/companies/_ci_baseline/engagements/2026/review_queue/v1/queue_{phase1,phase2,integrated}.parquet`
- 측정: `scripts/audit_phase2_review_band_distribution.py`

#### 결과 — 3 컬럼 wiring 정상

| Column              | Present | Non-empty count |
| ------------------- | ------- | --------------- |
| phase1_review_band  | ✓       | 41,129          |
| phase2_review_band  | ✓       | 41,129          |
| phase12_review_band | ✓       | 41,129          |

#### phase2_review_band 분포
...
PHASE2 등급 진입 0건. **본 측정 한계 (D062, 2026-05-21)**: family_top_subdetectors_by_case 가 Stage7 측정 스크립트에서 비어있어 max_evidence_tier 가 None. ML-only ECDF ≥ 0.995 + coverage ≥ 2 분기도 매칭 안 됨. **실제 운영 경로 (dashboard 의 phase2_inference_service) 에서 sub-detector 정보가 채워지는지는 별도 조사 필요**.

#### phase12_review_band 분포

```
immediate: 0       ← P1 immediate AND P2 immediate, P2 immediate=0 이라 0
review:    23,954  ← P1 immediate + P1 review (P2 모두 candidate)
candidate: 17,175  ← P1 candidate
none:      0
```

#### phase12 교차표 — 정책 정합 검증

| 통합 band | P1 / P2 조합                | Count  |
| --------- | --------------------------- | ------ |
| immediate | (empty)                     | 0      |
| review    | P1=immediate / P2=candidate | 3,729  |
| review    | P1=review / P2=candidate    | 20,225 |
| candidate | P1=candidate / P2=candidate | 17,175 |
| none      | (empty)                     | 0      |

통합 정책 "양측 모두 강한 경우만 즉시검토, 한쪽만 강하면 검토대상" 이 정확히 작동. `P1=immediate / P2=candidate → phase12=review` 는 교집합 정책에 따른 의도된 격하 (P1 만 강해서는 통합 즉시검토 진입 불가).

#### 추가 발견 — PHASE1 cache stale

phase1_review_band 의 immediate=3,729건. fy2022 stage_1 측정의 21건과 격차 큼.

원인: `artifacts/stage7_phase1_case_result.pkl` 의 timestamp **May 17 10:51** — 사용자가 `config/phase1_case.yaml` 의 `priority_band.high` 를 0.75 → 0.90 으로 상향한 시점 (Stage 1, May 20) **이전**. Stage7 cache 의 priority_band 가 이전 yaml (high=0.75) 기준으로 매겨져 있다. 본 측정의 PHASE1 분포는 이전 정책 기준이라 운영 count 가 아니다. wiring 검증에는 영향 없다.
...
| ---------------------------- | ----------------------------------------------- |
| 3 컬럼 wiring                | ✓ 통과                                          |
| phase2_review_band 매핑 로직 | ✓ 정합 (모두 candidate — Stage7 한계 정합)      |
| phase12_review_band 정책     | ✓ "max band 취하되 즉시검토만 교집합" 정확 작동 |
| max_evidence_tier 채움       | ✗ Stage7 한계 — sub-detector 정보 부재          |
| coverage_breadth_q95 채움    | ✗ Stage7 한계                                   |
| max_family_ecdf 채움         | ✗ Stage7 한계                                   |
| PHASE1 cache yaml 정합       | ✗ May 17 이전 priority_band 기준                |
| 운영 count 확정              | ✗ 본 측정 목적 외                               |

**정책/컬럼 wiring 검증 통과. 운영 분포 검증은 아직 아님.**

### 다음 단계

### P2-C 전 운영 경로 조사 결과

dashboard 실 운영 경로에서 sub-detector 정보 채움 여부를 조사했다.

| 호출 위치                                                                       | 평가                                          |
| ------------------------------------------------------------------------------- | --------------------------------------------- |
| `src/services/phase2_inference_service.py::_attach_phase2_case_overlays`        | 5개 overlay 입력 전달                         |
| `src/pipeline.py::_build_phase2_case_overlays`                                  | 5개 overlay 입력 전달                         |
| `tools/scripts/phase1_phase2_integration_stage7.py::build_case_overlay_payload` | sub-detector 제외 3개 입력만 전달 — 측정 한계 |

실 운영 경로는 `phase2_case_family_aggregator.build_phase2_case_family_overlay_inputs` 를 통해 `family_scores_by_case`, `family_ecdf_by_case`, `family_top_subdetectors_by_case`, `family_roles`, `family_q95_thresholds` 를 모두 전달한다. 따라서 dashboard 실 경로에서는 `max_evidence_tier`, `coverage_breadth_q95`, `max_family_ecdf` 가 채워진 상태로 `Phase2CaseOverlay` 가 만들어진다.

결정: P2-C UI 진입 가능. 단 운영 count 는 Stage7 cache 가 아니라 fresh inference 또는 실제 dashboard 경로에서 별도 확인한다.

### P2-C UI 반영

- `dashboard/tab_phase2.py`: PHASE2 KPI 를 `phase2_review_band` 기준 `즉시검토 / 검토대상 / 후보` 로 표시. Lane 표에 `Phase2 등급` 추가.
- `dashboard/components/review_queue_browser.py`: 통합 큐 표에 `통합 등급 / PHASE1 등급 / PHASE2 등급` 우선 표시. 정렬 점수 컬럼은 기본 본문 표에서 숨김.
- 사용자 본문에서 `RRF`, `k=60`, `Noisy-OR` 같은 알고리즘명 노출 제거.
- 합성 truth KPI 문구를 "발견 부정 전표" 에서 "검증 라벨 매칭 전표" 로 격하.
- `dashboard/tab_phase1.py`, `dashboard/tab_overview.py` 는 변경하지 않음.

### 산출물

- `config/phase2_review_band.yaml` (정책 잠금 문서)
- `src/services/phase2_case_contract.py::classify_phase2_review_band` / `classify_phase12_review_band` helper
- `tools/scripts/phase1_phase2_integration_stage7.py::_build_family_q95_thresholds` / `_build_family_ecdf_by_case` 보강
- `scripts/audit_phase2_review_band_distribution.py` (옵션 B 측정 도구)
- `tests/modules/test_services/test_phase2_review_band.py` (30 잠금 테스트)
- `dashboard/tab_phase2.py`
- `dashboard/components/review_queue_browser.py`

### 관련 문서

- `docs/spec/DECISION.md` D062
- `docs/spec/CONSTRAINTS.md`
- `docs/guide/users/08_PHASE2_FAMILY_STRUCTURE.md`, `09_REVIEW_QUEUE_RRF_FUSION.md`

### 상태

완료 (2026-05-21). P2-prep / A / B / B 보강 / D / C / E 완료. 남은 리스크는 운영 count 확정이며, 이는 fresh inference 또는 실제 dashboard 경로에서 별도 측정한다.
...
해결: 룰 결과를 6개 감사 주제(Theme) queue로 묶는다.
...
> 옛 7번째 queue `관계사·내부거래·순환구조`(`intercompany_cycle`)는 PHASE1-1에서 제거됨 → PHASE1-2 family(graph/relational)로 이관(SoT [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) §7.3). L3-03만 계정분류 topic booster로 잔존.
...
1. PHASE2는 처음부터 **비지도 학습 + zero-day 탐지**로 설계됐다(`docs/spec/PHASE2_GOVERNANCE_DESIGN.md` 결정 5 옵션 R). truth 라벨을 외우는 supervised 학습이면 V4에 없는 새 부정 유형을 못 잡는다. 정답 단서를 차단하면 zero-day 능력이 오히려 강화된다.
...
5. **PHASE1은 fraud detector가 아니다.** 룰 hit 모두를 부정으로 보지 않고, 7개 감사 주제 queue 안에서 증거 조합·금액 중요성·반복성·통제 우회·관계사 구조로 case priority를 매긴다 (TS-9).
...
**관련 문서**: [00-dataset.md](../archive/completed/raw-plan/00-dataset.md) | [DECISION.md D010](DECISION.md)
...
- `timing_anomaly`: L3-04, L3-05, L3-06, L3-07
...
- `L3-05` 같은 booster 룰과 `L4-06` 같은 combo-only 룰은 단독 점수보다 결합 증거로 해석
...
- `L3-01`은 폐기되었다(2026-06-20). 계정-업무 불일치 정답표 의존·통합점수 미참조로 제거하고, 희소 계정 조합은 `L4-04`가 데이터 기반으로 포착한다.
...
1. **기준서 전면 재탐색**
   ...
   - → `docs/spec/DETECTION_REFERENCE.md`에 기록.
...
| completed/raw-plan/05-detection.md      | 신규 탐지 룰 후보 (Phase 1~3)                              |
| completed/raw-plan/02-ingest.md         | 확장 컬럼 매핑 (16개)                                      |
| completed/raw-plan/03-feature.md        | 신규 파생변수 (11개)                                       |
| completed/raw-plan/03a-preprocessing.md | 확장 컬럼 전처리 + White Box                               |
| completed/raw-plan/04-validation.md     | L3 검증 확장 (통제 효과성, TB 대사)                        |
| completed/raw-plan/05a-detection-ml.md  | Phase 2 신규 탐지 (TrendBreak 등 5건)                      |
| completed/raw-plan/06-db.md             | GL 확장 + Trial Balance DDL                                |
| completed/raw-plan/08-llm.md            | Phase 3 NLP/LLM (2건)                                      |
| (구) TASKS.md → `dev/active/<plan>/`    | Phase 1b 확장 3건 + DataSynth 확장 5건                     |
...
**관련 문서**: [DETECTION_RULES.md](DETECTION_RULES.md) | [DETECTION_REFERENCE.md](DETECTION_REFERENCE.md) | `generation_principles.md` (현재 체크아웃에 없음) | [DECISION.md D011~D013](DECISION.md) | [DETECTION_RULES §6 갭 분석](DETECTION_RULES.md) | [E2E 테스트 결과](../../tests/modules/test_detection/test-results/e2e-detection-datasynth.md)
...
**관련 문서**: [CONSTRAINTS.md §ML 학습 전략](CONSTRAINTS.md) | [completed/raw-plan/05a-detection-ml.md](../archive/completed/raw-plan/05a-detection-ml.md) | `dev/active/phase2-*` (구 TASKS.md §Phase 2)
...
**관련 문서**: [completed/NEW_TASKS.MD](../archive/completed/NEW_TASKS.MD) | [CONSTRAINTS.md §회사별 매핑 프로파일](CONSTRAINTS.md)
...
**관련 문서**: [CLAUDE.md §DATASYNTH 생성 규칙](../../CLAUDE.md) | `generation_principles.md` §14 ML 안전성 (현재 체크아웃에 없음)

---

## TS-6: Historical LLM architecture experiment — superseded by local-first boundary

**분류**: 아키텍처 | **해결일**: 2026-04-09 | **현재 상태**: D068(2026-05-26)로 superseded

> 2026-05-26 업데이트: 아래 내용은 historical record다. 하이브리드(로컬 ML + 상용 API LLM) 결정은 폐기되었고, active product path는 외부 LLM/API 없이 PHASE1/PHASE2 로컬 분석과 deterministic Local Evidence Brief만 사용한다. 원장 근거, case evidence, rule hit, PHASE2 family signal은 외부 서비스로 전송하지 않는다.
...
3. **하이브리드 (로컬 머신러닝 + 외부 API LLM)** ← 당시 선택, 현재 D068로 폐기
...
| LLM 풀어쓰기 (Phase 3)  | 외부 API                    | historical only, D068로 제거 |
...
**관련 문서**: [CONSTRAINTS.md §하드웨어 제약](CONSTRAINTS.md) | [CONSTRAINTS.md §비식별화](CONSTRAINTS.md) | [PROJECT_OVERVIEW.md](../guide/PROJECT_OVERVIEW.md)
...
**관련 문서**: [DATASYNTH_PHASE1_RULE_TRUTH_DRAFT.md](../archive/completed/DATASYNTH_PHASE1_RULE_TRUTH_DRAFT.md) | [DATASYNTH_PATCH_WORKFLOW.md](../archive/completed/DATASYNTH_PATCH_WORKFLOW.md) | [DATASYNTH_UPDATE_CHECKLIST.md](../archive/completed/DATASYNTH_UPDATE_CHECKLIST.md)
...
> **Superseded in part by [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) (2026-06-14)**: 아래 TS-9·TS-9 Addendum의 7개 queue 중 7번째 `관계사·내부거래·순환구조`(`intercompany_cycle`)는 삭제됐다. 순환거래·내부거래·관계사 구조는 PHASE1-1 룰 topic이 아니라 PHASE1-2 family(graph/relational)의 정식 대상으로 이관됐고(SoT §7.3), IC01~03·GR01/03의 PHASE1-1 점수경로는 제거됐다(L3-03만 계정분류 topic booster로 잔존). 따라서 현행 공식 queue는 **6개**다. 또한 `fraud_combo_floor`/High floor `0.75` 등 숫자값은 명명된 tier(순서형 HIGH/MEDIUM/LOW/CONTEXT)로 대체됐다. 아래 7개 queue·floor 숫자 서술은 결정 당시의 역사 기록으로 보존하며, 운영 기준은 SoT를 따른다.
...
| Queue                      | 포함 룰                                         | 목적                                                            |
| -------------------------- | ----------------------------------------------- | --------------------------------------------------------------- |
| 원장기록·데이터정합성      | L1-01, L1-02, L1-08                             | 전표가 원장 기록으로서 유효한지 확인                            |
| 승인·권한·업무분장 통제    | L1-04, L1-05, L1-06, L1-07, L1-09, L3-02, L3-12 | 승인권한 초과, 자기승인, 승인생략, 수기우회, 업무범위 집중 확인 |
| 결산·기간귀속·입력시점     | L3-04, L3-05, L3-06, L3-07, L3-11, L4-05        | 결산 말, 휴일·야간, 사후입력, cutoff 전표 확인                  |
| 계정분류·거래실질 불일치   | L1-03, L2-04, L3-09, L3-10, L4-04               | 계정 조합과 거래 설명이 업무 실질과 맞는지 확인                 |
...
| 결산수정 조작 의심 | `결산수정 조작 의심` | 결산·기간귀속·입력시점     | `(L3-04 or L3-07 or L3-11 or L1-08) + L4-03 + (L3-10 or L4-04)`                         |
...
AI 학습 시 입력에서 빼는 컬럼 집합이다. 비유하면 **부정 관련 검토 신호 식별 AI를 학습시킬 때 "빨간 모자, 큰 가방, 비싼 시계 같은 결정적 단서는 보여주지 마라"라고 차단하는 것**과 같다. AI는 말투·자세·글씨체 같은 **간접 단서**로 학습하게 된다.

#### 6.2 왜 정답 단서를 차단해도 괜찮은가

PHASE2 학습은 처음부터 **비지도 학습 + 본 적 없는 부정 발견**으로 설계됐다(`docs/spec/PHASE2_GOVERNANCE_DESIGN.md`의 결정 5 옵션 R, `docs/spec/DECISION.md` 같은 영역 참조). 두 가지로 정당화된다.
...
PHASE1에서도 같은 원칙이 적용됐다. `feedback_phase1_truth_recall_guard` 메모리와 `docs/spec/CONSTRAINTS.md`의 자동 감시 정책:
...
- 별도 설계 PR 의무 (`docs/spec/PHASE2_GOVERNANCE_DESIGN.md` 결정 5)
...
`docs/spec/DECISION.md` D044(Rust 승격 PR 템플릿)을 이 결정에도 적용:
...
이 규칙은 메모리(`feedback_phase1_truth_recall_guard`)와 `docs/spec/CONSTRAINTS.md`에 박혀 있고, TS-10의 V7 deny-list 결정도 이 원칙을 PHASE2로 확장한 것이다.
...
`docs/spec/RULE_DETAIL_METADATA_V1_LOCK.md`에서 **32개로 못 박았다**. 계산식은 단 하나만 인정한다.
...
`docs/spec/PHASE1_SEPARATE_BENCHMARK_SPEC.md`에서 5개 룰을 **별도 평가 단위**로 분리했다.
...
| **B** | 회계 의미 재정비 후에야 검증 가능 | L2-02 중복지급, L2-04 비용 자산화, L4-04 희소 계정쌍 등       |
...
- ~~**관계사 순환 (intercompany_cycle)**~~ — PHASE1-1 topic에서 제거됨. 그래프 분석이 필요하므로 PHASE1-2 family(graph/relational)의 정식 대상으로 이관(SoT [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) §7.3). 이 Medium 보관소 대상이 아니다.
...
1. **PHASE1은 부정 판정기가 아니다.** 검토 후보를 추리는 1차 단계다(2절).
...
**관련 문서**: [PHASE1_TOPIC_SCORING_V1_LOCK.md](../archive/completed/PHASE1_TOPIC_SCORING_V1_LOCK.md) · [RULE_DETAIL_METADATA_V1_LOCK.md](RULE_DETAIL_METADATA_V1_LOCK.md) · [DETECTION_RANKING_CRITERIA.md](DETECTION_RANKING_CRITERIA.md) · [PHASE1_SEPARATE_BENCHMARK_SPEC.md](PHASE1_SEPARATE_BENCHMARK_SPEC.md) · [PHASE1_RULE_RELATIONSHIP_MAP.md](PHASE1_RULE_RELATIONSHIP_MAP.md) · [.github/workflows/phase1-kpi-guard.yml](../../.github/workflows/phase1-kpi-guard.yml)

---

## TS-12: PHASE2 점수가 통합 정렬에 미반영 + truth case 분모 인플레이션

**분류**: 탐지 정렬 / 평가 | **정리일**: 2026-05-18 | **상태**: 진행 중 (수정안 토론 단계, 코드 변경 전)

### 1. 이 절이 다루는 것

V7 fixed3 PHASE2 inference 결과(`docs/guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md`)에서 TOP-N recall이 낮게 나오는 두 개의 구조적 원인을 분리해서 기록한다. 하나는 정렬식 결함(코드 측), 다른 하나는 recall 분모 정의(평가 측)다. 둘은 별개 문제이고 해결 경로도 다르다.

핵심 한 줄: **"통합 정렬"이라는 라벨이 가리키는 큐는 사실 PHASE1 단독 정렬이다. PHASE2 점수는 옆에 표시만 되고 정렬에 들어가지 않는다. 동시에 truth recall 분모는 truth document 수가 아니라 case 수라서 1.28배 부풀려 있다.**

### 2. 측정된 사실

V7 fixed3 (manipulation v7 candidate fixed3, 1,032,864행 / 317,997 doc / truth 620 doc) 위에서 PHASE2 5 family inference 실행 후 PHASE1 case 41,129개의 review queue를 만든 결과:

```
구간        PHASE1만 잡은 truth   통합 정렬이 잡은 truth   차이
TOP 100              27                    27              0
TOP 500             115                   115              0
TOP 1,000           156                   156              0
TOP 2,000           215                   215              0
...
PHASE1과 통합 정렬의 TOP-N에서 잡힌 truth가 **모든 구간에서 정확히 일치**한다. 같은 큐를 PHASE1로 한 번, "통합"으로 한 번 정렬했는데 결과 행이 동일하다는 뜻이다.

PHASE2 점수 단독으로 row-level top-N을 잡으면 다른 결과가 나온다.

```
PHASE2 점수 상위 100   18 truth (case 단위 18/795)
PHASE2 점수 상위 1k   159 truth (PHASE1 단독 156과 비슷)
PHASE2 점수 상위 2k   266 truth (PHASE1 단독 215보다 51건 많음)
```

PHASE1과 PHASE2의 상관계수는 +0.07 (max) / -0.23 (mean)으로 거의 독립이다. 두 신호가 서로 다른 case를 잡는다는 뜻이며, 합쳐 쓰면 이론상 한 쪽 단독보다 많이 잡을 수 있다.

```
PHASE1 상위 1% OR PHASE2 상위 1% (합집합)    truth 155 / recall 19.50%
PHASE1 상위 1% AND PHASE2 상위 1% (교집합)   truth  13 / precision 36.11%
PHASE1 상위 1% 단독                          truth 108 / recall 13.58%
PHASE2 상위 1% 단독                          truth  66 / recall  8.30%
```

합집합 recall이 단독보다 6%p 높고 교집합 precision이 PHASE1 단독의 26.21% 대비 9.9%p 높은데, 현재 정렬은 이 두 효과를 모두 버리고 있다.

### 3. 원인 — 두 개 분리

#### 3.1 정렬식이 PHASE2를 인자로 받지 않는다 (코드 측)

`tools/scripts/phase1_phase2_integration_stage7.py` `build_queue_rows` 함수에서 review queue 정렬은 다음 키 순서로 수행된다.

```python
queue_df = queue_df.sort_values(
    by=[
        "phase1_composite_sort_score",   # 1차
        "phase1_triage_rank_score",      # 2차
        "total_amount",                  # 3차
        "rule_count",                    # 4차
    ],
    ascending=False,
...
같은 함수 주석은 "PHASE2 overlay 는 정렬에 영향 주지 않음"이라고 명시한다. V1 lock 규약상 PHASE1 priority_score 원본을 100% 보존해야 하고, PHASE2 점수는 `phase2_unsupervised_selection_score_max / _mean`과 `phase2_adjusted_priority` 컬럼에 따로 적재만 한다.

`src/services/phase2_case_contract.py:_adjusted_priority`는 `0.7 × phase1 + 0.3 × phase2_mean`을 계산해 `phase2_adjusted_priority` 컬럼에 저장하지만, 이 값은 어디서도 정렬 키로 쓰이지 않는다. 즉 PHASE2는 큐 옆에 표시되는 보조 정보일 뿐이고 case 순위에는 기여하지 않는다.

이게 §2 표에서 "통합 정렬" 결과가 PHASE1 단독과 완전히 일치하는 직접적 원인이다.

#### 3.2 truth case 분모가 truth document 대비 1.28배 부풀어 있다 (평가 측)

```
truth document 수        620
truth case 수            795
case / document 비율    1.28
```

DataSynth가 심은 부정 거래는 document 단위로 620건이다. PHASE1 case builder는 한 document가 여러 topic에 동시에 hit하는 경우 topic마다 별도 case를 만든다(`primary_topic` 결정 후 그룹화). 결과적으로 truth document 1개가 평균 1.28개의 case로 매핑된다.

recall을 case 단위로 계산하면 분자(잡은 truth case)도 같은 비율로 늘기 때문에 큰 영향이 없을 것처럼 보이지만, 실제로는 다음 두 이유로 분모만 부풀려진다.

- 한 truth document가 만든 여러 case 중 가장 priority가 높은 case 1개만 TOP-N에 들어가면, 나머지 case들은 분모에 남아 있어도 분자에 못 들어간다. 동일 truth가 multi-counting되는 분모.
- 같은 document의 다른 topic case는 정의상 PHASE1 priority가 낮을 가능성이 크다(주제가 약해서 다른 topic으로 갈렸으므로). 이 case들은 TOP-N에서 빠지고 분모에만 남는다.

`docs/guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §1/§2의 모든 recall 수치는 분모 795(case 단위)다. document 단위 recall로 환산하면 동일한 검토 case 수에서 더 높은 값이 나올 가능성이 크다. 다만 변환 함수가 어디에도 적용되지 않아 측정값이 없다.

### 4. recall이 "낮게 보이는" 데 기여하는 부가 요인

§3의 두 가지가 주된 원인이지만 다음 요인도 같이 작용한다. 본 절에서는 해결책을 다루지 않는다.

- **family 분포 왜곡**: `timeseries TS02 (unusual_frequency)`가 86.3% 행 hit. 점수가 ECDF/rank 정규화 없이 family score에 합산되면 다른 family 신호를 묻는다.
- **intercompany family 부분 dormant**: 3 sub 중 IC01만 부분 활성, IC02/IC03 hit 0. 5 family 다양성 손실. 데이터 enrichment 작업 필요.
- **supervised 트랙 비활성**: truth prevalence 0.06%로 `low_signal_fallback`. VAE 단독 + 4 rule-style proxy로 운영 중.
- **Recall 수학적 상한**: TOP 100 / 795 = 12.6%. AUROC 0.93 모델이 1.93% prevalence에서 TOP 100에 27건 모은 것은 무작위 대비 14배 enrichment로, 절대 수치는 정상 범위.

### 5. 영향

- TOP-100 운영 정책에서 PHASE2 단독으로 잡히는 18 truth 중 PHASE1 단독 27 truth에 포함되지 않는 case가 누락된다.
- 교집합 36 case(precision 36%)가 별도 부스트 없이 일반 PHASE1 순위 안에 묻혀 있다.
- AUROC 0.93의 VAE가 review queue 순위에 기여하지 못한다. inference 비용만 발생하고 정렬 효용이 0이다.
- recall 보고가 case 단위 단일 분모로만 산정되어 실제 운영 효율(document 단위 회수율)이 과소 보고된다.

### 6. 수정 방향 (코드 변경 전 토론 항목)

본 절은 결정이 아니라 토론 항목이다. 본 TS-12의 후속 작업으로 다룬다.

#### 6.1 정렬 — 3개 큐 + RRF (2026-05-18 확정)

**큐 구조 (확정)**: PHASE1 단독 큐 + PHASE2 단독 큐 + 통합 큐 3개 분리.

| 큐          | 정렬 키                            | V1 lock                                              |
| ----------- | ---------------------------------- | ---------------------------------------------------- |
| PHASE1 단독 | composite_sort_score (기존 그대로) | 완전 유지 (priority_score 값·산정식 무변경)          |
| PHASE2 단독 | phase2_unsupervised_score_max      | lock 무관                                            |
| 통합        | Reciprocal Rank Fusion (RRF, 아래) | lock 무관 (PHASE1 case 점수 값은 컬럼에 그대로 표시) |

**통합 큐 정렬식 (확정)**: Reciprocal Rank Fusion.

```
RRF_score(case) = Σ 1/(k + rank_i(case))
rank_i ∈ {phase1_composite, phase2_unsupervised, phase2_timeseries, phase2_relational, phase2_duplicate, phase2_intercompany}
k = 60 (Cormack 2009 권장값, 산업 표준 default)
```

- `rank_phase1_composite`: case를 `phase1_composite_sort_score` 내림차순 정렬한 순위.
- `rank_phase2_*`: case를 PHASE2 5-family document max score 내림차순 정렬한 순위.
- 두 reciprocal 합으로 최종 정렬. 점수 분포(0~1 vs 0~∞ 등) 차이 무관.

**채택 사유 (단일)**: PHASE1 ↔ PHASE2 상관계수가 +0.07 / −0.23으로 사실상 무상관이다. 두 ranker가 독립일 때 RRF의 fusion 효과가 가장 크다는 것이 Cormack 2009 §4의 핵심 결과다. 본 데이터에서 두 신호의 union recall(상위 1% 합집합 19.50%)이 단독(13.58% / 8.30%)보다 6%p 이상 크다는 것이 독립성의 정량 증거다.

**2026-05-19 확장 사유**: PHASE2 5-family document max score 상관 측정에서 최대 Spearman |ρ|=0.2108, 평균 |ρ|=0.0745로 모두 |ρ|<0.3이다. 따라서 `unsupervised`만 통합 정렬에 넣던 2-way RRF를 `unsupervised/timeseries/relational/duplicate/intercompany` 5-family RRF로 확장한다. 근거 산출물: `artifacts/phase2_family_correlation_matrix_20260519.md`.

**채택하지 않은 후보**:

| 후보                                    | 미채택 사유                                                                      |
| --------------------------------------- | -------------------------------------------------------------------------------- |
| max(p1_pct, p2_pct)                     | RRF의 단순 변종, 교집합 부스트 효과 없음                                         |
| α × p1 + (1−α) × p2                     | α 결정에 truth 사용 시 fitting 위험, α 고정 정당화 어려움                        |
| Borda Count                             | RRF와 유사하나 산업 표준 위치가 약함                                             |
| CombSUM / CombMNZ                       | 점수 분포 정규화 필요, 본 케이스 분포 차이로 불안정                              |
| Cross-encoder / LambdaMART              | 라벨 학습 필요, fitting 위험                                                     |
| SEAD quantile fusion (Amazon ICML 2025) | 검토했으나 구현 복잡도가 RRF 대비 3~4배. 도메인 차별성 vs 안정성에서 안정성 선택 |

**k=60 고정 정책**: k를 V7 fixed3 truth로 튜닝하면 `feedback_phase1_truth_recall_guard` 위반이다. 60은 학술/산업 default로 받아들이고 도메인 정합성으로 정당화한다.

**구현 위치 예정**: `tools/scripts/phase1_phase2_integration_stage7.py` `build_queue_rows`에서 단일 `queue.parquet`만 만들던 부분을 3개 큐(`queue_phase1.parquet` / `queue_phase2.parquet` / `queue_integrated.parquet`)로 분리. 통합 큐는 RRF 점수 컬럼(`rrf_score`) 신설 후 그것으로 정렬.

**산업 표준 근거**: RRF는 Microsoft Azure AI Search, OpenSearch 2.19, Elasticsearch, Vespa, Weaviate, MongoDB, MariaDB의 hybrid search 기본 fusion 방식이며 2024-2025 RAG 시스템의 사실상 표준이다.

#### 6.2 truth case 분모 인플레이션 해결 후보

PHASE1 case 구조를 그대로 둔 채 평가만 보정하는 방법과, case 구조 자체를 손대는 방법이 있다.

| 해결 후보                                                                | 변경 범위        | 영향                                                                                | 위험                                                                          |
| ------------------------------------------------------------------------ | ---------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| (a) recall을 document 단위로 병행 보고                                   | 평가 스크립트만  | 가장 가벼움. case 단위 분모와 동시 게시. 운영 의미가 두 가지로 분리됨               | UI에서 두 수치 동시 노출 시 사용자 혼란. 어느 쪽이 "정답"인지 정책 결정 필요  |
| (b) recall을 truth document 기준으로만 단일 보고                         | 평가 스크립트만  | 명확. document 분모로 일원화                                                        | case 단위 신호(같은 doc 여러 topic) 손실. PHASE1 case 구조 평가 정보가 사라짐 |
| (c) case 단위로 unique truth document 보존하며 분자 카운트 정의 변경     | 평가 스크립트만  | "TOP-N 안의 truth document 합집합" 정의로 분자 산정. case 분모 유지하지만 분자 보정 | recall 정의 비표준. 외부 비교 어렵게 됨                                       |
| (d) PHASE1 case builder에서 truth document 1개 = case 1개로 강제         | case 구조 자체   | 근본 해결. 같은 doc이 여러 topic에 있어도 대표 case만 남김                          | PHASE1 운영 의미 변경. 같은 doc의 다른 topic 신호가 큐에서 사라짐. 비범위     |
| (e) primary_topic 분기 정책 재설계 (한 doc 다 topic case 생성 정책 폐기) | case 구조 + 정책 | 근본 해결                                                                           | PHASE1 V1 lock 광범위 영향. 본 TS 범위 밖                                     |

추천: **(a)**가 비용 대비 효과 가장 큼. document 단위 recall을 보조로 추가 보고하면 truth 정의가 명확해진다. (c)는 정의가 비표준이라 외부 보고에 부적합. (d)/(e)는 PHASE1 구조 변경이라 별도 sprint.

#### 6.3 fitting 위험 분리

위 6.1, 6.2 모두 **truth label을 학습 신호로 쓰지 않는다**. 정렬 식 가중치(A~D안의 α 등)를 V7 fixed3 truth로 grid search하면 fitting이며 `feedback_phase1_truth_recall_guard` 메모와 §3 KPI guard 위반이다. 가중치는 도메인 정합성으로만 결정하고 truth 기반 튜닝은 금지.

### 7. 결정 진행 현황

| 항목                            | 상태                                                                                                                          | 결정                                                                                                                             |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| 큐 구조                         | ✅ 확정 (2026-05-18)                                                                                                          | 3개 분리 (PHASE1 / PHASE2 / 통합)                                                                                                |
| 통합 정렬 식                    | ⚠️ 정정 (2026-05-19) — §9 참조                                                                                                | RRF 5-way 확장 시도 실패. 2-way RRF는 일부 깊이에서만 유효. Phase A.5 결과 보고 폐지/유지 결정                                   |
| V1 lock 개정                    | ✅ 자동 해결                                                                                                                  | 3큐 구조로 PHASE1 단독 큐는 lock 유지, 통합 큐는 lock 무관                                                                       |
| RRF 통합 큐 구현                | ✅ 구현 (2026-05-18, [D058](DECISION.md#d058-phase1phase2-통합-큐에-reciprocal-rank-fusion-rrf-적용--3개-큐-분리-2026-05-18)) | `src/services/queue_fusion.py` + 3큐 parquet + 4탭 UI                                                                            |
| recall 분모 정책                | ⚠️ 정정 (2026-05-19) — §9 참조                                                                                                | **case 단위 통일**, 분모 case 620 (1 truth doc → 1 truth case 매핑). doc 단위는 외부 비교 보조 지표                              |
| V7 PHASE2 문서 doc 단위 수정    | ✅ 완료                                                                                                                       | `docs/guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` doc 단위 보강                                                    |
| V7 fixed3 PHASE1 단독 결과 문서 | ✅ 완료 (2026-05-18)                                                                                                          | `docs/archive/completed/DETECTION_RESULTS_MANIPULATION_V7_FIXED3.md` + `artifacts/phase1_manipulation_v7_fixed3_evaluation.json` |
| CONTRACT_V3 작성                | ✅ 완료 (2026-05-18)                                                                                                          | `docs/guide/DETECTION_RESULTS_CONTRACT_V3.md` 신규 작성                                                                          |
| 큐 미진입 truth doc 80건        | ✅ TS-13 신설                                                                                                                 | 본 문서 TS-13 참조                                                                                                               |

### 8. 비고

- 본 TS-12는 PHASE2 inference 자체에는 문제가 없다는 진단을 포함한다. VAE AUROC 0.93은 시스템이 정상 작동한다는 증거다. 손실은 정렬·집계·정의 계층에서 발생한다.
- "낮다"는 평가는 절대 수치(TOP 100 recall 3.4%)가 아니라 enrichment(무작위 대비 14배)와 합집합 potential(19.5% > 13.58%) 대비 활용 부족을 기준으로 한다.

### 9. 2026-05-19 정정 — RRF 5-way 실패 + doc-level 가설 부분 반증 + 단위 case 통일

본 절은 2026-05-18 §6.1 결정(RRF k=60 2-way 채택)과 §6.2 (a)(외부 KPI doc 단위) 결정 이후 측정으로 드러난 사실을 추가 기록한다. 기존 결정은 폐기하지 않고 정정 형태로 누적한다.

#### 9.1 RRF 5-way 확장 시도 실패

family간 상관 측정(`artifacts/phase2_family_correlation_matrix_20260519.{json,md}`) 결과 최대 |ρ|=0.2108, 평균 0.0745로 Cormack 2009 RRF 독립성 가정 만족. 5-way RRF(PHASE1 + VAE + timeseries + relational + duplicate + intercompany)로 확장 구현 후 측정(`artifacts/phase1_phase2_integration_report_5way_20260519.{json,md}`):

```
TOP-N      2-way recall     5-way recall    Δ
   100        16.77%          15.32%       -1.45%p
   500        43.23%          26.77%      -16.46%p   ← 가장 심함
 1,000        53.71%          46.94%       -6.77%p
 2,000        63.55%          55.00%       -8.55%p
```

직접 원인: intercompany ranker가 nonzero 29 case(전체 41,129 중 0.07%)만 가지고 그 29 case에 강제로 RRF rank 1~30을 부여 → §6 매트릭스상 intercompany는 모든 시나리오 0% hit인데 그 29 case가 TOP-30 점령으로 truth case 밀어냄.

학습: **상관계수가 낮다(독립성)는 RRF의 필요조건이지 충분조건이 아니다**. sparse ranker가 소수 case에 강한 prior를 주면 informative ranker 신호가 묻힌다. 5-way 채택은 measurement 후 검증으로만 정당화 가능했어야 했다.

#### 9.2 doc-level ranking 가설 부분 반증

사용자(빌더)가 "case grouping이 정답 doc을 평균에 묻는다"는 가설로 doc-level scoring/ranking 전환을 제안. Phase A 측정(`artifacts/doc_level_ranking_phase_a_20260519.{json,md}`)에서 4 PHASE1 variant + 4 PHASE2 variant 산출하고 case-level baseline과 비교:
...
진단: **case grouping은 단순 UI 묶음이 아니라 evidence bundle**이었다. 같은 분개의 multi-row + multi-rule corroboration이 V1 lock 산식의 0.12 secondary + 0.10 corroboration + 0.08 materiality 가중치에 응축되어 있고, doc-level로 풀면 이 신호가 사라진다. corroboration weight 0.05 보너스로는 case-level 산식이 갖던 0.30+ corroboration 정보를 못 따라잡는다.

학습: case 정의는 PCAOB AS 2401 ¶54-57의 "journal entry" 개념과 정합한 evidence-bundled unit이며, 단일 line item(doc) 단위 ranking은 fraud detection 표준에 비추어 무리한 단위 환원이었다.

#### 9.3 단위 정책 정정 — case 단위 통일

기존 §6.2 결정 (a) "doc 단위 병행 보고"는 정렬 단위와 평가 단위 불일치를 야기. 2026-05-19 사용자 확정으로 다음과 같이 정정:

```
정렬 단위 = case (V1 lock 유지)
평가 분자/분모 단위 = case (정렬-평가 단위 일치)
truth 분모 = case 620 (1 truth doc → 1 truth case 매핑 정책 신설)
외부 표준(doc) 단위 = 보조 지표로만 병행 표시
```

##### 1 truth doc → 1 truth case 매핑 정책

PHASE1 case_builder는 한 doc이 여러 topic에 hit하면 topic마다 case를 만들어 truth doc 620 → truth case 795(1.28배 인플레이션)가 발생했다. 매핑은 **measurement 단계에서만** 적용한다:

```
규칙: truth doc 1개 → 그 doc이 매핑된 case 중 priority_score 가장 높은 1개만 truth case로 카운트
효과: truth case 분모 795 → 620 (인플레이션 제거)
영향: case_builder 코드 0 변경 (V1 lock 보존), 측정 스크립트만 정책 적용
```

##### 외부 표준 정당화

본 프로젝트의 case = "같은 분개에 속한 doc + topic 묶음"은 PCAOB AS 2401 ¶54-57의 journal entry 개념과 정합하며, ISA 240 ¶A45의 corroborating evidence 원리(단일 증거 대신 누적 증거)와 일치한다. doc 단위는 line-item에 가깝고, case 단위가 audit standard의 분개 단위. Phase A 측정은 이 정합성을 정량 증거로 뒷받침한다(doc-level이 case-level의 1/3 수준).

#### 9.4 결정 변경 요약

| 항목                 | 2026-05-18 결정        | 2026-05-19 정정                     | 근거                                  |
| -------------------- | ---------------------- | ----------------------------------- | ------------------------------------- |
| RRF 5-way            | (당시 미시도)          | 폐기 — sparse ranker prior 위험     | 5-way 측정 -16.46%p                   |
| doc-level ranking    | (미고려)               | 시도 후 폐기 — evidence bundle 손실 | Phase A 측정 1/3 수준                 |
| 평가 분자/분모 단위  | doc (외부 KPI)         | **case 620** (정렬-평가 일치)       | AS 2401 journal entry 정합            |
| 외부 표준 단위       | doc 단독               | case 단독, doc은 보조 지표          | Phase A로 case = evidence bundle 확인 |
| TS-13 80건 회수 경로 | (c) PHASE2 doc 큐 권고 | 무효 — TS-13 §6 참조                | 80건 회수 0/80                        |

#### 9.5 Sprint 진행 상태

본 정정은 `dev/active/doc-level-ranking/doc-level-ranking-context.md` sprint에서 통합 관리. 옵션 가(case 유지) + 옵션 다(80건 PHASE2 row-level 진단) 진행 중. Phase A.5 측정 후 RRF 폐지/유지 최종 결정 + ceiling 확정 공시.

**관련 문서**: [DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md](../guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md) · [PHASE2_FITTING_AUDIT.md](PHASE2_FITTING_AUDIT.md) · [completed/S9_phase2_value_baseline.md](../archive/completed/S9_phase2_value_baseline.md) · [DETECTION_RANKING_CRITERIA.md](DETECTION_RANKING_CRITERIA.md) · `tools/scripts/phase1_phase2_integration_stage7.py` · `src/services/phase2_case_contract.py` · `dev/active/doc-level-ranking/doc-level-ranking-context.md` · `artifacts/phase2_family_correlation_matrix_20260519.{json,md}` · `artifacts/phase1_phase2_integration_report_5way_20260519.{json,md}` · `artifacts/doc_level_ranking_phase_a_20260519.{json,md}`

---

## TS-13: PHASE1 룰 커버리지 갭 — 큐 미진입 truth 80건

**분류**: 탐지 룰 커버리지 | **정리일**: 2026-05-18 | **상태**: 진행 중 (분석 완료, 정책 결정 전)

### 1. 이 절이 다루는 것

V7 fixed3 review queue 분석에서 truth 전표 620건 중 540건만 PHASE1 case로 묶여 review queue에 진입했고, 80건은 `queue.document_ids_joined` 기준으로 큐에 없었다. 따라서 현재 통합 RRF 정렬은 이 80건을 회수할 수 없다. RRF는 PHASE1 case row를 재정렬하는 방식이므로, case row가 없는 document는 순위가 생기지 않는다.

분석 산출물:

- `artifacts/ts13_uncovered_truth_80_analysis.json`
- `artifacts/ts13_recovery_path_evaluation.md`
- 재현 스크립트: `tools/scripts/ts13_analyze_uncovered_truth.py`

주의: 최초 가설은 "어떤 PHASE1 룰도 hit하지 않음"이었지만, detector raw `details` 대조 결과 80건 모두 일부 PHASE1 raw/review score가 있었다. 더 정확한 진단은 **raw 룰 신호는 있으나 row/case seed priority가 낮아 case builder에 진입하지 못한 문서**다. 즉 TS-13은 "룰 부재" 단독 문제가 아니라 **case seed threshold / non-seeding context 문제**다.
...
| ---------------------------- | -----: |
| truth 전표                   |    620 |
| review queue 진입 truth 전표 |    540 |
| queue 미진입 truth 전표      |     80 |
| recall ceiling               | 87.10% |
| 시나리오 분포 합             |     80 |
| 미적용 룰 분류 합            |     80 |

추출 방식은 `queue.document_ids_joined`를 `;`로 split한 document 집합과 `manipulated_entry_truth.csv`의 truth document 집합 차집합이다. PHASE2 점수는 `tools/scripts/phase1_phase2_integration_stage7.py::score_phase2`를 호출해 재생성한 ECDF 점수 기준으로 측정했다. truth label은 score 산정에 사용하지 않았다.

### 3. 시나리오 분포

| 시나리오                           | truth 전체 | 미진입 | 미진입 내 비중 | 시나리오 내 미진입률 |
| ---------------------------------- | ---------: | -----: | -------------: | -------------------: |
| embezzlement_concealment           |         76 |     39 |         48.75% |               51.32% |
| fictitious_entry                   |        168 |     32 |         40.00% |               19.05% |
| period_end_adjustment_manipulation |         92 |      8 |         10.00% |                8.70% |
| circular_related_party_transaction |         34 |      1 |          1.25% |                2.94% |
| expense_capitalization             |        100 |      0 |          0.00% |                0.00% |
| suspense_account_abuse             |        100 |      0 |          0.00% |                0.00% |
| approval_sod_bypass                |         29 |      0 |          0.00% |                0.00% |
| unusual_timing_manipulation        |         21 |      0 |          0.00% |                0.00% |

미진입은 `embezzlement_concealment`와 `fictitious_entry`에 강하게 편중된다. 특히 횡령은폐 시나리오는 76건 중 39건(51.32%)이 queue에 진입하지 못했다.

### 4. 공통 특성

| 축               | 관찰                                                                                                                                                              |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| fiscal_year      | 2022 27건, 2023 29건, 2024 24건. 특정 연도 쏠림은 약하다.                                                                                                         |
| company_code     | C002 34건(42.5%)이 가장 높지만 전체 truth 분포(C002 36.13%) 대비 과도한 단일 회사 결함으로 보기는 어렵다.                                                         |
| business_process | P2P 39건(48.75%), O2C 32건(40.00%)에 집중. R2R은 8건, Intercompany는 1건.                                                                                         |
| source           | automated 35건(43.75%), recurring 22건(27.50%)으로 자동/반복 source 비중이 높다. manual은 14건(17.50%)으로 전체 truth 41.13% 대비 낮다.                           |
| user_persona     | junior_accountant 32건(40.00%), senior_accountant 32건(40.00%). senior_accountant는 전체 truth 58.06% 대비 낮고, junior_accountant는 전체 truth 22.90% 대비 높다. |
| line_amount      | 미진입 80건 중 71건(88.75%)이 5억원 초과. 전체 truth의 5억원 초과 비중 40.32%보다 매우 높다.                                                                      |

요약하면, 미진입 80건은 P2P/O2C, automated/recurring source, 고액 전표 쪽에 몰려 있다. 다만 이는 V7 fixed3 합성 데이터 특성일 수 있으며, 실데이터에서 같은 비율로 PHASE1 case seed를 우회한다고 일반화하면 안 된다.
...
| (b) 임계값/seed 미달 |   80 | raw 룰 신호는 있으나 row `risk_level`이 Normal이거나 case seed 조건을 넘지 못해 case가 생성되지 않았다. |
| (c) 데이터 결손      |    0 | 주요 입력 컬럼 전체 결손으로 평가가 불가능했던 패턴은 확인되지 않았다.                                  |

상위 near-miss raw 신호는 `L3-04` 41건, `L4-03` 39건, `L4-05` 20건, `L3-05` 18건, `L3-06` 10건, `L4-06` 10건, `L3-03` 1건이었다. `L3-05`, `L3-06`, `L4-05`, `L4-06`은 V1 lock상 booster/combo context 성격이 강해 단독 case seed로 쓰기 어렵다. `L3-04`와 `L4-03`도 해당 문서들에서는 row-level `anomaly_score`가 Normal band에 머문 케이스가 많아 case builder에 진입하지 못했다.

이 결론은 "룰을 하나 더 만들면 바로 해결"이 아니라, **Normal/low-priority raw hit를 어디까지 case로 올릴지**의 운영 임계값 문제에 가깝다.

### 6. 회수 경로 평가

| 옵션                           |                                                                         측정 회수 (초기) | 비용/위험                                                                                                        | 판단 (2026-05-19 정정)                                                                                                                                                                                         |
| ------------------------------ | ---------------------------------------------------------------------------------------: | ---------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| (a) 룰 임계값 완화             |                                                                                    80/80 | raw 양수 점수만으로 case seed를 허용하면 정상 document 176,179건까지 확장될 수 있는 stress upper bound가 나온다. | V1 운영 큐 품질을 크게 훼손하므로 비권장. truth recall 향상을 근거로 threshold를 낮추면 fitting이다.                                                                                                           |
| (b) 신규 룰 추가               |                                                                                   미실행 | 32 룰 고정 정책과 충돌한다. `PHASE1_TOPIC_SCORING_V1_LOCK.md` 개정 및 `docs/spec/DECISION.md` 결정 필요.         | 본 sprint 비범위. 향후 K-SA 240, 금감원 감리지적 사례, PCAOB AS 2401, K-SOX 근거가 있을 때만 검토.                                                                                                             |
| (c) ~~PHASE2 단독 큐~~ ⚠️ 무효 | top 500 docs 4건, top 1,000 docs 10건, top 2,000 docs 12건, top 1% docs 12건 (초기 추정) | 기존 RRF는 case-bound라 0건 회수. 별도 PHASE2 document queue가 필요하다.                                         | **2026-05-19 Phase A 측정으로 무효화**. PHASE2 단독 document queue 실제 측정 결과 **80건 회수 0건**(`artifacts/doc_level_ranking_phase_a_20260519.json` M4). 초기 추정은 score_phase2 재생성 시 매핑 오류였음. |
| (d) 회수 불가 인정             |                                                                                        0 | V7 fixed3 기준 ceiling 87.10%(doc) / 등가 case ceiling을 운영 문서에 명시해야 한다.                              | **2026-05-19 권고 변경 — 채택**. 모든 회수 경로 측정에서 0/80이므로 ceiling 확정 공시가 유일 정합.                                                                                                             |

### 6.1 2026-05-19 정정 — 권고 변경

기존 권고 "(c) PHASE2 단독 document queue + ceiling 명시"는 **취소**한다. 근거:

- `dev/active/doc-level-ranking/` sprint의 Phase A 측정에서 PHASE2 5 family score를 전체 317,997 doc에 대해 계산해 만든 doc-level queue를 만들어도 80 doc 중 0건만 TOP에 진입했다 (즉 PHASE2 row-level 신호가 80 doc에 대해 ranking 의미 있는 값을 주지 못한다).
- 측정 산출: `artifacts/doc_level_ranking_phase_a_20260519.json` §M4_ts13_recovery + `artifacts/doc_level_ranking_phase_a_20260519.md` §5.
- doc-level 정렬 자체가 case-level 대비 1/3 수준 recall로 별도 측정 실패(§9.2 of TS-12 참조).

새 권고: **(d) 회수 불가 인정 + ceiling 공시**. 단 Phase A.5 측정에서 80 doc의 PHASE2 row-level score 분포를 한 번 더 확인해 신호 자체가 0인지(탐지기 한계) vs 집계에서 묻혔는지(다른 산식 가능성) 진단 후 최종 ceiling 확정.

### 7. 결정 상태

| 항목                                    | 상태                   | 결정                                                                      |
| --------------------------------------- | ---------------------- | ------------------------------------------------------------------------- |
| 80건 ID 산출                            | ✅ 완료                | `artifacts/ts13_uncovered_truth_80_analysis.json`                         |
| 시나리오/특성/룰 matrix                 | ✅ 완료                | 같은 JSON 산출물                                                          |
| PHASE2 회수 측정 (초기, doc 추정)       | ✅ 완료                | `score_phase2()` 재생성 ECDF 기준, 추정값                                 |
| PHASE2 회수 측정 (Phase A, doc 큐 실측) | ✅ 완료 (2026-05-19)   | **0/80** `artifacts/doc_level_ranking_phase_a_20260519.json`              |
| PHASE1 임계값/룰 변경                   | ❌ 미실행              | V1 lock 때문에 본 sprint 비범위                                           |
| 권고 경로 (초기)                        | ❌ 취소 (2026-05-19)   | (c) PHASE2 단독 doc queue — 측정으로 무효화                               |
| 권고 경로 (정정)                        | ✅ 채택 (2026-05-19)   | **(d) 회수 불가 인정 + ceiling 87.10% 공시**, Phase A.5 진단 후 최종 확정 |
| case-level ceiling 재산정               | ⏳ Phase A.5 진행 예정 | 1 truth doc → 1 truth case 매핑(분모 case 620) 후 ceiling 재계산          |

### 8. 부수 발견 (2026-05-19)

PHASE2 5 family ECDF는 80 doc에 대해 informative하지 않다. 이는 **8 시나리오 중 fictitious_entry(32 미진입), embezzlement_concealment(39 미진입)** 두 시나리오가 80건의 88.75%를 차지하며, 두 시나리오 모두 5 family 매트릭스(`docs/guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §6)에서 다음 특성을 보이기 때문으로 추정:

- fictitious_entry: 큐 진입 64건에서는 timeseries 100% / VAE 86% 회복하지만, 큐 미진입 32건은 row anomaly_score 자체가 낮은 케이스로 PHASE2 family도 같이 약한 신호.
- embezzlement_concealment: 큐 진입 37건은 relational 100% / duplicate 100% 회복하나, 큐 미진입 39건은 P2P/O2C automated/recurring source 비중 높음(TS-13 §4) → 정상 패턴과 구별이 어렵다.

Phase A.5에서 row-level 5 family score 분포를 직접 측정해 본 추정을 검증.

**관련 문서**: [DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md](../guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md) · [PHASE1_TOPIC_SCORING_V1_LOCK.md](../archive/completed/PHASE1_TOPIC_SCORING_V1_LOCK.md) · [DETECTION_RULES.md](DETECTION_RULES.md) · `artifacts/ts13_uncovered_truth_80_analysis.json` · `artifacts/ts13_recovery_path_evaluation.md` · `artifacts/doc_level_ranking_phase_a_20260519.{json,md}` · `dev/active/doc-level-ranking/doc-level-ranking-context.md`

---

## TS-15: PHASE2 family 합산 — hierarchical RRF reject → Noisy-OR separated 채택

**분류**: 탐지 정렬 / 결합식 | **정리일**: 2026-05-19 | **상태**: 채택 완료 (V7 fixed3 측정 + 결정 8 갱신 진행)

### 1. 이 절이 다루는 것

PHASE2 5 family score 를 어떻게 결합해 운영 큐에 반영할지의 결정 과정. 세 단계로 진행됐다.

1. **1차 시도** — hierarchical RRF (active-ranker voter + coarse-booster). V7 fixed3 측정에서 baseline 대비 평균 -6.45pp 손실 → production reject.
2. **2차 시도** (사용자 비판: "RRF 좀 씨발 집착하지 말라고") — RRF 외 8 결합식 × 3 적용 방식 = 24 측정 + baseline 2 ⇒ Noisy-OR separated 가 두 측정 모두에서 전 깊이 양수 Δ 로 일관 우세.
3. **채택** — Noisy-OR separated 를 PHASE2 voter 로 변경. PHASE1+VAE 2-way RRF 의 VAE 자리를 5-family Noisy-OR 결과로 교체.

핵심 한 줄: **RRF 적용 범위는 PHASE1 ↔ (PHASE2 Noisy-OR) 2-way 전역 결합으로 유지. PHASE2 internal 결합은 RRF 가 아닌 Noisy-OR (`1 - Π(1 - ecdf_f)`) 로 정의한다.**

### 2. 1차 시도 — hierarchical RRF 의 실패 원인

RRF 는 "여러 ranker 가 서로 독립적이고 동등한 검색기" 일 때 강하다. PHASE2 5 family 는 그 가정을 깬다:

| family             | hit rate | score 형태           | voter 적합성       |
| ------------------ | -------: | -------------------- | ------------------ |
| unsupervised (VAE) | 99.9999% | 연속 분포 (q95=0.97) | 강 — 연속 분해능   |
| timeseries         |   87.23% | 2값 이산 (0.4/0.8)   | 약 — tie 블록 거대 |
| relational         |    2.94% | 희소                 | 중                 |
| duplicate          |   14.64% | 이산 cap (q99=0.60)  | 약 — cap 발생      |
| intercompany       |  0.0033% | 거의 0               | dormant            |

이걸 `1/(60+rank)` 같은 voter 형식으로 통일하면 unsupervised 의 연속 분해능이 duplicate(binary cap) · timeseries(2값) · intercompany(99.997% 0) 에 의해 **dilute** 된다. 측정:

```
                       baseline (PHASE1+VAE 2-way RRF)   hierarchical RRF   Δ
TOP 100                    2.90%                          2.42%          -0.48pp
TOP 500                    9.03%                          3.23%          -5.81pp
TOP 1,000                 14.35%                          3.71%         -10.65pp
TOP 2,000                 22.10%                         13.87%          -8.23pp
TOP 5,000                 34.52%                         28.23%          -6.29pp
```

평균 -6.45pp. `compute_phase2_internal_rrf` helper 는 reject 결정 후 `tests/modules/test_services/test_queue_fusion_hierarchical.py` 에 `pytest.mark.experimental_phase2_internal_rrf` 로 격리. supervised/transformer 활성화 시 재평가 조건 lock.

산출물: [artifacts/phase2_family_ranking_measurement_20260519.md](../../artifacts/phase2_family_ranking_measurement_20260519.md).

### 3. 2차 시도 — RRF 외 8 결합식 측정

비-RRF 결합 방식 8 개를 V7 fixed3 에서 측정. 두 baseline 분리:

- **측정 1 — PHASE2 internal 단독**: baseline = VAE ECDF 단독
- **측정 2 — PHASE1+PHASE2 통합 큐**: baseline = PHASE1 composite + VAE ECDF 2-way RRF k=60 (현 운영)

각 결합식을 3 적용 방식으로 측정: `phase2_only` / `separated` (PHASE2 internal → PHASE1 과 RRF) / `unified` (PHASE1 + 5 family 6 source 한번에 결합).

#### 측정 1 결과 — PHASE2 internal 단독 (VAE 단독 대비 Δ pp)

> 정정: 아래 표는 초기 batch-local ECDF measurement-only 결과다. 이후 운영 helper 는 0/NaN 무신호 row 를 0 contribution 으로 보존하도록 수정했다. 따라서 아래 Δ 는 결합식 탐색 이력이며 운영 성능 주장에 사용하지 않는다.

| aggregator        |   Δ TOP100 |  Δ TOP500 | Δ TOP1000 | Δ TOP2000 | Δ TOP5000 |
| ----------------- | ---------: | --------: | --------: | --------: | --------: |
| **🟢 noisy_or**   | **+10.16** | **+9.19** | **+5.65** | **+5.16** | **+0.81** |
| rank_product      |      -1.77 |     +1.45 |     +4.84 |     +1.29 |     -9.68 |
| evidence_vote     |      +0.16 |     +4.84 |     +3.39 |     +0.16 |     -9.35 |
| max               |      +1.61 |     +2.10 |     +1.77 |     +1.13 |     -3.39 |
| top_k_mean        |     +13.39 |     +8.39 |     +1.77 |     -1.77 |    -19.03 |
| tier_weighted_sum |      +0.81 |     +5.48 |     -1.29 |     -9.68 |    -23.23 |
| geometric_mean    |      +0.81 |     +5.16 |     -1.61 |     -9.68 |    -25.65 |
| cascade_boost     |      -1.77 |    -10.48 |    -18.39 |     -5.16 |    -12.10 |

초기 measurement-only 에서는 Noisy-OR 단독으로 전 깊이 모두 양수였다. zero-preserving 운영 helper 적용 후에는 이 향상 주장을 제거한다.

#### 측정 2 결과 — PHASE1+PHASE2 통합 큐 (PHASE1+VAE 2-way RRF 대비 Δ pp)

| aggregator                       | application   |  Δ TOP100 |  Δ TOP500 | Δ TOP1000 | Δ TOP2000 | Δ TOP5000 |
| -------------------------------- | ------------- | --------: | --------: | --------: | --------: | --------: |
| **🟢 noisy_or**                  | **separated** | **+3.39** | **+8.39** | **+4.35** | **+1.61** | **+1.77** |
| evidence_vote                    | separated     |     -0.48 |     +0.32 |     +8.55 |     +2.42 |     -6.29 |
| top_k_mean                       | separated     |     +2.58 |     +8.87 |     +4.03 |     -1.29 |    -10.16 |
| top_k_mean                       | unified       |     +0.48 |     +7.10 |     +3.87 |     +0.48 |     -7.26 |
| tier_weighted_sum                | unified       |     -0.32 |     +7.42 |     +2.42 |     -4.19 |    -16.61 |
| max                              | unified       |     +0.32 |     -0.32 |     -0.65 |     -0.16 |     -0.48 |
| hierarchical_rrf (reject 재확인) | separated     |     -0.48 |     -5.32 |    -10.81 |     -7.26 |     -7.42 |
| cascade_boost                    | unified       |     -2.90 |     -8.55 |    -14.52 |     -0.65 |     -9.68 |

Noisy-OR separated 만 전 깊이 모두 양수. 다른 결합식은 일부 깊이에서 baseline 보다 약함.

#### 두 측정의 공통 결론

1. **Noisy-OR 가 PHASE2 단독 / 통합 두 측정 모두에서 일관 우월**.
2. **분리(separated) 적용이 통합(unified) 적용보다 우월** — 통합은 PHASE1 voting 비중을 1/6 로 줄여 baseline 보다 약해짐.
3. **Max 는 baseline 과 거의 동일** (4 family 가 voter 로 안 들어감과 동치).
4. **합산 가중 방식 (tier_weighted_sum, geometric_mean)** 은 TOP 500~1,000 에서 양수지만 TOP 2,000~5,000 에서 큰 음수 — 깊은 영역에서 정상 case 누적.
5. **Cascade boost** 는 통합 적용에서 TOP 1,000 까지 0% — q95 임계가 동률 야기.

산출물: [artifacts/phase2_family_ranking_alt_aggregators_20260519.md](../../artifacts/phase2_family_ranking_alt_aggregators_20260519.md).

### 4. 채택 — Noisy-OR separated

결합식:

```
phase2_internal_noisy_or(doc) = 1 - Π_{f ∈ 5 families} (1 - ecdf_f(doc))

final_score(doc) = 1/(60 + rank_phase1_composite(doc))
                 + 1/(60 + rank_phase2_internal_noisy_or(doc))
```
...
| 구분             | 이전                 | 신규                                                        |
| ---------------- | -------------------- | ----------------------------------------------------------- |
| PHASE2 voter     | VAE ECDF 단독        | **5 family Noisy-OR ECDF**                                  |
| 결합식 적용 단계 | (1단계 — VAE 그대로) | (2단계 — 분리: PHASE2 internal Noisy-OR → PHASE1 2-way RRF) |
| 통합 RRF 식      | PHASE1 ↔ VAE         | PHASE1 ↔ PHASE2 Noisy-OR (식 동일, voter 만 교체)           |

운영 fixture (V7 fixed3, zero-preserving ECDF + Noisy-OR voter, `queue_integrated.parquet`):
...
**단조 우월 아님**. TOP 100/500 에서 +5.65/+2.26pp 개선, TOP 1,000/2,000 에서 -4.03/-3.87pp 손실. TOP 100~2,000 평균 Δ ≈ 0pp 로 **종합 truth recall 사실상 동률**. 분포가 상단으로 재배치된 형태.

§3 의 alt_aggregator measurement 표 (전 깊이 +1.61~+8.39pp 양수 Δ) 와 차이가 나는 이유:
- §3 measurement 는 `synthesize_phase1_composite` 근사 (rule-style family score 합) + 일반 batch-local ECDF 적용. 일반 ECDF 는 0/NaN 무신호 row 에도 percentile 중간값을 부여하여 결과가 부풀려졌다.
- 운영 production run 은 실제 PHASE1 `composite_sort_score` + **zero-preserving ECDF** (0/NaN 은 0 유지). 무신호 보존 정책 적용 후 깊은 영역에서 -4pp 손실이 드러났다.

따라서 §3 의 측정 결과는 **결합식 탐색 이력**(8 종 후보 중 Noisy-OR 가 우월) 으로만 사용하고, 운영 성능 주장에는 사용하지 않는다. **채택 사유는 truth recall 개선이 아니라 (a) 단일 voter + narrator attribution standardization, (b) 무신호 보존, (c) parameter 0개 / fitting 위험 0** 의 architecture 합치성이다.

### 5. 거버넌스 정합

| 정책                                        | 정합 여부 | 근거                                                                       |
| ------------------------------------------- | --------- | -------------------------------------------------------------------------- |
| `feedback_phase1_truth_recall_guard`        | ✅        | Noisy-OR 식은 weight 0개, parameter 0개. truth label 미사용.               |
| 결정 5 옵션 R (truth recall 직접 추구 금지) | ✅        | 식 자체가 분포만 사용 (ECDF). truth 와 무관.                               |
| 결정 3 옵션 Z (independent queue)           | ✅        | primary 큐 결합 구조 유지 (PHASE1+PHASE2 2-way RRF). voter 만 교체.        |
| 결정 8 (family ranking 정책)                | 갱신      | reject 범위가 hierarchical RRF only 로 축소. Noisy-OR separated 채택 lock. |

### 6. 후속 작업

1. `compute_phase2_internal_noisy_or` helper 추가 (`src/services/queue_fusion.py`) ✅
2. `phase2_inference_service` / `pipeline._build_phase2_case_overlays` 의 final RRF voter 가 Noisy-OR 결과 사용 ✅
3. V7 fixed3 production fixture — Noisy-OR voter 적용 후 doc recall TOP 100/500/1,000/2,000 = **22.42% / 45.48% / 49.68% / 59.68%** 회귀 lock (informational only, `tests/modules/test_services/test_phase1_phase2_integration_stage7.py::test_integration_doc_recall_integrated_top*`) ✅
4. `docs/spec/PHASE2_GOVERNANCE_DESIGN.md` 결정 8 갱신 — reject 범위 축소 + Noisy-OR 채택 + **깊이별 trade-off (TOP 100/500 +, TOP 1,000/2,000 −) 명시** ✅
5. `docs/guide/users/08_PHASE2_FAMILY_STRUCTURE.md` / `09_REVIEW_QUEUE_RRF_FUSION.md` 사용자 문서 반영 — **단조 우월 표현 금지, architecture standardization 사유 강조** ✅

### 7. 학습된 교훈

- **RRF 는 ranker 동등 가정이 필요하다**. 5 family 처럼 hit rate 차이 10^4 배, score 형태 (연속/이산/희소) 가 다른 source 를 voter 형식으로 통일하면 dilution 이 일어난다.
- **결합식 후보를 다양하게 측정해야 한다**. 처음에 RRF 만 고집해서 -6.45pp 손실을 한 번 보고 나서야 비-RRF 측정으로 전환했다. 측정을 통한 검증이 디자인 직관보다 우선.
- **분리 vs 통합 적용은 다른 의사결정이다**. 분리는 PHASE1 voting 비중 50% 보존, 통합은 1/6 로 축소. 둘 다 측정해야 답이 나온다.

**관련 문서**: [PHASE2_GOVERNANCE_DESIGN.md](PHASE2_GOVERNANCE_DESIGN.md) 결정 8 · [PHASE2_INTERFACE_DESIGN.md](PHASE2_INTERFACE_DESIGN.md) · [DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md](../guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md) · `artifacts/phase2_family_ranking_measurement_20260519.{json,md}` · `artifacts/phase2_family_ranking_alt_aggregators_20260519.{json,md}` · `tools/scripts/phase2_family_ranking_alt_aggregators.py` · `docs/archive/completed/phase2-family-ranking/`

---

## TS-17: PHASE2 family responsibility-map — 전체 620 recall과 owner-set target recall 분리

**분류**: PHASE2 평가 기준 / DataSynth diagnostic denominator | **정리일**: 2026-05-31 | **상태**: v3.3b current canonical responsibility map candidate 생성

### 1. 문제

PHASE2 native case family를 전체 620개 synthetic truth document 분모로만 비교하면 family 성능 해석이 왜곡된다.

예:

- Intercompany는 reciprocal/counterparty evidence family이므로 전체 anomaly 620개를 모두 잡는 family가 아니다.
- Duplicate는 left/right pair evidence를 제공하는 family이므로 전체 truth recall보다 duplicate-like target recall이 더 중요하다.
- Timeseries는 timing/window target이 실제로 무엇인지 먼저 분리해야 TOP100 실패 원인을 판단할 수 있다.
- Unsupervised는 broad statistical anomaly companion이므로 rule family와 같은 denominator로 보면 역할이 흐려진다.

따라서 전체 620개 기준 recall은 **portfolio contribution**으로 유지하되, family 성능 판단은 별도 **responsibility-map owner-set denominator**를 추가한다.

### 2. 결정

620개 synthetic truth document를 detector 결과가 아니라 DataSynth truth semantics 기준으로 먼저 owner family set에 배정한다.

정책:

1. Owner는 단일 값이 아니다. 한 truth case는 0개, 1개, 또는 여러 owner를 가질 수 있다.
2. 명확한 owner가 없으면 `no_clear_owner`를 허용한다.
3. Owner assignment는 detector-blind / score-blind / rank-blind / matched-result-blind로 수행한다.
4. Raw `document_id`, `row_id`, `index_label`, `phase2_case_id` 계열 식별자는 owner artifact와 문서에 저장하지 않는다.
5. LLM-assisted semantic labeling은 허용하되, strict schema와 sanitized summary만 사용한다. 기본 테스트는 live API call을 하지 않는다.
6. Owner assignment는 product scoring이 아니라 diagnostic denominator를 만들기 위한 offline 평가 보조다.
...
### 3. 현재 fixed5 baseline

현재 산출물은 deterministic/rule-only mode로 생성했다.
...
이 baseline은 첫 기준선이며 최종 owner taxonomy가 아니다. 특히 `phase1=586`, `unsupervised=289`, `multi-owner=520`, `no_clear_owner=0`은 과도 배정 가능성을 review해야 한다.

### 4. owner-set target recall

전체 620 denominator recall은 portfolio contribution으로 유지한다.
...
Owner-set target recall은 family target performance로 해석한다.
...
- IC locked 결론은 유지한다. Intercompany-owned target 34/34를 모두 잡았다.
- Relational locked 결론은 유지하되, owner-set recall과 PHASE1 uplift / relationship evidence companion 역할을 분리한다.
- Duplicate는 전체 620 기준이 아니라 92개 duplicate-owned target 기준으로 다시 본다. 다음 개선은 ranking 변경보다 sidecar/export pair evidence 품질이다.
- Timeseries는 113개 timing-window target 기준 native TOP500 0건이므로 TOP100/TOP500 실패 원인을 target-surface alignment 관점에서 다시 봐야 한다.
- Unsupervised는 289개 broad-statistical target 기준 native row queue 39건이며, optional document companion 역할과 분리해 해석한다.

### 5. fitting / leakage guard

Owner assignment는 다음을 사용하지 않는다.

- PHASE1/PHASE2 score
- rank
- matched result
- detector output
- raw document/row identifier
- PHASE2 case identifier
...
### 6. V2 revision — owner_role로 primary target과 context contribution 분리

2026-05-30 v2 산출물을 추가했다.
...
- V1 artifact는 inclusive baseline으로 유지한다.
- `expected_owners`는 inclusive compatibility set으로 유지한다.
- 새 필드 `owner_roles`를 추가한다.
- Official family target recall은 `owner_role=primary`만 denominator로 쓴다.
- `secondary`와 `companion_context`는 evidence contribution으로 별도 측정한다.
- PHASE1의 broad owner 성격은 `baseline_review`로 분리한다.

V2 role distribution:

| Owner        | Primary | Secondary | Companion | Baseline |
| ------------ | ------: | --------: | --------: | -------: |
| phase1       |     397 |         0 |         0 |      223 |
| intercompany |      34 |         0 |         0 |        0 |
| relational   |      29 |       110 |         0 |        0 |
| duplicate    |       0 |         0 |        92 |        0 |
| timeseries   |      21 |         0 |        92 |        0 |
| unsupervised |     168 |         0 |       121 |        0 |
...
- `period_end_adjustment_manipulation` 92건은 timeseries primary가 아니라 `companion_context`.
- Timeseries primary는 `unusual_timing_manipulation / after_hours_posting` 21건으로 제한.
- TOP100/500 tuning 대상이 아니라 context lane이라는 `PHASE2_TIMESERIES_ROLE_LOCK.md`와 정합.

Duplicate metadata gap:

- `duplicate_primary_denominator_status = metadata_insufficient`
- 현재 metadata에는 explicit duplicate-like injected metadata가 없다.
- DataSynth backlog: `injected_duplicate_like`, `duplicate_pair_semantic_group`, reference/amount/text similarity injection source.

V2 fitting/leakage guard:

- raw identifier leak count 0
- forbidden identifier key count 0
- PHASE2 case-id-like token count 0
- detector output / score / rank / matched result는 owner assignment에 사용하지 않음
- production ranking/gate/fusion 변경 없음

### 6.1 V2.1 policy cleanup — audit-rule-first / evidence-companion

2026-05-30 v2.1은 v2 artifact를 덮어쓰지 않고 별도 산출물로 추가했다.
...
- `fictitious_entry`는 `phase1=primary`, `unsupervised=companion_context`.
- `expense_capitalization`은 `phase1=primary`, `unsupervised=companion_context` 유지.
- `unsupervised primary`는 explicit broad-statistical-only owner metadata가 있을 때만 둔다. 현재 fixed5에는 없어 pending.
- `approval_sod_bypass`는 `phase1=primary`, `relational=secondary`.
- `relational primary`는 explicit relationship-primary detector/evidence contract가 생길 때까지 부여하지 않는다.

V2.1 role distribution:

| Owner        | Primary | Secondary | Companion | Baseline |
| ------------ | ------: | --------: | --------: | -------: |
| phase1       |     565 |         0 |         0 |       55 |
| intercompany |      34 |         0 |         0 |        0 |
| relational   |       0 |       139 |         0 |        0 |
| duplicate    |       0 |         0 |        92 |        0 |
| timeseries   |      21 |         0 |        92 |        0 |
| unsupervised |       0 |         0 |       289 |        0 |
...
- `relational_primary_denominator_status = pending_explicit_relationship_primary_semantics`.
- `duplicate_primary_denominator_status = metadata_insufficient` 유지.
- `unsupervised_primary_denominator_status = pending_explicit_broad_statistical_only_metadata`.
- Timeseries primary는 21로 유지한다. `period_end_adjustment_manipulation` 92건은 계속 `companion_context`.
- `no_clear_owner=0`은 ambiguity 부재가 아니라 현재 rule-only structure 결과다.
- Leakage guard는 v2와 동일하게 raw identifier leak 0, forbidden key 0, score/rank/matched-result owner assignment 미사용이다.

Pending 해제 trigger condition:
...
### 6.2 V3 fixed5_ownermeta_ic — family-specific primary flags

2026-05-30 v3는 `datasynth_manipulation_v7_candidate_fixed5_ownermeta_ic`의 owner metadata를 사용한다. v3 relocated owner policy into DataSynth metadata for traceability. v3/v3.1 are diagnostic responsibility maps, not production detector changes.
...
- `truth_owner_primary`는 legacy representative summary이며 exclusive owner가 아니다.
- Non-PHASE1 denominator는 family-specific primary flags를 사용한다.
- Co-primary를 허용한다. circular 34건은 IC primary이면서 relational primary다.
- Production ranking/gate/fusion, PHASE1 ranking, PHASE2 family ordering은 변경하지 않는다.

Primary denominator:

| Owner        | Primary docs | Source                                                 |
| ------------ | -----------: | ------------------------------------------------------ |
| phase1       |          192 | legacy `truth_owner_primary == "phase1"` fallback      |
...
### 6.3 V3.1 policy cleanup

v3.1 reconciles the DataSynth metadata with audit-rule-first responsibility policy. v3 artifact는 보존하고 v3.1 artifact를 별도로 둔다. DataSynth family metadata는 owner 후보 신호이며, 감사 의미와 충돌하는 scenario-wide stamp는 audit-rule-first policy가 override한다.
...
V3.1 companion/context:

| Lane                   | Truth docs | Role                              |
| ---------------------- | ---------: | --------------------------------- |
| relational secondary   |        105 | approval 29 + embezzlement 76     |
| duplicate companion    |         76 | duplicate-like evidence companion |
| timeseries context     |         92 | period-end context                |
| unsupervised companion |        339 | statistical companion/context     |
...
- `approval_sod_bypass`는 PHASE1 primary, relational secondary다.
- `embezzlement_concealment`는 PHASE1 primary, duplicate/relational/unsupervised companion이다.
- `suspense_account_abuse`는 PHASE1 primary, unsupervised companion이다.
- `fictitious_entry`는 v2.1에서 expense와 함께 PHASE1 primary로 통일했지만, v3.1에서는 existence assertion과 classification error를 분리한다. 거래 실재성 자체가 불명확한 outlier/existence assertion 문제이므로 VAE primary로 유지한다.
- `expense_capitalization`은 실재 거래는 존재하지만 계정분류가 잘못된 classification error이므로 PHASE1 primary로 유지한다.
- Duplicate primary는 `pending_pair_evidence_validation`이며 denominator 0은 0% 실패가 아니다.

V2.2 relational relmeta update:

- `datasynth_manipulation_v7_candidate_fixed5_relmeta`의 `relationship_edge_truth.csv/json` sidecar가 있으면 relational primary pending을 해제한다.
- Primary owner는 exclusive하지 않다. `circular_related_party_transaction` 34건은 IC와 relational co-primary다.
- Co-primary는 family target performance 평가용이며 portfolio recall에는 중복 합산하지 않는다.
- `multi_primary=0` 원칙은 v2.2에서 폐기하고 `co_primary_overlap_count=34`로 명시한다.
- `relationship_edge_truth`는 evaluation denominator이며 detector/ranking/fusion/UI scoring input이 아니다.
- R05/R06는 계속 context/diagnostic-only lane이다.
- v2.2 결과: relational primary 63 / matched 9 / recall 14.29%, relational secondary 76 / matched 8 / recall 10.53%, IC primary 34 / matched 34 / recall 100%.
- Production ranking, gate, fusion은 변경하지 않았다.

### 6.4 V3.2d fixed5_ownermeta_v32d — historical responsibility map

`datasynth_manipulation_v7_candidate_fixed5_ownermeta_v32d` 기준 responsibility map은 v3.3b 도입 후 historical responsibility map으로 보존한다. 이 작업은 denominator / owner-role 정합성 재정의이고, production ranking/gate/fusion, PHASE1 ranking, PHASE2 family ordering은 변경하지 않았다.
...
- VAE 49 exact TOP500 recall = 5 / 49. 기존 6/49 proration은 공식 수치에서 제외한다.
...
| Owner              | Primary docs | Status                 |
| ------------------ | -----------: | ---------------------- |
| phase1             |          516 | available              |
| intercompany       |           34 | available              |
| relational         |            0 | no primary denominator |
| duplicate          |            0 | no primary denominator |
| timeseries         |           21 | available              |
| unsupervised / VAE |           49 | available              |
...
- relational primary 0은 0% failure가 아니다. 현재 fixed5 v3.2d에는 relationship-only primary scenario가 없고 relational companion 139로 추적한다.
- duplicate primary 0은 0% failure가 아니다. 현재 fixed5 v3.2d에는 duplicate-primary scenario가 없고 duplicate companion 111로 추적한다.
...
- VAE primary 49는 `fictitious_existence_statistical`만 포함한다. Statistical companion 395는 primary recall이 아니라 companion lifecycle metric이다.
...
- Raw identifier leak count, forbidden identifier key count, forbidden identifier value count, PHASE2 case-id-like token count는 모두 0이다.
- Owner assignment는 detector output, score, rank, TOP-N matched result를 사용하지 않는다.

### 6.5 V3.3b fixed5_ownermeta_v33b — current canonical candidate

`datasynth_manipulation_v7_candidate_fixed5_ownermeta_v33b` 기준 responsibility map을 current canonical responsibility map candidate로 생성했다. v3.2d = historical responsibility map이다. 이 작업은 owner denominator / role 정합성 재정의이고, production ranking/gate/fusion, PHASE1 ranking, PHASE2 family ordering은 변경하지 않았다.
...
| Owner              | Primary docs | Status                                                |
| ------------------ | -----------: | ----------------------------------------------------- |
| phase1             |          483 | available; includes suspense rule-ownability override |
| intercompany       |           34 | available                                             |
| relational         |           20 | available                                             |
| duplicate          |           22 | available                                             |
| timeseries         |           21 | available                                             |
| unsupervised / VAE |           40 | available; suspense excluded                          |
...
| relational TOP500                 |           20 |     n/a | estimated proration 2 / 20 |
...
- relational primary 20은 `employee_vendor_hidden_relationship`이다. Circular related-party 34건은 IC primary + relationship companion이며 relational primary가 아니다.
- duplicate primary 22는 `time_shifted_duplicate` / row-score path다. Duplicate companion 71은 sidecar/export evidence다.
...
- VAE primary 40은 `fictitious_existence_statistical` statistical primary이며 feature-space check가 붙어 있다.
- TS product/default는 `ts_specific_top100_stabilized_surface` 기준 21 / 21이다. 이전 ordering 0 / 21은 사용자-facing 결과가 아니라 internal debug baseline이다.
- Raw identifier leak count, forbidden identifier key count, forbidden identifier value count, PHASE2 case-id-like token count는 모두 0이다.

### 6.6 V3.3d fixed5_ownermeta_v33d — current canonical candidate

`datasynth_manipulation_v7_candidate_fixed5_ownermeta_v33d` 기준 responsibility map을 current canonical responsibility map candidate로 생성했다. v3.3b = historical responsibility map이다. 이 후보는 relationship, duplicate, statistical owner evidence에서 journal-visible shortcut token을 제거했다. shortcut token hits in journal = 0이다.

| 항목     | 파일                                                                                               |
| -------- | -------------------------------------------------------------------------------------------------- |
| Script   | `tools/scripts/measure_phase2_family_responsibility_recall_v33d_fixed5_ownermeta_v33d_20260601.py` |
| Artifact | `artifacts/phase2_family_responsibility_recall_v33d_fixed5_ownermeta_v33d_20260601.json`           |
| Test     | `tests/modules/test_services/test_phase2_family_responsibility_recall_v33d.py`                     |

V3.3d denominator:

| Owner/lane            | Truth docs |
| --------------------- | ---------: |
| phase1 primary        |        483 |
| intercompany primary  |         34 |
| relational primary    |         23 |
| duplicate primary     |         19 |
| timeseries primary    |         21 |
| VAE primary           |         40 |
| relational companion  |        116 |
| duplicate companion   |         71 |
| timeseries context    |         92 |
| statistical companion |        404 |
...
| relational                 |  0 / 23 |
| duplicate                  |  8 / 19 |
| timeseries product/default | 21 / 21 |
| VAE                        |  0 / 40 |

Policy notes:

- Suspense 100건은 PHASE1 primary + statistical companion으로 evaluator override한다.
- TS product/default는 `ts_specific_top100_stabilized_surface` 기준 21 / 21이다. 이전 ordering 0 / 21은 사용자-facing 결과가 아니라 internal debug baseline이다.
- v3.3d는 denominator와 shortcut 제거 측면의 개선이며, 낮은 detector recall은 별도 detector-surface follow-up으로 본다.
...
- Recall 하락은 shortcut removal과 realistic 318k scale이 함께 드러낸 honest measurement 결과다.

### 7. 남은 결정

현재 historical rule-only map은 다음 이유로 reviewer 검토 이력으로 남긴다.

1. `phase1=586`이 너무 넓을 수 있다. `generic PHASE1-reviewable`과 `PHASE1 primary responsibility`를 분리할지 검토한다.
2. `unsupervised=289`가 broad context를 과도하게 owner로 붙인 결과일 수 있다. `primary unsupervised target`과 `statistical context`를 분리할지 검토한다.
3. `multi-owner=520`은 owner set 정책상 허용되지만, denominator가 부풀 수 있다. 필요하면 `owner_roles = primary / secondary / context` schema로 확장한다.
4. `no_clear_owner=0`은 fixed5 rule-only map 결과다. 애매한 truth가 정말 없는지 LLM-assisted 또는 human review가 필요하다.
5. Timeseries owner denominator 113이 진짜 TS-primary target인지, mixed timing context인지 분리해야 한다. v3.2d에서는 TS primary 21과 timeseries context 92로 분리했다.

### 8. 후속 작업

1. Claude/reviewer에게 owner rule sanity check를 요청한다.
2. 필요하면 low-confidence 또는 multi-owner 과다 case에 LLM-assisted semantic labeling을 적용한다.
3. owner role schema(`primary / secondary / context`)가 필요하면 artifact v2로 확장한다.
4. owner-map v2가 나오면 PHASE2 locked/progress family 결론을 재측정한다. 현재 canonical candidate는 v3.3b다.

**관련 문서**: [docs/guide/users/16_PHASE2_RESPONSIBILITY_MAP_DECISION.md](../guide/users/16_PHASE2_RESPONSIBILITY_MAP_DECISION.md) · [docs/spec/debugging/PHASE2_FAMILY_RESPONSIBILITY_RECALL_20260530.md](debugging/PHASE2_FAMILY_RESPONSIBILITY_RECALL_20260530.md) · [DETECTION_RESULTS_MANIPULATION_V7_FIXED4_PHASE2.md](../guide/DETECTION_RESULTS_MANIPULATION_V7_FIXED4_PHASE2.md) · `artifacts/phase2_family_responsibility_recall_fixed5_20260530.json` · `tools/scripts/measure_phase2_family_responsibility_recall_fixed5_20260530.py`
