...
| TS-15 | 탐지 설계/거버넌스 | PHASE1 priority_score 분포 불균형 → 가중합·floor·band컷 폐기, 순서형 tier(HIGH/MEDIUM/LOW/CONTEXT) 전환 + HIGH 조합 근거화 + 17건 재감사 A안 셋째 다리 확장 | 0.75 클러스터링, priority_floors dead code, count fitting, tier 전환, HIGH_COMBO_GROUNDING, A안, D060/D061 | 설계 해결 (2026-06-16), 코드 반영·과탐 가드 미결 |
...
**미결(코드 단계, 다음 세션)**: A안 셋째 다리 확장의 `topic_scoring.py` 반영 + 과탐 HARD 가드(정상 데이터 HIGH ≤ 2%) 재측정. 측정 전 완료 선언 금지.

### 상태

설계 해결 (2026-06-16). 가중합 폐기 → tier 체계 전환으로 priority_score 계층 재설계 대체 완료. A안 셋째 다리 확장 설계 확정. **코드 반영 + 과탐 가드 재측정만 미결**(다음 세션).
