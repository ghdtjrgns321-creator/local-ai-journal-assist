...
§32(a)(iii) → 현재 31개 L1~L4 룰 전체 (연중 대상)
...
→ 이 프로젝트 자체가 A46의 구현체. 현재 31개 L1~L4 룰 = CAATs 쿼리를 Python으로 자동화한 것.
...
→ 현재 31개 L1~L4 룰 전체가 §8⑥의 "준수 여부 검토" 수단
...
현재 31개 L1~L4 룰 전체가 330호가 요구하는 전표 실증절차를 구현한 것이다.
통제테스트(TOE) vs 실증절차: 내부통제 양호 → TOE 표본 검사, 미흡 → 실증절차 확대.

```
→ 현재 31개 L1~L4 룰 = 330호 실증절차의 자동화
...
현재 31개 L1~L4 룰은 전표 데이터 기반 패턴 탐지에 집중하며, 증빙 대조는 미포함. L3-12는 L1-06과 분리된 업무범위 review signal이다.
...
```
감사기준서 240호 §32     "전표의 적정성을 테스트하여야 한다"    → 현재 31개 L1~L4 룰 전체
...
감사기준서 330호           실증절차, 재무제표-장부 대사              → 현재 31개 L1~L4 룰 전체 (실증절차)
...
> **귀속 주석 (2026-06-14, tier 재설계)**: 위 표의 "순환거래(10건) → L3-03" 매핑에서 순환거래 패턴 자체는 **PHASE1-2 family(graph) 탐지기**에 귀속된다. 한 전표만으로는 A→B→C→A 구조가 보이지 않아 PHASE1-1 룰(전표 단위 점수경로)로 표현 불가하기 때문이다. 그 근거(IFRS 10 §B86 / K-IFRS 1110·1024 / ISA 600 / 550 §23)도 PHASE1-2 family에 귀속한다. L3-03은 PHASE1-1에서 account_logic booster(관계사 모집단 맥락)로만 잔존하며 단독 case seed가 아니다. IC01~03·GR01/03의 PHASE1 점수경로는 제거되었다. 표의 건수 수치 자체는 금감원 실증 기록이므로 유지한다. SoT: [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) §7.3.
>
> **핵심 발견**:
...
4. **순환거래** (10건) → PHASE1-2 family(graph) 탐지기가 담당. PHASE1-1 룰 점수경로 아님. L3-03만 account_logic booster로 잔존 (관계사 모집단 맥락). 근거(IFRS 10 §B86 / K-IFRS 1110·1024 / ISA 600·550 §23)는 PHASE1-2 family에 귀속. SoT: [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) §7.3
...
Could (2~3점) → future local-only NLP/graph 검토
...
> **귀속 주석 (2026-06-14, tier 재설계)**: "순환거래·비정상 시점" 카테고리 중 **순환거래의 근거(550 §23, IFRS 10 §B86 / K-IFRS 1110·1024 / ISA 600)는 PHASE1-2 family(graph)에 귀속**한다. 순환거래는 전표 단위 룰(PHASE1-1)로 표현 불가한 구조 패턴이므로 family 탐지기가 맡는다. 비정상 시점은 결산·시점 booster로 PHASE1-1에 흡수된다. 위 표의 유형 수·근거는 도출 기록으로 유지한다. SoT: [PHASE1_TIER_EVIDENCE_BASIS.md](PHASE1_TIER_EVIDENCE_BASIS.md) §4·§7.3.
