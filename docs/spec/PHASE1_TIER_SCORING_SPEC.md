...
sort_key = (
    ...
    time_severity_score       desc,   # OFF-TIME set(L3-05·L3-06·L4-05) 합산 — 근무시간 외 입력 정황
    rule_count                desc,   # 발화 룰 총수
    materiality_score         desc,   # 금액(중요성) — 최후 tiebreak
)
```

- `time_severity_score` = OFF-TIME set 합산: 주말+공휴일(L3-05 weekend_holiday)·L4-05 작성자 집중 = 2(high), 주말·심야 단독 = 1(medium). **tier 게이트는 미참여**(시간 신호만으로 tier 승격 불가) — within-tier 정렬과 UI 표시 전용. 정의·근거 SoT: [`HIGH_COMBO_GROUNDING.md`](HIGH_COMBO_GROUNDING.md) OFF-TIME 보조축 절(§2(3)). `independent_primary_count` 다음·`materiality_score` 위에 둬 "서로 다른 1차 신호 수"를 1순위로 보존하면서 시간 정황을 금액보다 위에 둔다(anti-burying 일관). L4-05는 작성자 맥락 연결값이며 case 점수 병합 아님(3-surface 불변식).
