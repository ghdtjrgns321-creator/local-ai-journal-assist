...
아래의 `유형 수`는 DataSynth 원천 anomaly 유형 기준이며, 실제 Phase 1 구현 룰 수와 1:1로 대응하지 않는다. Phase 1은 Must 유형을 감사 검토 가능한 세부 룰로 확장해 현재 **31개 transaction/review row 카드**로 운영하며(L4-02 Benford macro는 2026-06-15 PHASE1-2 family로 이관, canonical count 제외), L3-12 업무범위 집중 검토는 L1-06과 분리된 review 룰로 둔다. 카운트 단일 출처는 [RULE_DETAIL_METADATA_V1_LOCK.md §Rule Count Policy](RULE_DETAIL_METADATA_V1_LOCK.md#rule-count-policy)다.

```
판정     유형 수   적용 단계   구현/운영 단위                       FSS 6대 패턴 커버
────────────────────────────────────────────────────────────────────────────────────
Must      20개    Phase 1    31 row 카드 + 1 macro 카드 (L4-02)    6/6 (전부 커버)
...
## 2. Phase 1: 룰 기반 탐지 (31개 transaction/review row 카드 + 1개 macro 카드 (L4-02 Benford))

이 절은 **31개 transaction/review row 카드**로 구성된다. row 카드는 `L1~L4` 전표 행 단위 구현 룰이다. L4-02 Benford macro(계정·월 단위 분포 검정)는 2026-06-15 PHASE1-2 family로 이관되어 canonical count에서 제외된다. L3-12는 L1-06의 명시적 SoD 위반과 분리된 사용자 업무범위 집중 review 룰이며, 사용자 큐와 우선순위 해석은
...
그래프 신호는 위 31개 카드 수에는 넣지 않지만, Phase 1 결과 화면에서는 Account / Process Queue 또는
관계사·연결 구조 drill-down의 보조 finding으로 결합한다. 카운트 단일 출처는 [RULE_DETAIL_METADATA_V1_LOCK.md §Rule Count Policy](RULE_DETAIL_METADATA_V1_LOCK.md#rule-count-policy)이며, 레거시 문서의 "32/33 rules" 표현은 `31 canonical + L4-02/Benford macro(PHASE1-2)` 로 normalize한다.
...
1. **Row-level anomaly score**
   ...
   - IC01/IC02/IC03은 L1~L4 룰 수에 포함하지 않는 관계사 보조 finding이다. `IntercompanyMatcher` 결과가 aggregate 입력에 포함된 경우, 대사 예외가 row 대표 점수에서 숨지 않도록 `intercompany_exception_score`와 `intercompany_exception_reasons`를 별도 기록한다. `IC02` 또는 `IC03` 단독은 row `Low` floor `0.20`, `IC01` 단독은 `ic01_evidence_level`에 따라 row `Low`/`Medium` floor를 나누며, 2개 이상 IC 예외 결합은 row `Medium` floor `0.40`을 적용한다.
2. **Case priority score**

   > **[tier 전환 예정]** 아래 가중합 priority_score(`0.25*control + …`)와 `high >= 0.90/medium >= 0.75` band컷은 **현행 코드 동작**이다. 설계상 이 체계는 폐기 확정이며, band는 순서형 tier(HIGH/MEDIUM/LOW/CONTEXT) 직접 매핑으로 대체된다(SoT `PHASE1_TIER_SCORING_SPEC.md` §5 폐기·대체 매핑표). 코드 반영은 다음 세션 — 본 절은 전환 전 현행 동작 기록.
...
- **Phase1 정규화 계약**
  ...
  - source가 자동 계열(`patterns.approval_limit_exceeded_review.sources`)이면 버킷 불문 review로 강등하되, **위장 의심 행(`source_trust.lone_automated_mask` 기준 단독 자동)은 강등을 받지 못하고 원래 버킷을 유지한다** (2026-06-13, OPEN_ISSUES #18 — critical 초과를 source 위조로 review 강등시키는 회피 차단).
...
- **기본 예외는 명시적으로 allowlist에 들어간 시스템 자동처리만 둔다**
  ...
  - **위장 게이트 (2026-06-13)**: allowlist `sources`에 해당해도 위장 의심 행(`source_trust.lone_automated_mask` 기준 단독 자동)은 `allowed_system` 0점 예외를 받지 못하고 일반 자기승인 경로로 평가된다. persona 기반 allowlist(`user_personas`)는 게이트 대상이 아니다 (OPEN_ISSUES #18).
...
- **유령 승인자 서브패턴 (`unknown_approver`, 2026-06-13 신설)**
  - L1-07 본래 후보는 `approved_by` **공란**이다. 이 서브패턴은 반대로 **비공란인데 그 ID가 직원 마스터에 없는** 행을 별도로 잡는다 — 한도검사(L1-04)는 한도 미해소로 제외하고 L1-07 공란 후보도 아니라서, 마스터에 없는 승인자를 적으면 승인통제 룰 전체를 회피하던 사각을 막는다.
  - 판정: feature `approver_in_master`(`amount_features._compute_approver_info` — `employees.json`의 `user_id` 멤버십, 공란→NA·비공란 미존재→False) ∧ 비공란. score 고정 `0.55`(검토등급), `queue_label`/`reason_code`/`bucket` = `unknown_approver`. 직원 마스터가 없으면 컬럼 미생성으로 graceful 비활성(공란 후보 경로는 불변).
  - 운영 해석: "부정 확정"이 아니라 승인권자 명부와 대조할 검토 후보다. 퇴사자·외부 컨설턴트·표기 차이(대소문자)·마스터 부분추출로 정상일 수 있으므로 annotation에 승인자 값을 노출해 감사인이 대조·종결하게 한다(합성데이터에선 0건). 시점 정합(마스터 유효기간)·표기 정규화는 향후 과제(OPEN_ISSUES #19).
- **운영 원칙**
...
- **PHASE1 점수 흐름**
  ...
  - **구현 반영 (2026-06-13)**: fallback 3종 발화 경로를 통합 fallback 루프로 구현했다(`b04_duplicate_payment` 내). 같은 (회사·거래처) 그룹에서 document_id가 다르고 45일 이내인 후행 문서를, 선행 reference 상태로 분기한다 — 선행 ref 있음+후행 공백=`mixed`, 두 ref 다름=`amount_partner`, 둘 다 공백=`blank`(이 경우만 정확 금액 일치, 나머지는 `min(2%,10만원)` 허용오차). 한 문서가 복수 경로에 걸리면 우선순위 `mixed>amount_partner>blank` 하나만 부여. 정기 반복(월 단위 3회+, `_l202_recurring_profile`)은 `recurring_suppressed`로 제외. floor는 `reference_match`만 받는다(R1-A `floor_eligible_labels` 게이트).
...
- **PHASE1 점수 유입**
  ...
  - `IC01` 단독 예외는 `ic01_evidence_level` sidecar 에 따라 floor 가 차별 적용된다. `high` 와 `review_stale` 은 row 최소 Medium `0.40`, `review` 는 row 최소 Low `0.20`. 2개 이상 IC 예외 결합은 evidence level 과 무관하게 row 최소 Medium `0.40` 으로 표시한다 (D065 결정).
  - 이 보정은 기존 `flagged_rules` 확정 룰 참조 생성 기준을 바꾸지 않는다. 출력에는 `intercompany_exception_score`, `intercompany_exception_reasons`로 별도 추적하며, IC01 hit 시 reason 문자열에 `IC01[high]` / `IC01[review_stale]` / `IC01[review]` qualifier 가 부착된다.
- **평가/리포트 표시 방식**
...
- **IC01 evidence level sidecar 정책**: IC01 hit 의 신뢰도는 detector 결과 DataFrame 의 두 sidecar column (`ic01_evidence_level`, `ic01_review_reason`) 으로 분류한다. 두 column 은 canonical 31 rule count 외부의 `intercompany_sidecar` surface 산출물이다.
...
- **평가/리포트 표시 방식**
  ...
  - **위장 게이트 (2026-06-13)**: source가 자동 계열이어도 위장 의심 행(`source_trust.lone_automated_mask` — 자동 주장 ∧ batch_id/job_id 결측 ∧ 같은 날 동류 ≤10건)은 `normal_system_context` 감면(0.20)을 받지 못하고 `confirmed_after_hours`(0.45)에 남는다. persona/actor 기반 분류는 게이트 대상이 아니다. 근거: source 한 칸 위조로 심야 감면을 받는 비대칭 차단 (OPEN_ISSUES #18).
...
| `L1-09` | `src/detection/fraud_layer.py::FRAUD_RULE_EXPLANATIONS` | `L1-09` key |
...
| `L3-01` | `src/detection/integrity_layer.py::INTEGRITY_RULE_EXPLANATIONS` | `L3-01` key |
