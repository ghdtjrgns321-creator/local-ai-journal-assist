...
- **판정 (binary flag — 점수 가감·밴드 폐기)**
  - 아래 둘 중 하나면 **flag 1.0**.
    ...
    - **(B) 1:1 거울 쌍**: 같은 `gl_account` + 반대 방향(한쪽 차변 X / 다른쪽 대변 X) + **금액 정확일치**(config `reversal_amount_tolerance`, 기본 `0.0` = ±0%) + 다른 `document_id`. 시간 윈도우(config `reversal_mirror_window_days`, 기본 45일) 내.
      - **금액 ±0% 근거 (2026-07-02)**: 역분개는 ERP 표준상 exact reverse(mirror-image / switch Dr-Cr / change sign)라 원전표와 금액이 **정확히** 같다. 구 `±2%`는 L2-02 중복지급 tolerance("부분지급·수수료로 살짝 다른 재지급")를 전용한 값으로 역분개 근거가 없었고, 정상 순환거래(AR 발생↔회수 등 우연 동액)를 대량 오탐했다(정상 v51 거울쌍 24,466라인 중 금액 정확일치 1.2%·±2%근사 98.8%·reference공유 0.6% → 무관거래 우연매칭이 압도적). ±0%로 정상 FP 17,434→207(98.8%↓), 설계 역분개 recall 100% 유지(설계 역분개는 s0 ERP 연결이 담당). 상세: [results/normal/L2-05_REVERSAL_TOLERANCE_DECISION.md](results/normal/L2-05_REVERSAL_TOLERANCE_DECISION.md).
