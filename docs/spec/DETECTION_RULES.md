...
- **발화 기준 (binary flag)**: 매출 계정 라인의 금액이 같은 매출계정 모집단 대비 양(+)의 z-score 임계 초과면 `1.0`, 아니면 `0`.

  ```
  is_revenue_account AND amount_zscore > zscore_threshold  →  1.0
  그 외                                                    →  0
  ```
  ...
  - `amount_zscore` 절대값은 `row_annotation`에 사실값으로만 남겨 표시·동점정렬에 통합점수 쪽에서 쓴다(룰 점수에 차등 반영 금지).
- **임계값 근거 (3.0σ)**
...
- **한계**
  ...
  - 매출 금액은 우편향(right-skew)이라 평균/표준편차 z-score는 robust하지 않다(σ가 부풀려져 이상치를 가릴 수 있음). 더 견고한 대안은 수정 z-score(중앙값+MAD, Iglewicz-Hoaglin 임계 3.5) 또는 로그변환 후 z-score다. 현행은 최소변경 원칙상 평균/std·3.0σ를 유지하고 MAD/로그 대안은 향후 개선 후보로 둔다.
- **조합 시 위험도 해석**
