...
| S3 | Trivial baseline | [`docs/archive/completed/S3_trivial_shortcut_baseline.md`](../archive/completed/S3_trivial_shortcut_baseline.md), `artifacts/stage3_trivial_shortcut_baseline.json` |
...
| S8 | Stacking OOF protocol 재검증 | [`docs/archive/completed/S8_stacking_oof_audit.md`](../archive/completed/S8_stacking_oof_audit.md), `artifacts/S8_stacking_oof_ablation.json` |
| S9 | Phase 2 ML 부가가치 baseline + zero-day 대안 | [`docs/archive/completed/S9_phase2_value_baseline.md`](../archive/completed/S9_phase2_value_baseline.md), [`docs/archive/completed/S9_zero_day_protocol_alternatives.md`](../archive/completed/S9_zero_day_protocol_alternatives.md) |
| S10 | 종합 판정 (본 문서) | [`docs/spec/PHASE2_FITTING_AUDIT.md`](PHASE2_FITTING_AUDIT.md), `tests/datasynth_quality_gate/results/phase2_fitting_audit/SUMMARY.md` |
...
### 6.1 `docs/spec/CONSTRAINTS.md` — §ML 학습 전략 신규 sub-section 추가

**적용 위치**: `## ML 학습 전략: 비지도학습 중심 + 지도학습 프레임워크` 섹션 말미에 추가

```diff
@@ docs/spec/CONSTRAINTS.md (## ML 학습 전략 섹션 말미) @@
...
### 6.2 `docs/spec/DECISION.md` — D040 신규 결정 추가

**적용 위치**: `### D039: DataSynth v23 운영 기준본 변경` 다음 (또는 마지막 D 항목 다음)

```diff
@@ docs/spec/DECISION.md (마지막 D 항목 다음) @@
...
+- **관련 audit**: `docs/spec/PHASE2_FITTING_AUDIT.md`,
+  `artifacts/S4_evaluation_protocol.md`, `docs/archive/completed/S9_phase2_value_baseline.md`
...
+  `docs/spec/PHASE2_FITTING_AUDIT.md`
...
+- **관련 audit**: `docs/spec/PHASE2_FITTING_AUDIT.md`,
+  `docs/archive/completed/S9_zero_day_protocol_alternatives.md` §3.3
...
- `docs/archive/completed/phase2_ml_feasibility.md` 말미에 다음 footnote 추가:

```markdown
> **Phase 2 Fitting Audit 통과 (Stage 0~10, 2026-05-15)**: 17 위험 항목 GREEN.
> 상세: `docs/spec/PHASE2_FITTING_AUDIT.md` 및
> `tests/datasynth_quality_gate/results/phase2_fitting_audit/SUMMARY.md`.
```
...
## 9. IC family structural fitting 진단 (2026-06-01, rule-based 보강)

§0~8 은 ML 트랙 fitting 이 대상이다. 본 절은 rule-based IntercompanyMatcher 의
v33d 공식 평가 100% recall (34/34) 이 어떤 구조에 의존하는지 측정한 결과를 기록한다.
측정 산출물: `artifacts/ic_noise_probe_20260601.json`,
`artifacts/ic_truth_strength_probe_20260601.json` /
스크립트 `tools/scripts/ic_noise_probe_20260601.py`,
`tools/scripts/ic_truth_strength_probe_20260601.py`. 탐지기·데이터 미변경, 읽기 전용.

### 9.1 측정 결과

| 측정 | 지표 | 값 |
|---|---|---|
| raw score 렌즈 | 정상 IC FP rate (score>0) | 6,156 중 3,873 = 62.9% |
| case-tier 렌즈 | 정상 IC FP (case 화) | 168 = 2.7% (unmatched/IC01 은 case 제외, invariant #54) |
| case-tier 렌즈 | truth recall / review_cost | 34/34, review_cost = 34 (truth 가 정렬 1~34위 독점) |
| case-tier 렌즈 | precision@K | p@34=100% · p@100=34% · p@500=17% |
| 노이즈 ablation | 정상 IC 0~100% perturbation | recall·review_cost 불변 (truth 미접촉이라 무력) |
| truth 강도 시뮬 | truth 를 정상과 같은 tier 로 강등 | review_cost 34 → 167 (5배), p@34 100% → 23.5% |

### 9.2 근본 원인 — structural tier separability

```
strong tier (reciprocal_flow): truth 34건 독점  · 정상 IC 0건
moderate tier (amount_mismatch): 정상 IC 212건 · truth 0건
```

truth (`circular_related_party_transaction`) 는 단일 전표 안 receivable+payable
self-balanced 구조라 `reciprocal_flow` strong tier 로 분류된다. 정상 IC
(ic_generator seller_doc/buyer_doc) 는 양변이 서로 다른 전표라 단일전표 reciprocal
조건을 못 만족해 strong 진입 자체가 막힌다. tier 가 정렬 1차 키이므로 truth 34건은
score·노이즈와 무관하게 항상 1~34위를 독점한다. 100% recall 은 score 우수성이 아니라
이 tier 분리에 의존한다.

### 9.3 현실 괴리

- 현실 정상 IC 는 단일 전표에 양변이 함께 오는 경우가 흔하다 (수기 결산 분개, 통합
  전표). 이 경우 정상 IC 도 reciprocal strong 으로 올라 truth 와 섞이며, 9.1 시뮬과
  동일하게 review_cost 가 5배로 증가하고 precision 이 1/4 로 하락한다.
- 현실 부정 IC 는 양 법인 분리 기표가 더 흔하다. 이 경우 truth 가 moderate 로 떨어져
  정상 212건에 묻힌다.
- 즉 IC 100% 는 "단일전표 reciprocal=이상, 양doc분리=정상" 이라는 합성 데이터 구조적
  관습에 의존하며 현실엔 이 관습이 없다.

### 9.4 후속 권고

- DataSynth: truth IC 일부를 양 법인 분리 기표로, 정상 IC 일부를 단일전표 양변으로
  생성해 tier 분리를 깨고 실난이도를 노출.
- 평가: recall@top500 단독 대신 precision@K / review_cost 를 상시 병기 (9.1 probe 가
  측정 코드).
- 해석: 현재 IC 100% 는 capability 결론이 아니라 "구조 분리 가정 하 상한" 으로 표기.

---

## 10. Manipulation truth 신호 유효성 전수 검사 (2026-06-02)

§9 의 IC circular NOSIGNAL 결함이 다른 manipulation scenario / family truth 에도
있는지 8 family 전수 검증한 결과. 데이터셋
`datasynth_manipulation_v7_independent_semanticfix8e_familyfix_20260602` (truth 620 docs),
detector/data 미변경, 출력↔라벨 사후 join 측정.

### 10.1 판정 요약

```
family / scenario                     n    의도 owner         verdict          핵심 근거
------------------------------------- ---- ------------------ ---------------- ----------------------------------------
circular_related_party_transaction    34   intercompany→graph NOSIGNAL         graph 0/34, cycle 0개(회사 3개·네임스페이스 불일치)
unusual_timing_manipulation           21   timeseries         DETECTOR-BLIND   구조 실재(야간1.0·backdating0.62)·TS01/TS02 0/21
embezzlement.relationship             24   relational         HEALTHY          R05 20/24 lift 6.33, 공유 bank/addr/phone 엣지 실재
embezzlement.duplicate                28   duplicate          SHORTCUT         L2-03d 5/28 lift 1.06(≈정상), relational 28/28 포착
embezzlement.phase1                   24   phase1             WEAK             risk med+ 0.29(타 phase1 0.84~1.0 대비 약함)
fictitious_entry(phase1 129)          129  phase1             HEALTHY          risk med+ 0.84 lift 10x, L4-05·L3-04 발화
expense_capitalization                100  phase1             HEALTHY          risk med+ 1.0, L2-04 자본화 룰 발화
suspense_account_abuse                100  unsupervised       HEALTHY          VAE q95 lift 4.69 + L3-07 rule co-signal
approval_sod_bypass                   29   phase1             HEALTHY          risk med+ 0.97, L1-09 SoD·L1-04·L1-05 발화
broad_statistical(unsupervised)       139  unsupervised       HEALTHY          ECDF median 0.973, q95 lift 4.69·q99 8.58
```

### 10.2 핵심 발견

- **circular = NOSIGNAL (§9 재확인)**: 8e 에서도 truth 34 docs 는 cycle 0개, 회사 3개뿐,
  `company_code(C001)` vs `trading_partner(IC-C002)` 네임스페이스 불일치로 엣지 폐합 불가.
  graph GR01/GR03 0/34, IC 구조 룰 IC01/IC02/IC03 0/34. 100% 포착은 `ic_unmatched_prob`(ML
  prob, 34/34) + `ic_reciprocal_flow_prob`(self-balanced 단일전표 마커, 22/34) 컨닝.
- **timeseries = DETECTOR-BLIND (신규 유형)**: NOSIGNAL 4종과 다른 케이스. 야간 posting
  100%·backdating 62%·주말 33% 로 **구조는 강하게 실재**하나 TimeseriesDetector TS01/TS02
  (계절성·추세파단)가 posting-hour/backdating 신호를 읽지 않아 0/21. 데이터 결함이 아닌
  **detector 커버리지 갭**. 단 night 100% 균일성은 고정 마커화 위험.
- **embezzlement.duplicate = SHORTCUT**: time_shifted L2-03d 가 정상과 구분력 0(lift 1.06).
  설계상 12/28 export_only(미탐지 의도) + 11 row_score 이나 실측 5/28. 동일 truth 를
  relational 이 28/28 포착 → 실제 신호는 relational.
- **phase1 4 scenario = HEALTHY**: 의도 룰이 scenario 별로 정확 발화(approval_sod→L1-09,
  expense_cap→L2-04, period_end→L1-05, fictitious→L4-05). risk Medium+ lift 10~12x.
- **unsupervised = HEALTHY**: VAE ECDF 가 통계 이상점 강분리. 단 suspense 는 L3-07/L3-08
  가계정 룰도 발화 → 'statistical only' 보다 rule+unsupervised co-owner 가 정확.

### 10.3 DataSynth 수정 우선순위

| 순위 | 대상 | 조치 |
|---|---|---|
| P1 | circular (NOSIGNAL) | 진짜 3-hop+ cross-company 순환을 동일 네임스페이스로 재생성 → graph 재배정; IC primary 는 pair-level reconciliation 불일치 truth 신규(self-balanced 금지) |
| P1 | timeseries (DETECTOR-BLIND) | detector 갭: TimeseriesDetector 에 posting-hour·backdating sub-signal 추가가 1차; night 100% 시간대 분산 |
| P2 | embezzlement.duplicate (SHORTCUT) | time_shifted L2-03 구분력 0 — amount/reference 근접 강화 또는 relational companion 으로 재배정 |
| P2 | embezzlement.phase1 (WEAK) | risk med+ 0.29 — L4-03 은닉 신호 강화 또는 owner 재배정 |
| P3 | suspense owner 라벨 | rule co-signal 공존 — 'broad_statistical_only' → 'rule+unsupervised co-owner' 명시 |

산출물: `artifacts/manipulation_truth_signal_audit_20260602.json` (+ rulestyle / structure_check /
phase1_unsup 분할 JSON 3종).

### 10.4 8o 재검증 (2026-06-03)

§10.3 권고를 반영해 재생성한 `semanticfix8o_familyfix` 에서 문제 4종을 같은 기준으로 재측정.
circular=relational cycle 재정의, 신규 IC mismatch primary(24), duplicate fingerprint 제거,
TS date marker 분산.

```
family        8e 판정          8o 구조                        8o 탐지                              8o 판정
------------- ---------------- ------------------------------ ------------------------------------ ------------------
circular(34)  NOSIGNAL         ✅ 진짜 5-cycle C001→…→C005→C001 ❌ GR01 0/34                          데이터 회복·탐지기 미연결
ic_mismatch   (shortcut owner) ✅ self_balanced 0·2전표 불일치  △ IC01/02/03 0, ic_unmatched lift21.6 HEALTHY
duplicate(26) SHORTCUT         fingerprint 제거                ❌ 1/26 lift0.22                      신호 소멸
timing(21)    DETECTOR-BLIND   △ 날짜 4→21 분산·night=1.0 잔존  ❌ TS01/02 0/21                       변동 없음(탐지기 갭)
```

판정 상세:

- **IC mismatch = 회복 성공**: self-balanced 단일전표 컨닝 완전 제거(rate 0.0), 회사별 단일전표
  2-doc cross-company 불일치 구조 실재. `ic_unmatched_prob` lift 21.6(19/24)로 잡힘. 단
  결정론 룰 IC01/IC02/IC03 은 여전히 0/24 — ML prob 만 발화(부차 권고).
- **circular = 데이터는 회복, 탐지기 미연결**: 네임스페이스 정규화(IC-C002→C002) 후 진짜 5-hop
  cycle 확인(NOSIGNAL 해소). 그러나 GraphDetector GR01 이 0/34 — 근본 2원인: ① journal 에
  `is_intercompany` 컬럼 부재로 GR01 `_REQUIRED_COLS` 미충족 → **GR01 통째 skip** ② GR01 이
  노드를 `company_code`("C002") vs raw `trading_partner`("IC-C002")로 만들어 IC- 접두 정규화
  없음 → cycle 미폐합. 금액(전건 ≥100M)은 무관. 데이터 cycle 은 고쳤으나 **detector 계약**(컬럼·
  네임스페이스) 미충족.
- **duplicate = 컨닝 제거됐으나 신호 소멸**: L2-03d lift 0.22(truth 0.038 < normal 0.176),
  1/26. fingerprint 는 제거됐으나 제품경로 중복신호가 대체 생성되지 않음. relational 26/26·
  intercompany 26/26 이 잡음 → 실제로 relational-signal embezzlement.
- **timing = 부분**: date 집중 marker(4 dates) 는 21 distinct 로 분산. 그러나 night_rate 1.0
  잔존 + TimeseriesDetector TS01/TS02 가 posting-hour/backdating 을 안 읽어 0/21 — §10.2 에서
  지적한 detector 갭 그대로(데이터 아닌 detector 수정 사항).

후속(detector vs data 분리):

| 구분 | 대상 | 조치 |
|---|---|---|
| **detector(Python)** | circular GR01 | `_REQUIRED_COLS` 의 `is_intercompany` 의존 완화 또는 파생, edge 노드에 `IC-` 접두 정규화 추가 |
| **detector(Python)** | timing TS | posting-hour·backdating sub-signal 추가 |
| **detector(Python)** | IC IC01-03 | genuine 2-doc unmatched 에 결정론 reconciliation 룰이 발화하도록 점검 |
| **data(Rust)** | circular | trading_partner 를 GR01 이 읽는 토큰(정규화 company 또는 is_intercompany 동반)으로 방출 |
| **data(Rust)** | duplicate | 제품경로 tier 에 amount/reference 근접 실중복 생성 또는 relational owner 재배정 |
| **data(Rust)** | timing | night_rate 100% 시간대 분산 |

산출물: `artifacts/manipulation_truth_reverify_8o_20260603.json`.
...
| 2026-06-01 | §9 IC family structural tier separability 진단 추가 (noise/truth-strength probe 측정) |
| 2026-06-02 | §10 Manipulation truth 신호 유효성 8 family 전수 검사 추가 (circular NOSIGNAL·timeseries DETECTOR-BLIND·duplicate SHORTCUT 외 HEALTHY 6) |
| 2026-06-03 | §10.4 8o 재검증 추가 (IC mismatch HEALTHY 회복·circular 데이터회복+GR01 미연결·duplicate 신호소멸·timing 변동없음) |
