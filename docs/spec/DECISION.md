...
- **결정**: D071 tier 전환 후 HIGH 조합을 근거화하던 중, FSS HIGH 158건 전수 재집계에서 기존 5조합 밖 17건을 발견했다. 17건 원문(수법)을 분석한 결과 11건이 조합1(가공전표)·조합2(횡령은폐)와 **같은 스토리, 셋째 정황(2차 정황)만 다른** 사례였다. 따라서 **A안 — 신규 조합을 만들지 않고 기존 조합1·2의 인정 2차정황 풀을 FSS 실증에 맞게 확장**한다.
  - 조합1(가공전표) 셋째 다리: `(L4-04|L2-03)` → `+ L3-03·L3-10·L1-05·L3-11` 추가(과탐 가드로 L3-04·L1-09 제외 — 아래).
  - 조합2(횡령은폐) 통제 분기: `승인우회(L1-04~07)` 필수 → `or (L2-05 역분개 + L3-02 수기 + L4-03 고액)` 분기 추가(승인 흔적 없는 은폐 포착, 고액 동반으로 정상 clearing 구분).
...
- **과탐 HARD 가드 (2026-06-17 측정·반영 완료)**: 정상 v42j 2022(14,070 case) 측정 — wide(6 신규 다리) **5.245% FAIL**. 다리별 분해에서 **기말(L3-04) 734/738·승인일공백(L1-09) 334**가 과발화 주범, L3-03·L3-10·L1-05는 정상 0. 핸드오프는 L3-04/L1-05를 위험으로 추측했으나 데이터는 L3-04·L1-09가 노이즈 → 기말(closing_timing 영역)·L1-09(근거 약함) 제외 → narrowed **0.334% PASS**(≤2%). 최종 2차정황 `L4-04·L2-03·L3-03·L3-10·L1-05·L3-11`(FSS 10/11 커버).
- **상태**: 코드·측정·문서 반영 완료(2026-06-17). 코드: `topic_scoring.py::_FICTITIOUS_SECONDARY_RULES` 신설·조합2 reversal+manual+고액 분기. 부수: `phase1_case_builder._precomputed_string_column` nullable-boolean fillna 버그 수정(전체연도 빌드 차단 선행버그).
