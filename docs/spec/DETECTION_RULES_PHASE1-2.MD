...
> - **자기 큐(별도 목록, 고액/대량 정렬)** — **Benford(L4-02)·D01·D02 + 라운드넘버 밀집도·첫등장/희소 거래처.** "전표 한 장으론 안 보이고 계정/거래처를 모아야 보이는" 분석적 검토 신호. PHASE1-1 전표가 안 떠도 "이 계정/거래처가 통째로 이상"을 독립 검토 표면으로 올린다(전표 tier에는 미참여 = 0). 채점 — 운영 정렬은 신호 강도(MAD·JSD·변동폭·합계금액·건수), 개발 검증은 coverage(precision/recall 아님).
> - **배지(PHASE1-1 전표 등급 줄의 맥락 꼬리표, 점수 비병합)** — **L4-05·L4-06·L3-12 + 라운드넘버 단건·첫등장/희소 거래처 전표.** 정상 업무에 흔해(HIGH_COMBO CONTEXT) 단독 전표 큐는 노이즈 → PHASE1-1 HIGH/MEDIUM/LOW 전표가 그 신호의 더미에 속하면 배지로만 표시(전표 등급 안 바꿈). 구현 `phase1_case_builder._compose_badge_tags` → `badge_tags`.
...
>
> **PHASE1-2 는 검토 목록을 소유하지 않는다 (2026-07-15 확정)**: 신호(finding)만 생성하고, 무엇을 몇 개 어떤 순서로 보여줄지는 화면이 정한다. 따라서 **finding 생성 경로는 절단하지 않는다** — 구 `macro_findings` 정렬 후 상위 100건 절단(커밋 18a5a47)은 제거했다. 신호 종류마다 점수 척도가 달라(Benford candidate_score 0~1 vs robust-z 무상한) 한 리스트를 자르면 척도 큰 종류가 칸을 독식하고, 백엔드가 미리 버린 신호는 화면이 되살릴 수 없다. 산출물 키는 단위별로 분리한다 — 계정/프로세스 단위 `metadata.macro_findings`, 거래처 단위 `metadata.partner_findings`.
>
> **단, overview 요약 경로는 예외**: `export/phase1_case_view.py:286 summarize_phase1_case_result` 는 `macro_findings[:5]` 로 자른다. 이는 개요 카드용 미리보기이고 전체 건수는 `macro_finding_count` 로 별도 노출되며 큐 접근자(`build_phase1_macro_finding_queue`·`build_phase1_partner_finding_queue`)는 `top_n=None` 기본이라 전량을 준다. 즉 **finding 은 안 버리고 요약만 자른다**.
>
> ※ 단위 SoT 주의 — [UNIT_MEASUREMENT_POLICY.md](UNIT_MEASUREMENT_POLICY.md) 는 PHASE1 탐지 단위를 **`document`·`flow` 두 가지로만** 규정하며 macro(계정/프로세스)·partner(거래처) 같은 **모집단 단위를 다루지 않는다**. 자기 큐 finding 은 전표 단위가 아니라 그 정책의 범위 밖이다(전표 tier 미참여 = 0). 정책 확장이 필요하면 별도 결정 대상.
>
> **UI**: PHASE1 1회 실행 → 한 결과창 소분류 — 검토 케이스(등급 + 배지) / 데이터 정합성 / 분석적 검토(Benford·D01/D02·라운드넘버 밀집도·첫등장/희소 자기 큐). PHASE2(VAE)는 별도 실행·별도 표시. UI 자체는 미구현(PLAN §3 Phase 5·6).
...
| 라운드넘버 밀집도 [신규]  | 둥근 금액 비정상 집중         | 계정·월·작성자 축별   | **자기 큐 + 배지**              | AS2401 §61(e); 밀집도=자기큐(이항검정), 단건=배지. `round_density_rules.py`                      |
| 첫등장/희소 거래처 [신규] | 원장 첫 등장·희소(rare)       | 거래처                | **자기 큐 + 배지**              | HIGH-6 GL-only 약근사(커버 비주장); 전수 목록·거래내용 묶음·고액/대량 정렬. `partner_signals.py` |
| 시계열 당기내 집중        | 특정 시점 거래 몰림           | 계정·시점(일/주/월)   | **자기 큐 폐기 → D02 드릴다운** | 당기내 baseline 이 결산 캘린더를 재발견(실측 §시계열). 2026-07-15                                |
...
> 자기 큐 **5종**(Benford·D01·D02·라운드넘버 밀집도·첫등장/희소 거래처)은 계정/거래처 단위 별도 검토 목록을 만들고, 배지 신호는 PHASE1-1 전표 등급 줄에 맥락 꼬리표로만 붙는다(점수 비병합). 구 6종의 시계열 당기내 집중은 2026-07-15 자기 큐에서 폐기(§시계열 당기내 집중). 재설계 SoT: `dev/active/phase1-2-code-rework/PLAN.md`, 실측 근거: `dev/active/phase1-2-code-rework/backend_verify.md`.

> **라운드넘버 밀집도 — 신규(dual: 자기 큐 + 배지)**: 부자연스럽게 둥근 금액(끝자리 0 다수 — 예 5,000,000 / 100,000,000). 근거 PCAOB AS2401 §61(e) **명문**(FSS 원문은 둥근 정도 미기록 → 태깅 0, 도구 경계 ≠ 감사 경계). **밀집도 = 자기 큐**(Benford 동렬), **단건 라운드넘버 = 배지**(전표 꼬리표).
>
> - **판정(2026-07-15 확정)**: baseline p = **원장 전체의 round 비율**(데이터에서 산출 → 산업·회사별 정상 둥근 금액률 차이를 흡수). 각 그룹의 round 건수 k 를 `Binomial(n, p)` **단측 이항검정**. 표본이 크면 사소한 차이도 유의해지므로 effect size 하한(`round_density_min_excess`)을 p-value 앞에 건다.
> - **모집단 = 계정·월·작성자 축별 독립**(복합키 아님). 복합키로 묶으면 그룹당 표본이 급감해 검정력이 죽는다. 축 이름은 finding 의 `scope`(`gl_account`/`posting_month`/`created_by`)로 노출.
> - **구현**: `src/detection/round_density_rules.py::compute_round_density_findings` → `phase1_case_builder._build_round_density_macro_findings` → `metadata.macro_findings`(rule_id=`ROUND-DENSITY`). `review_score` = 신호 강도(excess = baseline 대비 round 비율 초과분)이며 부정 확률이 아니다. row `anomaly_score` 기여 0.
> - **설정**: `round_density_min_sample`(100) · `round_density_alpha`(0.01) · `round_density_strong_alpha`(0.0001) · `round_density_min_excess`(0.05). 전부 `config/settings.py` read — §3 리터럴 금지.
> - **실측(3개년 통합)**: 정상 v53_r6 376,727행 baseline 5.52% → finding **1건**(작성자 ARCLERK011, 29.0%). fraud 381,791행 baseline 6.03% → finding **3건**(계정 9300·500860·500880, 11.8~14.0%).

> **is_round_number 정의 — 절대 단위 폐기(2026-07-15)**: 구 정의는 `amount % round_unit(100만원) == 0` 이었다. 그러나 원장 금액이 1,526원(p10)~2,057억원(max)으로 **8자릿수에 걸쳐** 있어 단일 절대 단위로는 잴 수 없다. 실측상 **거래의 64.9%가 100만원 미만**이라 구조적으로 판정 대상에서 빠졌고, 정상·fraud 양쪽 38만행 중 round 가 141행(**0.037%**)뿐이라 밀집도 검사기가 어느 데이터에서도 발화하지 못했다(데이터 아티팩트 아님 — 1,000원 배수는 7.1%로 라운드니스 자체는 존재).
>
> 현행: **금액 규모에 상대적인 둥근 정도** — 통화별 반올림 후 (1) 소수부 없음 (2) **유효숫자**(끝자리 0 제거 후 남은 자릿수) ≤ `round_max_significant_digits`(기본 2) (3) 총 자릿수 ≥ `round_min_digits`(기본 3, 10원·50원 같은 사소한 소액 배제). 근거 AS2401 §61(e) "끝자리 0 다수" 문언 + Nigrini 끝자리 분석. 실측 baseline: ≤2·≥3 → **5.16%**(≤1은 2.46%로 희박, ≤3은 14.49%로 과다).
> 구현 `src/feature/amount_features.py::add_is_round_number`·`significant_digit_stats`. 전환 소비처 5/5: settings · amount_features · `timeseries_rules.round_amount_score` fallback · QG `tier5_label.py` T5-17 · `presets/construction.yaml`(유효숫자 1자리) · `threshold_sidebar`. **등급 영향 0**(실측: is_round_number 0.033%→5.519% 인데 case HIGH/MEDIUM/LOW 1,186/26,837/432 불변, priority_score 합 ±0.000 — `phase1_case_builder:1820` 에서 tier 대표값이 weak_evidence 보너스를 덮어쓰기 때문).
...
> - **한도 직하 밀집도·매출 특화 시계열**: 미구현 후보로 **보류**(우선순위 낮음). ※ "당기 내 거래 집중" 시계열은 2026-06-30 자기 큐로 채택했으나 **2026-07-15 실측 후 폐기**(아래 §시계열 당기내 집중).

---

## 시계열 당기내 집중 — 자기 큐 폐기 → D02 드릴다운 (2026-07-15)

2026-06-30 PLAN §6 은 "D01/D02 는 **전기 대비**만 보므로, 당기 안에서 특정 시점에 몰리는 것은 별개 차원"이라며 당기 내 baseline(robust-z) 자기 큐를 채택했다. **구현 후 실측에서 이 전제가 성립하지 않았다.**

**측정 3라운드** (정상 v53_r6 376,727행·계정 57개, 3개년 통합):

| 라운드                     | finding | 분기말(3·6·9·12) 비중 | 12월 |
| -------------------------- | ------: | --------------------: | ---: |
| 계정·연도 자기 리듬 median |     864 |             **70.0%** |  307 |
| 분기말/평월 레인 분리 후   |     595 |             **52.4%** |  185 |

1. **결산 캘린더 재발견**: 당기 내 자기 리듬과 비교하면 "이 계정은 분기말에 바쁘다"를 계정마다 재발견한다. 이는 감사인이 이미 아는 사실이며 PHASE1-1 **L3-04(기말 전표)** 가 행 단위로 이미 커버한다.
2. **레인 분리로도 미해결**: 회계기수 % 3 == 0(분기말)/평월 레인을 나눠 각 레인 안에서만 baseline 을 잡아도 12월이 최다로 남는다(185건). **연말 결산은 분기 결산보다 무겁다** — 감가상각·충당금·법인세가 12월에만 얹히므로 같은 "분기말"로 묶어 3·6·9월과 비교해도 12월은 매번 튄다. (상위 예: 계정 400370 / 2024-12 / 1,004건 vs 중앙값 357.5, z=34.9)
3. **원리적 한계**: **한 해에 연말은 한 번뿐**이라 "이번 연말이 연말치고도 이상한가"의 비교 대상이 당기 안에 존재하지 않는다. 판정하려면 전기 연말과 비교해야 하는데, 그것은 PLAN §6 이 못박은 "전기비교 아님 · 당기 내 baseline" 전제와 정면 충돌한다.
4. **D02 중복**: "작년 같은 달과 비교"로 가면 **D02 가 이미 하는 일**이다(JSD 로 전기/당기 월별 분포 비교, 계정 단위, 잡는 신호 = "전기엔 고르게 발생하던 계정이 당기엔 결산월·특정 분기·특정 프로젝트 월에 몰리는 경우"). 통계만 바꾼 중복이 된다. 데이터가 3개년이라 같은 달 비교의 기준점이 1~2개뿐이라 median/MAD 가 성립하지 않고 D01 식 단순 변동률이 될 수밖에 없어 더욱 그렇다.

**DataSynth 게이트 판정 — 아티팩트 아님**: PLAN §6 은 "정상 과탐 시 생성 아티팩트인지 점검, 맞으면 Rust 수정(검사기 드롭 아님)"을 걸었다. 점검 결과 분기·연말 결산 집중(결산조정·감가상각·충당금)은 **실무상 정상**이며 DataSynth 를 고쳐 없애면 데이터가 오히려 비현실적이 된다. 따라서 PLAN §6 실패조건("정상 데이터 집중 과탐이 Rust 미수정 상태로 잔존")에 해당하지 않는다.

**결정**: 자기 큐 6종 → **5종**(시계열 제외). 시계열은 **D02 드릴다운**으로 재정의 — D02 가 "이 계정 월분포가 바뀌었다"를 띄우면 그 계정의 어느 달이 작년 같은 달 대비 몇 배인지 상세로 보여준다. 새 룰이 아니라 D02 상세정보다. **미구현**(다음 작업 단위).

**코드 상태**: `src/detection/timeseries_concentration_rules.py`(계정별 × 일/주/월 축별 robust-z, 기말/평월 레인 분리)와 단위테스트 9건은 **보존**하되 `phase1_case_builder` 배선은 **제거**했다(`_build_ts_concentration_macro_findings` 삭제). 당기내 robust-z 계산 자산은 D02 드릴다운에서 재사용 가능하다.

**[PHASE2_TIMESERIES_ROLE_LOCK.md](../archive/completed/PHASE2_TIMESERIES_ROLE_LOCK.md) supersede 취소**: PLAN §6 은 "본 결정으로 supersede"를 예고했으나, 시계열 자기 큐가 폐기되어 **supersede 하지 않는다**. TS01/TS02 결산 lane 역할 고정(결정 9, 2026-05-25)은 **유효하게 존속**하며 코드도 `timeseries` 를 phase2_only family 블록에서 계속 실행한다.

> ⚠️ 해당 lock 문서는 현재 `docs/archive/completed/` 에 있으나 CLAUDE.md 는 `docs/spec/` 로 링크해 **링크가 깨져 있다**(활성 문서 5곳이 spec 경로 참조, archive 경로 참조 0곳). supersede 취소로 lock 이 유효 존속하므로 위치 재확정이 필요하다 — `dev/active/docs-reorg/MAPPING.md` §경계 미확정 항목.
...
## 단일 법인 입구 가드 (2026-07-15 결정)

PLAN §3 Phase 1 은 "3법인 통합 GL 을 회사별로 돌려 합친다"(base+variance per company_code)를 최우선 과제로 뒀다. **폐기하고 입구 가드로 대체한다.**

- **근거**: DataSynth 데이터셋 15개 중 **14개가 이미 단일 회사(C001)** 이고, 다회사는 PLAN 작성과 같은 날 생성된 `datasynth_contract_v3_rebuild_20260630_r1` 1개뿐이다. 운영 DB 도 회사·연도별로 격리돼 있다(`data/companies/{id}/engagements/{year}/audit.duckdb`). 단일 법인 확정([CONSTRAINTS.md](CONSTRAINTS.md)) 하에서 회사별 실행 후 병합하는 기계(`flagged_indices` 위치 재매핑 + 탐지기별 metadata 병합)는 투기성 복잡도다.
- **구현**: `src/pipeline.py::single_company_scope_warnings` — `company_code` distinct ≥ 2 이면 `[분석범위제한]` 경고 1건을 `PipelineResult.warnings` 로 표면화. distinct ≤ 1 또는 컬럼 부재면 no-op. **결측/공백 `company_code` 는 회사로 세지 않는다**(세면 단일회사 원장이 다회사로 오판된다 — 실측 contract_v3_rebuild 에 결측 6행 존재).
- **실측**: 다회사(3사·결측 6행) → 경고 1건 / 단일회사 v53_r6 → 경고 0건.

**회사 오염 룰 7종** (2026-07-15 전수 조사 — 통계성 룰 **17개 중 7개**, 코드 줄 확인):

| 오염 룰 | 근거 (파일:줄)                               | groupby 키                                                               |
| ------- | -------------------------------------------- | ------------------------------------------------------------------------ |
| L4-01   | `feature/amount_features.py:183,201-203`     | `gl_account` 단독, 최종 fallback = **전체 df mean/std**                  |
| L1-06   | `fraud_rules_access.py:1343-1347`            | `created_by` 단독 — 회사별 권한이 합집합돼 실재하지 않는 toxic 조합 생성 |
...
| L4-06   | `anomaly_rules_batch.py:198,221-223,256-261` | 전역 / `posting_date` 단독                                               |
| L3-09   | `anomaly_rules_simple.py:810`                | 전역 `posting_date.max()`                                                |

안전(이미 `company_code` 를 groupby 키에 포함): **L4-02 · L4-03 · L4-04 · L2-02 · D01 · D02**.
의도적 교차(오염 아님): L3-03(IC 꼬리표) · L3-12(`company_count` 자체가 룰 목적).

> 관련 미해결 — `fraud_rules_groupby.py:355 _flag_o2c_offset_duplicate_entries` 는 L2-03 헬퍼 중 **유일하게 `company_code` 를 groupby 키에 포함**하지만 호출처가 0건인 죽은 코드다(`b05_duplicate_entry` 는 오염된 exact/reference 헬퍼만 호출). 처리 미정.
...
PHASE1-2 **첫등장/희소 거래처 신호**는 **GL 전표를 거래처 코드(`trading_partner`)로 group by 한 집계**만으로 만들 수 있는 **약근사**를 검토 신호로 surface한다. 이는 마스터 데이터가 아니라 원장 집계이므로 GL-only 경계를 깨지 않는다(단일 법인 범위 내 — 거래처 단위라 형제 회사 불필요). **dual 운영** — 거래처 전수 목록은 자기 큐, 그 거래처의 전표는 배지(정상 신규 공급처도 첫 등장이라 전표 단독 큐는 노이즈).

| 약근사 신호(정명)                      | GL 파생 방식                                                              | 한계 → 해석                                                |
| -------------------------------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **원장 첫 등장(first-seen-in-ledger)** | 전기 미등장 + 당기 등장 교차                                              | 정상 신규 공급처도 원장 첫 등장 → **검토 신호**, 적발 아님 |
| **원장 희소(rare-in-ledger) 거래처**   | 당기 거래처별 txn count 하위 분위수(`partner_rare_quantile`) 이하         | 정상 일회성 거래와 경계 모호 → **검토 신호**, 적발 아님    |
| **휴면 재활성(dormant reactivation)**  | 전기 활동 有 + 직전 회계연도 결번 + gap ≥ `partner_dormant_inactive_days` | 정상 거래 재개와 경계 모호 → **검토 신호**, 적발 아님      |

> **구현(2026-07-15)**: `src/detection/partner_signals.py::compute_partner_signals` — 옛 PHASE2 relational family(R01/R05/R07)는 **삭제**됐고(커밋 5d16525) 신뢰·재사용하지 않으며 base 경로에서 거래처 단위로 신규 구현했다. 산출 2종 —
> - **자기 큐**: `.partner_summary` → `phase1_case_builder._build_partner_findings` → `metadata.partner_findings`(rule_id=`PARTNER`). 금액 게이트 없이 **전수**, 거래내용(`gl_account`·`document_type`)별 묶음, 고액/대량 정렬, **절단 없음**. `macro_findings` 와 합치지 않는다 — 단위가 계정/프로세스가 아니라 거래처이고, 정렬 기준이 합계금액(원)이라 `review_score`(0~1) 기반 macro 정렬에 섞으면 척도가 어긋난다. 접근자 `export/phase1_case_view.py::build_phase1_partner_finding_queue`.
> - **배지**: `.row_badges` 3컬럼(`is_first_seen_partner`·`is_rare_partner`·`is_dormant_partner`) → `badge_tags`. 점수 비병합.
>
> 설정: `partner_rare_quantile`(0.10) · `partner_dormant_inactive_days`(180) · `partner_signal_min_population`(50). 다년 데이터 필요 — 단일 연도 실행 시 first-seen/dormant 는 가드로 빈 결과 + warning.
>
> **실측(3개년 통합 376,727행)**: 원장 거래처 2,966개 → 첫등장 568 · 희소 596 · 휴면 22 → 신호 거래처(합집합) **625개**(19%)가 그대로 `partner_findings` 625건.
...
- 세 약근사 모두 **HIGH-6 커버리지로 주장하지 않는다**. "가공거래처 적발/탐지 완료"가 아니라 "가공거래처 검토 후보를 GL 범위 내에서 일부 surface"로만 표기한다.
