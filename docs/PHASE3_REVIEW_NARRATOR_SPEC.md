...
> 작성일: 2026-05-14
> 상태: v2 정의 확정. 구현 매핑은 [TASKS.md](TASKS.md) Phase 3 섹션 참조.
> 단일 출처(SoT): 본 문서. CLAUDE.md / README.md / TASKS.md / raw-plan/08-llm.md 는 본 문서로 링크한다.
...
- **1차 방어선 (strict 단계)**: candidate별 입력 ID set으로 동적으로 채워진 JSON Schema enum으로 LLM 응답을 strict reject한다. LLM은 입력에 없는 ID를 응답으로 만들 수 없다. 구현 상세: [PHASE3_REWORK_PLAN.md §4.2](PHASE3_REWORK_PLAN.md).
...
**Provider 결정 (2026-05-14)**: GPT-5.4 / GPT-5.4-mini 단일 provider 유지. Claude Sonnet 4.6, Gemini 2.5 Flash 등과의 multi-provider A/B는 수행하지 않는다. 근거 4가지(citation 정합성 차이 실효 없음 + 자산 완성도 + 성능 차이 미미 + 비용 가드 충분)는 [PHASE3_REWORK_PLAN.md §4.1](PHASE3_REWORK_PLAN.md)에 기록. ChatClient Protocol은 ISP 설계라 향후 운영 데이터로 재평가 가능.
...
신규 WU 슬롯은 [TASKS.md](TASKS.md) Phase 3 섹션에서 정의한다.
...
- 2026-05-14: 인용 계약을 1차(strict schema enum) + 2차(citation_validator) 2계층으로 명시. 모델 정책에 GPT-5.4 단일 provider 유지 결정 기록. 상세 근거는 PHASE3_REWORK_PLAN.md §4.1 / §4.2.
