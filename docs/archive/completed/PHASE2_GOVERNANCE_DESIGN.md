...
> **C안 3-surface 정합 (2026-06-14, SoT [PHASE1_TIER_EVIDENCE_BASIS.md §7](PHASE1_TIER_EVIDENCE_BASIS.md))**:
> - **surface 귀속**: graph·relational·시계열·duplicate·intercompany 등 구조 단위 탐지기(family)는 **PHASE1-2 family** 에 귀속한다(결정론·근거·명명 탐지). **PHASE2 단독 surface 는 VAE(비지도) 하나뿐**이다. 본 문서가 "PHASE2 family lane / 5-family" 라고 부르던 duplicate/relational/timing/intercompany 는 surface 분류상 PHASE1-2 family 이며, 본 거버넌스 문서가 다루는 것은 그 family 탐지기 산출물의 **학습/모델 저장/KPI 가드 인프라**(VAE 중심)다. surface 귀속(PHASE1-2)과 거버넌스 인프라(PHASE2 학습/가드)는 다른 층위다.
> - **비병합 불변식**: 3 surface(PHASE1-1 룰 / PHASE1-2 family / PHASE2 VAE)는 절대 비병합 — 독립 탭/뷰/큐로 표시하고 단일 점수로 합치지 않는다. 본 문서의 Noisy-OR / RRF 통합 queue 서술(결정 8)은 이미 과거 기록으로 폐기되었으며, 현재 공식 계약과 정합한다.
> - **PHASE3 제거**: 외부 LLM narrator / PHASE3 ordering layer 는 active product path 에서 제거됐다. 본 문서에 남은 "narrator" 언급은 과거 기록이며 selected-case 설명은 deterministic Local Evidence Brief 만 사용한다.
...
>
> **PHASE1 거버넌스 정합**: [docs/spec/CONSTRAINTS.md §"PHASE1 CI KPI 가드 정책 (3-Layer 구조)"](CONSTRAINTS.md) · [tests/phase1_rulebase/kpi_baseline.json](../../tests/phase1_rulebase/kpi_baseline.json) · [tests/phase1_rulebase/nightly_kpi_guard.py](../../tests/phase1_rulebase/nightly_kpi_guard.py)
...
[CONSTRAINTS.md §Phase2 기본 모델 제약](CONSTRAINTS.md) 및 [phase2-unsupervised-autoencoder-plan.md](../archive/completed/phase2-unsupervised-autoencoder/phase2-unsupervised-autoencoder-plan.md) 기준 PHASE2 MVP는 다음과 같다.
...
[NEW_TASKS.MD RC-3 완료 항목](../archive/completed/NEW_TASKS.MD) 인프라 사용. `CompanyContext.model_dir` 가 이미 제공.
...
2. **family별 독립 review lane 유지**: duplicate / relational / timing / intercompany family lane별 신호와 설명 feature를 노출한다. (C안 정합: duplicate/relational/timing/intercompany 는 surface 분류상 **PHASE1-2 family**, unsupervised(VAE)는 **PHASE2 단독 surface**. 세 surface 비병합 — lane/큐를 단일 점수로 합치지 않는다.)
...
| 정정 기록 | [`docs/spec/TROUBLESHOOT.md` TS-15](TROUBLESHOOT.md#ts-15) |
...
| PR 변경 절차 | [`docs/spec/DECISION.md` D044 fitting-risk check](DECISION.md) — "PHASE2 sub-detector tier 변경 여부" 명시. truth recall 사유 금지. |
...
| 파일 | 역할 |
|------|------|
| `docs/spec/TROUBLESHOOT.md` TS-15 | 결정 과정 (hierarchical RRF reject → Noisy-OR 채택) 정리 |
| `docs/guide/users/08_PHASE2_FAMILY_STRUCTURE.md` | 사용자 문서 — Noisy-OR 채택 반영 |
| `docs/guide/users/09_REVIEW_QUEUE_RRF_FUSION.md` | 사용자 문서 — RRF 적용 범위 한정 + Noisy-OR 식 명시 |
| `docs/archive/completed/phase2-family-ranking/` | plan/context/tasks 3 파일 |
...
1. `docs/spec/PHASE2_TIMESERIES_ROLE_LOCK.md` §변경 절차 통과
2. `docs/spec/DECISION.md` D044 PR 템플릿 "PHASE2 sub-detector tier 변경" fitting-risk check 통과
...
| `docs/spec/PHASE2_TIMESERIES_ROLE_LOCK.md` | 락 문서 (변경 절차 포함) |
...
| 파일 | 상태 | 다음 단계 |
|------|------|---------|
| `docs/spec/PHASE2_GOVERNANCE_DESIGN.md` (본 문서) | 설계 완료 | review |
