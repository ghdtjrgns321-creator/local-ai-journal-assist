...
> **[ARCHIVED 2026-06-16]** docs/spec/ → docs/archive/completed/ 로 이관됨. 본 문서의 `priority_score >= 0.90/0.75` 2단계 분포 튜닝(Stage 0~2-B)은 구 가중합·floor 점수체계의 기록이며, 해당 체계는 tier(순서형 HIGH/MEDIUM/LOW/CONTEXT)로 폐기·대체됐다(SoT `docs/spec/PHASE1_TIER_EVIDENCE_BASIS.md`, 2026-06-14). 0.75 클러스터링·priority_floors dead code 문제와 그 귀결은 `docs/spec/TROUBLESHOOT.md` TS-15에 정리. 본 문서는 역사 기록으로만 보존한다.
...
- DataSynth truth recall/precision 은 **기록만**. 튜닝 기준으로 사용 금지 (`feedback_phase1_truth_recall_guard` memory).
