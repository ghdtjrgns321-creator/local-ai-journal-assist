...
|:----|:-------------------------|:-------------------------|:----------------------------------|
| L2-02 | auxiliary_account_number | "동일 벤더·금액·기간 내 2건+" | ❌ 여전히 NULL — Rust 수정 대기    |
| L3-06 | posting_date 시간정보     | "22시~06시"              | ✅ v1.2.0에서 datetime 포함        |

L2-02는 DataSynth Rust 코드에서 거래처 ID 생성 로직 추가가 필요하다.
L3-06은 v1.2.0에서 posting_date에 시분초가 포함되어 심야 탐지가 가능해졌다.

### 2. settings.py 임계값이 DataSynth 데이터 범위와 동떨어짐 → v1.2.0에서 해결

| 룰      | v1.2.0 이전                          | v1.2.0 현재                                                  |
|:--------|:-------------------------------------|:-------------------------------------------------------------|
| L2-01/L3-02 | 단일 승인한도 5천만원, 최대 금액 770만 | 6단계 `[10M, 100M, 1B, 5B, 10B, 50B]`, lognormal mu=14.0    |
| L1-06     | 41명 전원 5개 프로세스 → 전원 SOD 해당 | 150명, 6개 프로세스, SOD 위반률 1%로 현실적 분포              |
...
| 룰  | v1.2.0 이전                          | v1.2.0 현재                                                  |
|:----|:-------------------------------------|:-------------------------------------------------------------|
| L3-03 | `intercompany_identifiers: []`       | `['1150', '2050', '4500', '2700']` (IC Receivable/Payable/Revenue/Accrued) |
...
| 룰  | 상황                                                     | v1.2.0 상태         |
|:----|:---------------------------------------------------------|:--------------------|
| L1-02 | DataSynth가 필수필드 100% 생성 → MissingField 주입 안 함  | ⚠️ MCAR 2% 주입 추가, 재검증 필요 |
| L4-02 | DataSynth 금액이 자연스러운 Benford 분포 → 위반 없음      | ⚠️ 미해결 (위반 주입 미구현) |

L1-02는 v1.2.0에서 `data_quality.missing_values.rate: 0.02` (MCAR) 설정이 추가되었으나,
protected_fields(document_id, company_code, posting_date)는 제외되므로 탐지 결과 재검증이 필요하다.
L4-02은 Benford 위반 금액을 의도적으로 주입하는 로직이 아직 미구현이다.
...
| L1-06 SOD 과탐 | 41명 소규모 시뮬레이션 | 150명 확대, SOD 위반률 1% | `datasynth.yaml` |
| L3-03 관계사 미식별 | `intercompany_identifiers: []` | IC GL prefix 4개 등록 | `audit_rules.yaml` |
| L3-06 심야 미탐지 | posting_date 시간정보 없음 | datetime (시분초 포함) 전환 | `schema.yaml`, DataSynth |
...
| 항목 | 원인 | 필요 조치 | 대상 |
|:-----|:-----|:---------|:-----|
| L2-02 거래처 중복 | auxiliary_account_number NULL | DataSynth Rust에서 거래처 ID 생성 | `je_generator.rs` |
| L1-02 필수필드 누락 | MCAR 2% 주입 추가됨, 탐지 결과 재검증 필요 | E2E 테스트 재실행 | `test_e2e_detection.py` |
| L4-02 Benford 위반 | 위반 금액 미주입 | fraud_type_distribution에 BenfordViolation 추가 | `datasynth.yaml`, Rust |
...
- [pre-plan/05-detection.md](raw-plan/05-detection.md) — detection 구현 가이드
