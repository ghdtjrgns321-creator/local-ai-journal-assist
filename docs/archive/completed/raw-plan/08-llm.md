...
> **Historical/deprecated only (2026-05-26)**: This raw plan is retained as historical design context. PHASE3 LLM Narrator, Text-to-SQL, Vanna, LLM rule feedback, and external LLM/API based memo generation are removed from the active product path. Active explanation must follow [LOCAL_FIRST_EVIDENCE_POLICY.md](../../../spec/LOCAL_FIRST_EVIDENCE_POLICY.md).

> **🔄 Phase 3 v2 Rescope 안내 (2026-05-14) ✅ 구현 완료 (Sprint A~G, 2026-05-15)**: Phase 3 단일 목표는 **Review Queue Narrator**다. 단일 출처: [PHASE3_REVIEW_NARRATOR_SPEC.md](../../abandoned/PHASE3_REVIEW_NARRATOR_SPEC.md), 완료 리포트 [completed/phase3_review_narrator_completion.md](../../abandoned/phase3_review_narrator_completion.md).
> 본 문서 내 Text-to-SQL(WU-20), Chat UI(WU-26), 룰 피드백 루프(WU-30) 절은 **구현물 보존, 신규 작업 없음**으로 분류한다. 신규 작업은 [NEW_TASKS.MD WU-31](../NEW_TASKS.MD) 참조.
...
user_persona 값:
  ...
  manager (3.6%) — 승인, 대규모 조정 (R2R 77.6% 집중)
""")

# 부정 유형 (fraud_type, 15종, 2026-04-14 실측)
vn.train(documentation="""
fraud_type 값 (총 6,262건, 1.96%, T5-31 자기승인 역방향 라벨 반영):
  SelfApproval(5,932), RevenueManipulation(86), UnauthorizedAccess(46),
  FictitiousTransaction(45), DuplicatePayment(36), ExpenseCapitalization(36),
  SplitTransaction(29), SuspenseAccountAbuse(18), TimingAnomaly(12),
  JustBelowThreshold(10), RoundDollarManipulation(7), ExceededApprovalLimit(7),
  ImproperCapitalization(4), FictitiousEntry(1), SegregationOfDutiesViolation(1)
""")

# 이상징후 유형 (anomaly_type, 상위 10종, 2026-04-14 실측)
vn.train(documentation="""
anomaly_type 값 (총 8,294건 entry + 8,337건 labels.csv, 2.60%):
  NewCounterparty(413), UnusualAccountPair(313), MissingRelationship(271),
  DormantAccountActivity(247), UnmatchedIntercompany(215),
  CircularTransaction(140), CentralityAnomaly(125),
  TransferPricingAnomaly(120), UnusualTiming(76), CircularIntercompany(73)
""")

# SoD 충돌 유형 (sod_conflict_type, 2026-04-14 실측)
vn.train(documentation="""
sod_conflict_type 값 (총 10,595건, 3.32%):
  3.32%는 sod_violation_rate(0.01) + anomalous_assignment_rate(0.015) +
  compatible_extension_rate(0.25) 누적 결과.
  주요 충돌: preparer_approver (자기승인), system_access_conflict (권한 충돌),
  requester_approver, payment_releaser, journal_entry_poster,
  master_data_maintainer, reconciler_poster (7종)
""")

# GL 계정 체계
vn.train(documentation="""
gl_account 체계 (414개 사용/431개 정의 GL 계정, K-IFRS):
...
전표 규모: 319,204건(document_id), 1,107,720건(라인아이템, 2026-04-14 실측)
""")
```
...
class InsightGenerator:
    ...
    def generate_summary_insight(self, pipeline_result) -> str:
        ...
        예: '319,204건 전표 중 6,262건(1.96%)이 부정으로, 8,294건(2.60%)이
            이상징후로 탐지되었습니다. 부정 유형별: SelfApproval 5,932건(94.7%),
            RevenueManipulation 86건, UnauthorizedAccess 46건이 상위 3종입니다.
            SoD 위반 10,595건(3.32%) 중 preparer_approver 충돌이 가장 빈번합니다.'
        """
    ...
    def generate_entry_insight(self, entry: dict) -> str:
        ...
        예: '이 전표(fraud_type: DuplicatePayment)는 동일 reference
            ...
            created_by JA-015(junior_accountant)가 주말에 입력했습니다(L3-05+L2-02).'
        """

    def generate_benford_insight(self, benford_result: dict) -> str:
        """Benford 분석 결과 해석.

        예: '1,107,720 라인아이템의 first_digit 분포는 MAD 0.00146으로
            Benford 우수 적합 판정(Close Conformity). digit 1의 관측 빈도 30.7%는
            기대값 30.1%에 근접하며 전체적으로 실제 회계 분포를 잘 모사합니다.'
        """
```
...
- `has_other_risk_signals()`: 고액(L2-01/L1-04), 심야(L3-06), 수기(L3-02) 등 다른 룰에서 플래그된 건
- **예상 LLM 호출 건수**: 전체의 ~2~5% (구조화 패턴 미매칭 + 위험 플래그 보유)

### 구현 위치

`insight_generator.py`의 `generate_entry_insight()`에 L3-08 LLM 분류 로직을 통합하거나,
...
출력: 자연어 위험 사유서 (1~3문장)
  예: "이 전표(fraud_type: SplitTransaction)는 동일 거래처 V-000042에 대해
      동일 일자에 4,900만원 × 3건으로 분할 기표되었습니다(L2-01).
...
구현 방향:
  ...
  3. 룰 ID(L2-01, L3-06 등)를 사유서에 인용 → 감사조서 추적성 확보

호출 조건:
  - anomaly_score > 0 AND risk_level IN ('HIGH', 'CRITICAL')인 전표만 대상
  - 예상 대상: 부정 6,262건 + 이상징후 8,294건 중 HIGH 이상 = ~5,000~8,000건
...
- **근거**: 감사기준서 240호 §32(c)
- **입력**: L4-03(이상고액) + L4-01(매출이상) 탐지 결과에서 플래그된 전표의 적요+계정+금액
