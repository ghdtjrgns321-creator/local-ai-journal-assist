...
> **Historical/deprecated LLM context (2026-05-26)**: Any Phase 3, LLM preprocessing advisor, or external API references in this raw plan are historical only and not active product capability. Active evidence policy: [LOCAL_FIRST_EVIDENCE_POLICY.md](../../../spec/LOCAL_FIRST_EVIDENCE_POLICY.md).

> **PHASE1 역할 원칙**: PHASE1은 `fraud`를 확정하거나 정답 라벨을 맞히는 단계가 아니다. PHASE1의 목적은 전수 모집단에서 규칙 위반, 정책 위반, 이상 징후, 분석적 검토 신호를 넓게 올려 **감사인이 봐야 할 항목과 우선순위**를 만드는 것이다. DataSynth의 `is_fraud`/`is_anomaly`와 precision/recall은 개발 검증 보조 지표이며, 운영 해석은 예외 처리 대상, 감사인 리뷰 대상, 고위험 후보를 구분하는 review queue 기준으로 한다.

> **🔄 Phase 3 v2 Rescope (2026-05-14) ✅ 구현 완료 (Sprint A~G, 2026-05-15)**: Phase 3 단일 목표는 [Review Queue Narrator](../../abandoned/PHASE3_REVIEW_NARRATOR_SPEC.md), 완료 리포트 [completed/phase3_review_narrator_completion.md](../../abandoned/phase3_review_narrator_completion.md). 본 문서 내 LLM 전처리 제안 (Ollama → 전처리 추천) 절은 Phase 3 v2 **비범위** (WU-29 흡수 → WU-18 구현 보존). VAE+LLM 동시 VRAM 위험·`keep_alive=0` 메모리 관리 등은 historical v1 기록. [DECISION.md §D041](../../../spec/DECISION.md) 참조.
...
**테스트**: [52 tests passed](../../../../tests/modules/test_eda/test-results/eda-profiling.md) — 7개 테스트 파일
...
**테스트**: [62 tests passed](../../../../tests/modules/test_preprocessing/test-results/preprocessing-test-summary.md) — 8개 테스트 파일, 11개 구현 모듈
...
- **profiler.py:** 수치/범주/시간형 컬럼별 프로파일링 + 대용량 샘플링 동작 검증 ✅ [52 passed](../../../../tests/modules/test_eda/test-results/eda-profiling.md)
- **report.py:** EDAProfile → 대시보드 요약 변환 + JSON 직렬화 ✅ (eda-profiling 리포트에 포함)
- **sklearn Pipeline:** Pipeline별 fit/predict + cross_val_score 동작 검증 ✅ [62 passed](../../../../tests/modules/test_preprocessing/test-results/preprocessing-test-summary.md)
- **LLM 전처리 제안:** 프롬프트 → JSON 파싱 + 유효한 전처리 옵션 검증 ✅ [llm-preprocessing-advisor](../../../../tests/modules/test_llm/test-results/llm-preprocessing-advisor.md)
...
> **UX 단계 정의**: [ux-flow.md](../../../guide/ux-flow.md) 참조
...
[ux-flow.md 3가지 UX 디자인 원칙](../../../guide/ux-flow.md#3가지-ux-디자인-원칙)을 본 단계에도 적용:
