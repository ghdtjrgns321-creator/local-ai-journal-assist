...
> **Historical/deprecated LLM context (2026-05-26)**: Any Phase 3, Review Queue Narrator, Chat, Text-to-SQL, or external LLM references in this raw plan are historical only and not active product capability. Active evidence policy: [LOCAL_FIRST_EVIDENCE_POLICY.md](../../../spec/LOCAL_FIRST_EVIDENCE_POLICY.md).

> **PHASE1 역할 원칙**: PHASE1은 `fraud`를 확정하거나 정답 라벨을 맞히는 단계가 아니다. PHASE1의 목적은 전수 모집단에서 규칙 위반, 정책 위반, 이상 징후, 분석적 검토 신호를 넓게 올려 **감사인이 봐야 할 항목과 우선순위**를 만드는 것이다. DataSynth의 `is_fraud`/`is_anomaly`와 precision/recall은 개발 검증 보조 지표이며, 운영 해석은 예외 처리 대상, 감사인 리뷰 대상, 고위험 후보를 구분하는 review queue 기준으로 한다.

> **🔄 Phase 3 v2 Rescope (2026-05-14) ✅ 구현 완료 (Sprint A~G, 2026-05-15)**: Phase 3 단일 목표는 [Review Queue Narrator](../../abandoned/PHASE3_REVIEW_NARRATOR_SPEC.md), 완료 리포트 [completed/phase3_review_narrator_completion.md](../../abandoned/phase3_review_narrator_completion.md). 본 문서 내 `tab_chat.py` (Text-to-SQL) / `tab_export.py` (Excel/PDF 감사조서) 절은 historical v1 기록 (구현 보존, Phase 3 v2 완료 기준 제외). Phase 3 v2는 RC-4 대시보드 review queue 탭에 Narrator 출력을 표시하는 방향으로 통합한다. [DECISION.md §D041](../../../spec/DECISION.md) 참조.
...
> 참조: `generation_principles.md`, `PREVIEW.md` (현재 체크아웃에 없음)
...
| 라인아이템(line) | 1,107,720건                                      |
| 컬럼             | 39개 (header 24 + line 15)                       |
| 회사코드         | C001(본사, 서울), C002(울산공장), C003(천안공장)  |
| 통화             | KRW (단일)                                       |
| 회계연도         | 2022 (1~12월)                                    |
| GL 계정          | 414개 사용 / 431개 정의 (K-IFRS / SAP 한국 표준)  |
...
| 차원               | 값                                                                | 대시보드 활용           |
|:-------------------|:------------------------------------------------------------------|:------------------------|
| `business_process` | O2C(26.5%), R2R(25.3%), P2P(24.2%), TRE(9.4%), H2R(8.6%), A2R(6.0%) | 필터, 히트맵, 프로세스별 뷰 |
| `company_code`     | C001(본사), C002(울산), C003(천안)                                | 필터, 법인별 KPI 비교   |
| `user_persona`     | senior(28.7%), automated_system(24.7%), junior(24.4%), manager(10.2%), controller(10.1%) + typo 변형 | 필터, 페르소나×위험 교차표 |
| `source`           | manual(70.1%), automated(18.8%), recurring(7.4%), adjustment(3.7%) | 필터, 수기 전표 하이라이트 |
...
| 지표              | 건수    | 비율   | 대시보드 활용                   |
|:------------------|:--------|:-------|:--------------------------------|
| 이상징후 전표     | 8,294   | 2.60%  | Summary KPI, Explorer 필터      |
| 부정(is_fraud) 전표 | 6,262 | 1.96% | Summary KPI, 부정유형 Treemap   |
| SoD 위반          | 10,595  | 3.32%  | SoD 분석 뷰                     |
| 차대변 균형 불일치 | 25     | 0.008% | L1-01 룰 위반 카운트 (전부 라벨됨) |

### 부정 유형 Top 5 (15종 중, 2026-04-14 실측)

| 유형                   | 건수 | 대응 탐지 룰         |
|:-----------------------|:-----|:---------------------|
| SelfApproval           | 5,932 | L1-05 자기승인 (T5-31 역방향 라벨) |
| RevenueManipulation    | 86   | L4-01 수익 조작        |
| UnauthorizedAccess     | 46   | L1-07 승인 우회        |
| FictitiousTransaction  | 45   | L1-04 가공 거래        |
| DuplicatePayment       | 36   | L2-02 중복 지급        |

### 시간 패턴 (탐지 연계, 2026-04-14 실측)

```
시간대              비율      탐지 연계
late_night(0~6)     1.5%     L3-06 심야 전기
morning_spike(8~11) 29.7%    정상 업무 피크
eod_rush(16~18)     21.4%    마감 전 러시
overtime(18~22)     13.1%    야근 (주의)
midnight(22~24)      0.7%    L3-06 심야 (고위험)

주말 전표            2.8%    L3-05 주말 전기 (30,756건)
12월(연말 결산)     39,514건  L3-04 기말 대규모 (×1.49 평균)
```
...
└── components/
    ...
    ├── explorer_detail.py     # 행 상세 패널 (룰 점수 차트) ✅
    ├── explorer_whitelist.py  # HITL 예외 처리 (whitelist CRUD) ✅
...
def hourly_heatmap(df: DataFrame) -> go.Figure:
    ...
    심야(0~6)/주말 영역에 점선 박스 오버레이 → L3-05/L3-06 탐지 연계."""
...
def layer_score_radar(scores: dict) -> go.Figure:
    """선택 전표의 L1/B/C/Benford 점수 방사형 차트.
...
def render(result: PipelineResult) -> dict:
    ...
    - 위반 룰 선택 (multiselect): L1-01~L1-03, L4-01~L2-04, L3-04~L3-09 (24개 룰)
...
### `tab_summary.py` — Tab 1: Executive Summary ✅ 구현 완료

구현 파일: `tab_summary.py`, `_kpi.py`, `components/charts/rule_charts.py`
...
def render(result: PipelineResult):
    ...
    2. 총 라인아이템수  COUNT(*)
    3. 이상 전표수      risk_level != 'Normal' (document_id 기준)
    4. 이상 비율(%)     이상 전표수 / 총 전표수
    5. 이상 금액 합계   전표(document_id) 단위 groupby 후 합산 (라인 중복 방지)
    6. 부정 의심 건수   flagged_rules에 B 룰 포함 전표 수 (document_id nunique)

    ── 데이터 품질 (KPI 바로 아래 배치) ──
    데이터 완전성(%) | 처리 시간 | 파이프라인 경고 expander

    ── Row 1: 전체 현황 (st.columns([2, 1])) ──
    좌: rule_violation_bar(df) — 24개 룰 위반 건수 가로 바
        flagged_rules 파싱 → explode → value_counts
        색상: L1=파랑, L2=빨강, L3/L4=노랑
    우: risk_donut(df) — 위험 등급 도넛 차트

    ── Row 2: 시계열 + 히트맵 (st.columns(2)) ──
    좌: monthly_trend(df) — 월별 이상 전표 추이
    우: risk_heatmap(df) — fiscal_period × business_process 히트맵

    ── Row 3: 차원별 분석 (st.tabs 3개) ──
    탭 "프로세스별" → process_distribution_bar(df)
    탭 "페르소나별" → persona_risk_matrix(df)
    탭 "법인별"     → company_comparison(df)
    (1:1:1 columns 대신 st.tabs로 변경 — 한글 라벨 겹침/잘림 방지)

    성능: @st.cache_data로 KPI 재계산 최소화
    """
```
...
def render(result: PipelineResult):
    ...
    ✅ 해결: fiscal_period가 batch_ledger SELECT에 추가됨 (WU1에서 수정).
          line_number, line_text 등 나머지는 Explorer 탭(WU4)에서 필요 시 추가.
    ...
    page_size=100 (1,107,720 라인 직접 로드 방지)

    ── 행 선택 시 상세 패널 ──
    document_rule_detail 쿼리 → track_name, rule_code, score 표시
    룰별 점수 breakdown 가로 바 차트 (L1/L2/L3/L4 체계 색상 구분)
...
- 시간대별 히트맵에 심야(0~6)/주말 영역 점선 박스 → L3-05/L3-06 탐지 영역 표시
...
| 요소               | 내용                                                              |
|:-------------------|:-----------------------------------------------------------------|
| KPI 카드           | SoD 위반 건수(10,595), 위반 비율(3.32%), 위반 사용자 수           |
| 충돌 유형 바 차트  | sod_conflict_type × 건수 (preparer_approver 87%, system_access_conflict 8% 등) |
...
|:---------|:-------------------|:------------------------------|:--------------------------------|
| P2P      | KR, KZ, WE         | L2-02(중복지급), L2-01/L1-04(승인한도), L3-02(수기) | Sankey (KR→WE→KZ), 벤더 집중도 |
| O2C      | DR, DZ, WL         | L4-01(매출이상), L3-04(기말대규모), L4-03(고액)   | 매출 월별 추이, 고객 집중도    |
| R2R      | SA, IC              | L3-08(위험적요), L4-04(비정상 계정조합), L1-01     | 결산 수정 히트맵, 계정 네트워크 |
| H2R      | HR                  | L1-06(SoD), L3-05/L3-06(주말/심야)                | 급여 변동 추이                 |
| TRE      | KZ, SA              | L1-05(자기승인), L1-07(승인생략)                 | 자금 이동 흐름                 |
| A2R      | AA                  | L4-03(이상고액), Phase 2 ML(ImproperCapitalization) | 자산 취득/처분 타임라인   |
...
## 구현 순서
1. ✅ `components/data_uploader.py` — 파일 업로드 + `run_from_dataframe()` (메모리 직접 읽기, tempfile 미사용)
2. ✅ `components/charts/` — Plotly 차트 래퍼 17종 (8모듈 서브패키지 분리, 계층적 샘플링 적용, EDA 차트 4종 포함)
3. ✅ `components/filters.py` — 사이드바 공통 필터 (기본 4 + 차원 6 + 개발 2, 벡터화 정규식 매칭)
4. ✅ `components/preset_selector.py` — 환경 프리셋 드롭다운 (UX 4단계 C) + `config/presets/*.yaml`
5. ✅ `components/threshold_sidebar.py` — 실시간 임계값 튜닝 슬라이더 23개 (UX 4단계 A)
5a. ✅ `components/rule_panel.py` — 룰 컨트롤 패널: 가중치/임계값/27룰 토글
5b. ✅ `components/_redetect.py` — 재탐지 헬퍼 + "적용" 버튼 (features 건너뛰기)
6. ✅ `tab_summary.py` + `_kpi.py` + `charts/rule_charts.py` — KPI 6개 + 차트 7종 + @st.cache_data
...
> 상세: [ux-flow.md → 3가지 UX 디자인 원칙](../../../guide/ux-flow.md#3가지-ux-디자인-원칙)
...
WU7(2026-03-28)에서 인제스트 오케스트레이터 + 매핑 리뷰 UI 구현으로 다수 해결.

| 과제                            | 현상                                               | 해결 방향                                    | 발견 위치                                                    | 상태 |
|:--------------------------------|:---------------------------------------------------|:---------------------------------------------|:-------------------------------------------------------------|:-----|
| Parquet 헤더 탐지 스킵          | 불필요한 헤더 탐지 시도                            | 오케스트레이터 `source_format` 분기          | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ✅ WU7 30c |
| 멀티시트 UI 선택                | active_sheet가 데이터 양 무관                      | 시트 목록 + 행 수 → 사용자 선택 (UI-2)      | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ✅ WU7 30d |
| Fuzzy 추천 부정확               | monat→debit_amount 등 오추천                       | ReviewItem UI 확인/변경 + Fuzzy 슬라이더 (UI-3) | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ✅ WU7 30d |
| 매핑 프로파일 학습              | 반복 업로드 시 매번 매핑                           | 프로파일 우선 적용                           | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ⬜ 후속 |
| ReviewItem UI 노출              | 판단 근거 데이터 준비됨, UI 미구현                 | 3-tier 시각 피드백(초록/노랑/빨강)           | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ✅ WU7 30d |
| 차단 vs unmapped 미구분         | 타입 차단 사유 미표시                              | ReviewItem.reason 세분화                     | [02-ingest §미해결](02-ingest.md#미해결-이슈-발견--해결-교차-참조) | ✅ WU7 30d |
| ~~is_after_hours 날짜 경계~~    | ✅ **버그 아님** — `dt.hour` 기반 자정 걸침 정확 처리 확인 | —                                            | [03-feature §G](03-feature.md#g-미해결-이슈-발견--해결-교차-참조) | ✅ 해당없음 |
| 업종별 영업일 차이              | 단일 bdate_range 기준 (공휴일 미반영)              | 업종 선택 UI + holidays 패키지 연동          | [04-validation §5](../../../../tests/modules/test_validation/test-results/validation-all-results.md) | ⬜ 후속 |
| fiscal_period_mismatch NaN(SAP) | sap-merged에서 전체 NaN                            | 매핑 리뷰 UI에서 원인 규명                   | [e2e-sap-merged.md §3](../../../../tests/test_feature/test-results/e2e-sap-merged.md) | ✅ WU7 30d |
| sap-merged debit/credit 미매핑  | amount 카테고리 전체 스킵 (5개 피처 미생성)        | 수동 매핑 조정 + 중복 금액 퀵픽스 (UI-4)    | [e2e-sap-merged.md §3](../../../../tests/test_feature/test-results/e2e-sap-merged.md) | ✅ WU7 30d |
| schreyer-fraud gl_account 오매핑 | 캐스팅 후 결측률 100%                             | 타입 호환성 재확인                           | [ingest-validation-datasets.md](../../../../tests/test_ingest/test-results/ingest-validation-datasets.md) | ⬜ 후속 |
| datetime `.values`→`.array`      | 타임존 정보 손실 (`.values`가 ndarray 반환)       | `.array` 전환으로 타임존 보존               | [eda-profiling.md §코드리뷰](../../../../tests/modules/test_eda/test-results/eda-profiling.md)                    | ⬜ 후속 |
| `top_values` 반환 타입 계약      | tuple 반환이지만 대시보드 JSON 직렬화 시 list 기대 | 인터페이스 문서 + 타입 통일                 | [eda-profiling.md §코드리뷰](../../../../tests/modules/test_eda/test-results/eda-profiling.md)                    | ⬜ 후속 |
...
## 구현 시 주의사항
- **session_state:** ✅ `audit_` 접두사 키 6개 확정 (`_state.py`). `init_state()`로 방어적 초기화. FilterState TypedDict로 구조 문서화.
...
- **대용량 대응:** 1,107,720 라인아이템을 AgGrid에 직접 로드하지 않음. batch_ledger 쿼리가 anomaly_score DESC 정렬이므로 상위 N건만 초기 표시. AgGrid pagination(page_size=100) 적용.
