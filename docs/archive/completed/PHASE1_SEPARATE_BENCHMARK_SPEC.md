...
> Current PHASE1 scoring note (2026-04-27, macro 이관 갱신 2026-06-15): 별도 benchmark 대상 룰 중 `L4-02`(Benford)는 2026-06-15부로 PHASE1-2 macro family로 이관됐다. PHASE1-1 transaction queue에서 `macro_only`로 중화되어 row-level `normalized_score`는 case priority에 직접 기여하지 않으며(점수 기여 0), 평가는 PHASE1-2 macro-finding(Benford population benchmark) surface에서 수행한다. `L4-03`, `L4-04`, `L4-05`는 transaction queue에 남지만 `rule_scoring.py`의 `evidence_strength`와 `scoring_role`을 거친 `normalized_score`만 evidence type score에 합산된다.
