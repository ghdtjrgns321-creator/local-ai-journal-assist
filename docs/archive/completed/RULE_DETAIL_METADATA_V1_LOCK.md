...
- L1~L4 canonical transaction/detail rule count is exactly **29** (2026-06-20: was 31→30 with `L3-01` MisclassifiedAccount 폐기 → L4-04 희소쌍이 역할 대체, then 30→29 with `L3-08` MissingOrCorruptedDescription 폐기 — 적요 의미정합성은 결정론 룰 범위 밖이라 폐기. 2026-06-15: was 32; `L4-02` moved to PHASE1-2 family).
- Legacy "32/33 rules" means `29 canonical L1~L4 transaction rows + L4-02/Benford macro (now PHASE1-2 family)`; it never means 33 canonical transaction rules.
...
The v1 canonical L1~L4 transaction/detail rule count is fixed at **29** (2026-06-20: 31→30 `L3-01` 폐기, then 30→29 `L3-08` 폐기; 2026-06-15: was 32).

The 29 are all `presenter_surface=transaction_detail` 행/review row cards (review-only `L3-12` 포함). `L4-02`(Benford macro)는 2026-06-15 PHASE1-2 family로 이관되어 더 이상 count에 포함되지 않는다. `L3-01`(MisclassifiedAccount)과 `L3-08`(MissingOrCorruptedDescription)은 2026-06-20 폐기되어 count에서 제외된다.

Any external doc may state the total `29`, but must not claim a different split such as `29 + 1 macro`, `30`, `31`, `32`, `33 rules`, or count `Benford`/`L4-02`/`L3-01`/`L3-08` in the canonical L1~L4 transaction count.

Included in the 29:

- Canonical `L1-01` through `L4-06` 행 rows as defined by the L1~L4 rule set, **except `L4-02`, `L3-01`, and `L3-08`(폐기 2026-06-20)**.
...
- `L3-08` (MissingOrCorruptedDescription 폐기 → 적요 의미정합성은 결정론 룰 범위 밖, 2026-06-20)
...
- Any legacy dashboard or document text saying "32/33 rules" must be interpreted as `29 canonical + L4-02/Benford macro(PHASE1-2)` (L3-01·L3-08 폐기 반영).
...
- A canonical transaction count other than 29.
...
- Legacy "33 rules" copy is encountered and normalized to `29 canonical + L4-02/Benford macro(PHASE1-2)`.
...
| `include_in_l1_l4_transaction_count` | Whether this entry contributes to the canonical 29 count.                                                                                                  |
...
- `L3-05`, `L3-06`, `L3-10`, `L3-12`, `L4-05`, `L4-06` cannot generate standalone violation copy.
...
- Legacy "33 rules" display paths normalize to `29 canonical + L4-02/Benford macro(PHASE1-2)`.
...
본 절은 IC01 의 evidence level sidecar 정책을 별도 lock 으로 명시한다. canonical 29 transaction/detail rule count 정책 (§Rule Count Policy) 과 `RULE_DETAIL_METADATA_REGISTRY` 본문은 변경하지 않는다.
...
신규 sidecar column 2 종 (canonical 29 count 외부, `intercompany_sidecar` surface 의 일부):
