...
이 문서는 `DETECTION_RULES.md`의 룰 정의가 실제 설정·코드·UX 조정면과 어떻게 연결되는지 정리한다. **현 구현(config·code) 기준 파라미터 레퍼런스이며 tier/점수 설계 SoT가 아니다** — 설계 SoT는 [HIGH_COMBO_GROUNDING.md](HIGH_COMBO_GROUNDING.md)·[UNIT_MEASUREMENT_POLICY.md](UNIT_MEASUREMENT_POLICY.md)·[PHASE1_TIER_SCORING_SPEC.md](PHASE1_TIER_SCORING_SPEC.md)다.

현재 기준:

- PHASE1-1 canonical 전표 룰은 **29개**다(`L1~L4` 행 룰, `L4-02`·`L3-01`·`L3-08` 제외·`L3-12` review-only 포함; 카운트 SoT [RULE_DETAIL_METADATA_V1_LOCK.md](RULE_DETAIL_METADATA_V1_LOCK.md)). `Benford(L4-02)`·`D01/D02`·`L4-05`는 PHASE1-2 macro/family로 분리됐다(실행은 되지만 PHASE1-1 점수 기여 0, 별도 surface).
...
> **현황(코드 기준)**: 아래 `priority_weights`·`priority_band`·`priority_floors`는 `config/phase1_case.yaml`에 **deprecated fallback**으로만 잔존한다(파일 주석 명시: "retained for old artifacts/views while topic_scoring drives PHASE1 v1 case ranking"). 실제 PHASE1 큐 랭킹은 순서형 tier(`topic_scoring.compute_topic_tiers`)가 구동하며, tier 발화·정렬 SoT는 [HIGH_COMBO_GROUNDING.md](HIGH_COMBO_GROUNDING.md)·[PHASE1_TIER_SCORING_SPEC.md](PHASE1_TIER_SCORING_SPEC.md)다. 아래 표는 구 아티팩트/뷰 호환 참조용이며, 순서형 tier 마이그레이션 완료 시 제거 대상이다.
...
| priority floor  | `phase1_case.priority_floors`                                                                                                                                  | config `priority_floors` 목록의 룰별 최소 우선순위 (deprecated fallback)                                       |
