...
`docs/spec/DECISION.md:514` D055 는 `ic_unmatched_reference=True` 를 IC01 high-confidence unmatched IC evidence 로 수용한다고 명시. 본 plan 의 P0 (sidecar 의존 제거) 와 P1 (IC01 재정의) 은 D055 를 정면으로 뒤집는다. 따라서 P3-6 에서 명시적 supersede 결정문 (`D0xx`) 을 신규 추가하고, D055 본문 상단에 "Superseded by D0xx (YYYY-MM-DD)" 라인을 부착한다.
...
- 결정: `docs/spec/DECISION.md` 에 D0xx 신규 결정문 추가. D055 본문 상단에 "Superseded by D0xx (YYYY-MM-DD)" 라인 부착. D055 본문 자체는 그대로 둠.
- 합의: 사용자 보정 확인.
...
- D065 정합: 본 결정으로 D065 결정문의 "정책" 절에 "Sidecar 저장 위치" / "Review-only 신호의 confirmed 격상 방지" 두 조항 추가 (`docs/spec/DECISION.md` D065).
...
- `docs/spec/DETECTION_RULES.md:1085~1132` — L3-03 + IC01/IC02/IC03 정책 (T-P3-1 갱신 완료)
- `docs/spec/DETECTION_REFERENCE.md:153~241` — 감사기준서 매핑 (IFRS 10 / K-IFRS 1110·1024 / ISA 600 신규 절, T-P3-2 갱신 완료)
- `docs/spec/RULE_DETAIL_METADATA_V1_LOCK.md` — canonical rule count + evidence level sidecar 정책 (T-P3-3 갱신 완료)
- `docs/spec/PHASE1_TOPIC_SCORING_V1_LOCK.md` — floor 차별 정책 보조 절 (T-P3-4 갱신 완료)
- `docs/spec/PHASE1_RULE_RELATIONSHIP_MAP.md` — intercompany_structure evidence type + evidence level sidecar 주석 (T-P3-5 갱신 완료)
- `docs/spec/DECISION.md:514` — D055 (2026-05-18) — Superseded by D065 (2026-05-23)
- `docs/spec/DECISION.md:565~` — D065 (2026-05-23) — 본 plan 의 supersede 결정문 (T-P3-6 추가 완료)
