...
> **C안 3-surface 정합 (2026-06-14, SoT [PHASE1_TIER_EVIDENCE_BASIS.md §7](PHASE1_TIER_EVIDENCE_BASIS.md))**: row feature contract / ml_score 결합 정책은 3-surface 분리(PHASE1-1 룰 / PHASE1-2 family / PHASE2 VAE)를 전제한다. 본 문서가 다루는 `ml_score` 는 **PHASE2 VAE 단독 surface** 의 산출이며, graph·relational·시계열 등 구조 단위 탐지기(family)는 surface 분류상 **PHASE1-2 family** 다. 결정 3 옵션 Z(독립 parallel queue, PHASE1 ranking 비파괴)는 이미 "독립 surface, 점수 미병합" 불변식과 정합한다 — 세 surface 를 단일 final_score 로 합치지 않는다. historical PHASE3(외부 LLM narrator/ordering)는 active product path 에서 제거됐다.
...
- Local Evidence Brief UI 구현 (별도 local-first policy)
...
- `docs/spec/LOCAL_FIRST_EVIDENCE_POLICY.md`
...
- 결정 1 row matrix contract: 부분 존재. 단, **row-level provenance 식별자 (`document_id` + `row_index` / `record_id`)** 가 ML 입력 deny 와 Local Evidence Brief source 사이에서 명시적으로 분리되어 있지 않다. 명세 필요.
...
| 컬럼 | 역할 | ML 입력 | Local Evidence Brief provenance |
|------|------|--------|------|
| `document_id` | 전표 ID | ❌ deny | ✅ `journal_id` |
| `row_index` | 전표 내 행 번호 (0-based) | ❌ deny | ✅ `line_no` |
| `record_id` | 전체 GL 내 unique row 식별자 | ❌ deny | ✅ row 식별 보조 |

**원칙**: 식별자는 학습 입력 금지, Local Evidence Brief provenance 출력 전용. row matrix는 식별자 없는 feature 만 보유. inference 시 `row_index` 보존을 위한 별도 키 컬럼 (예: `_row_key`) 을 matrix 와 분리해서 유지.
...
> **변경 (2026-05-24, standalone contract lock 반영 / 2026-05-26 local-first 반영)**: 본 표의 이전 정책은 `flagged_rule_count`, `review_rule_count`, `flagged_rule_severity_max` 를 화이트리스트로 include 하고 multi-hot 만 deny 하던 안이었다. Phase 2 Standalone Contract lock 아래에서는 PHASE1 rule hit 의 **모든 row-level 파생 컬럼** 을 ML 입력으로 받지 않는다. PHASE1 rule hit 정보는 Local Evidence Brief provenance 와 dashboard overlay 에서만 사용한다.
...
| `flagged_rule_ids` / `flagged_rules` | rule_id list (raw) | ❌ exclude — Local Evidence Brief provenance 전용 |
...
| `is_synthetic`, `is_mutated` | DataSynth technical marker | synthetic-row marker; real customer data does not carry this as business substance |
...
| V7/r4l 파생 shortcut | `near_threshold_gap_amount`, `near_threshold_gap_ratio`, `near_threshold_limit_amount`, `approver_limit_amount`, `approver_can_approve_je`, `line_number`, `is_synthetic`, `is_mutated` |
...
#### (2-b) r4l preprocessing defense gate

v42/r4l domain audit에서 데이터 쪽 NULL marker를 제거하더라도 PHASE2 전처리 방어선은 별도로 잠근다.

- `src/preprocessing/constants.py`의 `SYNTHETIC_ONLY_COLUMNS` 또는 `LEAKAGE_DENY_COLUMNS`에는 `is_synthetic`, `is_mutated`가 포함되어야 한다.
- `is_fraud`, `is_anomaly`, `fraud_type`, `anomaly_type`은 라벨/평가 전용이며 row matrix feature에는 들어가지 않는다.
- `gl_account`는 계정코드이므로 숫자 크기나 z-score 의미가 없다. CSV에서 정수처럼 파싱되더라도 PHASE2 row matrix에서는 categorical role로 강제하거나 문자열로 정규화해야 한다.
- r4l smoke 기준: PHASE2 preprocessing plan/matrix에 `is_synthetic`, `is_mutated`, 라벨 컬럼이 없어야 하며, `gl_account`가 numeric/amount role로 분류되면 FAIL.
- v42/r4l 기준 numeric 생존 컬럼 목록은 preprocessing 리포트에 저장해야 한다. 현 정책상 금액 원천 컬럼은 V6/V7 deny 정책으로 입력 feature에서 제외되며, numeric 생존 컬럼은 합성 marker가 아닌 안전한 운영/파생 수치만 허용한다.
- 2026-06-13 v42j/r4l_b smoke: `is_synthetic`, `is_mutated`, `is_fraud`, `is_anomaly`는 exclude; `gl_account`는 `domain_code_categorical`로 include; 20,000행 matrix에는 synthetic marker 컬럼 0개.
...
#### (4) PHASE2 / historical PHASE3 출력 컬럼 (전 단계 출력 재투입 금지)
...
| `review_narrative_*`, `priority_rank` | historical PHASE3 출력. Active product path에서는 제거됨 |

### 2.5 feature_id 명명 규칙 (Local Evidence Brief provenance 호환)

`Phase2AutoencoderMatrixBuilder` 의 `feature_names_` 는 변환기별로 자동 생성된다. Local Evidence Brief와 local diagnostic views가 feature provenance를 일관되게 표시하도록 다음 규칙을 lock한다:
...
Local evidence provenance validator는 표시되는 `feature_id`가 PHASE2 `feature_names_` (또는 `output_feature_groups_` 키)에 존재함을 검증해야 한다.
...
| 설명 가능성 | 높음 — ML 출력이 rule 과 독립적이므로 Local Evidence Brief provenance와 충돌 없음 |
| 회귀 위험 | 낮음 — 현 코드 정책 유지 |
...
| 설명 가능성 | 중간 — ml_score top_features 가 prior 컬럼에 집중되면 local explanation이 "ML이 rule 점수를 재진술"하는 형태로 환원 |
...
- inference 후 PHASE1 prior 와 PHASE2 ml_score 를 별도 score 로 dashboard / Local Evidence Brief 에서 결합.

| 평가 항목 | 결과 |
|---------|------|
| fitting 위험 | 낮음 — 학습 단계 분리 |
| 새 신호 발견 가능성 | 높음 — 옵션 A 와 동일 |
| 운영 복잡도 | 높음 — dashboard 결합 가중치와 Local Evidence Brief가 두 score 를 동시 표시 |
...
- dashboard 와 Local Evidence Brief 는 단일 final_score 만 본다.
...
| 설명 가능성 | 중간 — local explanation이 "rule 점수 + ml 점수 = final" 형태로 단순 표시 가능하지만 가중치 변경 시 ranking 흔들림 |
...
- Local Evidence Brief: PHASE1 `case_id` / `composite_sort_score` 를 1차 source 로, PHASE2 `model_id` / `feature_id` / `score` 를 보조 provenance 로 표시.
...
4. Local Evidence Brief provenance 계약은 rule_id / feature_id / journal_id 를 **서로 다른 type** 으로 유지한다. 두 score 가 독립이어야 근거 다양성이 보장된다.

### 4.3 dashboard 노출 정책 (2026-05-26 lock)

옵션 Z 채택 하에서 dashboard tab 구조:

- `tab_phase1`: rule-based review queue 와 `composite_sort_score` 정렬을 유지한다. PHASE2는 PHASE1 ranking을 덮어쓰지 않는다.
- `tab_phase2`: 전체 요약 + 분석 영역별 family lane + lane-local 통계만 노출한다.
- `tab_review_queue`: Historical candidate-builder PHASE3 policy is removed from active product path. Active queue views use PHASE1/PHASE2 local results only.

금지 UI:

- PHASE1 + PHASE2 통합 band / 통합 rank / 통합 score
- PHASE2 global 또는 integrated score tab
- 5-family Noisy-OR rank / score distribution
- rank percentile 기반 PHASE2 검토 등급 분포
- PHASE2 점수로 정렬된 "위험 신호별" master table

허용 UI:

- PHASE2 summary ribbon: 분석 대상 case 수, Phase 2 신호 case 수, 활성 lane 수, 최상위 lane
- 분석 영역별 family lane 카드와 세부 탐지 항목 hit
- Lane × evidence_tier matrix
- family별 case-family contribution hit bar
- subdetector hit distribution

PHASE2 backend metadata(`phase2_family_scores`, `max_family_ecdf`, `phase2_review_band` 등)는 provenance/debug 또는 lane-local 설명에 사용할 수 있지만, user-facing global review order나 통합 점수로 노출하지 않는다.
...
**(4) 부수효과 — `phase2_review_band` metadata 영향 허용 (UI global band 노출 금지)**
...
Dashboard 정책: 위 `phase2_review_band` 는 service/debug metadata로만 취급한다. `tab_phase2` 는 이를 PHASE2 global 검토 등급, 통합 score, 또는 master ranking tab 으로 노출하지 않고, family lane 근거 강도와 Local Evidence Brief provenance에 한정해 사용한다.
...
### 4.4 Local Evidence Brief selected-case explanation 정책

- Local Evidence Brief는 PHASE1 review queue 또는 PHASE2 family lane 에서 사용자가 선택한 case를 설명하는 deterministic local layer다.
- `priority_rank` / `priority_score` 는 새로 산출하지 않는다. 기존 queue/lane display metadata를 표시할 때만 참조 가능하다.
- Local Evidence Brief는 후보 순서, lane 순서, official review order 를 변경하지 않는다.
- provenance validator 는 **evidence id 존재** 를 검증한다. provenance 결과를 queue ordering 변경 근거로 사용하지 않는다.

#### 4.4.1 family signal provenance 보강 (PHASE2 GOVERNANCE 결정 8 정합, 2026-05-19)

Local Evidence Brief input builder는 case overlay 의 다음 필드를 local evidence payload에 노출한다. 외부 LLM prompt payload나 narrator 호출은 active product path가 아니다.

| Evidence field | 출처 overlay 필드 | 역할 |
|---|---|---|
...
5. **Local Evidence Brief provenance 호환**
   - PHASE2 `ml_scores.top_features[].feature_id` 가 본 §2.5 명명 규칙과 일치함을 입력 단계에서 검증.
6. **D044 PR 템플릿 확장** (`docs/spec/DECISION.md`)
   - PHASE2/local evidence handoff checklist: feature deny-list 적용 여부, train/test split, OOF leakage 점검, provenance 검증.

## 7. 변경 이력

- 2026-05-15: 초안 작성. 결정 1 명세 보강 (row contract), 결정 2 옵션 A 권장, 결정 3 옵션 Z 권장. 사용자 확정 후 lock 예정.
- 2026-05-25: §4.3.2 추가 — IC family lane evidence_role priority (옵션 2). 4개 IC internal probability column 을 `phase2_subdetector_tiers.yaml` 에 등록(17→21) + `phase2_lane_sort.py` 에 `ic_role_priority` secondary dim 도입. score 합성 / KPI 가드 / family score 분포 모두 비변경. §4.3.2 (4) 부수효과 — `phase2_review_band` metadata 영향은 의도된 변화로 계약 고정하되, dashboard global band/score 노출은 금지 (ISA 550 ¶A20 + PCAOB AS 2401 §B7 인용, `TestIntercompanyInternalProbReviewBandImpact` 6 회귀).
