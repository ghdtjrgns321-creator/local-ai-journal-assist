...
| TS-15 | 탐지 설계/거버넌스 | PHASE1 priority_score 분포 불균형 — Stage 1 봉합 + Stage 2-A fitting 위험으로 보류, priority_score 계층 재설계로 이관 | 0.75 클러스터링, priority_floors dead code, max merge, count fitting, D060/D061 | 진행 중 (2026-05-21) |
...
- `docs/CONSTRAINTS.md` §프로젝트 포지셔닝
- `docs/users/10_PORTFOLIO_POSITIONING.md`
- `docs/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §외부 shadow 검증
...
- `docs/PHASE1_SCORE_DISTRIBUTION_LOG.md` — Stage 별 누적 측정 로그
- `docs/DECISION.md` D060 (0.90 critical 승격 원칙 5조), D061 (Stage 2-A 보류 + 계층 재설계)
- `docs/CONSTRAINTS.md` §PHASE1 priority_score 의 명시적 제약
- `docs/users/07_PHASE1_PRIORITY_RANKING.md`

### 상태

진행 중 (2026-05-21). Stage 1 봉합 완료, Stage 2-A 보류, Stage 5 (priority_score 계층 재설계) 대기.
...
- `docs/DECISION.md` D062
- `docs/CONSTRAINTS.md`
- `docs/users/08_PHASE2_FAMILY_STRUCTURE.md`, `09_REVIEW_QUEUE_RRF_FUSION.md`
...
**관련 문서**: [00-dataset.md](completed/raw-plan/00-dataset.md) | [DECISION.md D010](DECISION.md)
...
**관련 문서**: [CONSTRAINTS.md §ML 학습 전략](CONSTRAINTS.md) | [completed/raw-plan/05a-detection-ml.md](completed/raw-plan/05a-detection-ml.md) | `dev/active/phase2-*` (구 TASKS.md §Phase 2)
...
**관련 문서**: [completed/NEW_TASKS.MD](completed/NEW_TASKS.MD) | [CONSTRAINTS.md §회사별 매핑 프로파일](CONSTRAINTS.md)
...
V7 fixed3 PHASE2 inference 결과(`docs/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md`)에서 TOP-N recall이 낮게 나오는 두 개의 구조적 원인을 분리해서 기록한다. 하나는 정렬식 결함(코드 측), 다른 하나는 recall 분모 정의(평가 측)다. 둘은 별개 문제이고 해결 경로도 다르다.
...
`docs/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §1/§2의 모든 recall 수치는 분모 795(case 단위)다. document 단위 recall로 환산하면 동일한 검토 case 수에서 더 높은 값이 나올 가능성이 크다. 다만 변환 함수가 어디에도 적용되지 않아 측정값이 없다.
...
| V7 PHASE2 문서 doc 단위 수정 | ✅ 완료 | `docs/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` doc 단위 보강 |
| V7 fixed3 PHASE1 단독 결과 문서 | ✅ 완료 (2026-05-18) | `docs/completed/DETECTION_RESULTS_MANIPULATION_V7_FIXED3.md` + `artifacts/phase1_manipulation_v7_fixed3_evaluation.json` |
| CONTRACT_V3 작성 | ✅ 완료 (2026-05-18) | `docs/DETECTION_RESULTS_CONTRACT_V3.md` 신규 작성 |
...
| (b) 신규 룰 추가 | 미실행 | 32 룰 고정 정책과 충돌한다. `PHASE1_TOPIC_SCORING_V1_LOCK.md` 개정 및 `docs/DECISION.md` 결정 필요. | 본 sprint 비범위. 향후 K-SA 240, 금감원 감리지적 사례, PCAOB AS 2401, K-SOX 근거가 있을 때만 검토. |
...
PHASE2 5 family ECDF는 80 doc에 대해 informative하지 않다. 이는 **8 시나리오 중 fictitious_entry(32 미진입), embezzlement_concealment(39 미진입)** 두 시나리오가 80건의 88.75%를 차지하며, 두 시나리오 모두 5 family 매트릭스(`docs/DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md` §6)에서 다음 특성을 보이기 때문으로 추정:
...
**관련 문서**: [DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md](DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md) · [PHASE1_TOPIC_SCORING_V1_LOCK.md](PHASE1_TOPIC_SCORING_V1_LOCK.md) · [DETECTION_RULES.md](DETECTION_RULES.md) · `artifacts/ts13_uncovered_truth_80_analysis.json` · `artifacts/ts13_recovery_path_evaluation.md` · `artifacts/doc_level_ranking_phase_a_20260519.{json,md}` · `dev/active/doc-level-ranking/doc-level-ranking-context.md`
...
4. `docs/PHASE2_GOVERNANCE_DESIGN.md` 결정 8 갱신 — reject 범위 축소 + Noisy-OR 채택 + **깊이별 trade-off (TOP 100/500 +, TOP 1,000/2,000 −) 명시** ✅
5. `docs/users/08_PHASE2_FAMILY_STRUCTURE.md` / `09_REVIEW_QUEUE_RRF_FUSION.md` 사용자 문서 반영 — **단조 우월 표현 금지, architecture standardization 사유 강조** ✅
...
**관련 문서**: [PHASE2_GOVERNANCE_DESIGN.md](PHASE2_GOVERNANCE_DESIGN.md) 결정 8 · [PHASE2_INTERFACE_DESIGN.md](PHASE2_INTERFACE_DESIGN.md) · [DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md](DETECTION_RESULTS_MANIPULATION_V7_FIXED3_PHASE2.md) · `artifacts/phase2_family_ranking_measurement_20260519.{json,md}` · `artifacts/phase2_family_ranking_alt_aggregators_20260519.{json,md}` · `tools/scripts/phase2_family_ranking_alt_aggregators.py` · `dev/active/phase2-family-ranking/`
...
**관련 문서**: [docs/users/16_PHASE2_RESPONSIBILITY_MAP_DECISION.md](users/16_PHASE2_RESPONSIBILITY_MAP_DECISION.md) · [docs/debugging/PHASE2_FAMILY_RESPONSIBILITY_RECALL_20260530.md](debugging/PHASE2_FAMILY_RESPONSIBILITY_RECALL_20260530.md) · [DETECTION_RESULTS_MANIPULATION_V7_FIXED4_PHASE2.md](DETECTION_RESULTS_MANIPULATION_V7_FIXED4_PHASE2.md) · `artifacts/phase2_family_responsibility_recall_fixed5_20260530.json` · `tools/scripts/measure_phase2_family_responsibility_recall_fixed5_20260530.py`
