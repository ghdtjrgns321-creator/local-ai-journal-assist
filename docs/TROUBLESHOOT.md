...
| TS-14 | 제품 포지셔닝 | 부정탐지 모델에서 설명 가능한 감사 리뷰 큐 도구로 범위 고정 | 실데이터 부재, golden set 부재, 포트폴리오, review queue, narrator | 해결 |
...
| 데이터셋 | 역할 |
|---|---|
| DataSynth V7 fixed3 | 메인 포트폴리오 데모. 복식부기 전표 구조, 계정, 승인, 적요, 관계사, 의도 주입 truth가 있어 PHASE1/2/3 전체 기능을 보여주기에 적합 |
...
- PHASE3 Narrator는 판단자가 아니라 근거 요약과 절차 제안자다.

### 결과

이 결정으로 프로젝트는 중단되는 것이 아니라 범위가 명확해졌다. 메인 데모는 DataSynth에서 PHASE1 rule queue, PHASE2 unsupervised/rule-style family, PHASE3 narrator를 end-to-end로 보여준다. 외부 데이터는 보조 검증으로 두고, supervised 계열은 trusted golden set이 생길 때까지 dormant로 둔다.
