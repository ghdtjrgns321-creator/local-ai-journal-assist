...
|-------------------|----------------------|----------------------|
| 행수              | 332K (고정)           | 1,105,510 (조절 가능)  |
| 컬럼수            | 12 (SAP 필드)         | 29 (ACDOCA 매핑)      |
| 부정 레이블        | 없음 (IF/LOF 1%)     | fraud 2.1% + anomaly 7.7% |
| 부정 시나리오      | 0종                   | 132개 내장 중 프로젝트 대상 52개 (8종 비율 지정) |
| 복식부기 보장      | 원본 의존             | 생성 시 강제           |
| Benford 분포      | 원본 의존             | 생성 시 준수           |
| 재현성             | 불가                  | seed 2024 고정         |
| GL 계정            | 가변                  | 430개 정의 / 397개 사용 |
| 입력자             | 가변                  | 152명                  |
...
3. **3축 평가 체계 수립**
   ...
   - 축 3: DataSynth 29컬럼으로 즉시 탐지 가능 여부 — 0~3점
...
detection 코드 (구현)
  ↓ 테스트
DataSynth 데이터 (검증: 미국 제조사 기본값)
```

항목별 갭:

| 항목           | settings.py (한국 기준)    | DataSynth 실태 (미국 기준)   | 갭                    |
|---------------|--------------------------|----------------------------|----------------------|
| 승인한도       | 5,000만 원                | 최대 금액 770만              | 한도가 데이터 범위 밖   |
| 직무분리 임계   | 동일인 3개+ 프로세스       | 152명 중 151명이 3개+ 담당   | 소규모 환경에서 전원 위반 |
| 심야 기준      | 22시~06시                 | posting_date에 시간 없음     | 원천 데이터 구조 부재   |
| 거래처 ID      | auxiliary_account_number  | 전부 NULL                   | 원천 데이터 구조 부재   |
| Benford 위반   | MAD > 0.012              | 금액이 Benford 적합 분포     | 위반 데이터 미주입     |
...
3. **프로젝트 구현 가능성 판정**
   - 각 갭에 대해 "이 프로젝트에서 구현할 수 있는가"를 실제 데이터 구조(29컬럼, UUID, timestamp 등)와 대조하여 5단계로 판정.
...
| 판정 | 건수 | 내용 |
|------|------|------|
| **구현 가능** | 18건 | 29컬럼으로 즉시 가능(3건) + DataSynth 컬럼 추가(7건) + Phase 2~3(8건) |
...
- **사용자 풀**: 152명 → 150명 persona별 분리 (주니어 60, 시니어 30, 컨트롤러 10, 매니저 10, 자동화 40)
- **시간 정보**: intraday 시간대 분포 활성화. 한국 실무 반영: 심야(22~06시) 0.02, 야근(18:30~22) 0.3, 결산기 야근 0.7
- **승인한도**: KRW 6단계 [100만, 500만, 1000만, 2500만, 5000만, 1억]
...
| 카테고리 | 추가 컬럼 | 탐지 활용 |
|---------|----------|----------|
| 승인 | `approved_by`, `approval_timestamp`, `approval_level` | 승인 누락/지연/자기승인 정밀화 |
| 증빙 | `has_attachment`, `supporting_doc_type`, `invoice_amount`, `delivery_date`, `tax_amount` | 증빙 누락, 금액 불일치, 컷오프, 부가세 검증 |
| 변경 이력 | `changed_by`, `change_date`, `changed_field` | 기말 수정 집중, 무단 수정 |
| 전표번호 | `document_number` (순차 int) | 번호 갭 탐지 |
| IP | `ip_address` | 비정상 접근 |
...
DataSynth 재생성 전(현재까지)의 중간 결과:

| 지표              | 1차 E2E (Before)              | 재설정 후 (Expected)         |
|-------------------|------------------------------|------------------------------|
| 검증 가능 룰       | 14/22 (8개 0건)               | 22/22 + 신규 룰 후보          |
| B07 과탐률         | 99.96%                        | ~10%                          |
...
1. DataSynth Rust 코드 수정 (approval.rs 한국식 + 확장 컬럼 CSV 출력)
2. 데이터 재생성 + E2E 재테스트
3. Phase 1b 확장 3건 구현 (역분개, Top-side JE, 시간대 입력자 집중)
4. 신규 탐지 룰의 성능 평가
