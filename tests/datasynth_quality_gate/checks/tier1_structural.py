...
def t1_01(con: duckdb.DuckDBPyConnection, labels_con: duckdb.DuckDBPyConnection | None) -> CheckResult:
    """행수/컬럼수 정합 — Stage에 따라 39(S1) 또는 46(S2) 허용."""
    start = time.perf_counter()
    row_count = con.execute("SELECT COUNT(*) FROM je").fetchone()[0]
    col_count = len(con.execute("PRAGMA table_info('je')").fetchall())
    elapsed = (time.perf_counter() - start) * 1000

    # Why: Stage 1=39컬럼, Stage 2=46컬럼. 둘 다 허용하여 하위 호환 유지.
    valid_cols = col_count in (39, 46)
    expected = "rows>0; cols∈{39,46}"
    actual = f"rows={row_count:,}; cols={col_count}"
    status = "PASS" if row_count > 0 and valid_cols else "FAIL"
...
def t1_07(con: duckdb.DuckDBPyConnection, labels_con: duckdb.DuckDBPyConnection | None) -> CheckResult:
    """기간 범위 — config start_date/period_months 기반 다기간 대응."""
    start = time.perf_counter()
    ...
    valid_years = exp["valid_fiscal_years"]
    valid_years_sql = ", ".join(str(y) for y in valid_years)

    # fiscal_year가 유효 연도 밖인 건수
    bad_fy = con.execute(f"""
        SELECT COUNT(*) FROM je WHERE fiscal_year NOT IN ({valid_years_sql})
    """).fetchone()[0]
    ...
    # posting_date 범위: config 기간 밖은 WARNING
    out_of_range = con.execute(f"""
        SELECT COUNT(*) FROM je
        WHERE CAST(posting_date AS DATE) < '{start_date}'
           OR CAST(posting_date AS DATE) > '{end_date}'
    """).fetchone()[0]
    ...
    expected = f"fiscal_year∈{valid_years}, period=1~12, posting_date∈[{start_date},{end_date}]"
    actual = f"bad_fy={bad_fy:,}, bad_fp={bad_fp:,}, date_out_of_range={out_of_range:,}"

    return CheckResult(
        check_id="T1-07", tier=1, name="기간 범위",
        status=status, expected=expected, actual=actual,
        detail={"bad_fiscal_year": bad_fy, "bad_fiscal_period": bad_fp,
                "date_out_of_range": out_of_range, "valid_years": valid_years},
        elapsed_ms=elapsed,
    )
...
# ---------------------------------------------------------------------------
# T1-15 ~ T1-16: change_log 구조 검증 (Stage 2)
# ---------------------------------------------------------------------------
...
def t1_15(con: duckdb.DuckDBPyConnection, labels_con: duckdb.DuckDBPyConnection | None) -> CheckResult:
    """change_log 구조 — 필수 6컬럼 존재."""
    start = time.perf_counter()
    if not _has_table(con, "change_log"):
        return CheckResult(check_id="T1-15", tier=1, name="change_log 구조",
                           status="SKIP", expected="change_log 테이블", actual="테이블 미존재",
                           elapsed_ms=(time.perf_counter() - start) * 1000)

    cols = [r[1] for r in con.execute("PRAGMA table_info('change_log')").fetchall()]
    required = ["document_id", "changed_by", "change_date", "changed_field", "old_value", "new_value"]
    missing = [c for c in required if c not in cols]
    elapsed = (time.perf_counter() - start) * 1000

    return CheckResult(
        check_id="T1-15", tier=1, name="change_log 구조",
        status="PASS" if not missing else "FAIL",
        expected=f"필수 {len(required)}컬럼 존재",
        actual=f"missing={missing}" if missing else f"OK ({len(cols)}컬럼)",
        elapsed_ms=elapsed,
    )


def t1_16(con: duckdb.DuckDBPyConnection, labels_con: duckdb.DuckDBPyConnection | None) -> CheckResult:
    """change_log 보호필드 NOT NULL — document_id, changed_by, change_date."""
    start = time.perf_counter()
    if not _has_table(con, "change_log"):
        return CheckResult(check_id="T1-16", tier=1, name="change_log NOT NULL",
                           status="SKIP", expected="change_log 테이블", actual="테이블 미존재",
                           elapsed_ms=(time.perf_counter() - start) * 1000)

    nulls = con.execute("""
        SELECT COUNT(*) FROM change_log
        WHERE document_id IS NULL OR changed_by IS NULL OR change_date IS NULL
    """).fetchone()[0]
    elapsed = (time.perf_counter() - start) * 1000

    return CheckResult(
        check_id="T1-16", tier=1, name="change_log NOT NULL",
        status="PASS" if nulls == 0 else "FAIL",
        expected="보호필드 NULL=0", actual=f"null_rows={nulls:,}",
        elapsed_ms=elapsed,
    )
