...
TIER_NAMES = {
    ...
    4: "Reverse Leakage (Normal Perfection)",
    5: "Compound Feature Leakage",
    6: "Line-level GL Pair Structure",
}


def save_report(report: QualityGateReport, output_dir: Path | None = None) -> tuple[Path, Path]:
    """JSON + Markdown 저장."""
    if output_dir is None:
        output_dir = Path(__file__).parent / "results"
    output_dir.mkdir(parents=True, exist_ok=True)

    json_path = output_dir / "ml_fitting_report.json"
    md_path = output_dir / "ml_fitting_report.md"
    ...
    for tier in report.tiers:
        ...
        for c in tier.checks:
            ...
            lines.append(f"| {c.check_id} | {c.name} | {c.status} | {c.expected} | {actual_short} |")

    # FAIL/WARNING 상세
    fails = [c for t in report.tiers for c in t.checks if c.status in ("FAIL", "WARNING")]
    if fails:
        lines.extend(["", "## 상세"])
        for c in fails:
            ...
            if c.detail:
                lines.append("- 상세:")
                lines.append(f"```json")
                lines.append(json.dumps(c.detail, ensure_ascii=False, indent=2)[:2000])
                lines.append("```")
