...
|:-----|:----------:|:------:|:--------:|:-------------------------------------|:----------------------------------------------|
| A01  |           3 |      3 |  100.0%  | UnbalancedEntry                      | sum(debit) != sum(credit), 허용오차 0.01       |
| A02  |          21 |     21 |  100.0%  | MissingField                         | NULL in gl_account / document_type / posting_date |
| A03  |           5 |      5 |  100.0%  | InvalidAccount                       | GL not in CoA (449개)                          |
| B01  |           4 |      4 |  100.0%  | RevenueManipulation                  | GL starts with '4' (revenue)                  |
| B02  |          27 |     20 |   74.1%  | JustBelowThreshold                   | amount in 90~99.99% of label threshold        |
| B06  |           5 |      5 |  100.0%  | SelfApproval                         | created_by == approved_by                     |
| B07  |          12 |     12 |  100.0%  | SegregationOfDutiesViolation         | sod_violation == True                         |
| B09  |           1 |      1 |  100.0%  | SkippedApproval                      | approved_by is NULL                           |
| B11  |          12 |     12 |  100.0%  | ImproperCapitalization               | asset GL(15xx) AND expense GL(5-8xxx) co-exist |
| C02  |           5 |      5 |  100.0%  | WeekendPosting                       | posting_date 요일 = 토(5)/일(6)               |
| C03  |         213 |    213 |  100.0%  | AfterHoursPosting, UnusualTiming     | posting_hour in 22:00~06:59                   |
| C04  |          30 |     27 |   90.0%  | BackdatedEntry, LatePosting          | \|posting_date - document_date\| > 30일       |
| C05  |          12 |     11 |   91.7%  | WrongPeriod                          | fiscal_period != posting_date.month           |
| C06  |          19 |      9 |   47.4%  | VagueDescription                     | blank/≤2chars/vague keywords in line_text     |
| C11  |          20 |     10 |   50.0%  | ReversedAmount                       | intra-doc DR↔CR swap OR reversal-dup pair     |
| **합계** | **389** | **358** | **92.0%** |                                  |                                                |
...
**B02 탐지기 FN 별도 원인 (27건 전수 미탐 — PART 3 참조):** `employees.json` `user_id`와 JE `created_by` 네이밍 불일치로 approval_limit 조회 자체가 불가능. 탐지기 recall 78%의 직접 원인.
...
- C06 탐지기 `vague_keywords.yaml` 업데이트 시 recall 향상 가능

#### C11 — 검증률 50.0% (FN 10건)

DataSynth의 C11 주입 전략 2가지:
...
| 검사 항목                  | 건수     | 비율   | 판정    | 비고                          |
|:---------------------------|:-------:|:------:|:-------:|:------------------------------|
| A01: 불균형 전표 (문서 기준) |       8 |  0.01% | ★ 오염  | 정상 문서에 차변≠대변이 존재   |
| B09: approved_by NULL (행)  |   1,005 |  0.10% | ★ 오염  | 488개 문서, 승인자 누락        |

### 허용 가능 패턴 (참고용)

| 검사 항목                  | 건수      | 비율   | 판정    | 비고                          |
|:---------------------------|:--------:|:------:|:-------:|:------------------------------|
| B07: sod_violation=True (행) | 111,668 | 10.83% | [참고] ★ 높음 | 10,374개 문서 — DataSynth 파라미터 점검 권장 |
| B06: 자기승인 (행)           |  98,909 |  9.60% | [참고]  | 내부 정책상 허용 가능         |
| C02: 주말 전기 (행)          |  82,877 |  8.04% | [참고]  | 24/7 운영 환경 허용            |
| C03: 야간(22-06) 전기 (행)   |  22,449 |  2.18% | [참고]  | 야간 배치 처리 허용            |

**B07 주의:** 정상 데이터의 10.83%가 `sod_violation=True`로 표시됨. 이는 DataSynth가 SoD 위반 플래그를 anomaly 주입과 독립적으로 생성하는 과정에서 발생한 것으로, 탐지기가 이 컬럼을 직접 사용하면 False Positive 폭발 위험이 있음. `sod_conflict_type` 기반 로직 보강 또는 임계치 적용 필요.
...
| 완전 검증 룰 (100%)         | A01, A02, A03, B01, B06, B07, B09, B11, C02, C03  |
| 부분 검증 룰 (<100%)        | B02(74%), C04(90%), C05(92%), C06(47%), C11(50%)   |
| 정상 데이터 오염            | A01 8건(문서), B09 1,005행(488 문서)               |
| 주요 DataSynth 설계 결함    | B02 user_id 불일치, C04/C05 날짜 동시수정, C11 패턴1 |
| C06/C11 낮은 검증률         | 코드 버그 아님 — DataSynth 주입 방식과 탐지 로직 간 갭 |
