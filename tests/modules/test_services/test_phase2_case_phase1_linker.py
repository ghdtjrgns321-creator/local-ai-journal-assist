...
def _unsupervised_case(
    *,
    ...
    return UnsupervisedCase(
        ...
        max_score_row_ref=refs[0] if refs else None,
        top_features=(),
        model_id="vae_v1",
        schema_hash="sha:abc",
    )
...
def test_empty_phase1_returns_case_set_with_no_refs() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="d1", left_pos=0, right_pos=1),),
    )
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ()
    assert result.diagnostics["linked_count"] == 0
...
def test_position_match_creates_phase1_case_refs() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_001", left_pos=5, right_pos=7),),
    )
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_alpha",)


def test_no_position_overlap_returns_empty_phase1_refs() -> None:
    """row_position 이 겹치지 않으면 phase1_case_refs 는 빈 tuple 유지."""
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_001", left_pos=100, right_pos=101),),
    )
    phase1 = _phase1_result(
...
def test_linker_returns_case_set_with_linked_true() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_001", left_pos=0, right_pos=1),),
    )
    phase1 = _empty_phase1()
...
def test_linker_preserves_phase2_family_score() -> None:
    ...
    linked = result.case_set.timeseries_cases[0]
...
def test_linker_preserves_phase2_case_generation_reason() -> None:
    ...
    assert result.case_set.timeseries_cases[0].case_generation_reason == reason
...
def test_linker_idempotent_when_case_set_already_linked() -> None:
    ...
    first_refs = first.case_set.timeseries_cases[0].phase1_case_refs
    second_refs = second.case_set.timeseries_cases[0].phase1_case_refs
...
def test_phase1_case_refs_sorted_alphabetically_in_tuple() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_001", left_pos=1, right_pos=2),),
    )
    ...
    refs = result.case_set.timeseries_cases[0].phase1_case_refs
...
def test_pair_case_left_row_position_matches_phase1() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_left_only",)
...
def test_pair_case_right_row_position_matches_phase1() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_right_only",)
...
def test_phase2_cases_without_phase1_overlap_keep_empty_refs() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_match", left_pos=1, right_pos=2),),
...
def test_linker_resets_stale_refs_when_phase1_becomes_empty() -> None:
    ...
    """
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="p2_dup_1", left_pos=10, right_pos=11),),
    )
    ...
    assert linked_once.case_set.timeseries_cases[0].phase1_case_refs == ("p1_alpha",)
    ...
    assert linked_again.case_set.timeseries_cases[0].phase1_case_refs == ()
...
def test_linker_resets_stale_refs_when_phase1_positions_change() -> None:
    ...
    assert linked_first.case_set.timeseries_cases[0].phase1_case_refs == ("p1_alpha",)
    assert linked_first.case_set.timeseries_cases[1].phase1_case_refs == ("p1_alpha",)
    ...
    assert linked_second.case_set.timeseries_cases[0].phase1_case_refs == ()
    assert linked_second.case_set.timeseries_cases[1].phase1_case_refs == ()
...
def test_linker_preserves_match_for_some_resets_others() -> None:
    ...
    assert linked_second.case_set.timeseries_cases[0].phase1_case_refs == ("p1_alpha",)
    assert linked_second.case_set.timeseries_cases[1].phase1_case_refs == ()
...
def _duplicate_case_with_docs(
    *,
    ...
    """left/right 의 document_id 를 명시 지정한 TimeseriesCase fixture."""
...
def test_doc_id_mode_matches_across_different_positions() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_reload",)
...
def test_auto_mode_diagnostics_records_resolved_key_mode() -> None:
    ...
    res_doc = link_phase2_to_phase1(
        case_set=Phase2CaseSet(timeseries_cases=(case_doc,)),
        phase1=phase1,
        key_mode="auto",
    )
    ...
    res_pos = link_phase2_to_phase1(
        case_set=Phase2CaseSet(timeseries_cases=(case_no_doc,)),
        phase1=phase1,
        key_mode="auto",
    )
...
def test_invalid_key_mode_raises_value_error() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_x", left_pos=0, right_pos=1),),
    )
    phase1 = _empty_phase1()
...
def test_diagnostics_records_match_precision_row_for_position_mode() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_p", left_pos=10, right_pos=11),),
    )
    phase1 = _empty_phase1()
    result = link_phase2_to_phase1(case_set=case_set, phase1=phase1, key_mode="position")
...
def _duplicate_case_full(
    *,
    ...
    """row_ref 두 개를 받아 TimeseriesCase 를 만드는 fixture."""
    return TimeseriesCase(
        phase2_case_id=case_id,
        batch_id="batch_test",
        family="timeseries",
...
def test_doc_line_mode_accepts_none_row_ref_map_after_phase2() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_x", left=left, right=right),),
    )
...
def test_doc_line_mode_matches_via_doc_and_normalized_line() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_dl_alpha",)
    assert result.diagnostics["key_mode_used"] == "doc_line"
    assert result.diagnostics["match_precision"] == "row"
...
def test_company_doc_mode_matches_via_company_and_doc() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_cd_alpha",)
    assert result.diagnostics["key_mode_used"] == "company_doc"
    assert result.diagnostics["match_precision"] == "document"
...
def test_label_mode_matches_via_canonical_label_hash() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_label_alpha",)
...
def test_match_precision_row_for_label_doc_line_position() -> None:
    ...
    right = _row_ref_full(6, document_id="DOC_X", raw_line_number="0002")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_prec", left=left, right=right),),
    )
    rrm = [
        _row_ref_map_entry(
            position=5,
...
def test_match_precision_document_for_doc_id_company_doc() -> None:
    ...
    right = _row_ref_full(6, document_id="DOC_Y", company_code="CO_A")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_dprec", left=left, right=right),),
    )
    rrm = [
        _row_ref_map_entry(
            position=5,
...
def test_doc_line_mode_accepts_empty_row_ref_map_with_hit_hash_direct() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_y", left_pos=0, right_pos=1),),
    )
    result = link_phase2_to_phase1(
        case_set=case_set,
        phase1=_empty_phase1(),
...
def test_company_doc_mode_accepts_empty_row_ref_map_with_hit_hash_direct() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_z", left_pos=0, right_pos=1),),
    )
    result = link_phase2_to_phase1(
        case_set=case_set,
        phase1=_empty_phase1(),
...
def test_hash_mode_still_requires_salt() -> None:
    ...
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case(case_id="dup_s", left_pos=0, right_pos=1),),
    )
...
def test_label_mode_uses_hit_canonical_label_hash_when_present() -> None:
    ...
    right = _row_ref_full(6, index_label_raw=6, document_id="DOC_Y")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_hit_hash", left=left, right=right),),
    )
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_hit_hash",)
...
def test_label_mode_falls_back_to_row_ref_map_when_hit_hash_empty() -> None:
    ...
    right = _row_ref_full(6, index_label_raw=6, document_id="DOC_Y")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_fallback", left=left, right=right),),
    )
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_legacy",)
...
def test_doc_line_mode_uses_hit_doc_and_line_hash_when_present() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_dl_hit",)
...
def test_doc_line_mode_falls_back_to_row_ref_map_when_hit_hash_empty() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_dl_legacy",)
...
def test_company_doc_mode_uses_hit_hashes_when_present() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_cd_hit",)
...
def test_company_doc_mode_falls_back_to_row_ref_map_when_hit_hash_empty() -> None:
    ...
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_cd_legacy",)
...
def test_auto_coverage_passes_when_all_hits_have_canonical_label_hash() -> None:
    ...
    right = _row_ref_full(6, index_label_raw=6, document_id="DOC_Y")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_cov_hit", left=left, right=right),),
    )
    ...
    assert result.diagnostics["key_mode_used"] == "label"
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_cov_hit",)
...
def test_auto_coverage_mixed_hits_uses_row_ref_map_fallback_for_legacy_hits() -> None:
    ...
    right = _row_ref_full(6, index_label_raw=6, document_id="DOC_Y")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_mixed", left=left, right=right),),
    )
    ...
    assert result.diagnostics["key_mode_used"] == "label"
    assert result.case_set.timeseries_cases[0].phase1_case_refs == ("p1_mixed",)
...
def test_auto_coverage_fails_when_hit_lacks_hash_and_row_ref_map() -> None:
    ...
    right = _row_ref_full(6, index_label_raw=6, document_id="DOC_Y")
    case_set = Phase2CaseSet(
        timeseries_cases=(_duplicate_case_full(case_id="dup_no_cov", left=left, right=right),),
    )
