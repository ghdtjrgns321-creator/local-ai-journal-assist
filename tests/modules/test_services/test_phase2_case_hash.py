...
def test_raw_hash_invariant_under_with_phase1_refs() -> None:
    ...
    case = _make_case()
    case_set_before = Phase2CaseSet(duplicate_cases=(case,))
    case_with_refs = case.with_phase1_refs(("case-a", "case-b"))
    case_set_after = Phase2CaseSet(duplicate_cases=(case_with_refs,))
...
def test_raw_hash_deterministic_repeat() -> None:
    ...
    case_set = Phase2CaseSet(duplicate_cases=(_make_case(),))
...
def test_linked_hash_payload_contains_sorted_phase1_case_refs() -> None:
    ...
    set_unsorted = Phase2CaseSet(duplicate_cases=(case_unsorted,))
    set_sorted = Phase2CaseSet(duplicate_cases=(case_sorted,))
...
def test_linked_hash_changes_when_phase1_refs_set_changes() -> None:
    ...
    hash_a = compute_linked_case_hash(Phase2CaseSet(duplicate_cases=(case_a,)))
    hash_ab = compute_linked_case_hash(Phase2CaseSet(duplicate_cases=(case_ab,)))
...
def test_linked_hash_invariant_under_input_order() -> None:
    ...
    h1 = compute_linked_case_hash(Phase2CaseSet(duplicate_cases=(case_1,)))
    h2 = compute_linked_case_hash(Phase2CaseSet(duplicate_cases=(case_2,)))
    assert h1 == h2
...
def _make_case_with_label(index_label: object) -> DuplicateCase:
    """index_label 만 바꾼 minimal DuplicateCase fixture (row_refs 1개).
...
def test_canonical_dict_replaces_pd_timestamp_index_label_with_canonical_string() -> None:
    ...
    # left_ref 도 동일하게 canonicalize.
    assert payload["left_ref"]["index_label"] == canonicalize_ref_key(ts)
...
def test_raw_hash_stable_under_pd_timestamp_index_label_instance_difference() -> None:
    ...
    set_a = Phase2CaseSet(duplicate_cases=(case_a,))
    set_b = Phase2CaseSet(duplicate_cases=(case_b,))
...
def test_raw_hash_stable_under_np_int64_index_label() -> None:
    ...
    set_np = Phase2CaseSet(duplicate_cases=(case_np,))
    set_py = Phase2CaseSet(duplicate_cases=(case_py,))
