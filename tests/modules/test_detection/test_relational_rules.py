...
class TestR01NewCounterparty:
    ...
    def test_missing_amount_columns_graceful_zero(self):
        """debit_amount/credit_amount 없음 → 원본 index 유지한 0점."""
        df = pd.DataFrame(
            {
                "trading_partner": ["V01", "V99"],
    ...
    def test_empty_dataframe(self):
        ...
        df = pd.DataFrame(
            columns=["trading_partner", "posting_date", "debit_amount", "credit_amount"]
        )
        scores = r01_new_counterparty(df, lookback_days=90, large_quantile=0.90)
        assert len(scores) == 0
...
class TestR02DormantAccountActivity:
    ...
    def test_missing_amount_columns_with_min_amount_graceful_zero(self):
        """min_amount > 0이고 amount 컬럼 없음 → 원본 index 유지한 0점."""
        df = pd.DataFrame(
            {
                "gl_account": ["5100", "5100"],
                "posting_date": pd.to_datetime(["2023-01-01", "2023-08-01"]),
            },
            index=[10, 20],
        )
        scores = r02_dormant_account_activity(
            df,
            inactive_days=180,
...
class TestR03TransferPricingAnomaly:
    ...
    def test_graceful_missing_is_intercompany(self):
        ...
        df = _make_df(
            [{"trading_partner": "SUB01", "gl_account": "4500", "debit_amount": 10_000_000}]
        )
        scores = r03_transfer_pricing_anomaly(df, deviation_threshold=0.15, min_ic_pairs=3)
        assert (scores == 0.0).all()
        ...
        df = pd.DataFrame(
            {
                ...
                "trading_partner": ["SUB01", "SUB01", "SUB01"],
