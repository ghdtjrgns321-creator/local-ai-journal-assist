"""DuplicateDetector 독립 트랙 테스트 — B05a/b/c/d 4개 서브룰."""
...
def exact_pair_df() -> pd.DataFrame:
    """B05a: gl+amount+date 정확 일치 쌍 1조 + 고유 행 1건."""
    return pd.DataFrame({
...
def split_df() -> pd.DataFrame:
    """B05c: 100만 단건 + 50만+50만 분할 (같은 gl, 윈도우 내)."""
    return pd.DataFrame({
...
class TestB05aExact:
    ...
    def test_unique_not_flagged(self, unique_df: pd.DataFrame) -> None:
        """모든 행 고유 → B05a score = 0."""
        result = DuplicateDetector().detect(unique_df)
        # Why: B05d도 체크하므로 전체 0인지 확인
        assert result.details.get("B05a", pd.Series(0.0)).sum() == 0.0
...
class TestB05cSplit:
    def test_split_detected(self, split_df: pd.DataFrame) -> None:
        ...
        assert result.details["B05c"].iloc[0] > 0  # 타겟(100만)
        assert result.details["B05c"].iloc[1] > 0  # 분할1(50만)
        assert result.details["B05c"].iloc[2] > 0  # 분할2(50만)

    def test_no_split_unrelated(self, unique_df: pd.DataFrame) -> None:
        """무관 금액 → B05c score = 0."""
        result = DuplicateDetector().detect(unique_df)
        assert result.details["B05c"].sum() == 0.0
...
class TestB05dTimeShift:
    def test_timeshift_flagged(self, timeshift_df: pd.DataFrame) -> None:
        ...
        assert result.details["B05d"].iloc[0] > 0
        assert result.details["B05d"].iloc[1] > 0
    ...
    def test_timeshift_outside_window(self, timeshift_outside_window_df: pd.DataFrame) -> None:
        ...
        assert result.details["B05d"].sum() == 0.0
