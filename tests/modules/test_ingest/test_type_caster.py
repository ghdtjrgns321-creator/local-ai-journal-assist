...
class TestCastAmount:
    ...
    def test_korean_null_strings(self):
        """한국 ERP null 표현(미정, 해당없음, 없음) → NaN."""
        s = pd.Series(["미정", "해당없음", "없음", "1000"])
        result = cast_amount(s)
    ...
    def test_na_variants(self):
        """N/A, n/a 표현 → NaN."""
        s = pd.Series(["N/A", "n/a", "500"])
        result = cast_amount(s)
        assert result.isna().sum() == 2
        assert result.iloc[2] == 500.0

    def test_backslash_won_prefix(self):
        """백슬래시(₩ 인코딩 아티팩트) 통화 접두사 → 제거 후 파싱."""
        s = pd.Series(["\\1000", "\\5,000", "\\10000"])
        result = cast_amount(s)
        assert result.tolist() == [1000.0, 5000.0, 10000.0]

    def test_backslash_with_parentheses(self):
        """백슬래시 + 괄호 음수 조합."""
        s = pd.Series(["\\(1,000)", "\\0"])
        result = cast_amount(s)
...
class TestCastStr:
    ...
    def test_scientific_notation_recovery(self):
        """과학적 표기법(Excel 손상 ID) → best-effort 정수 문자열 복원."""
        s = pd.Series(["JE2025-0001", "2E+11", "2.00001E+11"])
        result = _cast_str(s)
        assert result.iloc[0] == "JE2025-0001"
        assert result.iloc[1] == "200000000000"
        assert result.iloc[2] == "200001000000"

    def test_scientific_notation_no_false_positive(self):
        """일반 문자열은 과학적 표기법 복원에 영향받지 않음."""
        s = pd.Series(["JE001", "ABC", "1110", "test-entry"])
        result = _cast_str(s)
        assert result.tolist() == ["JE001", "ABC", "1110", "test-entry"]
...
class TestCastBool:
    ...
    def test_float_string_bool(self):
        """cleaning.yaml의 '1.0'/'0.0' 불리언 표현 동작 확인."""
        s = pd.Series(["1.0", "0.0"])
        result = _cast_bool(s)
