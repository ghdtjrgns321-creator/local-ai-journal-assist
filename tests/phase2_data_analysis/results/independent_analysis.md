...
**AGI015 사용자 발견 의의**: 시스템이 아닌 실사용자가 심야 64건을 처리한 것은 룰 L3-06(심야 전기) + L4-05(비정상 시간 집중 입력)의 **타겟 케이스**. 이 사용자의 다른 지표(SOD, 금액, fraud 라벨)를 추가 조사 필요.
...
### 정성 해석
**Benford 적합성 최상위**. Phase 1 룰 L4-02(Benford 위반) recall이 낮게 나오는 것이 정상이며, 이는 "데이터 자체가 Benford를 준수한다 = 대량 조작이 없다"는 의미. VAE는 Benford 외 다차원 이상을 학습하므로 영향 없음.
...
### 정성 해석
1월/11월/12월이 **결산·법인세 정산·배당 시즌**으로 극단치가 대량 유입 — 합성 데이터가 한국 회계 연간 주기를 **정확히 모사**. 이는 L3-04(기말 대규모) 룰의 타겟 데이터가 현실적이라는 증거.
...
### 탐지 룰 영향
- `L1-05`(자기 승인): 2,473건 타겟 확보
- `L1-06`(직무분리 위반): 4,000건+ 타겟 확보
...
**해석**: L3-04(기말 대규모) 룰의 완벽한 타겟 — 기말 시점 효과가 **과도하게 강조**된 합성. 실전에서는 이렇게 일관된 12월 집중이 드뭐 Phase 1 룰 L3-04의 recall이 과도하게 높게 나올 위험. ML 모델이 이 편향을 학습하면 실전 일반화 실패 가능성.
...
- L3-06(심야 전기) 룰 타겟은 실제로 극소수 — `DuplicatePayment` 22시 1건, `TimingAnomaly` 0시 1건 등
- **L3-06 룰은 이 데이터셋에서 recall이 0에 수렴할 것**
...
2. **심야/시퀀스 패턴 탐지**: after_hours 0.27%는 너무 적음 → L3-06 + BiLSTM 시퀀스 모델의 실효성 검증 불가
