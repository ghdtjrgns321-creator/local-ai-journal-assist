...
**정성 해석**:
`night_owl_users` 10명 설정이 실제로 작동. JKANG390, HLEE252 등 **5명의 night-shift 실사용자가 700+ 심야 전표**를 생성. ISA 240 §33 "비정상 시간 집중 입력"의 완벽한 타겟 케이스 확보. C03(심야 전기) + C12(비정상 시간 집중) 룰 학습 데이터 실질적 확보.
...
- `manager`의 `system_access_conflict` 486건 + `senior`의 916건 = 권한 오남용 1,402건 (B07 타겟)
- `top3_approver_share 38.0%`는 실무 승인 체계의 현실적 분포 (Pareto 20/80 근사)

**B06(자기 승인) 타겟**: 5,786건 (충분)
**B07(직무분리) 타겟**: 10,404건 (충분)
...
- 12월 편향이 fraud_type별로 균등화됨 → C01(기말 대규모) 룰의 과적합 위험 해소
...
**Phase 1 C01(기말) 룰**: ⭐⭐⭐⭐☆ (편향 완화, 여전히 12월 비중 유지)
**Phase 1 C08(고액) 룰**: ⭐⭐⭐⭐⭐ (금액 정상화)
...
|----|-------|------|
| A01 차대 균형 | 0 (정상) | ✅ |
| A03 무효 계정 | 888888/777777 | ✅ |
| B06 자기 승인 | 5,786건 | ✅ |
| B07 직무분리 | 10,404건 | ✅ |
| B04 중복 지급 | 수 건 | ✅ |
| C01 기말 대규모 | 12월 12~45% | ✅ |
| C02 주말 전기 | weekend_activity 0.2 | ✅ |
| **C03 심야 전기** | **6,844건 (705건 실사용자)** | ✅ |
| C06 위험 적요 | risk_keywords 기반 | ✅ |
| C07 Benford | MAD 0.0016 | ✅ |
| C08 이상 고액 | positive 110만원 | ✅ |
| **C12 비정상 시간 집중** | **5 사용자 ×100+ 건** | ✅ |
...
> 위반 룰: **C01**(기말 대규모 전표) [ISA 240 §32 — 결산 시점 이상 거래],
> **B06**(자기 승인) [COSO 원칙 10 — 직무분리].
...
2. **Benford MAD 0.0016**은 너무 완벽 — 실전 데이터는 0.005~0.010이 일반적. 역설적으로 C07 룰 검증에는 "위반 케이스"가 부족
