"""Build v98 candidate by rebalancing manipulated-entry truth across companies.

The v84-v97 manipulated-entry truth was almost entirely C001. This patch keeps
the same year/scenario counts, cleans old manipulated text markers, then
reselects documents with company-balanced sampling from the current journal.
"""
...
SOURCE = ROOT / "data" / "journal" / "primary" / "datasynth_v97_candidate"
DEST = ROOT / "data" / "journal" / "primary" / "datasynth_v98_candidate"
LABELS = DEST / "labels"
...
SCENARIO_META = {
    "fictitious_entry": {
        ...
        "label": "supporting schedule true-up",
    },
    "period_end_adjustment_manipulation": {
        ...
        "label": "period-end estimate revision",
    },
    "embezzlement_concealment": {
        ...
        "label": "advance settlement clearing",
    },
    "circular_related_party_transaction": {
        ...
        "label": "intercompany netting settlement",
    },
    "approval_sod_bypass": {
    ...
    "unusual_timing_manipulation": {
        ...
        "label": "close backlog release",
    },
}

TEXT_LABELS = [re.escape(meta["label"]) for meta in SCENARIO_META.values()]
TEXT_PATTERN = re.compile(r"\s*\|\s*(?:" + "|".join(TEXT_LABELS) + r")")
...
def _old_manipulated_docs() -> set[str]:
    path = LABELS / "manipulated_entry_truth.csv"
    if not path.exists():
        return set()
...
def _existing_exclusion_docs(old_manipulated: set[str]) -> set[str]:
    exclude: set[str] = set()
    path = LABELS / "anomaly_labels.csv"
    if path.exists():
        frame = pd.read_csv(path, dtype=str, usecols=["document_id"])
        exclude.update(frame["document_id"].dropna().astype(str))
    # Old manipulated docs are cleaned before reselection and can be reused.
    return exclude - old_manipulated
...
def _balanced_pick(docs: pd.DataFrame, mask: pd.Series, count: int, used: set[str], salt: str) -> pd.DataFrame:
    ...
    candidates["_sort"] = (
        ...
        + "|"
        + candidates["document_type"].fillna("")
        + "|"
    ...
    buckets = {
        str(company): frame.sort_values("_sort").reset_index(drop=True)
        for company, frame in candidates.groupby("company_code", sort=True)
    }
    picks = []
    ...
    return pd.DataFrame(picks).drop(columns=["_sort"], errors="ignore")
...
def _scenario_mask(docs: pd.DataFrame, scenario: str) -> pd.Series:
    ...
    if scenario == "fictitious_entry":
        return has_approval & human & process.isin({"O2C", "R2R", "A2R"})
    if scenario == "period_end_adjustment_manipulation":
        return has_approval & (human | source.eq("recurring")) & (day.ge(25) | day.le(5))
    if scenario == "embezzlement_concealment":
        return has_approval & human & process.isin({"P2P", "TRE", "R2R"})
    if scenario == "circular_related_party_transaction":
        tp = docs["trading_partner"].fillna("").astype(str).str.strip()
        return has_approval & tp.ne("") & process.isin({"R2R", "O2C", "P2P", "TRE"})
    ...
    if scenario == "unusual_timing_manipulation":
        return has_approval & (weekend | day.ge(27) | day.le(3))
...
def _clean_old_text(df: pd.DataFrame, old_docs: set[str]) -> None:
    if not old_docs:
        return
    mask = df["document_id"].astype(str).isin(old_docs)
    for column in ("header_text", "line_text"):
        current = df.loc[mask, column].fillna("").astype(str)
        df.loc[mask, column] = current.map(lambda value: TEXT_PATTERN.sub("", value).strip())
...
def _patch_year(year: int, global_exclude: set[str], old_docs: set[str]) -> pd.DataFrame:
    path = DEST / f"journal_entries_{year}.csv"
    df = pd.read_csv(path, dtype=str, low_memory=False)
    _clean_old_text(df, old_docs)
    docs = _doc_frame(df)
    used = set(global_exclude)
    selected_frames: list[pd.DataFrame] = []

    for scenario, count in YEAR_SCENARIO_COUNTS[year].items():
        picked = _balanced_pick(docs, _scenario_mask(docs, scenario), count, used, scenario)
        if len(picked) < count:
            remaining = count - len(picked)
            fallback = _balanced_pick(
                docs,
                _fallback_mask(docs),
                remaining,
                used | set(picked["document_id"].astype(str)),
                f"{scenario}_fallback",
            )
        ...
        picked["evaluation_note"] = (
            "Evaluate whether any L1-L4 signal combination surfaces this manipulated entry; "
            "do not treat as rule-specific truth."
        )
...
def _write_manifest(truth: pd.DataFrame) -> None:
    summary = {
        "candidate_version": "v98",
        "source_baseline": str(SOURCE.relative_to(ROOT)).replace("\\", "/"),
        "patch_scope": "rebalance rule-agnostic manipulated-entry truth across companies",
        "total_manipulated_documents": int(len(truth)),
        "year_counts": {str(k): int(v) for k, v in truth["fiscal_year"].value_counts().sort_index().items()},
        "company_counts": {str(k): int(v) for k, v in truth["company_code"].value_counts().items()},
        "scenario_counts": {str(k): int(v) for k, v in truth["manipulation_scenario"].value_counts().items()},
        "scenario_company_counts": {
            f"{scenario}|{company}": int(v)
            for (scenario, company), v in truth.groupby(["manipulation_scenario", "company_code"]).size().items()
        },
        "anti_fitting_note": "Company balancing preserves existing scenario/year counts and does not target one detector rule.",
    }
    (DEST / "V98_MANIPULATED_COMPANY_DIVERSITY.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    (DEST / "FREEZE_V98_CANDIDATE.md").write_text(
        "# DataSynth v98 Candidate\n\n"
        f"Source baseline: `{summary['source_baseline']}`.\n\n"
        "Scope: rebalance manipulated-entry truth across C001/C002/C003.\n\n"
        f"```json\n{json.dumps(summary, ensure_ascii=False, indent=2)}\n```\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    old_docs = _old_manipulated_docs()
    exclude = _existing_exclusion_docs(old_docs)
    truth_frames = [_patch_year(year, exclude, old_docs) for year in YEARS]
        ...
        json.dumps(
            {
                ...
                "total": int(len(truth)),
                "company_counts": {
                    str(k): int(v) for k, v in truth["company_code"].value_counts().items()
                },
                "year_counts": {
