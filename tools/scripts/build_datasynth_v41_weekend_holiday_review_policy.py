...
NORMAL_CONTEXT_TARGETS = {2022: 1100, 2023: 1080, 2024: 1060}
WEEKEND_LABEL_TYPES = {"WeekendPosting"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build v41 L3-05 weekend/holiday review population and normal controls.")
    parser.add_argument("--source", required=True, help="Source dataset directory, normally datasynth_v40_candidate")
...
def classify_weekend_context(row: pd.Series) -> tuple[str, str]:
    if bool(row.get("is_holiday", False)) and not bool(row.get("is_weekend", False)):
...
def build_review_records(base: Path, year: int, weekend_label_docs: set[str], weekend_label_type_by_doc: dict[str, str], all_labeled_docs: set[str], existing_normal_docs: set[str]) -> list[dict]:
    ...
    holiday_set = build_holiday_set({year})
    ...
    df["is_weekend"] = df["posting_weekday"].ge(5)
    df["is_holiday"] = df["posting_date"].dt.date.isin(holiday_set)
    weekend = df[df["is_weekend"] | df["is_holiday"]].copy()
    doc_df = (
        ...
        records.append(
            {
                ...
                "is_weekend": bool(series["is_weekend"]),
                "is_holiday": bool(series["is_holiday"]),
...
def select_normal_context(review_records: list[dict]) -> list[dict]:
    ...
    selected: list[dict] = []
    context_targets = {
        "weekday_holiday": 0.36,
        "scheduled_batch": 0.22,
        "recurring_operation": 0.12,
        "close_calendar": 0.10,
        "operational_flow": 0.10,
        "backoffice_operation": 0.06,
        "extended_hours": 0.02,
        "manual_business_need": 0.02,
    }
...
def write_preview(output: Path, summary: dict) -> None:
    text = f"""# DataSynth v41 Candidate

v41 keeps v40 and fixes the L3-05 contract so review population covers both weekend and holiday postings.

## Summary

- Source baseline: `datasynth_v40_candidate`
...
- `V41_WEEKEND_HOLIDAY_REVIEW_POLICY.json`
"""
    (output / "PREVIEW.md").write_text(text, encoding="utf-8")
    (output / "FREEZE_V41_CANDIDATE.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    summary = {
        "candidate_version": "v41_candidate",
        "source_baseline": "datasynth_v40_candidate",
        "focus": "Separate L3-05 weekend/holiday review population from confirmed WeekendPosting anomalies and normal contexts",
    ...
    (output / "V41_WEEKEND_HOLIDAY_REVIEW_POLICY.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
