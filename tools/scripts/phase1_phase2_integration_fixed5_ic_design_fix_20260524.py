"""Stage7 재측정 — IC reciprocal flow 설계 변경 효과를 fixed5_normalcal5 PKL 로 측정.

PHASE2 5-family 점수는 helper 변경 반영을 위해 항상 재계산 (cache 무시). 산출물은
`_ic_design_fix_20260524` 접미사로 분리한다. truth label 기준 threshold 재튜닝은
하지 않으며 결과는 사후 검증으로만 사용한다.

변경 대상:
  - src/detection/intercompany_rules.py compute_reciprocal_flow_scores
    (single-document reciprocal IC flow probability)
  - src/detection/intercompany_matcher.py _build_result (internal column 합류)
  - config/settings.py ic_reciprocal_* 9 개 추가
"""
...
OUT_PHASE2_PARQUET = OUT_DIR / "stage7_fixed5_ic_design_fix_20260524_phase2_family_by_doc.parquet"
OUT_REPORT_JSON = OUT_DIR / "stage7_fixed5_ic_design_fix_20260524_report.json"

TOP_NS = [100, 500, 1000, 2000, 5000, 10000]

# baseline (pre-helper) 참조용 — 직접 사용하지 않고 보고서 baseline_reference 에만 기재
...
def _scenario_score_summary(
    ...
    for scenario, grp in merged.groupby("manipulation_scenario", sort=False):
        out[str(scenario)] = {
            ...
            "q95": float(np.quantile(grp["fs"], 0.95)) if len(grp) else 0.0,
...
def _ic_top_normal_vs_truth(
    phase2_by_doc: pd.DataFrame,
    truth_docs: set[str],
    col: str,
    thresholds: list[float],
) -> dict[str, Any]:
    """ic_score ≥ threshold 구간의 truth/normal 점유 — '정상이 부정보다 위' 정량."""
    if col not in phase2_by_doc.columns:
        return {}
...
def main() -> int:
    ...
    print(f"[ic-fix] loading PKL: {FIXED5_PKL.relative_to(ROOT)}")
    ...
    if FIXED5_PHASE1_CACHE.exists():
        print(f"[ic-fix] loading phase1 case cache: {FIXED5_PHASE1_CACHE.relative_to(ROOT)}")
    ...
    # PHASE2 5-family 점수는 IC helper 변경 반영을 위해 항상 재계산 (cache 무시).
    ...
    # family distribution + truth split + single recall
    family_score_cols = {
    ...
    # Noisy-OR TOP100 family dominant counts
    ...
    # ic_score ≥ 0.99 / 0.5 구간 truth vs normal 점유 (정상≥부정 ranking 정량)
    ic_top_split = _ic_top_normal_vs_truth(
        phase2_by_doc,
        truth_docs,
    ...
    report = {
        ...
        "fix_label": "ic_reciprocal_flow_split_20260524",
        ...
        "verification_principles": [
            ...
            "IC01/IC02/IC03 rule_id + sidecar 보존",
            "신규 ic_reciprocal_flow_prob 는 PHASE2 internal column",
    ...
    print("\n[ic-fix] intercompany family distribution")
    if "intercompany" in family_distribution:
        for key, value in family_distribution["intercompany"].items():
            print(f"  {key}: {value}")
    print("\n[ic-fix] intercompany truth split")
    if "intercompany" in family_truth_split:
        for key, value in family_truth_split["intercompany"].items():
            print(f"  {key}: {value}")
    ...
    print(
        f"\n[ic-fix] Noisy-OR PHASE2 TOP100 intercompany-dominant = {intercompany_dominant_top100}"
    )
    print(f"[ic-fix] circular truth (34) intercompany summary: {circular_truth_summary}")
    print(f"[ic-fix] ic_top threshold split: {ic_top_split}")
    return 0
