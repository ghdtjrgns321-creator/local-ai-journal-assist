"""Stage7 재측정 — TS evidence role 분리 설계 변경 효과를 fixed5_normalcal5 PKL 로 측정.

기존 캐시 (`artifacts/stage7_fixed5_normalcal5_phase2_family_by_doc_20260524.parquet`)
를 사용하지 않고 강제로 PHASE2 5-family 점수를 재계산한다. 산출물은
`_ts_design_fix_20260524` 접미사로 분리한다. truth label 기준 threshold 재튜닝은
하지 않으며 결과는 사후 검증으로만 사용한다.

변경 대상:
  - src/detection/timeseries_rules.py group_frequency_positive_robust_z_score
    (broad activity / cold-start / routine frequency suppression)
  - src/detection/timeseries_detector.py _build_result (strong/context 분리 결합식)
  - config/settings.py ts_group_min_active_days/excess_count/spike_ratio_min/
    cold_start_score_cap/strong_present_threshold 추가
"""
...
OUT_PHASE2_PARQUET = OUT_DIR / "stage7_fixed5_ts_design_fix_20260524_phase2_family_by_doc.parquet"
OUT_REPORT_JSON = OUT_DIR / "stage7_fixed5_ts_design_fix_20260524_report.json"
...
def main() -> int:
    ...
    print(f"[ts-fix] loading PKL: {FIXED5_PKL.relative_to(ROOT)}")
    ...
    if FIXED5_PHASE1_CACHE.exists():
        print(f"[ts-fix] loading phase1 case cache: {FIXED5_PHASE1_CACHE.relative_to(ROOT)}")
    ...
    # PHASE2 5-family 점수는 TS 설계 변경 반영을 위해 항상 재계산 (cache 무시).
    print("[ts-fix] scoring PHASE2 5 families on full df (no cache) ...")
    t_p2 = time.perf_counter()
    phase2_by_doc = stage7.score_phase2_families_by_document(df)
    print(
        f"[ts-fix]   phase2 scored docs={len(phase2_by_doc):,}  "
    ...
    print(f"[ts-fix]   wrote {OUT_PHASE2_PARQUET.relative_to(ROOT)}")

    # family distribution + truth split
    family_score_cols = {
    ...
    # Noisy-OR TOP 100 timeseries-dominant case count
    ...
    report = {
        ...
        "fix_label": "ts_evidence_role_split_20260524",
        ...
        "baseline_reference": {
            ...
            "phase2_family_by_doc": (
                "artifacts/stage7_fixed5_normalcal5_phase2_family_by_doc_20260524.parquet"
            ),
        },
        "verification_principles": [
            "fixed5 truth recall은 사후 보고만, threshold 튜닝에 사용 금지",
            "FIXED4 doc 미수정 (§12 비범위와 다른 scope — evidence role redefinition)",
            "phase2_subdetector_tiers.yaml tier 변경 없음 (weak 유지)",
            "TS01/TS02 rule_id 유지, TS03 추가 없음",
            "Phase1 columns/synthetic labels 입력 미사용",
        ],
    }
    ...
    print(f"\n[ts-fix] Noisy-OR PHASE2 TOP100 timeseries-dominant = {timeseries_dominant_top100}")
    print("\n[ts-fix] scenario ts score summary")
