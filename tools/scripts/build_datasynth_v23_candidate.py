...
TARGET_DIR = ROOT / "data" / "journal" / "primary" / "datasynth_v23_candidate"
...
def _date_shift(text: str, *, days: int = 0) -> str:
...
def _variant_amount(base_amount: int, variant_type: str) -> int:
    if variant_type == "near_amount":
        return max(1, round(base_amount * 1.014))
    return base_amount
...
def _patch_duplicate_pairs(rows: list[dict[str, str]], pairs: list[DuplicatePair]) -> None:
    ...
    for pair in pairs:
        ...
        source_amount = pair.payment_amount
        duplicate_amount = _variant_amount(pair.payment_amount, pair.variant_type)
        source_header = f"Payment {base_ref.replace('PAY:', '')} - {pair.vendor_code}"
        duplicate_header = source_header
        if pair.variant_type == "near_amount":
            duplicate_header = f"{source_header} adj"
        ...
        _write_payment_doc(
            ...
            payment_amount=duplicate_amount,
...
def _build_negative_control_groups(rows: list[dict[str, str]], pairs: list[DuplicatePair]) -> list[dict[str, Any]]:
    ...
    for year in YEARS:
        ...
        regular_created = 0
        confusing_created = 0
        while regular_created < 4 and cursor + 2 < len(candidates):
            chunk = candidates[cursor : cursor + 3]
            cursor += 3
            company = chunk[0][1][0]["company_code"]
            vendor = f"V-88{year % 100:02d}{regular_created + 1:02d}"
            amount = 150000 + regular_created * 20000
            posting_dates = [f"{year}-{month:02d}-25" for month in (2 + regular_created, 3 + regular_created, 4 + regular_created)]
            ...
            controls.append(
                {
                    "control_group_id": f"NC-{year}-REG-{regular_created + 1:03d}",
                    ...
                    "note": "Stable monthly recurring payment; intended true negative.",
                }
            )
            regular_created += 1

        while confusing_created < 2 and cursor + 1 < len(candidates):
            chunk = candidates[cursor : cursor + 2]
            cursor += 2
            company = chunk[0][1][0]["company_code"]
            vendor = f"V-87{year % 100:02d}{confusing_created + 1:02d}"
            amount = 220000 + confusing_created * 35000
            posting_dates = [f"{year}-02-05", f"{year}-02-16"]
            header = f"Manual payment run - {vendor}"
            for (_, doc_rows), posting_date in zip(chunk, posting_dates, strict=True):
                _write_payment_doc(
                    doc_rows,
                    company_code=company,
            ...
            controls.append(
                {
                    "control_group_id": f"NC-{year}-AMB-{confusing_created + 1:03d}",
                    "control_type": "ambiguous_repeat_payment_negative_control",
                    "pair_domain": "P2P",
                    "company_code": company,
                    "vendor_code": vendor,
                    "document_ids": [doc_id for doc_id, _ in chunk],
                    ...
                    "reference_pattern": "blank",
                    "note": "Irregular same-amount repeat payment; realistic borderline false-positive candidate.",
                }
            )
            confusing_created += 1
...
def _patch_anomaly_sidecars(target_dir: Path, pairs: list[DuplicatePair]) -> None:
    ...
    _write_csv(labels_dir / "anomaly_labels.csv", list(patched_csv[0].keys()), patched_csv)
...
def _write_freeze_note(target_dir: Path, pairs: list[DuplicatePair], controls: list[dict[str, Any]]) -> None:
    note = f"""# DataSynth V23 Candidate

- Frozen at: {datetime.now().astimezone().isoformat(timespec="seconds")}
- Base source: `data/journal/primary/datasynth/` freeze `v20.4`
- Scope: `B04 DuplicatePayment` harder P2P candidate

## Design Choice

- Keeps `DuplicatePayment` as `P2P + KZ`.
- Intentionally introduces a small amount of ambiguity so the benchmark is not perfect-fitting.
- Uses mixed duplicate variants plus borderline negative controls.
...
- negative controls include:
  - stable recurring monthly controls
  - ambiguous same-amount repeat controls

## Snapshot

- labeled duplicate documents: {len(pairs)}
- recurring / ambiguous negative-control groups: {len(controls)}
...
"""
    (target_dir / "FREEZE_V23_CANDIDATE.md").write_text(note, encoding="utf-8")
