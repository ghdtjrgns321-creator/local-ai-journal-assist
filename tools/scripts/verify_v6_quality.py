"""Verify DataSynth manipulation V6 candidate against the five quality gates.

Read-only against input datasets. Produces aggregate-only artifacts:
- artifacts/datasynth_v6_quality_verification.json
...
The default V6 input is
``data/journal/primary/datasynth_manipulation_v6_candidate``. If it is absent,
the script checks the fallback candidate names used during V6 handoff.
"""
...
DEFAULT_V6 = ROOT / "data" / "journal" / "primary" / "datasynth_manipulation_v6_candidate"
FALLBACK_V6_NAMES = (
    "datasynth_manipulation_v5_candidate_fixed10",
    "datasynth_manipulation_v6_active",
    "v6_active",
)
OUT_JSON = ROOT / "artifacts" / "datasynth_v6_quality_verification.json"
OUT_MD = ROOT / "artifacts" / "datasynth_v6_quality_verification.md"
TRUTH_CHECK = (
    ROOT
    / "tests"
    / "datasynth_quality_gate3"
    / "results"
    / "manipulation_v6_candidate_truth_check.json"
)
...
def resolve_v6(cli_path: str | None) -> tuple[Path, list[str]]:
    candidates: list[Path] = []
    if cli_path:
        candidates.append((ROOT / cli_path).resolve() if not Path(cli_path).is_absolute() else Path(cli_path))
    candidates.append(DEFAULT_V6)
    candidates.extend(ROOT / "data" / "journal" / "primary" / name for name in FALLBACK_V6_NAMES)
...
def first_digit_benford_gap(digit: pd.Series) -> pd.Series:
    ...
    actual = digit.map(expected).fillna(0.0)
    return 1.0 - actual
...
def gate2(v4_docs: pd.DataFrame, cand_docs: pd.DataFrame, v5_truth: pd.DataFrame, cand_truth: pd.DataFrame, guard: dict[str, Any] | None) -> dict[str, Any]:
    ...
    taxonomy = truth_taxonomy(v5_truth, cand_truth, "v5_fixed9", "v6")
    noise_delta = {
    ...
    block["pass"] = (
        block["balance"]["pass"]
        and taxonomy["v5_fixed9_docs_subset_of_v6"]
...
def gate3(docs: pd.DataFrame) -> dict[str, Any]:
    targets = {
        "approval_contract_gap": ("< 0.80", ">= 5%", 0.05),
        "approval_matrix_gap": ("< 0.80", ">= 5%", 0.05),
        "near_threshold_ratio_to_limit": ("< 0.80", ">= 3%", 0.03),
        "days_backdated": ("< 0.80", ">= 2%", 0.02),
        "is_suspense_account": ("< 0.80", ">= 1%", 0.01),
        "is_intercompany": ("< 0.80", "contract_v2 IC ratio maintained", 0.0),
        "master_counterparty_intercompany": ("< 0.80", "contract_v2 IC ratio maintained", 0.0),
        "amount_magnitude": ("< 0.80", "normal distribution overlaps manipulation amount area", None),
        "supply_amount_invoice_amount": ("< 0.80", "normal distribution overlaps manipulation amount area", None),
        "first_digit": ("< 0.80", "Benford normal distribution", None),
        "approval_lag_abs": ("< 0.80", "normal distribution added", None),
    }
    labels = docs["is_truth"].astype(int)
    normal = docs.loc[~docs["is_truth"]]
    checks = {}
    for feature, (auc_threshold, occurrence_target, min_rate) in targets.items():
...
def gate5(v5_docs: pd.DataFrame, cand_docs: pd.DataFrame, v5_truth: pd.DataFrame, cand_truth: pd.DataFrame, journal: pd.DataFrame) -> dict[str, Any]:
    ...
    block = {
        "normal_manual_entry_pct": {
            ...
            "v6": cand_noise["manual_entry_pct"],
        ...
        "normal_weekend_posting_pct": {
            ...
            "v6": cand_noise["weekend_posting_pct"],
        ...
        "truth_docs_preserved": {
            ...
            "v6": int(cand_truth["document_id"].nunique()),
            "pass": int(v5_truth["document_id"].nunique()) == 620 and int(cand_truth["document_id"].nunique()) == 620 and taxonomy["v5_fixed9_docs_subset_of_v6"],
        },
...
def blocked_result(v6: Path, checked: list[str]) -> dict[str, Any]:
    truth_check = {
        "generated_at": datetime.now(UTC).isoformat(),
        "dataset": rel(v6),
        "status": "blocked",
        "reason": "V6 candidate dataset not found",
    ...
    result = {
        ...
        "mode": "read-only",
        "status": "BLOCKED",
        "blocked_reason": "No V6 candidate directory with journal_entries.csv was found.",
        "checked_paths": checked,
        "outputs": {
            "json": rel(OUT_JSON),
            "markdown": rel(OUT_MD),
        ...
        "verdict": {
            "go_no_go": "BLOCKED",
            "hard_failures": ["v6_candidate_missing"],
            "soft_failures": [],
...
def write_markdown(result: dict[str, Any]) -> None:
    if result.get("status") == "BLOCKED":
        lines = [
            "# DataSynth V6 Candidate Quality Verification",
            "",
    ...
    g3 = gates["gate_3_enrichment_natural_occurrence"]["checks"]
    ...
    lines = [
        ...
        table(
            [
                ...
                ["Gate 3 - enrichment natural occurrence", status_word(gates["gate_3_enrichment_natural_occurrence"]["pass"])],
        ...
        table(
            [
                ...
                ["V5 fixed9 truth docs subset of V6", g2["truth_taxonomy"]["v5_fixed9_docs_subset_of_v6"], status_word(g2["truth_taxonomy"]["v5_fixed9_docs_subset_of_v6"])],
                ...
                ["Noise floor delta <= 10pp", g2["noise_floor"]["delta_v6_minus_v4"], status_word(g2["noise_floor"]["delta_within_10pp"])],
        ...
        "",
        "## Gate 3 - Enrichment Natural Occurrence",
        "",
        table(
            [
                ["Column", "Source", "Max AUROC", "AUROC Target", "Normal Occurrence", "Occurrence Target", "Verdict"],
                ["---", "---", "---:", "---", "---:", "---", "---"],
                *[
                    [
                        key,
        ...
        "",
        "## V5 fixed9 to V6 Change Matrix",
        "",
        table(
            [
                ["Area", "V5 fixed9", "V6", "Classification"],
                ["---", "---", "---", "---"],
        ...
        "## GO / NO-GO",
        "",
        f"V6 generation verification verdict: **{verdict_block['go_no_go']}**.",
        "",
        "HARD failures: "
        ...
        "- `artifacts/datasynth_v6_quality_verification.json`",
        "- `tests/datasynth_quality_gate3/results/manipulation_v6_candidate_truth_check.json`",
        "- `tools/scripts/verify_v6_quality.py`",
    ]
    OUT_MD.write_text("\n".join(lines) + "\n", encoding="utf-8")


def run(cli_path: str | None = None) -> dict[str, Any]:
    v6, checked = resolve_v6(cli_path)
    if not v6.exists() or not (v6 / "journal_entries.csv").exists():
        result = blocked_result(v6, checked)
    ...
    v6_truth = load_truth(v6)
    v6_journal = load_journal(v6)
    v6_docs = doc_summary(v6_journal, set(v6_truth["document_id"]))
    v6_accounts = load_accounts(v6)
    truth_check = build_truth_check(v6, v6_journal, v6_truth, v6_docs)
    guard = read_json(ROOT / "artifacts" / "manipulation_v5_candidate_guard_fixed9.json")

    gates = {
        "gate_1_fixed9_generation_regression": gate1(v6_journal, v6_docs, v6_truth, v6_accounts),
        "gate_2_accounting_substance": gate2(v4_docs, v6_docs, v5_truth, v6_truth, guard),
        "gate_3_enrichment_natural_occurrence": gate3(v6_docs),
        "gate_4_quality_gate3": gate4(truth_check),
        "gate_5_no_new_defects": gate5(v5_docs, v6_docs, v5_truth, v6_truth, v6_journal),
    }
    result = {
        "generated_at": datetime.now(UTC).isoformat(),
        "dataset": rel(v6),
        ...
        "mode": "read-only",
        "inputs": {
            "manifest": next((rel(path) for path in v6.glob("*MANIFEST*.json")), None),
            "truth": rel(v6 / "labels" / "manipulated_entry_truth.csv"),
            "truth_check": rel(TRUTH_CHECK),
        },
        "gates": gates,
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify DataSynth manipulation V6 candidate quality gates.")
    parser.add_argument("--dataset", default=None, help="Optional V6 dataset directory.")
