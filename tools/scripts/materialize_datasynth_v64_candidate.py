"""Materialize DataSynth v64 candidate from the SkippedApproval manifest."""
...
TARGET_DIR = ROOT / "data" / "journal" / "primary" / "datasynth_v64_candidate"
...
def _patch_labels() -> dict[str, int]:
    ...
    manifest = pd.read_csv(MANIFEST_DIR / "skipped_approval_label_manifest.csv", dtype=str)
    if manifest.empty:
        return {"added": 0, "total_skipped_approval": int(labels["anomaly_type"].eq("SkippedApproval").sum())}
    meta_col = _metadata_col(labels)
    existing = set(labels.loc[labels["anomaly_type"].eq("SkippedApproval"), "document_id"].astype(str))
        ...
        new_row.update(
            {
                ...
                "severity": "4",
                "description": (
                    f"L1-07 SkippedApproval truth: {row['document_number']} amount "
                    f"{int(float(row['document_amount']))} has no approver"
                ),
                "is_injected": "True",
                "monetary_impact": str(float(row["document_amount"])),
                "related_entities": json.dumps([row["document_number"]], ensure_ascii=False),
                "injection_strategy": "SkippedApprovalTruthRepair",
    ...
    return {"added": len(new_rows), "total_skipped_approval": int(labels["anomaly_type"].eq("SkippedApproval").sum())}
...
def _write_docs(validation: dict[str, Any]) -> None:
    text = f"""# DataSynth v64 Candidate
...
source_baseline: data/journal/primary/datasynth_v63_candidate
included_manifests:
  - patch_manifest/skipped_approval_label_manifest.csv
...
- Add missing strict L1-07 `SkippedApproval` labels.
- Keep broad missing-approver documents as normal controls when they are system/recurring or below approval threshold.
- Do not force year-balanced positives.
...
```
"""
    (TARGET_DIR / "FREEZE_V64_CANDIDATE.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    (TARGET_DIR / "V64_VALIDATION.json").write_text(json.dumps(validation, ensure_ascii=False, indent=2), encoding="utf-8")
