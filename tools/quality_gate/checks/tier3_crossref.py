...
def _get_excluded_doc_ids(con: duckdb.DuckDBPyConnection, anomaly_types: list[str]) -> set[str]:
    """labels 테이블에서 제외할 document_id 집합."""
    try:
...
def t3_05(con: duckdb.DuckDBPyConnection) -> CheckResult:
    """employee company 일치 — je.company_code == employees.company_code."""
    s = time.perf_counter()
    try:
        con.execute("SELECT 1 FROM _employees LIMIT 1")
    except Exception:
...
def t3_09(con: duckdb.DuckDBPyConnection) -> CheckResult:
    ...
    s = time.perf_counter()
    excl = _get_excluded_doc_ids(con, ["SoDViolation", "SoDConflict"])
...
def t3_12(con: duckdb.DuckDBPyConnection) -> CheckResult:
    """employee approval_limit — 전표 금액 > limit (ExceededApprovalLimit 제외)."""
    s = time.perf_counter()
    try:
        con.execute("SELECT 1 FROM _employees LIMIT 1")
    except Exception:
    ...
    excl = _get_excluded_doc_ids(con, ["ExceededApprovalLimit"])
    ...
    return CheckResult(
        ...
        expected="초과=0 (ExceededApprovalLimit 제외)", actual=f"{violated:,}건",
        elapsed_ms=_elapsed(s),
    )
...
def t3_27(con: duckdb.DuckDBPyConnection) -> CheckResult:
    ...
    orphan = con.execute("""
        SELECT
            COUNT(*) FILTER (WHERE seller_document NOT IN (SELECT DISTINCT document_id FROM je))
          + COUNT(*) FILTER (WHERE buyer_document NOT IN (SELECT DISTINCT document_id FROM je))
        FROM _ic_pairs
    """).fetchone()[0]
