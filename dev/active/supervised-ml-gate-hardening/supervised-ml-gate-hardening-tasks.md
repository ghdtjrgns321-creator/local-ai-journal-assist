...
- [ ] **2-1** datasynth label gate 판정 추가
  - File: `src/preprocessing/label_strategy.py`
...
- [ ] **2-2** pseudo / hybrid fallback 차단 판정 추가
  - File: `src/preprocessing/label_strategy.py`
...
- [ ] **3-3** train metadata에 gate snapshot 포함
  - File: `src/detection/supervised_detector.py`
...
- [ ] **3-5** supervised detector 테스트 추가
...
- [ ] pseudo label로 supervised 모델이 저장되지 않는다.
- [ ] GT 라벨이어도 최소 양성 수/비율 미달이면 supervised 학습이 차단된다.
- [ ] pipeline detector status에서 supervised skip reason이 바로 보인다.
