...
- 3층(장부상태): 없는 AR 갚기·불가능 잔액 — 신축. 1차 방어 = 상태 인지 생성(존재하는 것만 건드림).
...
- **수용 게이트**: (a) 6패턴 각 ≥1 생성, (b) 3층 오라클 사고위반 0, (c) 라벨/힌트 누수 0(detection grep read 0), (d) 5벌 모두 (a)(b)(c) 통과. 미달 시 결함 적시(은폐 금지).
